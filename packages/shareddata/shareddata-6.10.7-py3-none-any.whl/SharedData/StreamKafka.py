...
class StreamKafka:
    ...
    def __init__(
        ...
        self.cache = shdata.cache(self.database, self.period, self.source, self.tablename)
        if not self.cache.header_get('stream->poll_counter'):
            self.cache.header_set('stream->poll_counter', 0)
        if not self.cache.header_get('stream->persist_counter'):
            self.cache.header_set('stream->persist_counter', 0)
    ...
    def poll(self, groupid = None, timeout=None):
        ...
        self.cache.header_incrby('stream->poll_counter', 1) # increment poll_counter
        return msgdict
            ...
            for msg in messages:
                if msg.value is not None:
                    if decompress:
                        ...
                        msgdict = msg.value
                    if 'mtime' in msgdict:
                        if isinstance(msgdict['mtime'], (int, np.integer)):
                            msgdict['mtime'] = pd.Timestamp(msgdict['mtime'], unit='ns')
                        if self.mtime is None or msgdict['mtime'] > self.mtime:
                            self.mtime = msgdict['mtime']                    
                    msgs.append(msgdict)

        self.cache.header_incrby(f'stream->{groupid}->poll_counter', len(msgs)) # increment poll_counter
        return msgs
    ...
    async def async_cache_stream_task(self):
        try:
            await self.cache.start()
            await self.async_subscribe(groupid='cache-group',offset='earliest')
        except Exception as e:
        ...
        while True:
            try:
                msgs = await self.async_poll(timeout=1.0, groupid='cache-group')            
                if not msgs:                
                    continue
                for msg in msgs:
                    await self.cache.set_async(msg)
            except Exception as e:
                errmsg = f"Failed to cache stream: {e}"
                Logger.log.error(errmsg)
                raise Exception(errmsg)

    async def async_persist_stream_task(self, partitioning='daily'):
        try:
        ...
        while True:
            try:
                ...
                self.cache.header_incrby(f'stream->persist_counter', len(data)) #TODO: MOVE TO COLLECTION CLASS
            except Exception as e:
