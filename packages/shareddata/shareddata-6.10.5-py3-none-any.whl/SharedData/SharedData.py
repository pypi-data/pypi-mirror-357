...
class SharedData:    
    ...
    def stream(self, database, period, source, tablename, user='master', 
               type='KAFKA',
               bootstrap_servers=None, replication=None, partitions=None, use_aiokafka=False):
        ...
        if (not path in self.data.keys()):
            ...
            if type == 'KAFKA':
                self.data[path] = StreamKafka(database, period, source, tablename, user,
    ...
    def cache(self, database, period, source, tablename, user='master', 
              type='REDIS', use_async_queue=False):
        ...
        if (not path in self.data.keys()):
            ...
            if type == 'REDIS':
                self.data[path] = CacheRedis(database, period, source, tablename, user, use_async_queue)
            else:
                raise Exception(f'Invalid  stream type {type}')
    ...
    def delete_stream(self, database, period, source, tablename, user='master'):
        path = f'{user}/{database}/{period}/{source}/stream/{tablename}'
        stream = self.stream(database,period,source,tablename,user=user)
        stream.delete()
        if path in self.data.keys():
            del self.data[path]
        del stream
