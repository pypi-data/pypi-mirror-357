...
class StreamKafka:
    ...
    async def run_async_cache_and_persist_tasks(
        ...
        shdata,
        partitioning = 'daily',
        snapshot_periods = None #['D1', 'M15', 'M1']
    ):
        """
        Run and await cache and persist tasks asynchronously.
        """    
        cache, cache_tasks, persist_tasks = await self.async_cache_and_persist_tasks(
            shdata, partitioning=partitioning, snapshot_periods=snapshot_periods)
        tasks = [*cache_tasks, *persist_tasks]
    ...
    async def async_cache_and_persist_tasks(
        ...
        persist_flush_task = asyncio.create_task(
            self.async_persist_flush_task(self.persist_queue,shdata, partitioning=partitioning, snapshot_periods=snapshot_periods)
        )
        cache_tasks = [cache_consumer_task, cache_flush_task]
        persist_tasks = [persist_consume_task, persist_flush_task]
        return cache, cache_tasks, persist_tasks
    
    async def async_cache_tasks(
        self,
        shdata,
    ...
    async def async_persist_flush_task(self, queue, shdata, partitioning = 'daily', snapshot_periods = None) -> None:
        while True:
            data = [ await queue.get() ]            
            while not queue.empty() and len(data)<5000:
            ...
            collection = shdata.collection(self.database, self.period, self.source, tablename, user=self.user, hasindex=False)
            collection.extend(data)
            
            if snapshot_periods is not None and 'date' in self.pkeys:
                for period in snapshot_periods:
                    ...
                    collection = shdata.collection(self.database, period, self.source, tablename, user=self.user)
