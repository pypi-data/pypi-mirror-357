...
class CacheRedis:
    ...
    def __setitem__(self, pkey, value):
        ...
        if not 'mtime' in value:
            value['mtime'] = pd.Timestamp.utcnow().tz_localize(None)
        
        if isinstance(self.data[pkey]['mtime'], int):
            self.data[pkey]['mtime'] = pd.Timestamp(self.data[pkey]['mtime'], unit='ns')
    ...
    def set_async(self, msg):
        pkey = self.get_pkey(msg)
        self[pkey] = msg
        self.consumer_counter += 1

    def set(self, value):        
        pkey = self.get_pkey(value)
