...
# ##################################################################
#
# Comments by Žiga Stržinar:
# - Initial source: https://github.com/oguzserbetci/generate-time-series/blob/master/dtw_mean.py
# - The DTW implementation was raising errors in path calculation,
#   DTW was therefore replaced with the dtw package.
#
####################################################################
import numpy as np
...
def dtw(x, y, path=False):
    # d_original, p_original = dtw_original(x, y, path=True)

    from dtw.dtw import dtw, DTW

    d: DTW = dtw(x, y)
    if path:
        p = np.hstack((d.index1.reshape(-1, 1), d.index2.reshape(-1, 1)))
        return d.distance, p
    return d.distance


def dtw_original(x, y, path=False):
