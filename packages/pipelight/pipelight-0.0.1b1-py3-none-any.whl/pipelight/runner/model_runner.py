...
class ModelRunner(pl.LightningModule):
    ...
    def configure_optimizers(self) -> Union[Sequence[Optimizer], Tuple[Tuple[Optimizer], Sequence[Union[LRScheduler, ModelScheduler]]]]:
        ...
        if len(self.__lr_schedulers) == 0 and len(self.__model_schedulers) == 0:
            self.configured_lr_schedulers = tuple()
            self.configured_model_schedulers = tuple()
            ...
            main_module = self.select_main_module()
            self.configured_model_schedulers = tuple(
                _init_model_scheduler(main_module, model_scheduler) for model_scheduler in self.__model_schedulers
            )
            return self.configured_optimizers, [*self.configured_lr_schedulers, *self.configured_model_schedulers]
...
def _init_model_scheduler(model: nn.Module, model_scheduler: Union[ModelScheduler, Callable[[Optimizer], ModelScheduler]]):
    if isinstance(model_scheduler, ModelScheduler):
        return model_scheduler
    elif isinstance(model_scheduler, Callable):
        return model_scheduler(model=model)
    else:
        raise TypeError(f"'model_scheduler' should be an instance of either '{ModelScheduler.__name__}' or '{Callable.__name__}'")
