...
class SaliencyTool(ClassDiscriminativeTool):
    def __init__(self, 
                 model: nn.Module,
                 abs: bool = True,
        ...
        if self.abs not in [False, True]:
            raise ValueError("Expected abs argument to be True, or False,",
    ...
    def _prepare_xai_maps(self, batch: torch.Tensor, target_idx: Union[int, List[int]]):
        ...
        scores = self.saliency.attribute(batch, abs=self.abs, target=target_idx)
