...
from servequery.core.serialization import SnapshotModel
from servequery.errors import ServeQueryError
from servequery.legacy.core import new_id
from servequery.legacy.ui.api.models import OrgModel
from servequery.legacy.ui.api.service import SERVEQUERY_APPLICATION_NAME
from servequery.legacy.ui.base import Org
from servequery.legacy.ui.storage.common import SECRET_HEADER_NAME
from servequery.legacy.ui.type_aliases import STR_UUID
from servequery.legacy.ui.type_aliases import DatasetID
...
from servequery.legacy.ui.workspace.cloud import ACCESS_TOKEN_COOKIE
from servequery.legacy.ui.workspace.cloud import TOKEN_HEADER_NAME
from servequery.legacy.ui.workspace.remote import RemoteBase
from servequery.legacy.ui.workspace.remote import T
from servequery.sdk.datasets import DatasetList
from servequery.sdk.datasets import RemoteDatasetsManager
from servequery.sdk.models import DashboardModel
from servequery.sdk.models import DashboardPanelPlot
from servequery.sdk.models import DashboardTabModel
from servequery.sdk.models import ProjectModel
from servequery.sdk.models import SnapshotLink
from servequery.sdk.prompts import RemotePromptManager
from servequery.ui.storage.local.base import SNAPSHOTS_DIR_NAME
from servequery.ui.storage.local.base import LocalState
from servequery.ui.utils import get_html_link_to_report
...
class _RemoteProjectDashboard(ProjectDashboard):
    ...
    def add_tab(self, tab: str):
        ...
        if any([t.title == tab for t in _dashboard_model.tabs]):
            raise ServeQueryError(f"Tab {tab} already exists in project {self._project_id} dashboard")
    ...
    def delete_panel(self, panel: str, tab: str):
        ...
        if _tab is None:
            raise ServeQueryError(f"Tab {tab} does not exist in project {self._project_id} dashboard")
...
class RemoteWorkspace(RemoteBase, WorkspaceBase):  # todo: reuse cloud ws
    ...
    def verify(self):
        try:
            ...
            assert response.json()["application"] == SERVEQUERY_APPLICATION_NAME
        except (HTTPError, JSONDecodeError, KeyError, AssertionError) as e:
            raise ValueError(f"ServeQuery API not available at {self.base_url}") from e
...
class CloudWorkspace(RemoteWorkspace):
    URL: str = "https://app.servequery.cloud"
    ...
    def __init__(
        ...
        if token is None:
            token = os.environ.get("SERVEQUERY_API_KEY", default=None)
        if token is None:
            raise ValueError(
                "To use CloudWorkspace you must provide a token through argument or env variable SERVEQUERY_API_KEY"
            )
        self.token = token
    ...
    def _request(
        ...
        try:
            ...
            if self._logged_in and e.get_message() == "ServeQueryError: Not authorized":
                # renew token and retry
                self._jwt_token = self._get_jwt_token()
                cookies[self.token_cookie_name] = self.jwt_token
                return super()._request(
