...
from servequery._pydantic_compat import ModelMetaclass
...
def pydantic_type_validator(type_: Type[Any], prioritize: bool = False):
    def decorator(f):
        from servequery._pydantic_compat import _VALIDATORS
...
ALLOWED_TYPE_PREFIXES = ["servequery."]

SERVEQUERY_TYPE_PREFIXES_ENV = "SERVEQUERY_TYPE_PREFIXES"
ALLOWED_TYPE_PREFIXES.extend([p for p in os.environ.get(SERVEQUERY_TYPE_PREFIXES_ENV, "").split(",") if p])
...
def get_value_fingerprint(value: Any) -> FingerprintPart:
    if isinstance(value, ServeQueryBaseModel):
...
EBM = TypeVar("EBM", bound="ServeQueryBaseModel")


class ServeQueryBaseModel(FrozenBaseModel, PolymorphicModel):
    class Config:
        type_alias = "servequery:base:ServeQueryBaseModel"
        alias_required = True
        is_base_type = True
...
@autoregister
class WithTestAndMetricDependencies(ServeQueryBaseModel):
    class Config:
        type_alias = "servequery:test:WithTestAndMetricDependencies"

    def __servequery_dependencies__(self):
...
class FieldPath:
    ...
    def list_nested_fields(self, exclude: Set["IncludeTags"] = None) -> List[str]:
        ...
        for name, field in self._cls.__fields__.items():
            ...
            from servequery.legacy.core import get_field_tags
    ...
    def _list_with_tags(self, current_tags: Set["IncludeTags"]) -> List[Tuple[List[Any], Set["IncludeTags"]]]:
        ...
        from servequery.legacy.core import BaseResult

        if issubclass(self._cls, BaseResult) and self._cls.__config__.extract_as_obj:
            return [(self._path, current_tags)]
        res = []
        from servequery.ui.backport import ByLabelCountValueV1
        from servequery.ui.backport import ByLabelValueV1
...
class AutoAliasMixin:
    ...
    def __get_type__(cls) -> str:
        ...
        if config is not None and config.__dict__.get("type_alias") is not None:
            return config.type_alias
        return f"servequery:{cls.__alias_type__}:{cls.__name__}"
