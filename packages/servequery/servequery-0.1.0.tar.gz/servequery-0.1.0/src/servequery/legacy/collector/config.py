...
from servequery.legacy.collector.storage import InMemoryStorage
...
from servequery.legacy.ui.workspace.remote import RemoteWorkspace
...
class IntervalTrigger(CollectorTrigger):
    class Config:
        type_alias = "servequery:collector_trigger:IntervalTrigger"
...
class RowsCountTrigger(CollectorTrigger):
    class Config:
        type_alias = "servequery:collector_trigger:RowsCountTrigger"
...
class RowsCountOrIntervalTrigger(CollectorTrigger):
    class Config:
        type_alias = "servequery:collector_trigger:RowsCountOrIntervalTrigger"
...
class CollectorConfig(Config):
    ...
    def is_cloud_resolved(self) -> bool:
        return self.is_cloud if self.is_cloud is not None else self.api_url == "https://app.servequery.cloud"
        ...
        if self._workspace is None:
            if self.is_cloud_resolved:
                ...
                if self.save_datasets:
                    warnings.warn("'save_datasets' is not supported for self-hosted ServeQuery UI")
