...
def _flatten_params_rec(obj: Any, paths: List[str]) -> List[Tuple[List[str], str]]:
    ...
    if isinstance(obj, BaseModel):
        for field_name, field in obj.__fields__.items():
            if isinstance(obj, ServeQueryBaseModel) and field_name == "type":
                continue
...
def _flatten_params(obj: ServeQueryBaseModel) -> Dict[str, str]:
...
def _get_hover_params(items: Set[TMT]) -> Dict[TMT, List[str]]:
    from servequery.ui.backport import TestV2Adapter
