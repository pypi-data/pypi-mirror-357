...
from servequery.pydantic_utils import FrozenBaseMeta
...
class MetricResult(PolymorphicModel, BaseResult, metaclass=WithFieldsPathMetaclass):  # type: ignore[misc] # pydantic Config
    class Config:
        type_alias = "servequery:metric_result:MetricResult"
...
@autoregister
class ColumnName(EnumValueMixin, ServeQueryBaseModel):
    class Config:
        type_alias = "servequery:base:ColumnName"
...
class BasePreset(ServeQueryBaseModel):
    class Config:
        type_alias = "servequery:base:BasePreset"
...
class Metric(WithTestAndMetricDependencies, Generic[TResult], metaclass=WithResultFieldPathMetaclass):
    ...
    def get_group(cls) -> str:
        if cls.__module__.startswith("servequery.legacy.metrics."):
...
class ColumnMetricResult(MetricResult):
    class Config:
        type_alias = "servequery:metric_result:ColumnMetricResult"
        field_tags = {
