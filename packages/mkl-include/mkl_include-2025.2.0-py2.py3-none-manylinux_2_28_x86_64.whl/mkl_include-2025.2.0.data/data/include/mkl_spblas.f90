...
!   applies to SOR interface; define type of (S)SOR operation to perform
    ENUM, BIND(C)
        ENUMERATOR :: SPARSE_SOR_FORWARD   = 110, & ! (omega∗L + D)∗x^1 = (D - omega*D - omega*U)∗alpha*x^0 + omega*b
...
!   Computes multiplication of sparse matrices: C = op(A) * B, result is dense
        ...
        FUNCTION MKL_SPARSE_S_SP2MD(opA,descrA,A,opB,descrB,B,alpha,beta,C,layout,ldc) & !
        ...
        END FUNCTION
        FUNCTION MKL_SPARSE_D_SP2MD(opA,descrA,A,opB,descrB,B,alpha,beta,C,layout,ldc) & !
        ...
        END FUNCTION
        FUNCTION MKL_SPARSE_C_SP2MD(opA,descrA,A,opB,descrB,B,alpha,beta,C,layout,ldc) & !
        ...
        END FUNCTION
        FUNCTION MKL_SPARSE_Z_SP2MD(opA,descrA,A,opB,descrB,B,alpha,beta,C,layout,ldc) & !
        ...
        FUNCTION MKL_SPARSE_SET_SORV_HINT(type,A,descr,expected_calls) &
                 BIND(C, name='MKL_SPARSE_SET_SORV_HINT')
            USE, INTRINSIC :: ISO_C_BINDING , ONLY : C_INT
            IMPORT SPARSE_MATRIX_T
            IMPORT MATRIX_DESCR
            INTEGER(C_INT)       , INTENT(IN) :: type           ! choice of forward, backward sweep or SSOR operation
            TYPE(SPARSE_MATRIX_T), INTENT(INOUT) :: A
            TYPE(MATRIX_DESCR)   , INTENT(IN) :: descr          ! sparse_matrix_type_t + sparse_fill_mode_t + sparse_diag_type_t
            INTEGER              , INTENT(IN) :: expected_calls
            INTEGER(C_INT) MKL_SPARSE_SET_SORV_HINT
        END FUNCTION
        ...
        FUNCTION MKL_SPARSE_S_SORV(type,descr,A,omega,alpha,x,b) &
                 BIND(C, name='MKL_SPARSE_S_SORV')
            USE, INTRINSIC :: ISO_C_BINDING , ONLY : C_INT, C_FLOAT
            IMPORT SPARSE_MATRIX_T
            IMPORT MATRIX_DESCR
            INTEGER(C_INT)       , INTENT(IN) :: type       ! choice of forward, backward sweep or SSOR operation
            TYPE(MATRIX_DESCR)   , INTENT(IN) :: descr
            TYPE(SPARSE_MATRIX_T), INTENT(IN) :: A
            REAL(C_FLOAT)        , INTENT(IN) :: omega
            REAL(C_FLOAT)        , INTENT(IN) :: alpha      ! alpha equals to 0 mean zero initial guess
            REAL(C_FLOAT)        , INTENT(INOUT), DIMENSION(*) :: x ! solution vector and alpha * x is initial guess
            REAL(C_FLOAT)        , INTENT(IN)   , DIMENSION(*) :: b ! right-hand side
            INTEGER(C_INT) MKL_SPARSE_S_SORV
        END FUNCTION

        FUNCTION MKL_SPARSE_D_SORV(type,descr,A,omega,alpha,x,b) &
                 BIND(C, name='MKL_SPARSE_D_SORV')
            USE, INTRINSIC :: ISO_C_BINDING , ONLY : C_INT, C_DOUBLE
            IMPORT SPARSE_MATRIX_T
            IMPORT MATRIX_DESCR
            INTEGER(C_INT)       , INTENT(IN) :: type       ! choice of forward, backward sweep or SSOR operation
            ...
            REAL(C_DOUBLE)       , INTENT(IN) :: alpha      ! alpha equals to 0 mean zero initial guess
            REAL(C_DOUBLE)       , INTENT(INOUT), DIMENSION(*) :: x ! solution vector and alpha * x is initial guess
            REAL(C_DOUBLE)       , INTENT(IN)   , DIMENSION(*) :: b ! right-hand side
            INTEGER(C_INT) MKL_SPARSE_D_SORV
        END FUNCTION
