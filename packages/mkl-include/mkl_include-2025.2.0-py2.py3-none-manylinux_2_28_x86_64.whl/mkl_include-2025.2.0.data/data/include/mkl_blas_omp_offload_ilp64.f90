...
module onemkl_blas_omp_offload_ilp64
  ...
  interface
     ...
     subroutine cgemm3m_batch_strided ( transa, transb, m, n, k, alpha, a, lda, stridea,        &
       ...
       complex,intent(in)                  :: a( lda, * ), b( ldb, * )
       complex,intent(inout)               :: c( ldc, * )
       !$omp  declare variant( cgemm3m_batch_strided:mkl_blas_cgemm3m_batch_strided_omp_offload_ilp64 ) match( construct={dispatch}, device={arch(gen)} ) append_args(interop(prefer_type("sycl","level_zero"),targetsync)) adjust_args(need_device_ptr:a,b,c)
     ...
     subroutine zgemm3m_batch_strided ( transa, transb, m, n, k, alpha, a, lda, stridea,        &
       ...
       complex*16,intent(in)              :: a( lda, * ), b( ldb, * )
       complex*16,intent(inout)           :: c( ldc, * )
       !$omp  declare variant( zgemm3m_batch_strided:mkl_blas_zgemm3m_batch_strided_omp_offload_ilp64 ) match( construct={dispatch}, device={arch(gen)} ) append_args(interop(prefer_type("sycl","level_zero"),targetsync)) adjust_args(need_device_ptr:a,b,c)
