...
def _kmock_aresponses(request: pytest.FixtureRequest) -> None:
    # This fixture is separated from `kmock` because it be sync to retrieve another async fixture.
    # Initialize `aresponses` strictly before `kmock` if it is used in the test at all.
    # If not used, assume that it is not retrieve at runtime later.
...
@pytest.fixture()
async def kmock(kmock_options: Dict[str, Any], _kmock_aresponses: None) -> apps.RawHandler:
    """
