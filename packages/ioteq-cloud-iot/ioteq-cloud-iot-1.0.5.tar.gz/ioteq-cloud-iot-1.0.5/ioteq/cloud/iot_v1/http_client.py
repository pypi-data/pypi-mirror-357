...
class HttpClient():
    def __init__(self,
                 ioteq_config:IOTEQConfig = None) -> None:

        self._ioteq_config = ioteq_config
        self._system_key = self._ioteq_config.system_key
        self._auth_token = self._ioteq_config.token
    ...
    def _get_api_url(self, api_name:str = None, is_web_hook:bool = True) -> str:
        api_web_hook_path = "api/v1/iot/webhook/execute"
        if not is_web_hook:
            api_web_hook_path = "api/v1/iot"

        return "{}:{}/{}/{}?".format(self._ioteq_config.api_url,
                                        "8443",
                                        api_web_hook_path,
                                        api_name)

    def _headers(self):
        return {'TeqConnect-UserToken':self._ioteq_config.token, 'Content-Type':'application/json'}
...
class SyncClient(HttpClient):
    ...
    def __init__(self, ioteq_config: IOTEQConfig = None) -> None:
        super().__init__(ioteq_config)
