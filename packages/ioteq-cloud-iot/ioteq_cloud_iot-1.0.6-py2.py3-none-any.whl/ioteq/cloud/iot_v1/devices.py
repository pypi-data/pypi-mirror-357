...
from .config_manager import IOTEQConfigManager
...
class IOTEQDeviceManager():

    def __init__(self) -> None:
        # create the IOTEQConfig object
        self._config_manager = IOTEQConfigManager()
    ...
    def _create_device_body(self, device: Device) :
        ...
        if not 'id' in device.credentials:
            device.credentials["id"] = str(uuid.uuid4())

        if not 'name' in device.credentials:
            device.credentials["name"] = ""

        if not 'Type' in device.credentials:
            device.credentials["Type"] = "psk"

        if not 'pskCredentials' in device.credentials:
            device.credentials["pskCredentials"] = {
                "username": "teqconnectMqttDefault",
                "password": "mqttIoteq2019"
            }

        if not 'certCredentials' in device.credentials:
            device.credentials["certCredentials"] = {
                "expirationTime": None,
                "publicKey": {
                    "format": None,
                    "key": None
    ...
    def registry_path(self,
        ...
        return "projects/{project}/registries/{registry}".format(
            project=project,
            registry=registry,
        )
    ...
    def send_command(self,
        ...
        ioteq_config = self._config_manager.regional_config(parent=request.parent)

        project = find_project_region_registry_from_parent(request.parent)

        params.update({'organization':ioteq_config.organization, 'registry': project['registry'], 'device': project['device'],
                       'name': "organizations/{}/{}".format(ioteq_config.organization,request.parent)})

        sync_client = SyncClient(ioteq_config=ioteq_config)
        return sync_client.post(api_name="cloudiot_device_cmd", request_params=params, request_body=body)
    ...
    async def send_command_async(self,
        ...
        params, body = self._prepare_for_send_command(request, name, binary_data, subfolder)
        ioteq_config = await self._config_manager.regional_config_async(request.parent)
        ...
        async_client = AsyncClient(ioteq_config=ioteq_config)
        return await async_client.post(api_name="cloudiot_device_cmd",
    ...
    def create(self, request: CreateDeviceRequest) -> Device:
        ...
        body.update({'name': "organizations/{}/{}/devices/{}".format(ioteq_config.organization,request.parent,request.device.id)})
        
        project = find_project_region_registry_from_parent(request.parent)

        params = {'organization':ioteq_config.organization, 'registry': project['registry']}

        sync_client = SyncClient(ioteq_config=ioteq_config)
        response = sync_client.post(api_name="cloudiot_device",
                                    request_params=params,
                                    request_body=body)
    ...
    async def create_async(self, request: CreateDeviceRequest,
        ...
        async_client = AsyncClient(ioteq_config=ioteq_config)
        response = await async_client.post(api_name="cloudiot_device", 
                                            request_params=params,
                                            request_body=body)
        # return the device only if status code is 200
        # other wise return None
    ...
    def get_device_list_response(self, request:ListDevicesRequest):
        ...
        params.update({'organization':ioteq_config.organization, 'registry': project['registry']})
    ...
    def get(self,
        ...
        params = {'organization':ioteq_config.organization, 'registry': project['registry'], 'device': project['device']}
        
        sync_client = SyncClient(ioteq_config=ioteq_config)
        response = sync_client.get(api_name="cloudiot_device", request_params=params)
    ...
    async def get_async(self,
        ...
        async_client = AsyncClient(ioteq_config=ioteq_config)
        response = await async_client.get(api_name="cloudiot_device", request_params=params)
    ...
    def delete(self,
        ...
        sync_client = SyncClient(ioteq_config=ioteq_config)
        response = sync_client.delete(api_name="cloudiot_device",request_params=params)
        return response
    ...
    async def delete_async(self,
        ...
        async_client = AsyncClient(ioteq_config=ioteq_config)
        response = await async_client.delete(api_name="cloudiot_device", request_params=params)
        return response
    ...
    def update(self,
        ...
        sync_client = SyncClient(ioteq_config=ioteq_config)
        response = sync_client.patch(api_name="cloudiot_device",request_body=body, request_params=params)
    ...
    async def update_async(self,
        ...
        async_client = AsyncClient(ioteq_config=ioteq_config)
        response = await async_client.patch(api_name="cloudiot_device",request_body=body, request_params=params)
    ...
    def states_list(self,
        ...
        ioteq_config = self._config_manager.regional_config(request.parent)
        params = {'name':"organizations/{}/{}".format(ioteq_config.organization,request.parent), 'numStates':request.numStates}
        sync_client = SyncClient(ioteq_config=ioteq_config)
    ...
    def config_versions_list(self,
        ...
        ioteq_config = self._config_manager.regional_config(request.parent)
        params = {'name':"organizations/{}/{}".format(ioteq_config.organization,request.parent), 'numVersions':request.numVersions}
        sync_client = SyncClient(ioteq_config=ioteq_config)
