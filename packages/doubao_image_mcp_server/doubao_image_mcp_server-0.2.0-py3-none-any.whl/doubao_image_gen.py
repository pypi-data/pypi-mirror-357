...
class DoubaoImageGenerator:
    ...
    async def generate_image(
        ...
        try:
            ...
            # 收集生成信息
            # Try to get actual seed from API response, fallback to input seed
            # 尝试从API响应中获取实际的seed值，如果没有则使用输入的seed
            actual_seed = seed
            if hasattr(response, 'data') and response.data and hasattr(response.data[0], 'revised_prompt'):
                # Some APIs return actual seed in response, check if available
                # 某些API会在响应中返回实际的seed值
                if hasattr(response.data[0], 'seed'):
                    actual_seed = response.data[0].seed
