...
class RiskReducerStrategyDDI(RiskReducerStrategy):
    ...
    @staticmethod
    @abstractmethod
    def wwmf_discrimination(wwmf: xr.DataArray) -> xr.DataArray:
        pass
    ...
    def process_period(self):
        ...
        wwmf = self.wwmf_discrimination(
            self.parent.parent.params["WWMF__SOL"].compute()
        )
...
class RiskReducerStrategySnow(RiskReducerStrategyDDI):
    ...
    @staticmethod
    def wwmf_discrimination(wwmf: xr.DataArray) -> xr.DataArray:
        return Wwmf.is_snow(wwmf) & (wwmf != 58)


class RiskReducerStrategyRain(RiskReducerStrategyDDI):
    intensity_parameter: ClassVar[str] = "EAU3__SOL"
