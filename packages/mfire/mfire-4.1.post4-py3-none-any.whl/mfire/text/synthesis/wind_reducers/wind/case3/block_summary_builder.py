...
class BlockSummaryBuilder(BaseSummaryBuilderMixin, SummaryKeysMixin, ABC):
    ...
    def compute(self, reference_datetime: Datetime, dataset: xr.Dataset) -> dict:
        wi_describers: list[str] = self._wind_intensities_describer(dataset)
        ...
        self._summary.update(
            {
                ...
                # Else,
                self.PCI_K: [
                    self._pci[i].summarize(reference_datetime, wi_describers[i])
