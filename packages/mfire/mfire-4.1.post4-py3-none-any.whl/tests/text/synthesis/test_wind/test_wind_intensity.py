...
class TestWindIntensity:
    ...
    def test_text_with_attenuation_exception(self, force):
        wi: WindIntensity = WindIntensity(force)
    ...
    def test_is_juxtaposed_with(self, f0, f1, expected):
        wi_0 = WindIntensity(f0)
        wi_1 = WindIntensity(f1)
    ...
    def test_speed_min(self):
        assert WindIntensity(35).speed_min == 25.0
        assert WindIntensity(70.0).speed_min == 25.0

    @pytest.mark.parametrize(
        "valid_time, data_wf, wind_intensity_exp",
        [
            (
    ...
    def test_creation_from_term(self, valid_time, data_wf, wind_intensity_exp):
        composite = CompositeFactory1x100().get(
            valid_time=valid_time, data_wind=data_wf
        )
        dataset = composite.weather_data()
        data_array: xr.DataArray = dataset["wind"].sel(valid_time=valid_time[0])
        wind_intensity: WindIntensity = WindIntensity.from_term_data_array(data_array)
        assert wind_intensity == wind_intensity_exp

    def test_comparison(self):
        assert WindIntensity(25.0) == WindIntensity(34.9)
        assert WindIntensity(35.0) == WindIntensity(49.9)
        assert WindIntensity(50.0) == WindIntensity(69.9)
        assert WindIntensity(70.2) == WindIntensity(90.8)

        assert WindIntensity(30.0) <= WindIntensity(35.0)
        assert WindIntensity(30.0) <= WindIntensity(40.0)
        assert WindIntensity(60.0) >= WindIntensity(40.0)
        assert WindIntensity(60.0) >= WindIntensity(54.0)

        assert WindIntensity(30.0) < WindIntensity(40.0)
        assert WindIntensity(60.0) > WindIntensity(49.0)
        assert WindIntensity(80.0) > WindIntensity(67.6)


class TestPci:
    WIND_INTENSITY = WindIntensity(37.0)
    WIND_PCI = Pci(
        ...
        [
            ...
            (
                Pci(
                    begin_time=Datetime(2023, 1, 1, 0, 0, 0),
                    end_time=Datetime(2023, 1, 1, 4, 0, 0),
                    wi=WindIntensity(34.9),
                ),
                Pci(
                    begin_time=Datetime(2023, 1, 1, 6, 0, 0),
                    end_time=Datetime(2023, 1, 1, 10, 0, 0),
                    wi=WindIntensity(51.0),
                ),
                False,
            ),
        ],
