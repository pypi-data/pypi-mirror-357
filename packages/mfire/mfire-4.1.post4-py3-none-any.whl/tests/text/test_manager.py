...
class TestManagerValidation:
    ...
    def test_compute_synthesis(self, period, assert_equals_result):
        inputs_dir = self.inputs_dir / "synthesis"
        config = JsonFile(inputs_dir / f"prod_task_config_{period}_fr.json").load()
    ...
    def test_compute_risk(self, period, root_path_cwd, assert_equals_result):
        np.random.seed(0)
        inputs_dir = self.inputs_dir / "risk"
        config = JsonFile(inputs_dir / period / "prod_task_config_fr.json").load()
