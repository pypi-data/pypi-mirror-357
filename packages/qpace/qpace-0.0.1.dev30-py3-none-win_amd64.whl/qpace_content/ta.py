...
`accdist`
    ...
    return _lib.Incr_fn_accdist_fc6dd3(ctx).collect()
...
`accdist`
    ...
    def __init__(self, ctx: Ctx):
        self.ctx = ctx
        self.inner = _lib.Incr_fn_accdist_fc6dd3(ctx)
...
`rsi`
    ...
    return _lib.Incr_fn_rsi_baa024(ctx).collect(_5620_src=src, _5621_length=length)
...
`rsi`
    ...
    def __init__(self, ctx: Ctx):
        self.ctx = ctx
        self.inner = _lib.Incr_fn_rsi_baa024(ctx)
        self.locals = RsiLocals(self.inner)

    def next(self, src: float, length: Optional[int]) -> Optional[float]:
        return self.inner.next(_5620_src=src, _5621_length=length)
