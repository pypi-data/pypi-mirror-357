...
def _wrap_elem(nodes, elem_fun, load_fun, elem):
    xyz = nodes[elem.nodes].T
    me, ke, fe = elem_fun(xyz)
    if load_fun is not None:
        fe = load_fun(xyz, elem.nodes)
    return elem.nodes, (me, ke, fe)
...
def poisson_neumann(
    ...
    with mp.Pool(threads) as pool:
        for conn, (me, ke, fe) in pool.imap_unordered(
                ...
                elements,
                chunk):
            _assemble_matrix(Ka, ke, conn, ndf)
            _assemble_vector(Fa, fe, conn, ndf)
...
def poisson_neumann(
    ...
    tic = perf_counter()
    with mp.Pool(threads) as pool:
        for conn, (_, ke, fe) in pool.imap_unordered(
                partial(_wrap_elem, nodes, elem_fun, load_fun),
                elements,
                chunk):
            _assemble_triplet(conn, ke, rows, cols, data)
            _assemble_vector(Fa, fe, conn, ndf)
