...
class TestTeamMethods:
    ...
    def test_roster_position_properties(self):
        ...
        for forward in forwards:
            assert forward.position in ["C", "L", "R", "LW", "RW"], f"Forward {forward} has invalid position: {forward.position}"
        
        for defenseman in defensemen:
            assert defenseman.position == "D", f"Defenseman {defenseman} has invalid position: {defenseman.position}"
        
        for goalie in goalies:
            assert goalie.position == "G", f"Goalie {goalie} has invalid position: {goalie.position}"
    ...
    def test_roster_utility_methods(self):
        ...
        if players:
            for player in players:
                if player.sweater_number:
                    ...
                    assert found_player is not None, f"Should find player with number {player.sweater_number}"
        ...
        if players:
            ...
            assert found_player is not None, f"Should find player with name {player.full_name}"
            assert found_player == player, "Found player should be the same as original"
