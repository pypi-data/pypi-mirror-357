...
class Autoencoder:
    ...
    def fit(self, train: cvtda.neural_network.Dataset, val: typing.Optional[cvtda.neural_network.Dataset]):
        ...
        for epoch in range(self.n_epochs_):
            ...
            for images, features, diagram_idxs in cvtda.logging.logger().pbar(train_dl, desc = f"Epoch {epoch}"):
                ...
                pred = self.decode_(self.encode_(images, features, diagram_idxs, train))
                loss = torch.nn.functional.mse_loss(pred, images.to(self.device_), reduction = 'mean')
                loss.backward()
    ...
    def init_(
        ...
        self.encoder_base_ = cvtda.neural_network.NNBase(
        ...
        self.decoder_ = torch.nn.Sequential(
            torch.nn.InstanceNorm1d(self.latent_dim_), torch.nn.Linear(self.latent_dim_, 128 * w * h),
            torch.nn.InstanceNorm1d(128 * w * h), torch.nn.Unflatten(1, (128, w, h)), torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(128, 64, kernel_size = 3, stride = 2, output_padding = 1, padding = 1), torch.nn.InstanceNorm2d(64), torch.nn.ReLU(),
            torch.nn.Conv2d(64, 64, kernel_size = 3, padding = 1), torch.nn.InstanceNorm2d(64), torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(64, 32, kernel_size = 3, stride = 2, output_padding = 1, padding = 1), torch.nn.InstanceNorm2d(32), torch.nn.ReLU(),
            torch.nn.Conv2d(32, 16, kernel_size = 3, padding = 1), torch.nn.InstanceNorm2d(16), torch.nn.ReLU(),
            torch.nn.Conv2d(16, images.shape[1], kernel_size = 3, padding = 1), torch.nn.Sigmoid()
        ).to(self.device_).train()
        ...
        cvtda.logging.logger().print(f'Encoder Base parameters: {sum(p.numel() for p in self.encoder_base_.parameters())}')
    ...
    def decode_(self, latent_vector: torch.Tensor) -> torch.Tensor:
        latent_vector = latent_vector.to(self.device_)
        return self.decoder_(latent_vector)
