...
from tensorkube.helpers import sanitise_assumed_role_arn, track_event, sanitise_name, is_valid_email
...
def configure(vpc_public_subnets, vpc_private_subnets, legacy, test, override_policy_checks):
    ...
    if legacy:
        ...
        try:
            ...
            user_alert_email = click.prompt(click.style("Please enter your email address to receive cluster issue alerts", bold=True, fg=CliColors.INFO.value), type=str)
            if not is_valid_email(user_alert_email):
                click.echo(click.style("Invalid email address. Please enter a valid email address.", fg=CliColors.ERROR.value))
                return
            cfn_configure(test, None)
