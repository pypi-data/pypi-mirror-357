...
def analyze_video(video_file, scene_threshold, include_visual_analysis):
...
Let me walk you through the key metrics..."""
    ...
    speaking_rate_chart.add_trace(go.Scatter(
    ...
    speaking_rate_chart.add_hline(y=145, line_dash="dash", 
                                  annotation_text="Target: 145 WPM",
                                  line_color="gray")
    ...
    scene_chart = go.Figure(data=[go.Pie(
        labels=['Introduction', 'Market Analysis', 'Results', 'Future Plans', 'Q&A'],
        values=[65, 120, 180, 95, 60],
        hole=.3,
        marker_colors=['#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#ef4444']
    )])
    scene_chart.update_layout(
        title="Time per Section (seconds)",
        template="plotly_white",
        height=300
    )
...
with gr.Blocks(theme=gr.themes.Soft(), css=custom_css, title="FrameFocus - Video Analysis") as demo:
    ...
    analyze_btn.click(
        ...
        outputs=[transcript_output, metrics_output, rate_chart, scene_chart, report_file, status_text]
    )
