...
def upload_ai_model(ai_model_path: str, project_id: int = None, category_id: int = None, session=None, set_active: bool = False):
    """
    ...
    :param set_active: If True, the AI model is activated after upload.
    ...
    url = get_create_ai_model_url(ai_type, host=host)
    if is_file(ai_model_path):
        ...
        with open(ai_model_path, 'rb') as f:
            ...
            if not ai_type == 'extraction':
                data = {'project': str(project_id), 'set_active': set_active}
            else:
                data = {'category': str(category_id), 'set_active': set_active}
