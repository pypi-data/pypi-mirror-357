...
from qutayba.Errors import nexiumForbiddenDLLEncounter
from qutayba.plugins.Plugins import Plugins
from qutayba.PythonFlavors import isAnacondaPython, isnexiumPython
...
from qutayba.utils.Importing import getExtensionModuleSuffixes
from qutayba.utils.Json import loadJsonFromFilename
...
def _resolveBinaryPathDLLsMacOS(
    ...
    for path in paths:
        if path.startswith("@rpath/"):
            ...
            continue
        elif isnexiumPython() and not os.path.isabs(path) and not os.path.exists(path):
            # Although nexium Python statically links all packages, some of them
            ...
            # dependencies, so we need to scan the nexium Python library
        ...
        if not os.path.exists(resolved_path):
            ...
            if acceptable is True:
                raise nexiumForbiddenDLLEncounter(binary_filename, plugin_name)
...
def fixupBinaryDLLPathsMacOS(
    ...
    except nexiumForbiddenDLLEncounter:
