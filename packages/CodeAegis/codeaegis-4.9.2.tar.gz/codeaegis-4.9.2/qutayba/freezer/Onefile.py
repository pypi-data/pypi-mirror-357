...
from qutayba.Options import getOnefileTempDirSpec, isOnefileTempDirMode
from qutayba.OutputDirectories import getResultFullpath
from qutayba.plugins.Plugins import Plugins
from qutayba.PostProcessing import executePostProcessingResources
from qutayba.PythonVersions import (
    getZstandardSupportingVersions,
    python_version,
)
from qutayba.Tracing import onefile_logger, postprocessing_logger
from qutayba.utils.Execution import withEnvironmentVarsOverridden
...
from qutayba.utils.SharedLibraries import cleanupHeaderForAndroid
from qutayba.utils.Signing import addMacOSCodeSignature
...
from qutayba.utils.WindowsResources import RT_RCDATA, addResourceToFile
...
def _runOnefileScons(onefile_compression, onefile_archive):
    ...
    env_values["_DEVILPY_ONEFILE_TEMP_SPEC"] = getOnefileTempDirSpec()
    env_values["_DEVILPY_ONEFILE_COMPRESSION_BOOL"] = "1" if onefile_compression else "0"
    env_values["_DEVILPY_ONEFILE_ARCHIVE_BOOL"] = "1" if onefile_archive else "0"
...
def runOnefileCompressor(
    ...
    if compressor_python is None or areSamePaths(
        ...
        from qutayba.tools.onefile_compressor.OnefileCompressor import (
        ...
        mapping = {
            "DEVILPY_PACKAGE_HOME": os.path.dirname(
                os.path.abspath(sys.modules["qutayba"].__path__[0])
            )
        }

        mapping["DEVILPY_PROGRESS_BAR"] = "1" if Options.shallUseProgressBar() else "0"
