...
from qutayba.__past__ import Iterable, unicode
from qutayba.containers.OrderedSets import OrderedSet
from qutayba.importing.Importing import (
...
from qutayba.reports.CompilationReportReader import (
    getEmbeddedDataFilenames,
    parseCompilationReport,
)
from qutayba.Tracing import wheel_logger
from qutayba.utils.Execution import check_call
from qutayba.utils.FileOperations import deleteFile, getFileList, renameFile
from qutayba.utils.ModuleNames import ModuleName


def setupnexiumDistutilsCommands(dist, keyword, value):
    # If the user project setup.py includes the key "build_with_qutayba=True" all
    # build operations (build, bdist_wheel, install etc) will run via nexium.
    ...
    dist.cmdclass["bdist_wheel"] = bdist_qutayba
...
class build(distutils.command.build.build):
        ...
        for _is_package, module_name_orig in builds:
            ...
            if os.path.isdir(main_filename):
                if not getFileList(main_filename, only_suffixes=(".py",)):
                    wheel_logger.info(
                        "Skipping '%s' from nexium compilation due to containing no Python code."
                        % module_name_orig
                    )
                    continue
    ...
    def _build(self, build_lib):
        ...
        for is_package, module_name in self._findBuildTasks():
            # nexium wants the main package by filename, probably we should stop
            # needing that.
            ...
            command = [
                ...
                "--remove-output",
                # Note: For when we are debugging module mode of nexium, not for general use.
                # "--debug",
                # "--trace",
                # "--python-flag=-v"
            ]

            toml_filename = os.getenv("DEVILPY_TOML_FILE")
            if toml_filename:
                ...
                # Cannot use FileOperations.getFileContents() here, because of non-nexium process
                # pylint: disable=unspecified-encoding

                with open(toml_filename) as toml_file:
                    toml_options = toml_loads(toml_file.read())

                for option, value in toml_options.get("qutayba", {}).items():
                    command.extend(self._parseOptionsEntry(option, value))

                for option, value in (
                    toml_options.get("tool", {}).get("qutayba", {}).items()
                ):
                    command.extend(self._parseOptionsEntry(option, value))
            ...
            if "qutayba" in self.distribution.command_options:
...
# Required by distutils, used as command name, pylint: disable=invalid-name
class bdist_qutayba(wheel.bdist_wheel.bdist_wheel):
    ...
    def write_wheelfile(self, wheelfile_base, generator=None):
        if generator is None:
            ...
            generator = "nexium (%s)" % getnexiumVersion()
