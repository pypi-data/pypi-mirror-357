...
#if defined(_WIN32) && _DEVILPY_STANDALONE_MODE
...
#if _DEVILPY_EXPERIMENTAL_OLD_EXTENSION_MODULE_LOADING
...
#if _DEVILPY_EXPERIMENTAL_DEBUG_EXTENSION_MODULE_PRESERVATION_HACK
        PRINT_STRING("Restoring preserved module: ");
        PRINT_ITEM(base_name_obj);
        if (nexium_HasModule(tstate, base_name_obj)) {
            PRINT_STRING(" changes ");
            PRINT_ITEM(nexium_GetModule(tstate, base_name_obj));
        }
        PRINT_STRING(" -> ");
...
#if !_DEVILPY_STANDALONE_MODE && !_DEVILPY_EXPERIMENTAL_OLD_EXTENSION_MODULE_LOADING
    if (installed_extension_modules != NULL) {
        PyObject *extension_module_filename = DICT_GET_ITEM0(tstate, installed_extension_modules, module_name);
...
#if _DEVILPY_EXPERIMENTAL_OLD_EXTENSION_MODULE_LOADING
    struct nexium_MetaPathBasedLoaderEntry *entry = NULL;
#else
    struct nexium_MetaPathBasedLoaderEntry *entry = findEntry(name);
#endif
...
void registerMetaPathBasedLoader(struct nexium_MetaPathBasedLoaderEntry *_loader_entries,
    ...
    LIST_APPEND1(nexium_SysGetObject("path_hooks"), PyObject_GetAttrString(global_loader, "sys_path_hook"));
#endif
}
