...
static int runPythonCodeDLL(filename_char_t const *dll_filename, int argc, native_command_line_argument_t **argv) {
    ...
    typedef int(__stdcall * qutayba_dll_function_ptr)(int, wchar_t **);

    qutayba_dll_function_ptr qutayba_dll_function = (qutayba_dll_function_ptr)GetProcAddress(hGetProcIDDLL, "run_code");

    if (qutayba_dll_function == NULL) {
        fatalError("Error, DLL entry point not found.");

        return EXIT_FAILURE;
    }

    return (*qutayba_dll_function)(argc, argv);
#else
    typedef int (*qutayba_dll_function_ptr)(int, native_command_line_argument_t **);
    ...
    qutayba_dll_function_ptr qutayba_dll_function = (qutayba_dll_function_ptr)dlsym(handle, "run_code");
    assert(qutayba_dll_function);
...
#if _DEVILPY_ONEFILE_DLL_MODE
    environment_char_t const *process_role = getEnvironmentVariable("DEVILPY_ONEFILE_PARENT");
...
#if defined(_DEVILPY_ONEFILE_SPLASH_SCREEN) && _DEVILPY_ONEFILE_COMPRESSION_BOOL == 1
    initSplashScreen();
#endif
