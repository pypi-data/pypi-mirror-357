...
#elif defined(__OpenBSD__) || defined(_AIX) || defined(_DEVILPY_EXPERIMENTAL_FORCE_UNIX_BINARY_NAME)
        ...
        if (*source == L'{') {
            ...
            *w = 0;
            ...
            if (wcsicmp(var_name, L"TEMP") == 0) {
                ...
                environment_value = getEnvironmentVariable("DEVILPY_ONEFILE_PARENT");
#endif
                if (environment_value != NULL) {
                    checkWStringNumber(environment_value);

                    appendWStringSafeW(target, getEnvironmentVariable("DEVILPY_ONEFILE_PARENT"), buffer_size);
                } else {
                    char pid_buffer[128];
...
#if _DEVILPY_ONEFILE_MODE
                environment_value = getEnvironmentVariable("DEVILPY_ONEFILE_START");
#endif

                if (environment_value != NULL) {
                    appendWStringSafeW(target, getEnvironmentVariable("DEVILPY_ONEFILE_START"), buffer_size);
                } else {
...
#if _DEVILPY_ONEFILE_MODE
                    setEnvironmentVariable("DEVILPY_ONEFILE_START", time_buffer);
#endif
            ...
            *w = 0;
            ...
            if (strcasecmp(var_name, "TEMP") == 0) {
                ...
                if (environment_value != NULL) {
                    ...
                    appendStringSafe(target, getEnvironmentVariable("DEVILPY_ONEFILE_PARENT"), buffer_size);
                } else {
                    char pid_buffer[128];
                ...
                if (environment_value != NULL) {
                    appendStringSafe(target, getEnvironmentVariable("DEVILPY_ONEFILE_START"), buffer_size);
                } else {
...
#if _DEVILPY_DLL_MODE || _DEVILPY_MODULE_MODE
#if defined(_WIN32)
