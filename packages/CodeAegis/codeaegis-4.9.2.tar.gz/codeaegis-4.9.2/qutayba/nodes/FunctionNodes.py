...
from qutayba import Options, Variables
from qutayba.Constants import isMutable
from qutayba.optimizations.TraceCollections import (
    TraceCollectionPureFunction,
    withChangeIndicationsTo,
)
from qutayba.PythonVersions import python_version
from qutayba.specs.ParameterSpecs import (
    ParameterSpec,
    TooManyArguments,
    matchCall,
)
from qutayba.Tracing import optimization_logger, printError
from qutayba.tree.Extractions import updateVariableUsage
from qutayba.tree.SourceHandling import readSourceLines
from qutayba.tree.TreeHelpers import makeDictCreationOrConstant2
...
class ExpressionFunctionRef(ExpressionNoSideEffectsMixin, ExpressionBase):
    ...
    def getFunctionBody(self):
        if self.function_body is None:
            ...
            from qutayba.ModuleRegistry import getModuleFromCodeName
...
class ExpressionFunctionCall(ChildrenHavingFunctionValuesTupleMixin, ExpressionBase):
    ...
    def computeExpression(self, trace_collection):
        ...
        if cost is not None and cost < 50:
            from qutayba.optimizations.FunctionInlining import (
