...
from qutayba import TreeXML
from qutayba.__past__ import unicode
from qutayba.build.DataComposerInterface import getDataComposerReportValues
from qutayba.build.SconsUtils import readSconsErrorReport
from qutayba.code_generation.ConstantCodes import getDistributionMetadataValues
from qutayba.containers.OrderedSets import OrderedSet
from qutayba.freezer.IncludedDataFiles import getIncludedDataFiles
from qutayba.freezer.IncludedEntryPoints import getStandaloneEntryPoints
from qutayba.freezer.Standalone import getRemovedUsedDllsInfo
from qutayba.importing.Importing import getPackageSearchPath
from qutayba.importing.Recursion import getRecursionDecisions
from qutayba.ModuleRegistry import (
    getDoneModules,
...
from qutayba.OutputDirectories import (
    getResultRunFilename,
    getSourceDirectoryPath,
    hasMainModule,
)
from qutayba.plugins.Plugins import getActivePlugins
...
from qutayba.Tracing import ReportingSystemExit, reports_logger
...
from qutayba.utils.MemoryUsage import getMemoryInfos
...
def _getReportInputData(aborted):
    ...
    qutayba_version = getnexiumVersion()
    qutayba_commercial_version = getCommercialVersion() or "not installed"

    qutayba_aborted = aborted

    qutayba_exception = sys.exc_info()
...
def writeCompilationReport(report_filename, report_input_data, diffable):
    ...
    exit_message = None
    if not report_input_data["qutayba_aborted"]:
        completion = "yes"
    elif report_input_data["qutayba_exception"][0] is KeyboardInterrupt:
        completion = "interrupted"
    elif report_input_data["qutayba_exception"][0] is SystemExit:
        completion = "error exit (%s)" % report_input_data["qutayba_exception"][1].code
    elif report_input_data["qutayba_exception"][0] is ReportingSystemExit:
        completion = (
            "error exit message (%s)" % report_input_data["qutayba_exception"][1].code
        )
        exit_message = report_input_data["qutayba_exception"][1].exit_message
    else:
        completion = "exception"

    root = TreeXML.Element(
        "qutayba-compilation-report",
        qutayba_version=report_input_data["qutayba_version"],
        qutayba_commercial_version=report_input_data["qutayba_commercial_version"],
...
_crash_report_filename = "qutayba-crash-report.xml"
...
def writeCompilationReports(aborted):
    ...
    if (
        ...
        # of reporting exit, these do not constitute definitive bugs of nexium but
