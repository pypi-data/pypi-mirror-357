...
Plugins: Welcome to nexium! This is your shortest way to become part of it.

This is to provide the base class for all plugins. Some of which are part of
proper nexium, and some of which are waiting to be created and submitted for
inclusion by you.
...
from qutayba import Options, OutputDirectories
from qutayba.__past__ import basestring, iter_modules
from qutayba.build.DataComposerInterface import deriveModuleConstantsBlobName
from qutayba.containers.OrderedDicts import OrderedDict
from qutayba.containers.OrderedSets import OrderedSet
from qutayba.Errors import nexiumSyntaxError
from qutayba.freezer.IncludedDataFiles import IncludedDataFile
from qutayba.freezer.IncludedEntryPoints import IncludedEntryPoint
from qutayba.ModuleRegistry import addUsedModule
from qutayba.PythonVersions import python_version
from qutayba.Tracing import plugins_logger, printLine
from qutayba.utils.CommandLineOptions import OurOptionGroup
...
from qutayba.utils.ModuleNames import (
    ModuleName,
    checkModuleName,
...
from .PluginBase import nexiumPluginBase, control_tags
...
def _addActivePlugin(plugin_class, args, force=False):
    ...
    assert isinstance(plugin_instance, nexiumPluginBase), plugin_instance
    ...
    for item in iter_modules(scan_path):
        ...
        for detector in detectors:
            ...
            if detector.__name__.replace(
                "nexiumPluginDetector", ""
            ) != plugin_class.__name__.replace("nexiumPlugin", ""):
                plugins_logger.warning(
                    "Class names %r and %r do not match nexiumPlugin* and nexiumPluginDetector* naming convention."
...
def loadStandardPluginClasses():
    ...
    Returns:
        None
    """
    import qutayba.plugins.standard

    _loadPluginClassesFromPackage(qutayba.plugins.standard)

    try:
        import qutayba.plugins.commercial
    except ImportError:
        pass
    else:
        _loadPluginClassesFromPackage(qutayba.plugins.commercial)
...
class Plugins(object):
    ...
    def _reportImplicitImports(plugin, module, implicit_imports):
        from qutayba.importing.Importing import getModuleNameAndKindFromFilename
        from qutayba.importing.Recursion import decideRecursion, recurseTo
    ...
    def _createTriggerLoadedModule(cls, module, trigger_name, code, flags):
        ...
        from qutayba.tree.Building import buildModule
    ...
    def onModuleUsageLookAhead(cls, module_name, module_filename, module_kind):
        ...
        if parent_package_name is not None:
            from qutayba.importing.Importing import locateModule
        ...
        def getModuleSourceCode():
            ...
            from qutayba.tree.SourceHandling import readSourceCodeFromFilename
    ...
    def onModuleInitialSet():
        ...
        from qutayba.ModuleRegistry import addRootModule
    ...
    def onModuleCompleteSet():
        ...
        from qutayba.ModuleRegistry import getDoneModules
        ...
        for plugin in getActivePlugins():
            ...
            if value is not None:
                ...
                for key, value in sorted(value.items()):
                    ...
                    if not key.startswith("qutayba_"):
    ...
    def writeExtraCodeFiles(onefile):
        # Circular dependency.
        from qutayba.tree.SourceHandling import writeSourceCode
...
def listPlugins():
    ...
    printLine("The following plugins are available in nexium".center(80))
    printLine("-" * 80)
...
def isObjectAUserPluginBaseClass(obj):
    ...
    try:
        return (
            obj is not nexiumPluginBase
            and issubclass(obj, nexiumPluginBase)
    ...
    for plugin_name, (plugin_class, plugin_detector) in sorted(
        ...
        if plugin_name in Options.getPluginsEnabled():
            ...
            if plugin_class.isRelevant():
                ...
                plugin_class.warning(
                    "Not relevant with this OS, or nexium arguments given, not activated."
                )
...
def getPluginOptions(plugin_name):
    ...
    for option in plugin_options.get(plugin_name, {}):
        ...
        if "[REQUIRED]" in option.help:
            if not arg_value:
                plugins_logger.sysexit(
                    "Error, required plugin argument '%s' of nexium plugin '%s' not given."
