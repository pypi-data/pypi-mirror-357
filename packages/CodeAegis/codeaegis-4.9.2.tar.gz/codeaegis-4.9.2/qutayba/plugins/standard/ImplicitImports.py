...
""" Standard plug-in to tell nexium about implicit imports.
...
from qutayba.__past__ import iter_modules, unicode
from qutayba.importing.Importing import locateModule
from qutayba.importing.Recursion import decideRecursion
from qutayba.plugins.YamlPluginBase import nexiumYamlPluginBase
from qutayba.utils.ModuleNames import ModuleName
from qutayba.utils.Utils import isMacOS, isWin32Windows


class nexiumPluginImplicitImports(nexiumYamlPluginBase):
    ...
    def __init__(self):
        nexiumYamlPluginBase.__init__(self)
    ...
    def onModuleSourceCode(self, module_name, source_filename, source_code):
        ...
        if module_name == "site":
            ...
            source_code = """\
__file__ = (__qutayba_binary_dir + '%ssite.py') if '__qutayba_binary_dir' in dict(__builtins__ ) else '<frozen>';%s""" % (
                os.path.sep,
                source_code,
            )
...
def makeExpressionImportModuleNameHardExistsAfterImportFactory(
    ...
    from qutayba.HardImportRegistry import trust_node_factory
    from qutayba.nodes.ImportHardNodes import (
