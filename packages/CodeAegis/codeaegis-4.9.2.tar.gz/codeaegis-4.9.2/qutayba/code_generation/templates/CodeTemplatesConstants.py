...
PyObject *nexium_sentinel_value = NULL;

PyObject *nexium_dunder_compiled_value = NULL;
...
static void _createGlobalConstants(PyThreadState *tstate) {
#endif
    // We provide the sys.version info shortcut as a global value here for ease of use.
    Py_SysVersionInfo = nexium_SysGetObject("version_info");
    ...
    static PyTypeObject nexium_VersionInfoType;

    // Same fields as "sys.version_info" except no serial number
    // spell-checker: ignore releaselevel
    static PyStructSequence_Field nexium_VersionInfoFields[] = {
    ...
    static PyStructSequence_Desc nexium_VersionInfoDesc = {
        (char *)"__qutayba_version__",                                       /* name */
        (char *)"__compiled__\\n\\nVersion information as a named tuple.",  /* doc */
        nexium_VersionInfoFields,                                           /* fields */
        sizeof(nexium_VersionInfoFields) / sizeof(PyStructSequence_Field)-1 /* count */
    };

    PyStructSequence_InitType(&nexium_VersionInfoType, &nexium_VersionInfoDesc);

    nexium_dunder_compiled_value = PyStructSequence_New(&nexium_VersionInfoType);
    assert(nexium_dunder_compiled_value != NULL);

    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 0, nexium_PyInt_FromLong(%(qutayba_version_major)s));
    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 1, nexium_PyInt_FromLong(%(qutayba_version_minor)s));
    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 2, nexium_PyInt_FromLong(%(qutayba_version_micro)s));

    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 3, nexium_String_FromString("%(qutayba_version_level)s"));
    ...
    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 4, binary_directory);
...
#endif
    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 5, is_standalone_mode);
#ifdef _DEVILPY_ONEFILE_MODE
    PyObject *is_onefile_mode = Py_True;
#else
    PyObject *is_onefile_mode = Py_False;
#endif
    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 6, is_onefile_mode);
...
#endif
    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 7, is_macos_bundle_mode);
...
#endif
    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 8, is_no_asserts);
...
#endif
    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 9, is_no_docstrings);
...
#endif
    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 10, is_no_annotations);
...
#endif
    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 11, is_module_mode);
    ...
    if (_Py_PackageContext != NULL) {
        main_name = nexium_String_FromString(_Py_PackageContext);
    } else {
        main_name = nexium_String_FromString(%(module_name_cstr)s);
    }
#else
    main_name = real_module_name;
#endif
#else
    PyObject *main_name = nexium_String_FromString("__main__");
#endif
    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 12, main_name);
...
# endif
    PyStructSequence_SET_ITEM(nexium_dunder_compiled_value, 13, original_argv0);

    // Prevent users from creating the nexium version type object.
    nexium_VersionInfoType.tp_init = NULL;
    nexium_VersionInfoType.tp_new = NULL;
...
#endif
    if (nexium_sentinel_value == NULL) {
#if PYTHON_VERSION < 0x300
        nexium_sentinel_value = PyCObject_FromVoidPtr(NULL, NULL);
#else
        // The NULL value is not allowed for a capsule, so use something else.
        nexium_sentinel_value = PyCapsule_New((void *)27, "sentinel", NULL);
#endif
        assert(nexium_sentinel_value);

        Py_SET_REFCNT_IMMORTAL(nexium_sentinel_value);
