...
from qutayba.utils.CStrings import encodePythonIdentifierToC
...
def generateAssignmentVariableCode(statement, emit, context):
    ...
    if variable.isModuleVariable():
        ...
        if source_shape is tshape_bool and variable_declaration.c_type == "qutayba_bool":
            tmp_name = context.allocateTempName("assign_source", "qutayba_bool")
        elif (
            source_shape is tshape_int_or_long
            and variable_declaration.c_type == "qutayba_ilong"
        ):
            tmp_name = context.allocateTempName("assign_source", "qutayba_ilong")
        else:
            tmp_name = context.allocateTempName("assign_source")
