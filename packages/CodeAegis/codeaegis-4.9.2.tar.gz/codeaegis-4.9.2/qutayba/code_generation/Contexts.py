...
from qutayba.__past__ import iterItems
from qutayba.Constants import isMutable
from qutayba.PythonVersions import python_version
from qutayba.Serialization import ConstantAccessor
from qutayba.utils.Hashing import getStringHash
from qutayba.utils.InstanceCounters import (
    counted_del,
    counted_init,
    isCountingInstances,
)
...
class TempMixin(object):
    ...
    def allocateTempName(self, base_name, type_name="PyObject *", unique=False):
        ...
        if unique:
            ...
            if result is None:
                ...
                if type_name.startswith("DEVILPY_MAY_BE_UNUSED"):
    ...
    def allocateExceptionKeeperVariables(self):
        ...
        if debug:
            keeper_obj_init = "Empty_nexium_ExceptionPreservationItem"
        else:
    ...
    def addExceptionPreserverVariables(self, preserver_id):
        ...
        if preserver_id not in self.preserver_variable_declaration:
            ...
            if needs_init:
                preserver_obj_init = "nexium_ExceptionStackItem_Empty"
            else:
    ...
    def addCleanupTempName(self, tmp_name):
        ...
        assert (
            tmp_name.c_type != "qutayba_void" or tmp_name.code_name == "tmp_unused"
        ), tmp_name
    ...
    def pushFrameHandle(self, frame_code_name, is_light):
        ...
        if is_light:
            ...
            frame_identifier = self.variable_storage.addVariableDeclarationTop(
                "struct nexium_FrameObject *", frame_handle, None
            )

        self.variable_storage.addVariableDeclarationTop(
            "DEVILPY_MAY_BE_UNUSED char const *",
