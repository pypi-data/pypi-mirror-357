...
""" CType classes for PyObject *, PyObject **, and struct nexium_CellObject *

"""

from qutayba.__past__ import iterItems, xrange
from qutayba.code_generation.ErrorCodes import (
    getErrorExitBoolCode,
    getReleaseCode,
)
from qutayba.code_generation.templates.CodeTemplatesVariables import (
...
from qutayba.Constants import getConstantValueGuide, isMutable
...
"""
    ...
    @classmethod
    def emitAssignmentCodeTonexiumBool(
        cls, to_name, value_name, needs_check, emit, context
    ):
        ...
        emit(
            "%s = %s == 0 ? DEVILPY_BOOL_FALSE : DEVILPY_BOOL_TRUE;"
            % (to_name, truth_name)
        )
...
class CTypePyObjectPtr(CPythonPyObjectPtrBase):
    ...
    def getCellObjectAssignmentCode(cls, target_cell_code, variable_code_name, emit):
        emit("%s = nexium_Cell_New0(%s);" % (target_cell_code, variable_code_name))
...
class CTypeCellObject(CTypeBase):
    c_type = "struct nexium_CellObject *"
    ...
    def getInitValue(cls, init_from):
        ...
        if init_from is not None:
            return "nexium_Cell_New1(%s)" % init_from
        else:
            return "nexium_Cell_NewEmpty()"
    ...
    def emitValueAccessCode(cls, value_name, emit, context):
        ...
        return VariableDeclaration(
            "PyObject *", "nexium_Cell_GET(%s)" % value_name, None, None
        )

    @classmethod
    def getVariableArgDeclarationCode(cls, variable_code_name):
        return "struct nexium_CellObject *%s" % variable_code_name
