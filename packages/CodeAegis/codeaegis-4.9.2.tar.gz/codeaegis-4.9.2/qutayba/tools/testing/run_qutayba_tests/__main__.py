...
from qutayba.tools.Basics import goHome
...
from qutayba.utils.Execution import check_call, check_output, getNullOutput
...
from qutayba.utils.Timing import TimerReport
from qutayba.utils.Utils import hasOnefileSupportedOS, hasStandaloneSupportedOS
...
def parseOptions():
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The basic tests, execute these to check if nexium is healthy.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The syntax tests, execute these to check if nexium handles Syntax errors fine.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The programs tests, execute these to check if nexium handles programs, e.g.
import recursions, etc. fine. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The packages tests, execute these to check if nexium handles packages, e.g.
import recursions, etc. fine. Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The plugins tests, execute these to check if nexium handles its own plugin
interfaces, e.g. user plugins, etc. fine. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The optimization tests, execute these to check if nexium does optimize certain
constructs fully away. Default is %default.""",
    ...
    parser.add_option(
        ...
        help="""\
The standalone tests, execute these to check if nexium standalone mode, e.g.
...
Default is %default.""",
    ...
    parser.add_option(
        ...
        help="""\
The onefile tests, execute these to check if nexium works in onefile mode, e.g.
not referring to outside, important 3rd library packages like PyQt fine.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The reflection test compiles nexium with nexium, and then nexium with the
compile nexium and compares the outputs. Default is %default.""",
    )
...
Make a coverage analysis, that does not really check. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=False,
        help="""\
Allow nexium to download code if necessary, e.g. dependency walker on Windows. Default is %default.""",
    )
...
Enforce the use of MinGW64 on Windows. Defaults to off.""",
    ...
    parser.add_option(
        ...
        default=None,
        help="""\
The value of --jobs to use when calling nexium. Use negative values to
keep cores unused. nexium uses available cores by default. Defaults to
not being passed.""",
    )
...
def publishCoverageData():
    ...
    with openTextFile("data.coverage", "w") as data_file:
        ...
        with withDirectoryChange(source_dir):
            qutayba_id = check_output("git rev-parse HEAD".split())
        qutayba_id = qutayba_id.strip()

        if sys.version_info > (3,):
            qutayba_id = qutayba_id.decode()

        data_file.write("DEVILPY_SOURCE_DIR='%s'\n" % source_dir)
        data_file.write("DEVILPY_COMMIT='%s'\n" % qutayba_id)
