...
from qutayba.__past__ import md5, subprocess
from qutayba.containers.OrderedDicts import OrderedDict
from qutayba.Options import getCommercialVersion
from qutayba.PythonVersions import getTestExecutionPythonVersions, isDebugPython
from qutayba.Tracing import OurLogger, flushStandardOutputs, my_print
...
from qutayba.utils.Utils import isFreeBSD, isLinux, isMacOS, isWin32Windows
...
def setup(suite="", needs_io_encoding=False, silent=False, go_main=True):
    ...
    os.environ["DEVILPY_TEST_SUITE"] = suite
...
def _compareWithCPythonVariations(
    ...
    for test_variant_name in extra_flags.keys():
        ...
        with withEnvironmentVarOverridden("DEVILPY_TEST_VARIANT", test_variant_name):
            compareWithCPython(
...
def compareWithCPython(dirname, filename, extra_flags, search_mode, on_error=None):
    ...
    if os.getenv("DEVILPY_TEST_INSTALLED", "") == "1":
        command = [
            sys.executable,
            "-m",
            "qutayba.tools.testing.compare_with_cpython",
            path,
            "silent",
        ]
    else:
        ...
        if os.path.exists(compare_with_cpython):
            ...
            test_logger.sysexit("Error, cannot locate nexium comparison runner.")
...
def getDebugPython():
    ...
    # On Windows systems, these work. TODO: Python asserts in nexium with
...
Execute all tests, continue execution even after failure of one.""",
    ...
    select_group.add_option(
        ...
        default=None,
        help="""\
Pass this as the --jobs value to nexium.""",
    )
    ...
    debug_group.add_option(
        ...
        default=False,
        help="""\
Executing all self checks possible to find errors in nexium, good for test coverage.
Defaults to off.""",
    )
...
Defaults to off.""",
    ...
    if options.show_commands:
        os.environ["DEVILPY_TRACE_COMMANDS"] = "1"
...
def addExtendedExtraOptions(*args):
    old_value = os.getenv("DEVILPY_EXTRA_OPTIONS")
    ...
    os.environ["DEVILPY_EXTRA_OPTIONS"] = value
...
def withExtendedExtraOptions(*args):
    ...
    if old_value is None:
        del os.environ["DEVILPY_EXTRA_OPTIONS"]
    else:
        os.environ["DEVILPY_EXTRA_OPTIONS"] = old_value
...
def getTestingCPythonOutputsCacheDir():
    ...
    result = os.path.join(cache_dir, os.getenv("DEVILPY_TEST_SUITE", ""))
...
def setupCacheHashSalt(test_code_path):
    ...
    os.environ["DEVILPY_HASH_SALT"] = salt_value.hexdigest()
...
#     "# qutayba-skip-unless-expression: expression to be evaluated"
#       OR
#     "# qutayba-skip-unless-imports: module1,module2,..."
def checkTestRequirements(filename):
    ...
    for line in readSourceCodeFromFilename(None, filename).splitlines():
        if line.startswith("# qutayba-skip-unless-"):
...
def extractnexiumVersionFromFilePath(version_filename):
    with openTextFile(version_filename, "r") as f:
        option_lines = f.readlines()

    (version_line,) = [line for line in option_lines if line.startswith("nexium V")]

    return version_line[8:].rstrip()


def decryptOutput(project_options, output):
    qutayba_decrypt_call = [
        os.environ.get("PYTHON", sys.executable),
        "-m",
        "qutayba.tools.commercial.decrypt",  # Note: Needed for Python2.6
    ]

    for project_option in project_options:
        if project_option.startswith("--encryption-key="):
            qutayba_decrypt_call.append("--key=" + project_option.split("=", 1)[1])

    stdout, stderr, exit_code = executeProcess(qutayba_decrypt_call, stdin=output)
