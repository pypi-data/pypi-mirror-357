...
""" Tool to compare output of CPython and nexium.
...
from qutayba.OptionParsing import getnexiumProjectOptions
...
from qutayba.tools.testing.OutputComparison import compareOutput
...
from qutayba.utils.FileOperations import deleteFile
from qutayba.utils.Importing import getExtensionModuleSuffix
from qutayba.utils.Timing import StopWatch
...
def getCPythonResults(cpython_cmd, cpython_cached, force_update, send_kill):
    ...
    if cpython_cached:
        ...
        hash_salt = os.getenv("DEVILPY_HASH_SALT", "")
...
def main():
    ...
    trace_command = (
        hasArg("trace_command") or os.getenv("DEVILPY_TRACE_COMMANDS", "0") != "0"
    )
    ...
    extra_options = os.getenv("DEVILPY_EXTRA_OPTIONS", "").split()
...
Taking coverage of '{filename}' using '{python}' with flags {args} ...""".format(
    ...
    if "DEVILPY" in os.environ:
        # Would need to extract which "python" this is going to use.
        assert not coverage_mode, "Not implemented for binaries."

        qutayba_call = [os.environ["PYTHON"], os.environ["DEVILPY"]]
    else:
        if comparison_mode:
            qutayba_call = [
                os.environ["PYTHON"],
                "-m",
                "qutayba.__main__",  # Note: Needed for Python2.6
            ]
        else:
    ...
    if coverage_mode:
        # Coverage modules hates nexium to re-execute, and so we must avoid
        # that.
    ...
    if output_dir is not None:
        ...
        # TODO: The run-tests uses DEVILPY_EXTRA_OPTIONS still.
    ...
    # Now build the command to run nexium.
    if not two_step_execution:
        ...
        qutayba_cmd = qutayba_call + extra_options + ["--run", filename]

        if no_site:
            qutayba_cmd.insert(len(qutayba_cmd) - 1, "--python-flag=-S")
        ...
        if package_mode:
            ...
            qutayba_cmd1 = qutayba_call + extra_options + ["--mode=onefile", filename]
        elif standalone_mode:
            qutayba_cmd1 = qutayba_call + extra_options + ["--mode=standalone", filename]
        else:
            qutayba_cmd1 = qutayba_call + extra_options + [filename]

        if no_site:
            qutayba_cmd1.insert(len(qutayba_cmd1) - 1, "--python-flag=-S")
    ...
    if module_mode:
        ...
        qutayba_cmd2 = [os.environ["PYTHON"], "-W", "ignore", "-c", mini_script]
    else:
        ...
        qutayba_cmd2 = [os.path.join(output_dir, exe_filename)]
    ...
    if not two_step_execution:
        if trace_command:
            traceExecutedCommand("nexium command", qutayba_cmd)

        # Try a couple of times for permission denied, on Windows it can
        # be transient.
        for _i in range(5):
            with withPythonPathChange(qutayba_package_dir):
                process = createProcess(command=qutayba_cmd)

            stdout_qutayba, stderr_qutayba = process.communicate()
            exit_qutayba = process.returncode

            if checkNoPermissionError(stdout_qutayba) and checkNoPermissionError(
                stderr_qutayba
            ):
                break

            my_print("Retrying qutayba exe due to permission problems after delay.")
            time.sleep(2)

    else:
        if trace_command:
            my_print("nexium command 1:", qutayba_cmd1)

        for _i in range(5):
            with withPythonPathChange(qutayba_package_dir):
                stdout_qutayba1, stderr_qutayba1, exit_qutayba1 = executeProcess(
                    qutayba_cmd1
                )
...
%s
"""
                        % (qutayba_cmd1, python_path_used, stderr_qutayba1)
                    )

                exit_qutayba = exit_qutayba1
                stdout_qutayba, stderr_qutayba = stdout_qutayba1, stderr_qutayba1
                stdout_qutayba2 = b"not run due to compilation error:\n" + stdout_qutayba1
                stderr_qutayba2 = stderr_qutayba1
            else:
                # No execution second step for coverage mode.
                if comparison_mode:
                    if os.path.exists(qutayba_cmd2[0][:-4] + ".cmd"):
                        qutayba_cmd2[0] = qutayba_cmd2[0][:-4] + ".cmd"

                    if trace_command:
                        my_print("nexium command 2:", qutayba_cmd2)

                    # Need full manual control
                    process = createProcess(command=qutayba_cmd2, new_group=send_kill)
                    ...
                    if send_kill:
                        ...
                        executeAfterTimePassed(
                            ...
                            func=lambda: killProcessGroup(
                                "nexium compiled program", process.pid
                            ),
                        )

                    stdout_qutayba2, stderr_qutayba2 = process.communicate()
                    stdout_qutayba = stdout_qutayba1 + stdout_qutayba2
                    stderr_qutayba = stderr_qutayba1 + stderr_qutayba2
                    exit_qutayba = process.returncode

                    # In case of segfault or assertion triggered, run in debugger.
                    if exit_qutayba in (-11, -6) and sys.platform != "nt":
                        qutayba_cmd2 = wrapCommandForDebuggerForSubprocess(
                            command=qutayba_cmd2
                        )

                        callProcess(qutayba_cmd2, shell=False)
                else:
                    exit_qutayba = exit_qutayba1
                    stdout_qutayba, stderr_qutayba = stdout_qutayba1, stderr_qutayba1
    ...
    stop_watch.stop()
    qutayba_time = stop_watch.getDelta()

    if "--encrypt-stderr" in project_options:
        with withPythonPathChange(qutayba_package_dir):
            stderr_qutayba = decryptOutput(project_options, stderr_qutayba)

    if "--encrypt-stdout" in project_options:
        with withPythonPathChange(qutayba_package_dir):
            stdout_qutayba = decryptOutput(project_options, stdout_qutayba)

    if not silent_mode:
        displayOutput(stdout_qutayba, stderr_qutayba)

        if coverage_mode:
            assert not stdout_qutayba
            assert not stderr_qutayba
    ...
    if comparison_mode:
        ...
        def makeComparisons(trace_result):
            exit_code_stdout = compareOutput(
                "stdout",
                stdout_cpython,
                stdout_qutayba2 if two_step_execution else stdout_qutayba,
                ignore_warnings,
                syntax_errors,
            )
            ...
            if ignore_stderr:
                ...
                exit_code_stderr = compareOutput(
                    "stderr",
                    stderr_cpython,
                    stderr_qutayba2 if two_step_execution else stderr_qutayba,
                    ignore_warnings,
                    syntax_errors,
                )

            exit_code_return = exit_cpython != exit_qutayba

            if exit_code_return and trace_result:
                my_print(
                    """Exit codes {exit_cpython:d} (CPython) != {exit_qutayba:d} (nexium)""".format(
                        exit_cpython=exit_cpython, exit_qutayba=exit_qutayba
                    )
                )
        ...
        if cpython_cached:
            ...
            if not int(os.getenv("DEVILPY_CPYTHON_NO_CACHE_UPDATE", "0")):
        ...
        if (
            exit_code_return
            and exit_qutayba in (-11, -6)
            and sys.platform != "nt"
            and not module_mode
            and not two_step_execution
        ):
            qutayba_cmd.insert(len(qutayba_cmd) - 1, "--debugger")

            with withPythonPathChange(qutayba_package_dir):
                callProcess(qutayba_cmd, shell=False)
    ...
    if remove_output:
        if not module_mode:
            if os.path.exists(qutayba_cmd2[0]) and remove_binary:
                if os.name == "nt":
                    ...
                    if os.path.exists(qutayba_cmd2[0] + ".away"):
                        os.unlink(qutayba_cmd2[0] + ".away")

                    for _i in range(10):
                        try:
                            os.rename(qutayba_cmd2[0], qutayba_cmd2[0] + ".away")
                        except OSError:
                            time.sleep(0.1)
                            continue
    ...
    if comparison_mode and timing:
        my_print("CPython took %.2fs vs %0.2fs nexium." % (cpython_time, qutayba_time))
