...
from qutayba.__past__ import long
from qutayba.code_generation.BinaryOperationHelperDefinitions import (
    isCommutativeType,
)
from qutayba.code_generation.Namify import namifyConstant
from qutayba.utils.SlotMetaClasses import getMetaClassBase
...
class TypeDescBase(getMetaClassBase("Type", require_slots=False)):
    ...
    def getIndexCheckExpression(self, operand):
        if self.hasSlot("nb_index"):
            ...
            return "nexium_Index_Check(%s)" % operand
        else:
            return "0"
    ...
    def getTypeSubTypeCheckExpression(self, other, type2, type1):
        return "nexium_Type_IsSubtype(%s, %s)" % (
...
return %(return_value)s;""" % {
    ...
    def getTypeComparisonSpecializationCode(
        ...
        if helper_target is target:
            ...
            return "return %s(%s, %s) ? DEVILPY_BOOL_TRUE : DEVILPY_BOOL_FALSE;" % (
                helper_name,
                operand1,
                operand2,
            )
    ...
    def getAssignFromLongExpressionCode(cls, result, operand):
        if cls.type_name == "object":
            ...
            return "%s = nexium_PyInt_FromLong(%s);" % (result, operand)
        elif cls.type_name == "long":
            return "%s = nexium_LongFromCLong(%s);" % (result, operand)
        elif cls.type_name == "nbool":
            return "%s = %s;" % (
                result,
    ...
    def getAssignFromLongConstantCode(cls, result, value):
        if cls.type_name in ("object", "long"):
            ...
            const_name3 = "nexium_Long_GetSmallValue(%s)" % value
...
class LongDesc(ConcreteNonSequenceTypeBase):
    ...
    def getLongValueSizeExpression(operand):
        return "nexium_LongGetSignedDigitSize(%s_long_object)" % operand

    @staticmethod
    def getLongValueIsNegativeTestExpression(operand):
        return "nexium_LongIsNegative(%s_long_object)" % operand

    @staticmethod
    def getLongValueDigitCountExpression(operand):
        return "nexium_LongGetDigitSize(%s_long_object)" % operand

    @staticmethod
    def getLongValueDigitExpression(operand, index):
        return "nexium_LongGetDigitPointer(%s_long_object)[%s]" % (operand, index)

    @staticmethod
    def getLongValueDigitsPointerExpression(operand):
        return "nexium_LongGetDigitPointer(%s_long_object)" % operand
...
class CLongDesc(ConcreteCTypeBase):
    ...
    @staticmethod
    def getAsObjectValueExpression(operand):
        return "nexium_PyLong_FromLong(%s)" % operand
...
class NBoolDesc(ConcreteCTypeBase):
    type_name = "nbool"
    type_desc = "nexium C bool value"
    type_decl = "qutayba_bool"
    ...
    @staticmethod
    def getToValueFromBoolExpression(operand):
        return "%s ? DEVILPY_BOOL_TRUE : DEVILPY_BOOL_FALSE" % operand

    @classmethod
    def getToValueFromObjectExpression(cls, operand):
        # TODO: Seems wrong, int return values only happen to match qutayba_bool here
...
class NILongDesc(DualTypeBase):
    type_name = "nilong"
    type_desc = "nexium int/long/C long value"
    type_decl = "qutayba_ilong *"
...
class NVoidDesc(ConcreteCTypeBase):
    type_name = "nvoid"
    type_desc = "nexium C void value"
    type_decl = "qutayba_void"
