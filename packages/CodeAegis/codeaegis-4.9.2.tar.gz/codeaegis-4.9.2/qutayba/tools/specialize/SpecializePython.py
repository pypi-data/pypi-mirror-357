...
import qutayba.code_generation.BinaryOperationHelperDefinitions
import qutayba.code_generation.CodeGeneration
import qutayba.code_generation.ComparisonCodes
import qutayba.code_generation.Namify
import qutayba.nodes.NetworkxNodes
import qutayba.nodes.PackageMetadataNodes
import qutayba.nodes.PackageResourceNodes
import qutayba.nodes.SideEffectNodes
import qutayba.nodes.TensorflowNodes
...
import qutayba.specs.BuiltinTypeOperationSpecs
import qutayba.specs.HardImportSpecs
import qutayba.tree.Building
from qutayba.containers.OrderedDicts import OrderedDict
from qutayba.containers.OrderedSets import OrderedSet
from qutayba.nodes.ImportNodes import hard_modules_non_stdlib
from qutayba.nodes.NodeMetaClasses import NodeCheckMetaClass
...
from qutayba.utils.Jinja2 import getTemplate
...
def _getMixinForShape(shape):
    ...
    if shape is tshape_str:
        return "qutayba.nodes.ExpressionShapeMixins.ExpressionStrShapeExactMixin"
    elif shape is tshape_list:
        return "qutayba.nodes.ExpressionShapeMixins.ExpressionListShapeExactMixin"
    elif shape is tshape_tuple:
        return "qutayba.nodes.ExpressionShapeMixins.ExpressionTupleShapeExactMixin"
    elif shape is tshape_int:
        return "qutayba.nodes.ExpressionShapeMixins.ExpressionIntShapeExactMixin"
    elif shape is tshape_bool:
        return "qutayba.nodes.ExpressionShapeMixins.ExpressionBoolShapeExactMixin"
    elif shape is tshape_none:
        return "qutayba.nodes.ExpressionShapeMixins.ExpressionNoneShapeExactMixin"
    elif shape is tshape_dict:
        return "qutayba.nodes.ExpressionShapeMixins.ExpressionDictShapeExactMixin"
    elif shape is tshape_bytes:
        return "qutayba.nodes.ExpressionShapeMixins.ExpressionBytesShapeExactMixin"
    else:
        assert False, shape
...
processTypeShapeAttribute(
    "tshape_type",
    qutayba.specs.BuiltinTypeOperationSpecs,
    python2_type_methods,
    python3_type_methods,
)
...
def emitGenerationWarning(emit, doc_string, template_name):
    ...
    for spec_descriptions in getSpecVersions(qutayba.specs.HardImportSpecs):
...
def makeAttributeNodes():
    filename_python = "qutayba/nodes/AttributeNodesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit("from qutayba.specs.BuiltinParameterSpecs import extractBuiltinArgs")

        emit("from qutayba.nodes.ConstantRefNodes import makeConstantRefNode")
        emit(
            """\
from qutayba.nodes.NodeMakingHelpers import (
    ...
    makeRaiseExceptionReplacementExpression
        ...
        emit(
            "from qutayba.nodes.KeyValuePairNodes import makeKeyValuePairExpressionsFromKwArgs"
        )

        emit("from qutayba.nodes.AttributeNodes import makeExpressionAttributeLookup")

        emit("from qutayba.PythonVersions import getDictFromkeysNoArgErrorMessage")
...
def makeBuiltinOperationNodes():
    filename_python = "qutayba/nodes/BuiltinOperationNodeBasesGenerated.py"
...
def makeChildrenHavingMixinNodes():
    ...
    filename_python = "qutayba/nodes/ChildrenHavingMixins.py"
    filename_python2 = "qutayba/nodes/ExpressionBasesGenerated.py"
    filename_python3 = "qutayba/nodes/StatementBasesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit1(
            """
from qutayba.nodes.Checkers import (
    checkStatementsSequenceOrNone,
...
def makeHardImportNodes():
    ...
    filename_python = "qutayba/nodes/HardImportNodesGenerated.py"
