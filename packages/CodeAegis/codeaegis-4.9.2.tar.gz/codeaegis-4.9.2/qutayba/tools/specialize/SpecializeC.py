...
import qutayba.Options

qutayba.Options.is_full_compat = False

# isort:start

import os

import qutayba.specs.BuiltinBytesOperationSpecs
import qutayba.specs.BuiltinDictOperationSpecs
import qutayba.specs.BuiltinListOperationSpecs
import qutayba.specs.BuiltinStrOperationSpecs
import qutayba.specs.BuiltinUnicodeOperationSpecs
from qutayba.code_generation.BinaryOperationHelperDefinitions import (
    getSpecializedBinaryOperations,
    isCommutativeOperation,
    parseTypesFromHelper,
)
from qutayba.code_generation.c_types.CTypePyObjectPointers import (
    make_list_constant_direct_threshold,
    make_list_constant_hinted_threshold,
)
from qutayba.code_generation.CallCodes import (
    getQuickCallCode,
...
from qutayba.code_generation.ComparisonHelperDefinitions import (
    getSpecializedComparisonOperations,
)
from qutayba.code_generation.ImportCodes import getImportModuleHardCodeName
from qutayba.containers.OrderedSets import OrderedSet
from qutayba.HardImportRegistry import (
    hard_modules,
    hard_modules_non_stdlib,
    hard_modules_version,
)
from qutayba.nodes.shapes.BuiltinTypeShapes import (
    tshape_bool,
    tshape_bytes,
    tshape_dict,
    tshape_int,
...
from qutayba.utils.Jinja2 import getTemplateC
...
def getDoExtensionUsingTemplateC(template_name):
    return getTemplateC(
        package_name="qutayba.code_generation",
...
def makeHelpersComparisonOperation(operand, op_code):
    ...
    filename_c = "qutayba/build/static_src/HelpersComparison%s.c" % op_code.capitalize()
    filename_h = "qutayba/build/include/qutayba/helper/comparisons_%s.h" % op_code.lower()
...
def makeHelpersComparisonDualOperation(operand, op_code):
    ...
    filename_c = (
        "qutayba/build/static_src/HelpersComparisonDual%s.c" % op_code.capitalize()
    )
    filename_h = (
        "qutayba/build/include/qutayba/helper/comparisons_dual_%s.h" % op_code.lower()
    )
...
def makeHelpersBinaryOperation(operator, op_code):
    ...
    filename_c = (
        "qutayba/build/static_src/HelpersOperationBinary%s.c" % op_code.capitalize()
    )
    filename_h = (
        "qutayba/build/include/qutayba/helper/operations_binary_%s.h" % op_code.lower()
    )
...
def makeHelpersInplaceOperation(operator, op_code):
    ...
    filename_c = (
        "qutayba/build/static_src/HelpersOperationInplace%s.c" % op_code.capitalize()
    )
    filename_h = (
        "qutayba/build/include/qutayba/helper/operations_inplace_%s.h" % op_code.lower()
    )
...
def makeHelpersBinaryDualOperation(operand, op_code):
    ...
    filename_c = (
        "qutayba/build/static_src/HelpersOperationBinaryDual%s.c" % op_code.capitalize()
    )
    filename_h = (
        "qutayba/build/include/qutayba/helper/operations_binary_dual_%s.h"
        % op_code.lower()
    )
...
def makeHelpersImportHard():
    filename_c = "qutayba/build/static_src/HelpersImportHard.c"
    filename_h = "qutayba/build/include/qutayba/helper/import_hard.h"
...
def makeHelperCalls():
    filename_c = "qutayba/build/static_src/HelpersCallingGenerated.c"
    filename_h = "qutayba/build/include/qutayba/helper/calling_generated.h"
    ...
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
            ...
            template = getTemplateC(
                "qutayba.code_generation", "CodeTemplateCallsPositional.c.j2"
            )
            ...
            template = getTemplateC(
                "qutayba.code_generation", "CodeTemplateCallsMixed.c.j2"
            )
...
def makeHelperLists():
    filename_c = "qutayba/build/static_src/HelpersListsGenerated.c"
    filename_h = "qutayba/build/include/qutayba/helper/lists_generated.h"
    ...
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
            ...
            template = getTemplateC(
                "qutayba.code_generation", "CodeTemplateMakeListSmall.c.j2"
            )
            ...
            template = getTemplateC(
                "qutayba.code_generation", "CodeTemplateMakeListHinted.c.j2"
            )
...
generate_builtin_type_operations = [
    ...
    # nexium-Python eventually.
    (
        "tshape_dict",
        dict_desc,
        qutayba.specs.BuiltinDictOperationSpecs,
        ("pop", "popitem", "setdefault"),
    ),
    (
        "tshape_list",
        list_desc,
        qutayba.specs.BuiltinListOperationSpecs,
        (
            "pop",
    ...
    # that we do not have easy access to, but we might one day for nexium-Python
    # expose it for the static linking of it and then we could in fact call
    # these directly.
    (
        "tshape_str",
        str_desc,
        qutayba.specs.BuiltinStrOperationSpecs,
        (
            "strip",
            "rstrip",
            "lstrip",
    ...
    (
        "tshape_unicode",
        unicode_desc,
        qutayba.specs.BuiltinUnicodeOperationSpecs,
        (
            "strip",
            "rstrip",
            "lstrip",
    ...
    (
        "tshape_bytes",
        bytes_desc,
        qutayba.specs.BuiltinBytesOperationSpecs,
        (
            "capitalize",
            "center",
            "count",
...
def makeDictCopyHelperCodes():
    filename_c = "qutayba/build/static_src/HelpersDictionariesGenerated.c"
...
def _getCheckForShape(shape):
    ...
    if shape is tshape_str:
        return "nexium_String_CheckExact"
...
def makeHelperBuiltinTypeMethods():
    ...
    filename_c = "qutayba/build/static_src/HelpersBuiltinTypeMethods.c"
    filename_h = "qutayba/build/include/qutayba/helper/operations_builtin_types.h"
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
