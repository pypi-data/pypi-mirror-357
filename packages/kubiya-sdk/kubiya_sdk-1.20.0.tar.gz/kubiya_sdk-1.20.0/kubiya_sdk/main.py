...
def server(ctx, host, port, reload):
    ...
    try:
        ...
        with sentry_sdk.start_transaction(op="server", name="Start Kubiya SDK Server"):
            ...
            # Configure uvicorn to reduce noise from health checks and metrics
            # Option 1: Disable access logging entirely (uncomment to use)
            # uvicorn.run(app, host=host, port=port, reload=reload, access_log=False)
            
            # Option 2: Use custom filter to suppress health/metrics logs
            log_config = {
                "version": 1,
                ...
                "handlers": {
                    ...
                    "access": {
                        "formatter": "access",
                        "class": "kubiya_sdk.utils.logger.HealthCheckFilter",
                        "stream": "ext://sys.stderr",
                    },
                },
                "loggers": {
