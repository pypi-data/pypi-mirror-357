...
def get_side_menu(context: Context, using: str = "available_apps") -> List[Dict[str, Any]]:
    ...
    options = get_resolved_settings(context.get("request", None))
...
@register.simple_tag(takes_context=True)
def get_top_menu(context: Any, user: AbstractUser, admin_site: str = "admin") -> List[Dict]:
    """
...
@register.simple_tag(takes_context=True)
def get_user_menu(context: Any, user: AbstractUser, admin_site: str = "admin") -> List[Dict]:
    """
...
@register.simple_tag(takes_context=True)
def get_dashub_settings(context:Any) -> Dict:
    """
    Get Practet Dashboard settings, update any defaults from the request, and return
    """
    settings = get_resolved_settings(context.get("request", None))
...
@register.simple_tag(takes_context=True)
def get_user_avatar(context: Any, user: AbstractUser) -> str:
    """
...
@register.simple_tag(takes_context=True)
def get_changeform_template(context: Any, adminform: AdminForm) -> str:
    """
...
@register.simple_tag(takes_context=True)
def get_changeform_template_class(context: Any, adminform: AdminForm) -> str:
    """
    Go get the correct change form template based on the modeladmin being used,
    the default template, or the overridden one for this modeladmin
    """
