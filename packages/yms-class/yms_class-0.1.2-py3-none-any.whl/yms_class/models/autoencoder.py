...
class CustomDataset(Dataset):
    def __init__(self, root_dir, transform=None, class_to_label=None):
        ...
        self.images = [f for f in os.listdir(root_dir) if f.endswith('.npy')]
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            image = self.transform(torch.from_numpy(image.reshape(6, 100, 100)).float()).view(-1)
...
if __name__ == '__main__':
    ...
    train = DataLoader(data, batch_size=10, shuffle=True)
    for x, y in train:
