...
def _ray_status() -> dict[str, any]:
    ...
    """
    status: dict[str, any] = {"ray_nodes": 0, "ray_actors": 0}
    try:
    ...
    try:
        # this is requires connecting to the ray dashboard
        ...
        live_actors = [
            a.actor_id
            for a in actors
            if a.class_name == "ApplierActor" and a.state == "ALIVE"
        ]
        status["ray_applier_actors"] = len(live_actors)
