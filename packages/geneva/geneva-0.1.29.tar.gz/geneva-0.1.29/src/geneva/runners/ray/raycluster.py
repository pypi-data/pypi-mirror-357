...
class ClusterStatus:
    ...
    pbar_actors = attrs.field(default=None)

    last_cnt_k8s = attrs.field(default=0)
    last_cnt_pods = attrs.field(default=0)
    last_cnt_actors = attrs.field(default=0)
    ...
    def get_status(self) -> None:
        ...
        try:
            ...
            cnt_actors = ray_status.get("ray_actors", 0)
            if self.pbar_actors is None:
                fmt = "{l_bar}{bar} {n_fmt}/{total_fmt} [{elapsed}]"
                self.pbar_actors = tqdm(
                    total=self.concurrency, desc="Workers ready", bar_format=fmt
                )

            self.pbar_actors.update(cnt_actors - self.last_cnt_actors)
            self.last_cnt_actors = cnt_actors
        except Exception:
