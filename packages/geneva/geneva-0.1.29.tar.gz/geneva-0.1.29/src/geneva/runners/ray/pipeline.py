...
def _run_column_adding_pipeline(
    ...
    * use_cpu_only_pool: If True will force schedule cpu-only actors on cpu-only nodes.
    ...
    _LOG.info(f"setting input plan len to {len(input_plan)}")
    plan_tracker.set_total.remote(len(input_plan))
    ...
    _LOG.info(f"setting fragment count {len(fragments)}")
    ...
    def _fragment_is_done(frag_id: int) -> bool:
        ...
        return dedupe_key in checkpoint_store
        # TODO update count for previously checkpointed and now skipped batches
    ...
    def _commit(frag_id: int, fut: ObjectRef, commit_granularity: int) -> None:
        ...
        writer_tracker.update.remote(1)
        dedupe_key = _get_fragment_dedupe_key(ds.uri, frag_id, map_task)
...
def dispatch_run_ray_add_column(
    ...
    **kwargs,
    ...
    hist.set_running(job.job_id, cloudpickle.dumps(obj_ref))
    return RayJobFuture(
        job_id=job.job_id,
        ray_obj_ref=obj_ref,
        fragment_tracker=fragment_tracker,
