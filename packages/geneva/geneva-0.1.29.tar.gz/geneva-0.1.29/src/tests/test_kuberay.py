...
def test_ray_status_with_real_actorstate(monkeypatch) -> None:
    ...
    # none of the ApplierActors are ALIVE → zero applier actors
    assert status["ray_applier_actors"] == 0
    assert status["ray_actors"] == 0
...
def test_ray_status_with_real_actorstate_no_ray(monkeypatch) -> None:
    ...
    # none of the ApplierActors are ALIVE → zero applier actors
    assert status.get("ray_applier_actors") is None
    assert status["ray_actors"] == 0
