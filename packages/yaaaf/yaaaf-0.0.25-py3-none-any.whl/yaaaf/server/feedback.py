...
def save_feedback(arguments: FeedbackArguments) -> Dict[str, Any]:
    ...
    try:
        ...
        _logger.info(f"Found {len(notes_with_artifacts)} notes with artifact IDs: {[note.artefact_id for note in notes_with_artifacts]}")
                ...
                try:
                    ...
                    if artifact:  # Make sure artifact exists
                        artifact_data = {
                            ...
                            "data": artifact.data.to_html(index=False) if artifact.data is not None else "",
                            "code": artifact.code if artifact.code is not None else "",
                            "image": artifact.image if artifact.image is not None else "",
                            "description": artifact.description if artifact.description is not None else "",
                        }
                        artifacts.append(artifact_data)
                        processed_artifact_ids.add(note.artefact_id)
                        _logger.info(f"Successfully retrieved artifact {note.artefact_id} of type {artifact.type}")
                except Exception as e:
                    _logger.warning(f"Could not retrieve artifact {note.artefact_id}: {e}")
        ...
        for agent_config in config.agents:
            if isinstance(agent_config, AgentSettings):
                agent_info = {
                    ...
                    "temperature": agent_config.temperature if agent_config.temperature is not None else config.client.temperature,
                    "max_tokens": agent_config.max_tokens if agent_config.max_tokens is not None else config.client.max_tokens,
            ...
            "notes": [
                {
                    ...
                    "internal": getattr(note, 'internal', False),  # Handle backward compatibility
                }
                for note in notes
            ],
            "artifacts": artifacts,
