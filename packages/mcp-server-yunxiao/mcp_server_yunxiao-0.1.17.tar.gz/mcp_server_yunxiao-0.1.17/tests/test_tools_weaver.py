...
class TestToolsWeaver(unittest.TestCase):
    ...
    def test_query_task_message(self):
        """测试查询任务列表接口"""
        result = query_task_message(
            region="ap-guangzhou",
            task_id=[self.test_task_id]
        )
        self._assert_valid_json(result)

    def test_describe_error_codes(self):
        """测试查询错误码接口"""
        result = describe_error_codes(region="ap-guangzhou")
        self._assert_valid_json(result)

    def test_describe_host_task(self):
        """测试查询宿主机任务接口"""
        result = describe_host_task(
            region="ap-guangzhou",
            host=self.test_ip,
            vs_task_id=123456
        )
    ...
    def test_query_host_task_log(self):
        """测试查询宿主机任务日志接口"""
        result = query_host_task_log(
            region="ap-guangzhou",
    ...
    def test_query_task_log(self):
        """测试查询任务日志接口"""
        result = query_task_log(
            ip=self.test_ip,
            task_id=self.test_task_id
        )
        self._assert_valid_json(result)
    ...
    def test_thread_check(self):
        """测试线程检查接口"""
        result = thread_check(ip=self.test_ip)
        self._assert_valid_json(result)
