...
try:
    ...
    class Deployer:
        ...
        def execute_python_file(
            cls, python_file: str, args: List[str] = None
...
def start(
    ...
    if no_interactive:
        ...
        try:
            ...
            if files:
                ...
                for file_path in files:
                    if os.path.exists(file_path):
                        ...
                        console.print(
                            f"⚠️  File not found: {file_path}", style="yellow"
                        )
                        ...
                        if env_manager.venv_path:
                            ...
                            if os.name == "nt":  # Windows
                                activate_script = os.path.join(
                                    env_manager.venv_path, "Scripts", "activate"
                                )
                                cmd = f'"{activate_script}" && python "{script_path}"'
                                shell = True
                            else:  # Unix/macOS
                                activate_script = os.path.join(
                                    env_manager.venv_path, "bin", "activate"
                                )
                        ...
                        try:
                            if shell:
                                result = subprocess.run(
                                    cmd, shell=shell, cwd=repo_path, env=env
                                )
                            else:
                            ...
                            console.print("\n" + "-" * 50)
                            console.print(
                                f"Script completed with return code: {result.returncode}"
                            )
            ...
            console.print(
                "   To run a script: digy local <repo> <script.py> [--args ...]"
            )
            return 0
...
def status(ctx):
    ...
    table.add_row(
        "Memory Usage",
        f"{mem_stats['usage_mb']:.2f} MB / {mem_stats['total_mb']:.2f} MB",
    )
...
def docker(
    ...
    if ctx.obj.get("auth_type"):
        auth_provider = get_auth_provider(
            ctx.obj["auth_type"], config_file=ctx.obj.get("auth_config")
        )
    ...
    try:
        ...
        if not any(
            repo_url.startswith(proto) for proto in ["http://", "https://", "git@"]
        ):
...
def examples():
    ...
    """
    console.print(Panel(examples.strip(), title="DIGY Examples", border_style="blue"))
...
def info():
    ...
    """
    console.print(
        Panel(info_text.strip(), title="DIGY Information", border_style="green")
    )
