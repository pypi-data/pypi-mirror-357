...
from relationalai.early_access.dsl.bindings.common import Binding, RoleBinding, ConceptBinding, BindableTable, \
    BindableAttribute, FilteringConceptBinding, ReferentConceptBinding
...
class Weaver:
    ...
    def __init__(self, model, config=None):
        ...
        self._ctor_binding_to_entity_map: dict[ConceptBinding, SimpleConstructorEntityMap] = OrderedDict()
        ...
        self._subtype_binding_to_entity_map: dict[Union[ConceptBinding, ReferentConceptBinding], EntitySubtypeMap] = OrderedDict()
        self._filtering_subtype_binding_to_entity_map: dict[FilteringConceptBinding, EntitySubtypeMap] = OrderedDict()
    ...
    def _generate_simple_ctor_entity_map(self, binding: ConceptBinding):
    ...
    def _lookup_supertype_identifier_role_maps(self, binding: ConceptBinding):
    ...
    def _lookup_constructing_role_map(self, binding: Union[ConceptBinding, ReferentConceptBinding],
                                      ctor_concept: qb.Concept):
        if self._binder.is_value_type_binding(binding):
            role_map = self._binding_to_value_map[binding]
        elif isinstance(binding, ConceptBinding):
    ...
    def _find_matching_entity_maps(entity_maps, concept: qb.Concept,
        ...
        for entity_map in entity_maps:
            if isinstance(source_or_col, BindableTable):
                if entity_map.table() == source_or_col and entity_map.value_player() is concept:
                    matching_entity_maps.add(entity_map)
            elif isinstance(source_or_col, BindableAttribute):
                if entity_map.column() is source_or_col and entity_map.value_player() is concept:
                    matching_entity_maps.add(entity_map)
    ...
    def _generate_subtype_entity_map(self, binding: Union[ConceptBinding, ReferentConceptBinding]):
    ...
    def _generate_filtered_subtype_entity_map(self, binding: FilteringConceptBinding):
    ...
    def _generate_semantic_predicate(self, bound_relationship: BoundRelationship):
        if bound_relationship.relationship._arity() == 1:
