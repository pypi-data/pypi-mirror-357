...
def mk_abstraction(vars: list[Tuple[lqp.Var,lqp.RelType]], value: lqp.Formula) -> lqp.Abstraction:
    return lqp.Abstraction(vars=vars, value=value, meta=None)

def mk_exists(vars: list[Tuple[lqp.Var, lqp.RelType]], value: lqp.Formula) -> lqp.Formula:
    if len(vars) == 0:
        return value
