...
def lqp_agg_op(names: UniqueNames, op: ir.Relation, aggr_arg_name: str, aggr_arg_type: lqp.RelType) -> lqp.Abstraction:
    ...
    name = agg_to_lqp.get(op.name, op.name)
...
def lqp_operator(names: UniqueNames, op: ir.Relation, aggr_arg_name: str, aggr_arg_type: lqp.RelType) -> lqp.Abstraction:
    ...
    if op.name not in agg_to_lqp:
        raise NotImplementedError(f"Unsupported aggregation: {op.name}")

    return lqp_agg_op(names, op, aggr_arg_name, aggr_arg_type)
