...
class Compiler(c.Compiler):
    def __init__(self, skip_denormalization:bool=False):
        rewrites = [
            Checker(),
            InferTypes(),
            Flatten(handle_outputs=False, handle_union=False)
        ]
...
class ModelToSQL:
    ...
    def _create_statement(self, task: ir.Logical):
        ...
        statements = []
        statements.extend(self._create_recursive_view(union) for union in unions)
        # TODO - this is simplifying soooo much :crying_blood:
        if updates and not lookups and not nots and not aggs:
            # TODO: this is assuming that the updates are all static values
            # insert static values: INSERT INTO ... VALUES ... or RawSource
            for u in updates:
                r = u.relation
                if r == builtins.raw_source:
                    lang, src = u.args[0], u.args[1]
                    if not (isinstance(lang, str) and lang.lower() == "sql"):
                        raise Exception(f"Unsupported language for RawSource: {lang}")
        ...
        elif lookups or outputs or nots or aggs:
            ...
            if updates:
                    ...
                    if not self._is_value_type_population_relation(r):
                        ...
                        statements.append(
                            sql.Insert(self._relation_name(r),
                                       ...
                                       self._make_select(lookups, aliases, nots, aggs, True)
                            )
                        )
            elif outputs:
                ...
                statements.append(self._make_select(lookups, aliases, nots, distinct=distinct))
        elif logicals:
            for logical in logicals:
    ...
    def _make_select(self, lookups: list[ir.Lookup], outputs: list[Tuple[str, ir.Value]|Tuple[str, ir.Value, ir.Task]],
                     nots: Optional[list[ir.Not]] = None, aggs: Optional[list[ir.Aggregate]] = None, distinct: bool = False) -> sql.Select:
        ...
        wheres.extend(self._generate_where_clauses(var_lookups, var_column, sql_vars, builtin_vars))
        ...
        return sql.Select(distinct, vars, froms, where, group_by)
        ...
        if aggs:
            ...
            for var in agg.group:
                if var in var_lookups:
                    ...
                    group_by.append(sql.VarRef(sql_vars[lookup], var_column[var, lookup].name, None))
                    ...
                    var_ref = self._resolve_builtin_var(builtin_vars, var)
    ...
    def _generate_select_nots(self, nots: Optional[list[ir.Not]], sql_vars: dict[ir.Lookup, str],
        ...
        if nots:
            for not_expr in nots:
                ...
                wheres.extend(self._generate_where_clauses(not_var_lookups, not_var_column, not_sql_vars, builtin_vars))
    ...
    def _is_table_creation_required(self, r: ir.Relation) -> bool:
        ...
        if builtins.is_builtin(r) or from_cdc_annotation in r.annotations or builtins.is_annotation(r):
            return False
