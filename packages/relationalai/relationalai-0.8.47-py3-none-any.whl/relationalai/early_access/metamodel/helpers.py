...
def get_outputs(lookup: ir.Lookup):
    """
        Return an array with the arguments of this lookup that are refering to output fields
    ...
    if lookup.relation == builtins.eq:
        ...
        for idx, f in enumerate(lookup.relation.fields):
            arg = lookup.args[idx]
            if isinstance(arg, Iterable):
                # deal with ListType fields that pack arguments in a tuple
                for element in arg:
                    if isinstance(element, ir.Var) and not f.input:
                        outputs.append(element)
            else:
                if isinstance(arg, ir.Var) and not f.input:
                    outputs.append(arg)
        return outputs
