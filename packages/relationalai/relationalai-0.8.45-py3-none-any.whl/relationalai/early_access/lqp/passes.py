...
from relationalai.early_access.rel.rewrite import Flatten, QuantifyVars, CDC
...
class DeduplicateVars(Pass):
    ...
    def dedup_output(output: ir.Output) -> List[Union[ir.Output, ir.Lookup]]:
        ...
        alias_names = [alias[0] for alias in output.aliases]
        new_output = output.reconstruct(
            output.engine,
            FrozenOrderedSet([tuple(alias) for alias in zip(alias_names, deduped_vals)]),
            output.keys,
            output.annotations,
        )
