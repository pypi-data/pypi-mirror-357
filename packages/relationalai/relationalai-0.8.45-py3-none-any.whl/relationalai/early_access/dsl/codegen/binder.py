...
from relationalai.early_access.dsl.bindings.common import Binding, ConceptBinding, RoleBinding, FilteringConceptBinding, \
...
class Binder:
    def __init__(self, model, reasoner: OntologyReasoner):
        ...
        self._identifier_bindings: OrderedSet[ConceptBinding] = OrderedSet()
        self._subtype_bindings: OrderedSet[Union[ConceptBinding, ReferentConceptBinding]] = OrderedSet()
        self._filtering_subtype_bindings: OrderedSet[FilteringConceptBinding] = OrderedSet()
    ...
    def constructor_bindings(self) -> OrderedSet[ConceptBinding]:
        """
    ...
    def is_constructor_binding(self, binding: Binding) -> bool:
        ...
        """
        return isinstance(binding, ConceptBinding) and binding in self._identifier_bindings
    ...
    def is_filtering_binding(binding: Binding) -> bool:
        ...
        """
        return isinstance(binding, FilteringConceptBinding) or binding.filter_by is not None

    def subtype_bindings(self) -> OrderedSet[Union[ConceptBinding, ReferentConceptBinding]]:
        """
        Returns the subtype bindings.
        """
        return self._subtype_bindings

    def filtering_subtype_bindings(self) -> OrderedSet[FilteringConceptBinding]:
        """
    ...
    def lookup_binding_role(self, binding: Binding):
        if isinstance(binding, RoleBinding):
            ...
            role = binding.role
        elif isinstance(binding, (ConceptBinding, FilteringConceptBinding, ReferentConceptBinding)):
    ...
    def lookup_subtype_reference_binding(self, binding: ConceptBinding):
        """
    ...
    def lookup_subtype_reference_binding_by_source(self, binding: Union[ConceptBinding, FilteringConceptBinding]):
        """
        Look up a constructor binding referenced by the subtype binding.
        """
        ctor_role = self._reasoner.subtype_unique_ref_scheme_role(binding.entity_type)
    ...
    def _classify_binding(self, binding: Binding):
        if isinstance(binding, Union[ConceptBinding, ReferentConceptBinding]):
            self._process_concept_binding(binding)
        elif isinstance(binding, RoleBinding):
            self._process_role_binding(binding)
        elif isinstance(binding, FilteringConceptBinding):
            self._process_filtering_subtype_binding(binding)
        else:
            raise ValueError(f"Unknown binding type: {type(binding)}")

    def _process_concept_binding(self, binding: Union[ConceptBinding, ReferentConceptBinding]):
        """
    ...
    def _process_filtering_subtype_binding(self, binding: FilteringConceptBinding):
        """
    ...
    def _classify_binding_by_type(self, binding: Binding, player: qb.Concept):
        if isinstance(binding, FilteringConceptBinding):
