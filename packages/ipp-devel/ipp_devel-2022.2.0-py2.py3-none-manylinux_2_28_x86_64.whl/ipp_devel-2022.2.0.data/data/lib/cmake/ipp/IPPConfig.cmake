...
if (NOT IPP_FIND_COMPONENTS)
    ...
    foreach (_component ${IPP_FIND_COMPONENTS})
        if (NOT _component IN_LIST ipp_components)
            message (FATAL_ERROR "Component '${_component}' not found! It must be one of the following values: ${ipp_components}!")
        endif ()
    endforeach ()
...
function (add_imported_library_target
          _component
          PATH_TO_LIBRARY
          PATH_TO_IMPORT_LIB
          LINKAGE_TYPE
    ...
    if (EXISTS "${PATH_TO_LIBRARY}")
        ...
        set (${component_found_variable} 1 PARENT_SCOPE)
        set (ipp_libraries ${${ipp_libraries_variable}})
        list (APPEND ipp_libraries IPP::${_component})
        set (${ipp_libraries_variable} ${ipp_libraries} PARENT_SCOPE)
    elseif (IPP_FIND_REQUIRED AND IPP_FIND_REQUIRED_${_component})
        ...
        set (${component_found_variable} 0 PARENT_SCOPE)
        set (${ipp_found_variable} 0 PARENT_SCOPE)
    endif ()
endfunction ()
    ...
    foreach (_component ${list_of_components})
        ...
        if (WIN32 AND ${_linkage_type} MATCHES "SHARED")
            get_filename_component(
                _lib
                "${ipp_cmake_package_dir}/${IPP_REDIST_REL_PATH}/${tl_path}${_ipp_library_prefix}${_component}${_ipp_library_suffix}"
                REALPATH)
            get_filename_component(
                _imp_lib
                "${ipp_cmake_package_dir}/${IPP_LIB_REL_PATH}/${tl_path}${_ipp_library_prefix}${_component}${_ipp_import_library_suffix}"
                REALPATH)
        else ()
            get_filename_component(
                _lib
                "${ipp_cmake_package_dir}/${IPP_LIB_REL_PATH}/${tl_path}${_ipp_library_prefix}${_component}${_ipp_library_suffix}"
                REALPATH)
            set(_imp_lib "")
        endif ()
        add_imported_library_target("${full_component_name}"
                                    "${_lib}"
                                    "${_imp_lib}"
                                    "${_linkage_type}"
                                    component_found
                                    ipp_found
                                    ipp_libraries)
        set (IPP_${full_component_name}_FOUND ${component_found} PARENT_SCOPE)
...
function (add_ipp_iw_include_directory iw_include_directory language)
    get_filename_component(
        _include_dir "${ipp_cmake_package_dir}/${IPP_INC_REL_PATH}/ipp/${iw_include_directory}"
        REALPATH)
    if (EXISTS "${_include_dir}")
