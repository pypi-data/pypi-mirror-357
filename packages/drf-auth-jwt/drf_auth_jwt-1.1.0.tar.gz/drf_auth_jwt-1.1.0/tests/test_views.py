...
class CustomUserUUIDObtainJSONWebTokenTests(TestCase):
    ...
    def test_jwt_login_json(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        decoded_payload = utils.jwt_decode_handler(response.data["token"])
        self.assertEqual(decoded_payload["username"], str(self.user.email))
...
class VerifyJSONWebTokenTestsSymmetric(TokenTestCase):
    ...
    def test_verify_jwt_fails_with_expired_token(self):
        ...
        self.assertRegex(response.data["non_field_errors"][0], "Signature has expired")
    ...
    def test_verify_jwt_fails_with_missing_user(self):
        ...
        response = client.post("/auth-token-verify/", {"token": token}, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertRegex(response.data["non_field_errors"][0], "User doesn't exist")
