    ...
    def from_edf_file(cls, record_filepath:str, signal_names:Union[str,list,None]=None):
        ...
        for sig_header, signal in zip(signal_headers, signals):
            ...
            if (signal_names is None) or (label in signal_names):
                output[label] = Signal(label=label, data=signal, fs=sig_header['sample_frequency'], start_time=output.start_date)
        return output
...
class EventRecord:
    ...
    def __index_to_key(self, start:Union[int,None]=None, stop:Union[int,None]=None) -> Tuple[Any,Any]:
...
def autocache(func:Callable, cache_folder:str, filename:str, cache_format:str='pickle', force:bool=False, ignore_kwargs:Union[List[str],None]=None) -> Any:
    ...
    def func_ref() -> str:
        ...
        try:
            if (module is not None) and (len(module.__package__)>0):
