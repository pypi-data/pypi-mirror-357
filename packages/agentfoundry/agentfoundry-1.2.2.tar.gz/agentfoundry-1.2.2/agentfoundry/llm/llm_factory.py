...
class LLMFactory:
    ...
    @staticmethod
    def get_llm_model(provider: str = None) -> ChatOpenAI or ChatOllama:
        """
        ...
        The configuration should define:
          ...
          - For Ollama:
              - OLLAMA_MODEL (default: "codegemma:7b-instruct")
        ...
        provider = provider or config.get("PROVIDER", "openai")  # Get the LLM provider from the config
        logger.info(f"Creating LLM model of type: {provider}")
        if provider == "ollama":
            model_name = config.get("OLLAMA_MODEL", "gemma3:27b")
            ...
            model = config.get("OPENAI_MODEL", "o3")
            logger.info(f"Using OpenAILLM model: {model}")
            try:
