...
from agentfoundry.chroma.client import ChromadbClient
...
class ChromaDBDataLoader:
    def __init__(self, data_directory: str, persist_directory: str, collection_name: str, chunk_size: int = 512):
        ...
        self.client = ChromadbClient(persist_directory=self.persist_directory)
    ...
    def load_pdfs(self) -> int:
        ...
        collection = self.client.client.get_or_create_collection(
            ...
            embedding_function=self.client.collection._embedding_function
        )
    ...
    def _store_chunks(self, chunks: List[Dict], pdf_file: str):
        ...
        self.client.store_results(chunks)
...
if __name__ == "__main__":
    ...
    persist_dir = Config().get("CHROMADB_PERSIST_DIR")
    ...
    collection_names = loader.client.list_collections()
    print("\nAvailable collections:")
    for col in collection_names:
        name = col.name
        collection = loader.client.get_collection(name)
