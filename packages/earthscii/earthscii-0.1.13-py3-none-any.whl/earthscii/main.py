...
def single_tile_mode(stdscr, args, width, height, aspect_ratio):
    ...
    try:
        ...
        return map_data, transform, zoom
...
def main(stdscr, args):
    ...
    if args.tilewalk:
        ...
        map_data, transform, zoom = single_tile_mode(stdscr, args, width, height, aspect_ratio)
    ...
    while True:
        try:
            ...
            angle_x, angle_y, angle_z, zoom, offset_x, offset_y, changed, show_more_info = handle_keys(
            ...
            if state != prev_state:
                ...
                render_overlay(
                    buffer,
                    angle_x, angle_y, angle_z, zoom,
                    lat, lon,
                    mode="globe" if is_global else "tile",
                    stride=args.stride, zscale=args.zscale,
                    extra=show_more_info
                )
...
if __name__ == '__main__':
    ...
    try:
        ...
        raise
def handle_keys(key, angle_x, angle_y, angle_z, zoom, offset_x, offset_y, show_more_info=False):
    changed = False
...
def render_overlay(buffer, angle_x, angle_y, angle_z, zoom, lat=None, lon=None, mode="tile", stride=None, zscale=None, extra=False):
    height, width = buffer.getmaxyx()

    # Line 1 – compact, vim-style status
    line1 = f"MODE: {mode} |"
    if lat is not None and lon is not None:
        line1 += f" LAT: {lat:.2f}  LON: {lon:.2f} |"
    else:
        line1 += " LAT: ----  LON: ---- |"
    line1 += f" X: {angle_x:>3}°  Y: {angle_y:>3}°  Z: {angle_z:>3}° | ZOOM: {zoom:.2f}"

    buffer.addstr(height - 2 if extra else height - 1, 0, line1.ljust(width - 1), curses.color_pair(3))

    # Line 2 – extra info (optional)
    if extra:
        extra_info = f" STRIDE: {stride or '--'}  ZSCALE: {zscale or '--'}"
        buffer.addstr(height - 1, 0, extra_info.ljust(width - 1), curses.color_pair(2))
