...
config = LockConfig(
    ...
    ttl_seconds=60,          # Lock expires after 60 seconds
    retry_times=3,           # Retry 3 times on failure
    retry_delay=1.0,         # Wait 1 second between retries
    heartbeat_interval=20    # Renew lock every 20 seconds
)
...
# Setup
config = LockConfig(table_name="app-locks", ttl_seconds=120)
lock_manager = DistributedLockAPI(config)
...
async def async_data_processing():
    ...
    config = LockConfig(table_name="async-locks", ttl_seconds=90)
    lock_manager = DistributedLockAPI(config)
...
config = LockConfig(
    # Required
    table_name="my-locks",              # DynamoDB table name

    # Lock behavior
    ttl_seconds=60,                     # Lock expiration time
    retry_times=3,                      # Number of acquisition retries
    retry_delay=1.0,                    # Delay between retries (seconds)
    heartbeat_interval=20,              # Lock renewal interval (seconds)

    # AWS configuration
    credentials=None,                   # AWS credentials (optional)
    region="us-east-1",                # AWS region
    endpoint_url=None,                 # DynamoDB endpoint (for testing)

    # Process identification
    owner_id=None                      # Unique owner ID (auto-generated if None)
)
```
