...
from ipulse_shared_base_ftredge.enums  import (ReviewStatus, LogLevel, Dimension,
...
class StructLog:
    ...
    def __init__(
        ...
        q_unit: Optional[Union[Dimension, Resource]] = None,
        ...
        self._q = q
        # Handle q_unit which can be either Dimension or Resource
        if q_unit is not None:
            # First try Dimension, then Resource if that fails
            self._q_unit = to_enum(q_unit, Dimension) or to_enum(q_unit, Resource)
        else:
            self._q_unit = None
    ...
    @property
    def q_unit(self) -> Optional[Union[Dimension, Resource]]:
        return self._q_unit
    
    @q_unit.setter
    def q_unit(self, value: Optional[Union[Dimension, Resource, str]]):
        if value is not None:
            # First try Dimension, then Resource if that fails
            self._q_unit = to_enum(value, Dimension) or to_enum(value, Resource)
        else:
            self._q_unit = None
    ...
    def getEvent(self, exclude_none: bool = True, as_dict: bool = False, with_q=False) -> Union[Dict[str, Any], Tuple[Any, ...]]:
        ...
        """
        event_details = {
            "level": str(self.level) if self.level and not isinstance(self.level, str) else self.level,
            "action": str(self.action) if self.action and not isinstance(self.action, str) else self.action,
            "resource": str(self.resource) if self.resource and not isinstance(self.resource, str) else self.resource,
            "source": str(self.source) if self.source and not isinstance(self.source, str) else self.source,  # Add source
            "destination": str(self.destination) if self.destination and not isinstance(self.destination, str) else self.destination,  # Add destination
            "progress_status": str(self.progress_status) if self.progress_status and not isinstance(self.progress_status, str) else self.progress_status,
            "alert": str(self.alert) if self.alert and not isinstance(self.alert, str) else self.alert
        }

        if with_q:
            event_details["q"] = self.q
            event_details["q_unit"] = str(self.q_unit) if self.q_unit and not isinstance(self.q_unit, str) else self.q_unit
