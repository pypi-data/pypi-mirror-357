...
from typing import Union, Optional, List, TypeVar, Iterable, Type
        ...
        while self._should_continue(iterations, used_time):
            ...
            try:
                ...
                self.curr_message.add_user(
                    f"Observation: parse llm response error: {str(e)} ;response data : {remove_think(llm_resp)}"
                )
                continue
            ...
            self.curr_message.add_user(
                f"Observation: {tool_result if isinstance(tool_result, (str, int, float)) else tool_result.to_str()}"
            )
