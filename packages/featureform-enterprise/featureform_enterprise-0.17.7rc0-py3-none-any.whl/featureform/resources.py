...
from .types import VectorType, type_from_proto, ListType
...
@typechecked
@dataclass
class StreamChannel(Location):
    """
    StreamChannel represents a resource that can be read in a streaming way.

    For example, this can be a Kafka topic.
    """
    name: str
    channel_name: str
    provider: str
    ...
    def from_proto(source_stream):
        return StreamChannel(
            ...
            channel_name=source_stream.channel_name,
...
@typechecked
@dataclass
class StreamInput(Location):
    name: str
    options: dict[str, str]

    def __init__(self, name: Union[str, StreamChannel], options: dict[str, str]):
        if isinstance(name, StreamChannel):
            self.name = name.name
    ...
    @staticmethod
    def from_proto(stream_input: pb.StreamInput) -> "StreamInput":
        return StreamInput(
            name=stream_input.name.name,
            options=stream_input.options,
        )

    def to_proto(self) -> pb.StreamInput:
        return pb.StreamInput(
...
class PrimaryData:
    ...
    def kwargs(self) -> Dict[str, Any]:
        ...
        elif isinstance(self.location, StreamChannel):
            primary_data_kwargs["stream_channel"] = pb.StreamChannel(
                name=self.location.name,
                channel_name=self.location.channel_name,
...
class SQLTransformation(Transformation):
    ...
    def __post_init__(self):
        if len(self.func_params_to_inputs):
    ...
    def to_proto(self) -> pb.Transformation:
        ...
        for i, inp in enumerate(self.streaming_inputs):
            if hasattr(inp, "name_variant"):
                ...
                self.streaming_inputs[i] = inp
            elif isinstance(inp, StreamInput):
                self.streaming_inputs[i] = (inp.name, "")
            else:
                self.streaming_inputs[i] = (inp, "")
        ...
        for placeholder in self._get_placeholders():
            ...
            # Verify this won't cause any regressions in existing SQL transformation logic
            if isinstance(name_variant, tuple):
                replacement = "{{ " + f"{name_variant[0]}.{name_variant[1]}" + " }}"
            elif isinstance(name_variant, str):
                replacement = "{{ " + name_variant + " }}"
            else:
                raise ValueError(f"Invalid input type: {name_variant}")
...
class FeatureVariant(ResourceVariant):
    name: str
    source: Any
    value_type: Union[VectorType, ScalarType, ListType, str]
    entity: str
    owner: str
    ...
    def __post_init__(self):
        if isinstance(self.value_type, str) and ScalarType.has_value(self.value_type):
            self.value_type = ScalarType(self.value_type)
        elif not isinstance(self.value_type, (VectorType, ScalarType, ListType)):
            raise ValueError(f"Invalid type {self.value_type}")
...
@typechecked
@dataclass
class StreamingFeatureViewUpdate:
    name: str
    provider: str
    view_name: str
    entity_column: str
    timestamp_column: str
    feature_mappings: dict[str, Union[HasNameVariant, tuple[str, str]]]
    output_mode: str
    ...
    @staticmethod
    def get_resource_type() -> ResourceType:
        return ResourceType.STREAMING_FEATURE_VIEW_UPDATE

    @staticmethod
    def get(stub, name: str) -> "StreamingFeatureViewUpdate":
        name_req = pb.NameRequest(name=pb.Name(name=name))
        streaming_feature_view_update = next(
            stub.GetStreamingFeatureViewUpdates(iter([name_req]))
        )
        if not streaming_feature_view_update:
            raise ValueError(f"Streaming Feature View Update {name} not found.")

        return StreamingFeatureViewUpdate(
            name=streaming_feature_view_update.name,
            provider=streaming_feature_view_update.provider,
            view_name=streaming_feature_view_update.view_name,
            entity_column=streaming_feature_view_update.entity_column,
            timestamp_column=streaming_feature_view_update.timestamp_column,
            feature_mappings={
                key: (value.name, value.variant)
                for key, value in streaming_feature_view_update.feature_mappings.items()
            },
            output_mode=streaming_feature_view_update.output_mode,
            inputs=[StreamInput.from_proto(input) for input in streaming_feature_view_update.inputs],
            owner=streaming_feature_view_update.owner,
            description=streaming_feature_view_update.description,
            transformation=Transformation.from_proto(
                streaming_feature_view_update.transformation
            ),
            created=(
                streaming_feature_view_update.created.ToDatetime().isoformat()
                if streaming_feature_view_update.created
                and streaming_feature_view_update.created.seconds != 0
                else None
            ),
            last_updated=(
                streaming_feature_view_update.last_updated.ToDatetime().isoformat()
                if streaming_feature_view_update.last_updated
                and streaming_feature_view_update.last_updated.seconds != 0
                else None
            ),
            status=streaming_feature_view_update.status.Status._enum_type.values[
                streaming_feature_view_update.status.status
            ].name,
            server_status=ServerStatus.from_proto(streaming_feature_view_update.status),
            error=streaming_feature_view_update.status.error_message,
        )

    def _create(self, req_id: uuid.UUID, stub) -> Tuple[Optional[str], Optional[str]]:
        print(f"Request ID: {req_id} for streaming feature view update:\n{self}")
        req = pb.StreamingFeatureViewUpdateRequest(
            streaming_feature_view_update=pb.StreamingFeatureViewUpdate(
                name=self.name,
                ...
                feature_mappings={
                    key: (
                        pb.NameVariant(name=value.name, variant=value.variant)
                        if hasattr(value, "name_variant")
                        else pb.NameVariant(name=value[0], variant=value[1])
                    )
                    for key, value in self.feature_mappings.items()
                },
                output_mode=self.output_mode,
                inputs=[input.to_proto() for input in self.inputs],
                owner=self.owner,
                description=self.description,
        ...
        stub.StreamFeatureViewUpdate(req)
        # TODO: create get_and_set_equivalent_variant
        return None, None
...
class ResourceState:
    ...
    def sorted_list(self) -> List[Resource]:
        resource_order = {
            ...
            ResourceType.FEATURE_VIEW: 10,
            ResourceType.STREAM_CHANNEL: 11,
            ResourceType.STREAMING_FEATURE_VIEW_UPDATE: 12,
        }
    ...
    def build_dashboard_url(self, host, resource_type, name, variant=""):
        ...
        resource_type_to_resource_url = {
            ...
            ResourceType.STREAM_CHANNEL: "stream-channel",
            ResourceType.STREAMING_FEATURE_VIEW_UPDATE: "streaming-feature-view-updates",
        }
