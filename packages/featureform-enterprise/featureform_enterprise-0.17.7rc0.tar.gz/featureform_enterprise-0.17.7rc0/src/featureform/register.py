...
from .enums import ResourceType, TransformationType
from .get import *
...
from .types import VectorType, pd_to_ff_datatype, ListType
...
@dataclass
class StreamingFeatureViewUpdateDecorator:
    registrar: "Registrar"
    ...
    def __call__(self, fn):
        if len(self.feature_mappings) == 0:
            raise ValueError("feature_mappings must be provided")
        self.name = fn.__name__
        if self.description == "" and fn.__doc__ is not None:
            self.description = fn.__doc__
        if self.owner == "":
            self.owner = self.registrar.must_get_default_owner()
        func_params = inspect.signature(fn).parameters
        if len(self.inputs) == 0:
            raise ValueError(
                f"Streaming feature view update {self.name} must have at least one topic input."
            )
        if len(func_params) == 0:
            raise ValueError(
                f"Streaming feature view update {self.name} must have at least one topic param."
            )
        if len(func_params) != len(self.inputs):
            raise ValueError(
                f"Streaming feature view update {self.name} must have the same number of params as inputs."
            )

        inputs = [input.name for input in self.inputs]
        self.func_params_to_inputs = {param: t for param, t in zip(func_params, inputs)}
        if self.transformation_type == TransformationType.SQL:
            self.__set_sql_query(fn(*inputs))
        elif self.transformation_type == TransformationType.DATAFRAME:
            self.__set_df_query()
        else:
            raise ValueError(
                f"Unsupported transformation type {self.transformation_type}"
            )
        self.registrar.add_resource(self.__to_resource())
        return fn

    def __to_resource(self):
        ...
        return StreamingFeatureViewUpdate(
            ...
            view_name=(
                ...
                if isinstance(self.feature_view, FeatureView)
                else self.feature_view
            ),
    ...
    def __get_transformation(self):
        if self.transformation_type == TransformationType.SQL:
            return SQLTransformation(
                ...
                is_streaming=True,
                streaming_inputs=self.inputs,
            )
        elif self.transformation_type == TransformationType.DATAFRAME:
            # return DFTransformation(query=self.query)
            raise NotImplementedError(
                "Dataframe streaming updates are not supported yet"
            )
        else:
            raise ValueError(
                f"Unsupported transformation type {self.transformation_type}"
    ...
    def __set_sql_query(self, query: str):
        if query == "":
            raise ValueError("Query cannot be an empty string")
        # TODO: validation
        self.query = query

    def __set_df_query(self):
        raise NotImplementedError("Dataframe streaming updates are not supported yet")
...
class OfflineSparkProvider(OfflineProvider):
    ...
    def sql_streaming_update(
        self,
        feature_view: Union[FeatureView, str],
        entity_column: str,
        feature_mappings: dict[str, Union[HasNameVariant, tuple[str, str]]],
        output_mode: str,
        inputs: List[Union[str, StreamChannel, StreamInput]],
        description: str = "",
        timestamp_column: str = "",
    ):
        """
        Register a SQL streaming update for a feature view.

        Args:
            feature_view (FeatureView or str): The feature view to update.
            feature_mappings (List[dict]): List of mappings for features to be updated.
            topics (List[SourceVariant, tuple[str, str]]): Kafka topics to read from.
            stream_options (dict): Options for the streaming update.

        Returns:
            StreamingFeatureViewUpdateDecorator: Decorator for the streaming update.
        """

        def map_input(input):
            if isinstance(input, StreamChannel):
                return StreamInput(input.name)
            elif isinstance(input, str):
                return StreamInput(input)
            elif isinstance(input, StreamInput):
                return input
            else:
                raise ValueError(f"Invalid input type: {type(input)}")

        return StreamingFeatureViewUpdateDecorator(
            registrar=self.__registrar,
            ...
            inputs=[map_input(input) for input in inputs],
            description=description,
        )
...
class KafkaProvider(OfflineProvider):
    ...
    def register_kafka_topic(
        ...
        **Examples**
        ...
        topic = StreamChannel(name=name, channel_name=topic, provider=self.name())
...
class ColumnResource(ABC):
    ...
    def __init__(
        ...
        elif isinstance(type, (ListType, VectorType)):
            self.type = type
        elif isinstance(type, str):
            if not ScalarType.has_value(type):
                raise ValueError(
                    f"Invalid type for column. Must be a ScalarType or one of {ScalarType.get_values()}"
                )
            self.type = ScalarType(type)
        else:
...
class FeatureColumnResource(ColumnResource):
    def __init__(
        self,
        transformation_args: tuple,
        type: Union[ScalarType, str, ListType],
        entity: Union[Entity, str] = "",
        name: str = "",
        variant: str = "",
        owner: str = "",
...
class Registrar:
    ...
    def register_column_resources(
        ...
        for feature in features:
            ...
            if variant == "":
                variant = self.__run
            if isinstance(feature["type"], (ListType, ScalarType, VectorType)):
                value_type = feature["type"]
            elif isinstance(feature["type"], str) and ScalarType.has_value(
                feature["type"]
            ):
                value_type = ScalarType(feature["type"])
            else:
                raise ValueError(
                    f"Invalid type for feature {feature['name']} ({variant}): {type(feature['type'])}. Must be a ScalarType, ListType, VectorType or one of {ScalarType.get_values()}"
                )
...
class ResourceClient:
    ...
    **Using the Resource Client:**
    ...
    def _get_feature_view(self, name, local=False):
    ...
    def _get_streaming_feature_view_update(self, name, local=False):
        """Get a streaming feature view update. Prints out information on streaming feature view update, and all resources associated with the streaming feature view update."""
        return get_streaming_feature_view_update(self._stub, name)

    def list_streaming_feature_view_updates(self, local=False):
        """List all streaming feature view updates. Prints a list of all streaming feature view updates."""
        return list_name_status(self._stub, "streaming_feature_view_updates")
...
class EmbeddingColumnResource(ColumnResource):
    def __init__(
        ...
        **Example**
        ...
        super().__init__(
            transformation_args=transformation_args,
            type=VectorType(ScalarType.FLOAT32, dims, True),
            resource_type=ResourceType.FEATURE_VARIANT,
            entity=entity,
            name=name,
            variant=variant,
...
DeletableResourceObjects = Union[
    ...
    OnlineProvider,
    OfflineProvider,
    # TODO: disable deletion of feature views until we have proper dependency lookup
    # for streaming feature view updates
    # FeatureView,
]
...
StringList = ListType(ScalarType.STRING)
