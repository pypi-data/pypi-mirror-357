...
def get_streaming_feature_view_update(stub, name):
    searchName = metadata_pb2.NameRequest(name=metadata_pb2.Name(name=name))
    try:
        for x in stub.GetStreamingFeatureViewUpdates(iter([searchName])):
            status = x.status.Status._enum_type.values[x.status.status].name
            rows = [
                ("NAME: ", x.name),
                ("VIEW NAME: ", x.view_name),
                ("DESCRIPTION: ", x.description),
                ("OWNER: ", x.owner),
                ("PROVIDER: ", x.provider),
                ("STATUS: ", x.status.Status._enum_type.values[x.status.status].name),
                ("VIEW NAME: ", x.view_name),
                ("ENTITY COLUMN: ", x.entity_column),
                ("TIMESTAMP COLUMN: ", x.timestamp_column),
                ("CREATED: ", x.created.ToDatetime().isoformat()),
                ("LAST UPDATED: ", x.last_updated.ToDatetime().isoformat()),
            ]
            if status == "FAILED":
                rows.append(("ERROR: ", x.status.error_message))
            format_rows(rows)
            format_pg("FEATURES MAPPINGS:")
            format_rows("KEY", "NAME", "VARIANT")
            for k, v in x.feature_mappings.items():
                format_rows(k, v.name, v.variant)
            format_pg()
            format_pg("INPUTS:")
            format_rows("NAME")
            for i in x.inputs:
        ...
        print("Streaming feature view update not found.")
