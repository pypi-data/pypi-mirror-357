...
class QCDA(object):
    ...
    def auto_compile(
        ...
        if qm_iset is not None or qm_layout is not None:
            iset_gtlist = qm_iset.one_qubit_gates[:]
            iset_gtlist.append(qm_iset.two_qubit_gate)
            post_opt_process = self._choice_opt_algorithm(
                circuit.get_all_gatetype(), keep_phase=keep_phase, iset=iset_gtlist
            )
    ...
    def _choice_opt_algorithm(self, gate_types: list, keep_phase=False, iset: bool = None):
        ...
        if clifford_only:
            ...
            return CommutativeOptimization(depara=depara_list, keep_phase=keep_phase, instruciton_set=iset)
