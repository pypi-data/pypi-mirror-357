...
class CommutativeOptimization(object):
    ...
    def __init__(self, para=['all'], depara=[], keep_phase=False, instruciton_set=None):
        """
        Args:
        ...
        self._instruction_set = instruciton_set
    ...
    @classmethod
    def parameterize(cls, gate: BasicGate, para=['x', 'y', 'z'], iset=None) -> Tuple[BasicGate, float]:
        """
        ...
        for p in para:
            try:
                ...
                if iset is not None and gate_type not in iset:
                    break
    ...
    @classmethod
    def deparameterize(cls, gate: BasicGate, depara=['x', 'y', 'z'], iset=None) -> Tuple[CompositeGate, float]:
        """
        ...
        for p in depara:
            try:
                ...
                enable_depara = True
                for g in g_list:
                    if iset is not None and g.type not in iset:
                        enable_depara = False
                        break
    ...
    def execute(self, gates: Union[Circuit, CompositeGate]) -> CompositeGate:
        ...
        for gate in opt_gates.flatten_gates():
            ...
            if self.para:
                gate, phase = self.parameterize(gate, self.para, self._instruction_set)
                phase_angle += phase
        ...
        for node in nodes:
            if node.identity or node.gate.type == GateType.gphase:
                ...
                gates_depara, phase = self.deparameterize(node.gate, self.depara, self._instruction_set)
