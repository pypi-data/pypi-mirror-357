...
class DashboardWidget(Static):
    ...
    def update_data(self, data: Dict[str, Any]):
        ...
        unrealized_status = "📈" if unrealized_pnl > 0 else "📉" if unrealized_pnl < 0 else "➖"
        table.add_row("Unrealized PnL", f"${unrealized_pnl:.2f}", unrealized_status)
        
        realized_pnl = metrics.get("total_realized_pnl", 0.0)
        realized_status = "📈" if realized_pnl > 0 else "📉" if realized_pnl < 0 else "➖"
        ...
        sample_data = {
            "metrics": {
                "metrics": {
                    ...
                    "total_unrealized_pnl": 1234.56 * (0.5 + random()) * (1 if random() > 0.3 else -1),
                    "total_realized_pnl": -234.78 * (0.5 + random()) * (1 if random() > 0.4 else -1)
                }
            }
        }
