...
export const getProjectDetail = async (id: any) => {
  return await request.get(_baseUrl + '/teaching/project/' + id);
};
...
export const getTaskDetail = async (id: string) => {
  return await request.get(_baseUrlCommon + '/job/teaching/notebook/' + id);
};
...
export const loadProjectVersion = async (data: {
  ...
  return await request.post(_baseUrl + '/teaching/version/action/load', {
    data
  });
};
