...
def pre_check_pdf_table_type(user_name, password, server_name, share_name, pdf_file_path, port, page_number=1):
    """
    ...
    Returns:
        tuple:
            is_normal_table (bool): True if all extracted tables are valid 2D lists.
            tables (list): List of tables (each table is a list of rows).
    """
    ...
    return is_normal_table, tables


def extract_table_from_pdf(user_name, password, server_name, share_name, port, pdf_file_path, page_number, table_index, first_column_name):
    """ This function is used to extract tables from a PDF file.
    Args:
        pdf_file_path(str): The path to the PDF file.
        page_number(int): The page number from which to extract the table.
    ...
    is_normal_table, tables = pre_check_pdf_table_type(user_name, password, server_name, share_name, pdf_file_path, port, page_number)
    if is_normal_table:
    ...
    for row in table:
        if row and row[0] == first_column_name:
    ...
    pdf_table_df = pd.DataFrame(pdf_table_dict)
    return pdf_table_df
...
def save_pdf_table_into_excel(user_name, password, server_name, share_name, port, pdf_file_path, page_number, table_index, first_column_name, save_column_names, excel_file_path,
    ...
    if not sheet_name:
        sheet_name = 'Sheet1'
    pdf_table_df = extract_table_from_pdf(user_name, password, server_name, share_name, port, pdf_file_path, page_number, table_index, first_column_name)
...
def save_excel_data_into_text(user_name, password, server_name, share_name, port, excel_file_path, text_file_path, sheet_name='Sheet1', save_column_names=''):
    ...
    excel_file_obj = smb_load_file_obj(user_name, password, server_name, share_name, excel_file_path, port)
    ...
    excel_data.to_csv(file_obj, index=False, sep='\t', header=True, encoding='utf-8')
    file_obj.seek(0)
