...
class WSNOCLIB : 
    ...
    def connect(self,auto_refresh_token=True) :
        ...
        try :
            self.logger.debug(f"self.headers={self.session.headers} self.adater={self.session.adapters}")
        except :
            pass
        try :
            # this is for STC sever.
            # the reconnect/refresh token will fail if old session is still up.
            self.session.close()
        except :
            pass
        #refresh the session
        ...
        self.logger.debug(f'Connecting to {self.address} using username: {self.username} password: {self.password}')
        URL = f"https://{self.address}/rest-gateway/rest/api/v1/auth/token"
        self.logger.debug(f'Login URL is {URL}')
        
        r = self.session.post(url = URL , auth=BAuth(self.username, self.password), verify=False, json={"grant_type": "client_credentials"})
    ...
    def logout(self,logout=True) :
        ...
        self.logger.debug(f"TOP SECRET INFO {self.username} {self.password}")
