...
def run() -> None:
    ...
    for gui in _choose_gui_order():
        try:
            ...
            print(f"[ipynotebook] Using GUI backend: {backend}")
            break
        except (webview.exceptions.UnsupportedGUI, ModuleNotFoundError) as e:
            print(f"[ipynotebook] GUI '{gui or 'default'}' failed: {e}")
            continue
    else:
        # If every backend fails, raise a helpful error
        raise RuntimeError(
            "ipynotebook could not find a working GUI backend.\n"
            "Install one of:\n"
...
# Enables:  python -m ipynotebook.app
