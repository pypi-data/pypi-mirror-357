...
def omni_kit_command(command: str = "CreatePrim", prim_type: str = "Sphere", prim_path: str = None) -> str:
    ...
    FOR CREATING A CUBE WITH SIZE 10:
    ...
    Note: CreatePrim creates objects with default size 1x1x1. 
    ...
    if is_duplicate_command("omni_kit_command", params):
        return f"Object {prim_type} at {prim_path if prim_path else 'auto-generated path'} has already been created. Skipping duplicate creation. Use transform tool to modify it if needed."
    
    try:
        # Get the global connection
        isaac = get_isaac_connection()
        
        # Send command to Isaac
        ...
        if result.get("status") == "success":
            ...
            if command == "CreatePrim":
                return (f"Successfully created {prim_type}{prim_path_info}. "
                       ...
                       f"To scale it, use: transform(prim_path=\"{actual_prim_path}\", scale=[desired_x, desired_y, desired_z])")
            else:
