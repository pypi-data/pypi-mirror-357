...
class OpenAIClient(AIAPIClient):
    ...
    def __init__(self, config: OpenAIClientConfig):
        ...
        # Store retry configuration for later use, but don't apply decorators yet
        # This avoids issues with method wrapping during initialization
        try:
            self.retry_config = config.retry_config if hasattr(config, 'retry_config') and config.retry_config else RetryConfig()
            
            # Prepare loggable version of retry_config
            if hasattr(self.retry_config, 'model_dump'):
                try:
                    loggable_rc_dict = self.retry_config.model_dump()
                    ...
                    logger.info(f"Using retry_config for OpenAIClient: {json.dumps(loggable_rc_dict, indent=2, default=str)}")
                except Exception as e:
                    logger.warning(f"Failed to log retry config: {e}")
        except Exception as e:
            logger.warning(f"Error setting up retry configuration: {e}")
            self.retry_config = None
            
        # We'll apply the retry decorators after all initialization is complete
        # This ensures that the methods are fully defined before wrapping
        try:
            if self.retry_config:
                # Safely apply decorators after initialization
                self._apply_retry_decorators()
        except Exception as e:
            logger.warning(f"Failed to apply retry decorators: {e}")
            # Continue without retry decorators - better than failing initialization
