...
def validate_commodity_attr(entries, unused_options_map, config_str):
    ...
    validmap = {attr: frozenset(values) if values is not None else None for attr, values in config_obj.items()}
        ...
        for attr, values in validmap.items():
            ...
            if value is None:
                errors.append(
                    CommodityError(
                        entry.meta,
                        "Missing attribute '{}' for Commodity directive {}".format(attr, entry.currency),
                        None,
                    )
                )
                continue
            if values and value not in values:
                errors.append(
                    CommodityError(
                        ...
                        + " directive {}; valid options: {}".format(entry.currency, ", ".join(values)),
                        None,
                    )
                )
