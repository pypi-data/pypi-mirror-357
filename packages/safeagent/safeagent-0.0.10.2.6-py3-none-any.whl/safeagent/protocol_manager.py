...
class ProtocolManager:
    ...
    """
    def __init__(self, protocol: str = None, access_manager: Optional[AccessManager] = None): # Added access_manager
        self.protocol = protocol or DEFAULT_PROTOCOL
        self.access_manager = access_manager or AccessManager() # Use provided or create new
    ...
    def _run_agent2agent(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for aid in agent_ids:
            ...
            def retrieve(agent_id=aid, user_input: str = inputs["user_input"], **kwargs):
                return vector_ret.query("Query for {}: {}".format(aid, user_input), top_k=2)

            def respond(retrieve: List[dict], agent_id=aid, **kwargs) -> dict:
                doc_ids = [d.get('id', 'N/A') for d in retrieve]
                prompt = "As {}, generate a one-sentence response based on documents: {}".format(aid, doc_ids)
        ...
        for aid, orch in agents.items():
            ...
            # Pass original inputs to each agent's orchestrator
            res = orch.run(inputs)
