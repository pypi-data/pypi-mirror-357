...
class ProtocolManager:
    ...
    def _run_mcp(self, inputs: Dict[str, Any]) -> Any:
        ...
        # Ensure _build_mcp_orchestrator receives the correct resources
    ...
    def _run_agent2agent(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for aid in agent_ids:
            ...
            def retrieve(agent_id=aid, user_input: str = inputs["user_input"], 
                ...
                g_docs = []
                if graph_ret: # Only query graph if graph_ret is initialized
            ...
            def respond(retrieve_docs: List[dict], agent_id=aid, llm=llm, **kwargs) -> dict: 
                doc_ids = [d.get('id', 'N/A') for d in retrieve_docs]
                prompt = "As {}, generate a one-sentence response based on documents: {}".format(agent_id, doc_ids)
                return llm.generate(prompt) # Use the captured llm
