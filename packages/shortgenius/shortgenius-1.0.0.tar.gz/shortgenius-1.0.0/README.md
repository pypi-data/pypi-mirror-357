...
[![PyPI version](<https://img.shields.io/pypi/v/shortgenius.svg?label=pypi%20(stable)>)](https://pypi.org/project/shortgenius/)
...
async def main() -> None:
    genres = await client.music.genres.list()
...
```sh
# install from PyPI
pip install shortgenius[aiohttp]
```
...
import os
import asyncio
from shortgenius import DefaultAioHttpClient
from shortgenius import AsyncShortgenius


async def main() -> None:
    async with AsyncShortgenius(
        api_key=os.environ.get("SHORTGENIUS_API_KEY"),  # This is the default and can be omitted
...
client.with_options(max_retries=5).music.genres.list()
```
...
client.with_options(timeout=5.0).music.genres.list()
```
...
genre = response.parse()  # get the object that `music.genres.list()` would have returned
print(genre)
```
