...
from typing import Annotated, Any, Dict, List, Literal, Optional, Tuple, TypedDict, Union, cast
...
async def search_rongda_doc(
    ...
    result = [
        RongdaDoc(
            doc_id=result.htmlpath or "", title=result.title, content_clip=result.content
        )
...
async def search_documents(
    ...
    send_reasoning(f"Finding documents with title: {title_keywords}, content: {content_keywords}, companies: {company_code}")
...
def create_search_agent() -> CompiledStateGraph:
    ...
    # Search tools
    search_tools: List[Tool|StructuredTool] = cast(List[Tool|StructuredTool], [search_documents, select_document])
    logger.debug(
