...
class Tool:
    ...
    def formulate_tool_schema(self):
        ...
        print(properties)  # Example output: {'a': {'type': 'integer'}, 'b': {'type': 'string'}}
        schema = {
            "type": "function",
            "function": {
                "name": self.name,
        ...
        print(schema)  # Example output: {'type': 'function', 'function': {'name': 'foo', 'description': 'A test function.', 'parameters': {'type': 'object', 'properties': {'a': {'type': 'integer'}, 'b': {'type': 'string'}}, 'required': ['a', 'b']}}}
        return schema