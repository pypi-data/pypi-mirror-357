...
from labdesk.utils import project
from labdesk.utils.git import checkout_ref, clone_repo, fetch_ref, git_pull, git_push
from labdesk.utils.logging import update_json_file
import labdesk.utils.config as config
from labdesk.utils.runner import pbs_del
...
def main():
    ...
    resource_group = os.environ.get('LABDESK_RESOURCE_GROUP', None)
    ...
    for _ in range(1):
        ...
        ref_name = f'refs/labdesk/{action["action_id"]}'
        ...
        env_vars = {f"LABDESK_{k.upper()}": str(v) for k, v in action.items()}
        env_vars['LABDESK_LOG_DIR'] = log_dir
