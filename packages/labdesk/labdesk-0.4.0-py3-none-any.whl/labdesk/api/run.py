...
def main(args):
    ...
    # Define the paths
    database_file = '.labdesk/database.txt'
    pbs_output_file = '.labdesk/pbs_script.pbs'
    ...
    for i, sequential_group in enumerate(selected_experiments):
        ...
        for j, (exp_name, exp_path) in enumerate(sequential_group):
            run_script_path = os.path.join(exp_path, '.labdesk', 'run_script.sh')
...
#PBS -j oe
#PBS -o /work/arosasco/diffusion_policy_og/.labdesk/pbs_out.txt
