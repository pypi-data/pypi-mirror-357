...
class FeatrixSphereClient:
    ...
    def _make_request(self, method: str, endpoint: str, max_retries: int = 3, **kwargs) -> requests.Response:
        """Make an HTTP request with error handling and retry logic for 503s."""
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if e.response.status_code == 503 and attempt < max_retries:
                    ...
                    print(f"503 Service Unavailable, retrying in {wait_time}s... (attempt {attempt + 1}/{max_retries})")
    ...
    def _get_json(self, endpoint: str, max_retries: int = 3, **kwargs) -> Dict[str, Any]:
        """Make a GET request and return JSON response."""
        response = self._make_request("GET", endpoint, max_retries=max_retries, **kwargs)
        return response.json()
    
    def _post_json(self, endpoint: str, data: Dict[str, Any] = None, max_retries: int = 3, **kwargs) -> Dict[str, Any]:
    ...
    def upload_df_and_create_session(self, df, filename: str = "data.csv") -> SessionInfo:
        """
        Upload a pandas DataFrame directly and create a new session.
    ...
    def predict(self, session_id: str, record: Dict[str, Any], max_retries: int = 3) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            max_retries: Number of retries for 503 errors (default: 3)
    ...
    def predict_table(self, session_id: str, table_data: Dict[str, Any], max_retries: int = 5) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            max_retries: Number of retries for 503 errors (default: 5 for batch operations)
