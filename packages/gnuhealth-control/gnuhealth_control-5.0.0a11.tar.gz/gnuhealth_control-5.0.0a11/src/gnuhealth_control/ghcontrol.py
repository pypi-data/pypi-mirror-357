...
VERSION = "5.0.0a11"  # gnuheath-control version
...
"""
    ...
    def setup(self):
        ...
        for task in TASKS:
            if task == 'install_finished':
                ...
                msg = "Please LOG OUT and re-login "
                msg_cont = "to activate the new environment"
                attr = curses.color_pair(2) | curses.A_BOLD
                self.show_logs(
                    win=logwin, row=row+1, col=2, data=msg, attr=attr)
                self.show_logs(
                    win=logwin, row=row+2, col=2, data=msg_cont, attr=attr)
            else:
    ...
    def update(self):
        ...
        show_logs(win=logwin, row=1, col=1, data="Updating...", attr=0)

        for package in distributions():
            pkg = package.metadata["Name"]
            row = 3
            show_logs(win=logwin, row=row, col=1, data=pkg, attr=0)
            logwin.refresh()
            ...
            if ("tryton" in pkg or "gnuhealth" in pkg):
                ...
                try:
                    ...
                    show_logs(win=logwin, row=row, col=1, data="ERROR", attr=0)
