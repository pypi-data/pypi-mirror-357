...
class QueryBuilder:
    ...
    def read_query(
        ...
        if use_cache and cache_client:
            # Import cache utilities when needed
            from ..cache import generate_cache_id, get_or_set_cache, datetime_serializer
            
            query_hash = generate_cache_id(parsed_query)
            id_cache_endpoint = f"{cache_key_prefix}:query:data:{query_hash}"

            return get_or_set_cache(
                cache_client,
                id_cache_endpoint,
                lambda: self.get_query_response(
                    db_collection, parsed_query, pipeline, pipeline_count, 
                    cache_client=cache_client, 
                    cache_key_prefix=cache_key_prefix,
                    use_cache=True,
                    clear_cache=clear_cache
                ),
                clear_cache=f"{cache_key_prefix}:query:data:*" if clear_cache else False,
                ttl=60*30,
    ...
    def build_filter_pipeline(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def expand_text_search_condition(field_name, operator, str_value):
            ...
            match operator:
                case "$containsi":
                    expanded_conditions.append({field_name: {"$regex": escaped_value, "$options": "i"}})
                case "$contains":
                    expanded_conditions.append({field_name: {"$regex": escaped_value}})
                case "$notContainsi":
                    expanded_conditions.append({field_name: {"$not": {"$regex": escaped_value, "$options": "i"}}})
                case "$notContains":
                    expanded_conditions.append({field_name: {"$not": {"$regex": escaped_value}}})
                case "$startsWith":
                    expanded_conditions.append({field_name: {"$regex": f"^{escaped_value}"}})
                case "$startsWithi":
                    expanded_conditions.append({field_name: {"$regex": f"^{escaped_value}", "$options": "i"}})
                case "$endsWith":
                    expanded_conditions.append({field_name: {"$regex": f"{escaped_value}$"}})
                case "$endsWithi":
                    expanded_conditions.append({field_name: {"$regex": f"{escaped_value}$", "$options": "i"}})
                case "$eqi":
                    expanded_conditions.append({field_name: {"$regex": f"^{escaped_value}$", "$options": "i"}})
                case "$nei":
                    expanded_conditions.append({field_name: {"$not": {"$regex": f"^{escaped_value}$", "$options": "i"}}})
                ...
                try:
                    ...
                    match operator:
                        case "$containsi" | "$contains":
                            ...
                            if str_value == str(int(float(str_value))):  # It's a whole number like "10"
                                ...
                                if len(str_value) <= 2 and int_val >= 0:
                                    ...
                                    expanded_conditions.append({
                                        field_name: {"$gte": int_val, "$lt": int_val + 1}
                                    })
                                    ...
                                    if int_val > 0:
                                        ...
                                        range_end = range_start + (10 ** (len(str(int_val)))) - 1
                                        if range_start != int_val:  # Avoid duplicate
                                            expanded_conditions.append({
                                                field_name: {"$gte": range_start, "$lte": range_end}
                                            })
                        
                        case "$eqi":
                            expanded_conditions.append({field_name: numeric_value})
                            if isinstance(numeric_value, int):
                                expanded_conditions.append({field_name: float(numeric_value)})
                        
                        case "$nei":
                            expanded_conditions.append({field_name: {"$ne": numeric_value}})
                            if isinstance(numeric_value, int):
                                expanded_conditions.append({field_name: {"$ne": float(numeric_value)}})
        ...
        def build_sub_pipeline(conditions, field_name=None):
            ...
            for operator, value in conditions.items():
                ...
                text_search_ops = ["$contains", "$notContains", "$containsi", "$notContainsi", 
                                 "$startsWith", "$startsWithi", "$endsWith", "$endsWithi", "$eqi", "$nei"]
                ...
                if operator in text_search_ops:
                    ...
                    if is_numeric_string(str_value):
                        ...
                        if operator == "$containsi":
                            ...
                            sub_pipeline = {"$regex": f"^{escaped_value}", "$options": "i"}
                            ...
                            sub_pipeline = {"$not": {"$regex": f"^{escaped_value}$", "$options": "i"}}
                        else:
                            sub_pipeline = {"$regex": escaped_value, "$options": "i"}
                else:
                    ...
                    match operator:
                        ...
                        case "$between":
                            if not isinstance(value, list) or len(value) != 2:
                                raise ValueError(f"$between operator requires an array with exactly 2 elements, got: {value}")
                            sub_pipeline = {"$gte": value[0], "$lte": value[1]}
                        case _:
                            if isinstance(value, dict):
                                sub_pipeline = {
                                    '_field': operator,
                                    '_conditions': build_sub_pipeline(value, field_name)
                                }
                            else:
                    ...
                    for condition in conditions:
                        ...
                        if isinstance(field_conditions, dict):
                            ...
                            if operator in text_search_ops:
                                ...
                                value_type, str_value = get_value_type_and_string(value)
                                expanded_conditions = expand_text_search_condition(field_name, operator, str_value)
    ...
    def build_fields_pipeline(self, fields_params: List[str], populate_params: Optional[List] = None) -> Dict[str, Any]:
    ...
    def build_distinct_pipeline(self, distinct_params: List[str], include_project: bool = True, pipeline: Optional[List] = None) -> List[Dict[str, Any]]:
    ...
    def build_pagination_pipeline(self, pagination_params: Dict[str, Any], pipeline: Optional[List] = None) -> List[Dict[str, Any]]:
    ...
    def sanitize_result(self, query: Dict[str, Any], data_result: Any, meta_result: Any) -> Dict[str, Any]:
                ...
                for relation in self.relations:
                    ...
                    if relation_name == query_relation:
                        ...
                        if isinstance(query_relations[query_relation], dict):
                            lookup_relation = self.get_lookup_relation(relation, query_relations[query_relation])
                            query_lookup_relations = [
                                *query_lookup_relations,
                                *lookup_relation[0]
                            ]
    ...
    def get_count_lookup_relations(self, query_relations: List[str]) -> List[Dict[str, Any]]:
    ...
    def make_aggregation_query(self, query: Dict[str, Any], base_pipeline: Optional[List] = None) -> List[Dict[str, Any]]:
        """Make aggregation query pipeline"""
        pipeline = (
            [] if not base_pipeline 
            else deepcopy(base_pipeline)
        )
        ...
        if "distinct" in query: 
            pipeline.extend(self.build_distinct_pipeline(query["distinct"], include_project=True, pipeline=pipeline))
        elif "fields" in query:
            pipeline.append(self.build_fields_pipeline(query["fields"], query.get("populate", None)))
    ...
    def make_count_aggregation_query(self, query: Dict[str, Any], base_pipeline: Optional[List] = None) -> List[Dict[str, Any]]:
        ...
        if "filters" in query:
            if not pipeline:
                ...
                if list(lookup_filters.keys()):
                    populate_from_filters = self.get_count_lookup_relations(list(lookup_filters.keys()))
        ...
        if "distinct" in query:
            pipeline.extend(self.build_distinct_pipeline(query["distinct"], include_project=False, pipeline=pipeline))

        pipeline.append({"$count": "totalCount"})

        return pipeline

    def get_query_response(self, db_collection, query: Dict[str, Any], pipeline: Optional[List] = None, pipeline_count: Optional[List] = None, cache_client=None, cache_key_prefix: str = "", use_cache: bool = False, clear_cache: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if use_cache and cache_client:
                ...
                meta_result = get_or_set_cache(
                    ...
                    clear_cache=f"{cache_key_prefix}:query:count:*" if clear_cache else False,
                    ttl=60*30,
                    serializer=lambda x: datetime_serializer(x[0]) if x else {"totalCount": 0}
                )
            else:
