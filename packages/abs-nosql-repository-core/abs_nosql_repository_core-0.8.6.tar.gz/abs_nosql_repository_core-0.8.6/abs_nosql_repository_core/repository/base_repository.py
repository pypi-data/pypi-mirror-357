...
class BaseRepository:
    ...
    def _get_sort_stages(self,pipeline: List[Dict[str, Any]],sort_order: List[SortSchema]) -> List[Dict[str, Any]]:
        top_level_sort: Dict[str, int] = {}

        for sort in sort_order:
            path_parts = sort.field.split(".")
            direction = 1 if sort.direction == SortDirection.ASC else -1

            if len(path_parts) == 1:
                # Simple top-level field sort
                top_level_sort[sort.field] = direction

            elif len(path_parts) == 2:
                # Top-level array sort: e.g. sections.order
                pipeline.append(self._sort_top_level_array(path_parts[0], path_parts[1], direction))

            else:
                # Fully nested sort
                pipeline.append(self._sort_deep_nested_array(path_parts, direction))

        if top_level_sort:
            pipeline.append({"$sort": top_level_sort})

        return pipeline


    def _sort_top_level_array(self,array_field: str, sort_key: str, direction: int) -> Dict[str, Any]:
        return {
            "$set": {
                array_field: {
                    "$cond": {
                        "if": {"$isArray": f"${array_field}"},
                        "then": {
                            "$sortArray": {
                                "input": f"${array_field}",
    ...
    def _sort_deep_nested_array(self,path_parts: List[str], direction: int) -> Dict[str, Any]:
        """
        Recursively builds a $set stage to sort deeply nested arrays.
        e.g. ['a', 'b', 'c', 'd', 'e'] -> sort array at 'e' by key, wrapping a->b->c->d in $map
        """
        *outer, array_field, sort_key = path_parts

        def build_recursive_map(levels: List[str], current_input: str) -> Any:
            """
            Recursively builds nested $map expressions.
            `levels` is a list of parent array fields.
            `current_input` is either "$" (top-level) or "$$this" (for inner levels).
            """
            if not levels:
                # We're at the parent of the array to sort
                return {
                    "$map": {
                        "input": f"{current_input}",
                        "as": "this",
                        "in": {
                            "$mergeObjects": [
                                "$$this",
                                {
                                    array_field: {
                                        "$cond": {
                                            "if": {"$isArray": f"$$this.{array_field}"},
                                            "then": {
                                                "$sortArray": {
                                                    "input": f"$$this.{array_field}",
                                                    "sortBy": {sort_key: direction}
                                                }
                                            },
                                            "else": f"$$this.{array_field}"
                                        }
                                    }
                                }
                            ]
            ...
            next_map = build_recursive_map(remaining_levels, "$$this." + current_level)

            return {
                "$map": {
                    "input": f"{current_input}.{current_level}",
                    "as": "this",
                    "in": {
                        "$mergeObjects": [
                            "$$this",
        ...
        return {
            "$set": {
                outer[0]: build_recursive_map(outer[1:], "$" + ".".join(outer))
            }
        }
