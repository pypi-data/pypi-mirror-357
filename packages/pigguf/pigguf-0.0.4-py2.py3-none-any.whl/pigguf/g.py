...
import logging, argparse, os, sys, enum, warnings
...
from gguf_connector.reader import GGUFReader, GGUFValueType, ReaderField
from gguf_connector.writer import GGUFWriter
from gguf_connector.const import TokenType, RopeScalingType, PoolingType, GGMLQuantizationType, Keys

logger = logging.getLogger("pig-reader")
icon_path = os.path.join(os.path.dirname(__file__), "logo.png")

# Map of key names to enum types for automatic enum interpretation
KEY_TO_ENUM_TYPE = {
    Keys.Tokenizer.TOKEN_TYPE: TokenType,
    Keys.Rope.SCALING_TYPE: RopeScalingType,
    Keys.LLM.POOLING_TYPE: PoolingType,
    Keys.General.FILE_TYPE: GGMLQuantizationType,
}

# Define the tokenizer keys that should be edited together
TOKENIZER_LINKED_KEYS = [
    Keys.Tokenizer.LIST,
    Keys.Tokenizer.TOKEN_TYPE,
...
class GGUFEditorWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("pig-reader")
    ...
    def edit_tokenizer_metadata(self, trigger_key):
        ...
        tokens_field = self.reader.get_field(Keys.Tokenizer.LIST)
        token_types_field = self.reader.get_field(Keys.Tokenizer.TOKEN_TYPE)
        scores_field = self.reader.get_field(Keys.Tokenizer.SCORES)
        ...
        if Keys.Tokenizer.LIST in self.metadata_changes:
            _, (_, tokens) = self.metadata_changes[Keys.Tokenizer.LIST]
        if Keys.Tokenizer.TOKEN_TYPE in self.metadata_changes:
            _, (_, token_types) = self.metadata_changes[Keys.Tokenizer.TOKEN_TYPE]
        if Keys.Tokenizer.SCORES in self.metadata_changes:
            _, (_, scores) = self.metadata_changes[Keys.Tokenizer.SCORES]
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if tokens_field:
                self.metadata_changes[Keys.Tokenizer.LIST] = (
                    GGUFValueType.ARRAY,
                    (tokens_field.types[1], new_tokens)
                )

            if token_types_field:
                self.metadata_changes[Keys.Tokenizer.TOKEN_TYPE] = (
                    GGUFValueType.ARRAY,
                    (token_types_field.types[1], new_token_types)
                )

            if scores_field:
                self.metadata_changes[Keys.Tokenizer.SCORES] = (
                    GGUFValueType.ARRAY,
            ...
            self.update_tokenizer_display(Keys.Tokenizer.LIST, new_tokens)
            self.update_tokenizer_display(Keys.Tokenizer.TOKEN_TYPE, new_token_types)
            self.update_tokenizer_display(Keys.Tokenizer.SCORES, new_scores)
    ...
    def save_file(self):
        ...
        try:
            ...
            arch = 'unknown'
            field = self.reader.get_field(Keys.General.ARCHITECTURE)
            if field:
                arch = field.contents()
            ...
            alignment = None
            field = self.reader.get_field(Keys.General.ALIGNMENT)
            if field:
