...
from chonk9k_api import Chonk9kAPI, AsyncChonk9kAPI, DefaultAioHttpClient
from chonk9k_api._utils import is_dict
...
logging.getLogger("chonk9k_api").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Chonk9kAPI]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Chonk9kAPI(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncChonk9kAPI]:
