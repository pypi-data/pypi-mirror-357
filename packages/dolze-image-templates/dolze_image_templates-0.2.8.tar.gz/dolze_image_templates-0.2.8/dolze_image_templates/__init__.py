...
from dolze_image_templates.data.template_variables import get_template_variables
...
def get_template_variable_status(template_name: str) -> dict[str, Any]:
    """
    Get the status of a template's variables.

    Args:
        template_name: Name of the template to get variable status for

    Returns:
        Dictionary containing variable status and example values

    Raises:
        ValueError: If template_name is not found in the registry
    """
    return get_template_variables(template_name)
