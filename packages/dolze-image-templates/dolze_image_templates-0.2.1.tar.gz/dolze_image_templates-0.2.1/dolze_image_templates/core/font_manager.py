...
class FontManager:
    ...
    def _scan_fonts(self) -> None:
        ...
        """
        logger.debug(f"[FontManager] Scanning for fonts in: {os.path.abspath(self.font_dir)}")
        
        # Check if the font directory exists and is accessible
        if not os.path.exists(self.font_dir):
            logger.warning(f"Font directory '{os.path.abspath(self.font_dir)}' does not exist.")
            # Try to create the directory
            try:
                os.makedirs(self.font_dir, exist_ok=True)
                logger.info(f"Created font directory: {os.path.abspath(self.font_dir)}")
                # If we just created the directory, it will be empty
                return
            except Exception as e:
                logger.error(f"Failed to create font directory '{os.path.abspath(self.font_dir)}': {e}")
                return
            
        if not os.path.isdir(self.font_dir):
            logger.error(f"Font path '{os.path.abspath(self.font_dir)}' is not a directory")
            return
            
        # Log the actual contents of the fonts directory for debugging
        try:
            contents = os.listdir(self.font_dir)
            logger.debug(f"Contents of font directory '{os.path.abspath(self.font_dir)}': {contents}")
        except Exception as e:
            logger.error(f"Failed to list contents of font directory '{os.path.abspath(self.font_dir)}': {e}")
            return
