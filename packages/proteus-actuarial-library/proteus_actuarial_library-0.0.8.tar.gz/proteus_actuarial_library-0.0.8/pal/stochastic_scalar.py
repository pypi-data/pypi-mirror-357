...
class StochasticScalar(ProteusStochasticVariable):
    ...
    def weighted_mean(self, weights: StochasticScalar, normalised=True) -> float:
        ...
        Args:
            ...
            normalised (bool): If True, the weights are assumed to be already normalised to sum to 1.
                If False, the weights will be normalised by dividing by their sum. Defaults to True.

        Returns:
            float: The weighted mean of the variable.

        """
        if self.n_sims != weights.n_sims:
            raise ValueError(
                "Weights must have the same number of simulations as the variable."
            )
        return sum(self * weights) / (weights.ssum() if not normalised else 1)
