...
class ResponseUtil(object):
    def __init__(self, response: Response):
        ...
        if isinstance(self.response.json(), dict):
            self.box = Box(convert_numbers_to_decimal(response.json()))
        elif isinstance(self.response.json(), list):
            self.box = BoxList(convert_numbers_to_decimal(response.json()))
        else:
    ...
    def __send_request(self, data):
        ...
        with allure.step("发送请求"):
            ...
            if request_obj.get('params'):
                with allure.step(f"请求params: {request_obj.get('params')}"):
                    logger.info(f"请求params: {request_obj.get('params')}")
            if request_obj.get('data'):
                with allure.step(f"请求data: {request_obj.get('data')}"):
                    logger.info(f"请求data: {request_obj.get('data')}")
            if request_obj.get('json'):
                with allure.step(f"请求json: {request_obj.get('json')}"):
                    logger.info(f"请求json: {request_obj.get('json')}")
