...
# 文件名称： create_config_page_project.py
# 项目描述： 生成 fastapi 配置页面模板
# 开发工具： PyCharm
import os
import shutil
import argparse
...
def create_config_page_project(args: Union[argparse.Namespace, dict]) -> None:
    """
    生成 fastapi 配置页面模板，复制模板文件到指定的项目目录，并根据用户输入定制项目名称和路径。
    ...
    # 设置模板文件源路径
    template_path = os.path.join(TEMPLATE_PATH, 'html', 'config_template')
    ...
    # 保存项目路径
    project_path = os.path.join(output_dir, 'config_page_template')
