...
class SlackClient(Messaging):
    ...
    def upload_file(
        self, filename: str, channel: str, thread_ts: Optional[str] = None
    ) -> SlackResponse:
        """
        ...
        Args:
            ...
            channel (str): The channel IDs where the file should be uploaded
        ...
        try:
            message_args = {"file": filename, "channel": channel}
            if thread_ts:
                message_args[thread_ts] = thread_ts
            logger.debug(f"Uploading file to channel(s) {channel}")
