...
class LoreWeaveParser:
    ...
    def sync_with_github_issues(self, use_github_api=True, issue_limit=30, github_token=None):
        """
        ...
        try:
            ...
            if not issues_data:
                if use_github_api:
                    # Skip sync if no issues were retrieved via the API
                    print("  ⚠️ No issues retrieved; skipping sync.")
                    return False
                else:
                    # Explicit offline mode uses bundled test issue
