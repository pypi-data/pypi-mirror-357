...
def _fetch_users(server, token, course, user_queries, include_computed_scores):
    ...
    if (len(user_queries) == 0):
        ...
        users = lms.api.user.resolve.fetch_and_resolve_users(server, token, course, user_queries,
            keys = None, include_role = True)
...
def _fetch_assignments(server, token, course):
    assignments = lms.api.assignment.list.request(server = server, token = token, course = course)
...
def _add_computed_scores(users, assignments, grades):
    for i in range(len(lms.api.user.common.COMPUTED_SCORE_KEYS)):
        key = lms.api.user.common.COMPUTED_SCORE_KEYS[i]
