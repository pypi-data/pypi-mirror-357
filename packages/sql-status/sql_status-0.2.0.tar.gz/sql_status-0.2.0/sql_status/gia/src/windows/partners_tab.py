...
class PartnersTab(QWidget):
    ...
    def init_ui(self):
        ...
        add_btn.setStyleSheet("background-color: #0C4882; color: white; font-family: 'Bahnschrift Light SemiCondensed';")
        add_btn.clicked.connect(self.open_add_partner_dialog)
    ...
    def open_add_partner_dialog(self):
        dlg = QDialog(self)
        dlg.setWindowTitle("Добавить партнёра")
        dlg.setMinimumWidth(500)
        form = QFormLayout(dlg)
        # Получаем типы партнёров
        type_combo = QComboBox()
        try:
            types = self.db.get_partner_types()
            for t in types:
                type_combo.addItem(t[0])
        except Exception:
            type_combo.addItem("Ошибка загрузки типов")
        ...
        password_input.setPlaceholderText("Пароль для входа")
        password_input.setEchoMode(QLineEdit.Password)
        form.addRow("Тип", type_combo)
        form.addRow("Наименование", name_input)
        form.addRow("Директор", director_input)
        form.addRow("Email (логин)", email_input)
        ...
        def save():
            try:
                ...
                type_id = self.db.get_partner_type_id(type_name)
                if not type_id:
                    type_id = self.db.add_partner_type(type_name)
                partner_name = name_input.text()
                director = director_input.text()
                ...
                if not email or not password:
                    QMessageBox.warning(dlg, "Ошибка", "Заполните Email и пароль")
                    return
                # 1. Добавляем партнёра
                self.db.execute_non_query(
                    "INSERT INTO partners (partner_type_id, name, director_name, email, phone, legal_address, inn, rating) VALUES (?, ?, ?, ?, ?, ?, ?, 0)",
                    (type_id, partner_name, director, email, phone, address, inn)
                )
                # 2. Получаем partner_id
                partner_id = self.db.get_single_value("SELECT partner_id FROM partners WHERE email=?", (email,))
                # 3. Регистрируем пользователя с ролью partner
                self.db.register_user(email, password, "partner", partner_id)
                dlg.accept()
                self.load_partners()
                QMessageBox.information(self, "Успех", "Партнёр и пользователь добавлены")
            except Exception as e:
