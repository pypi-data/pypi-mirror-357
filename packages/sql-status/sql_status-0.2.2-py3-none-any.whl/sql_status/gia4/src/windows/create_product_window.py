...
class CreateProductWindow(QDialog):
    ...
    def init_ui(self):
        ...
        # Материал (только выбор, без ввода)
        self.material_combo = QComboBox()
        self.material_combo.setEditable(False)
        self.material_combo.addItem("")
        try:
            rows = self.db.execute_query("SELECT name FROM materials")
            for row in rows:
                self.material_combo.addItem(row[0])
        except Exception:
            self.material_combo.addItem("Ошибка загрузки материалов")
        form.addRow(QLabel("Материал:"), self.material_combo)
    ...
    def create_product(self):
        ...
        if not type_name or not material_name or not name or not article or not min_price or not coef:
        ...
        if type_row:
            ...
            # Добавляем новый тип в выпадающий список
            self.type_combo.addItem(type_name)
            type_row = self.db.execute_query("SELECT product_type_id FROM product_types WHERE name=?", (type_name,))
            product_type_id = type_row[0][0] if type_row else None
        if not product_type_id:
            QMessageBox.warning(self, "Ошибка", "Не удалось определить тип продукции. Проверьте поле 'Тип'.")
            return
        # Проверяем, есть ли такой материал
        material_row = self.db.execute_query("SELECT material_id FROM materials WHERE name=?", (material_name,))
        if material_row:
            material_id = material_row[0][0]
        else:
            QMessageBox.warning(self, "Ошибка", "Материал не найден. Добавьте материал заранее.")
            return
        # Создаём продукт с product_type, product_type_id, material, material_id
        try:
            self.db.execute_non_query(
                "INSERT INTO products (product_type, product_type_id, material, material_id, name, article, min_price) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (type_name, product_type_id, material_name, material_id, name, article, min_price_val)
            )
