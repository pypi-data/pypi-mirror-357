from PyQt5.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QComboBox, QLineEdit, QMessageBox, QScrollArea, QFrame, QDialog, QTextEdit, QInputDialog
)
...
class OrdersTab(QWidget):
    ...
    def load_orders(self, filter_col=None, filter_val=None, sort_col=None, sort_desc=False, status_filter="Все"):
        self.clear_cards()
        # Для партнёра показываем только его заявки
        if self.role == "partner":
            ...
            where_clauses = []
            filter_val = self.orders_filter_input.text()
            if filter_val:
                where_clauses.append("(" + " OR ".join([
                    "pr.name LIKE ?", "pr.article LIKE ?", "a.status LIKE ?"
                ]) + ")")
                params.extend([f"%{filter_val}%"] * 3)
            status_val = self.orders_status_filter_combo.currentText() if hasattr(self, 'orders_status_filter_combo') else "Все"
            ...
            try:
                ...
                for row in rows:
                    self.add_partner_order_card(row)
    ...
    def add_partner_order_card(self, row):
        ...
        if status == "новая":
            ...
            btn_edit.clicked.connect(lambda _, app_id=application_id, qty=quantity: self.edit_partner_order(app_id, qty))
            ...
            btn_delete.clicked.connect(lambda _, app_id=application_id: self.delete_partner_order(app_id))
    ...
    def edit_partner_order(self, application_id, quantity):
    ...
    def delete_partner_order(self, order_id):
