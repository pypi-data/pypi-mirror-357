...
from backend.DB_Operations import fetch_properties_for_category, fetch_property_details_with_filter, fetch_property_details_without_filter, has_filter_column
...
def on_category_change(*args, category, cat_mapped_value, property_frame, root, on_final_uid=None, clear_uid_table=None):
    ...
    result = fetch_properties_for_category(conn)
    if not result:
        display_message(property_frame, "No properties found for the selected category.", msg_type="error")
        return
    ...
    def on_any_dropdown_change(*args):
        # Disable all dropdowns below the first empty one
        for i, widget in enumerate(dropdown_widgets):
            value = widget.var.get()
            if value not in widget.options:
                # Disable all below this
                for w in dropdown_widgets[i+1:]:
                    w.grid_remove()
                break
            else:
                # Enable the next one if not already enabled
    ...
    def create_property_dropdown(header_index, prop_values):
        ...
        has_filter = has_filter_column(conn, table_name)
        results = []
        if has_filter:
            ...
            for item in prop_values:
                ...
                results = fetch_property_details_with_filter(conn, table_name, item, cummulative_prop_val)
                if results:
                    break
                
        else:
            results = fetch_property_details_without_filter(conn, table_name)
        ...
        def on_property_change(*args, prop_var=prop_var, header_index=header_index):
            ...
            on_any_dropdown_change()  # <-- Check and update enabled/disabled state
        ...
        # Disable all except the first at creation
        if header_index != 0:
