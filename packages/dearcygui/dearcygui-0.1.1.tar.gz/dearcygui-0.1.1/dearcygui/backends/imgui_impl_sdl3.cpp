...
static void ImGui_ImplSDL3_PlatformSetImeData(ImGuiContext*, ImGuiViewport* viewport, ImGuiPlatformImeData* data)
{
    ...
    if (data->WantVisible)
    {
        ...
        r.x = (int)(data->InputPos.x/dpi);
        r.y = (int)(data->InputPos.y/dpi);
        r.w = 1;
...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event* event)
{
    ...
    switch (event->type)
    {
        case SDL_EVENT_MOUSE_MOTION:
        {
            ...
            if (ImGui_ImplSDL3_GetViewportForWindowID(event->motion.windowID) == NULL || window == NULL)
                return false;
...
#endif
    float dpi = SDL_GetWindowPixelDensity(focused_window);
    if (is_app_focused)
    {
        ...
        if (bd->MouseCanUseGlobalState && bd->MouseButtonsDown == 0)
        {
            ...
            io.AddMousePosEvent((mouse_x_global - window_x) * dpi, (mouse_y_global - window_y) * dpi);
        }
    }
}
...
void ImGui_ImplSDL3_NewFrame(int display_h, int display_w)
{
    ...
    // Do not let imgui handle dpi scaling
