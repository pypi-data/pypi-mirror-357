...
- Add io_options.fetch_html parameter to only retrieve HTML if requested.
  The default is `False`, which is a difference from the previous behavior.
  (Prior to v0.8.0 it was necessary to get the HTML to get the wikitext.)
- Get the timestamp associated with the Word page and write this to the default
  output text files.
