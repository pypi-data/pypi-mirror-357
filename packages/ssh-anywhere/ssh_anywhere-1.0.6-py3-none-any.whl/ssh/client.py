...
expect {{
    ...
    -re ".*'s password:" {{
        puts "DEBUG: User-specific password prompt detected"
        send "{escaped_password}\\r"
        exp_continue
    }}
    "Enter passphrase for key*:" {{
...
expect {{
    -re "\\$ |# |> " {{
        # Connected successfully (primary pattern)
    }}
    -re "\\$\\s*$" {{
        # Shell prompt ending with $ (fallback for no space)
    }}
    -re "#\\s*$" {{
        # Root prompt ending with # (fallback for no space)
    }}
    -re ">\\s*$" {{
        # Generic prompt ending with > (fallback for no space)
    }}
    timeout {{
