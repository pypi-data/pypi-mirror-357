...
class SSHClient:
    ...
    def __init__(self,
        ...
        self.username = str(username) if username is not None else os.getenv('USER', 'user')
        self.port = port
        self.password = str(password) if password is not None else None
        self.private_key_path = Path(str(private_key_path)).expanduser() if private_key_path else None
        self.jump_host = jump_host
        self.ssh_options = ssh_options or {}
        
        # Sanitize ssh_options values
        if self.ssh_options:
            sanitized_options = {}
            for key, value in self.ssh_options.items():
                sanitized_options[str(key)] = str(value) if value is not None else ""
            self.ssh_options = sanitized_options
...
'''
    ...
    def exec_sequential_commands(self, commands: list, timeout: int = 60, command_delay: float = 0.5) -> list:
        ...
        # Sanitize each command in the list
        sanitized_commands = []
        for cmd in commands:
            sanitized_cmd = str(cmd) if cmd is not None else ""
            if sanitized_cmd.strip():  # Only add non-empty commands
        ...
        if self.use_password_auth or self.key_passphrase:
            return self._execute_sequential_commands_with_expect(sanitized_commands, timeout, command_delay)
        else:
            return self._execute_sequential_commands_direct(sanitized_commands, timeout, command_delay)
...
# Wait for initial prompt - handle various prompt formats
expect {{
    -re ".*\\$\\s*$" {{
        # Shell prompt ending with $ (with or without space)
    }}
    -re ".*#\\s*$" {{
        # Root prompt ending with # (with or without space)
    }}
    -re ".*>\\s*$" {{
        # Generic prompt ending with > (with or without space)
    }}
    -re "\\$ |# |> " {{
...
expect {{
    ...
    # Connected successfully - handle various prompt formats
    -re ".*\\$\\s*$" {{
        # Shell prompt ending with $ (with or without space)
    }}
    -re ".*#\\s*$" {{
...
# Wait for command to complete - use a more robust approach
set command_complete 0
...
'''
    ...
    def interact(self, escape_char: str = '~'):
        ...
        # Sanitize inputs
        escape_char = str(escape_char) if escape_char is not None else '~'
        if len(escape_char) > 1:
            escape_char = escape_char[0]  # Use only first character
...
expect {{
    ...
    # Success - we got a shell prompt (handle various formats)
    -re ".*\\$\\s*$" {{
        interact
        exit 0
    }}
