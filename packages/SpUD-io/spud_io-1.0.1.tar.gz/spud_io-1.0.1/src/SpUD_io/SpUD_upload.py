...
def uploadRFAs(RFAs, connectParams, uploadUser, enableProgressBar=False):
    ...
    uploadDateSQL = sql.SQL('INSERT INTO uploads ({}) VALUES ({})').format(sql.SQL(', ').join([sql.Identifier('uid'), sql.Identifier('user'), sql.Identifier('date')]), sql.SQL(', ').join(['DEFAULT', uploadUser, uploadDate]))
    ...
    with connection.transaction():
        ...
        try:
            ...
            print(f'Excecute Error: {error}')
            print(f'Error inserting upload date')
            raise Rollback()
