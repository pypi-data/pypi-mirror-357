...
class Qatar_ID_Full_Name(BaseModel):
    """Extract the name from the OCR extracted text of a Qatari National ID or a Qatari Residency Permit"""
    full_name_ar: str = Field(..., description="Full name in Arabic")
    full_name_en: str = Field(..., description="Full name in English" )

@tool(args_schema=Qatar_ID_Full_Name)
def qatar_id_full_name_tool(full_name_ar, full_name_en):
    parts = full_name_en.split()
    # parts = [
    #     part for part in parts
    #     if not any(keyword in part.upper() for keyword in ["ISTRY", "MINISTRY", "MINISTER"])
    # ]
    try:
        first_name = parts[0]
        last_name = parts[-1]
    except Exception as e:
        return {'error':'covered_photo', 'error_detail':"Name in English not found in image."}

    return {'name':full_name_en, 'first_name':first_name, 'last_name':last_name}
...
def route(result):
    if isinstance(result, AgentFinish):
        ...
        tools = {
            "qatar_id_full_name_tool": qatar_id_full_name_tool,
        }
        return tools[result.tool].run(result.tool_input)


def extraction_chain_QAT_Name(ocr_text, openai_key):
    #Takes in OCR text of front ID
    try:
        gpt_model = 'gpt-4o'
        tools_func = [format_tool_to_openai_function(qatar_id_full_name_tool)]
        model = ChatOpenAI(model=gpt_model, temperature=0,
                           openai_api_key=openai_key)
        model = model.bind(functions=tools_func)

        prompt = ChatPromptTemplate.from_messages([
            ("system",
             "Extract the relevant information, if not explicitly provided do not guess, leave empty string. Extract partial info."
             ),
            ("user", "{ocr_text}")
        ])
        ...
        front_data = {'name':result.get('name',''), 'first_name':result.get('first_name',''), 'last_name':result.get('last_name','')}
...
def qatar_front_id_extraction(client, image_data, front_id_text, front_id_text_description, openai_api_key):
    ...
    front_data = extraction_chain_QAT_Name(front_id_text_description, openai_api_key)
