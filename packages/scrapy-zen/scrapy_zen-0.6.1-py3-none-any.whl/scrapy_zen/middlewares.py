...
class PreProcessingMiddleware:
    ...
    def process_request(self, request: Request, spider: Spider) -> None:
        ...
        request.meta['request_at'] = time.time()
        return None

    def process_response(self, request: Request, response: Response, spider: Spider) -> Response:
        request.meta["response_at"] = time.time()
        return response
    ...
    async def process_spider_output(self, response, result, spider):
        async for i in result:
            if isinstance(i, dict):
                i['scraped_at'] = datetime.now(timezone.utc).isoformat(timespec='milliseconds').replace('+00:00', 'Z')
                i['request_at'] = response.meta.get("request_at")
                i['response_at'] = response.meta.get("response_at")
            yield i
