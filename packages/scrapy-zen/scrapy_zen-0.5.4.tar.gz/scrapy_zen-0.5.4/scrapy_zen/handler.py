...
rnet_impersonate_fingerprints = [getattr(Impersonate, attr) for attr in dir(Impersonate) if not attr.startswith("__")]
...
class ZenDownloadHandler:
    ...
    def __init__(self, crawler: Crawler):
        ...
        self.rnet_handler = RnetDownloadHandler.from_crawler(crawler)
    ...
    def download_request(self, request: Request, spider: Spider) -> Deferred | Deferred[Response]:
        ...
        elif request.meta.get("rnet_impersonate"):
            impersonate = request.meta.get("rnet_impersonate")
            if impersonate == "random":
                request.meta["rnet_impersonate"] = random.choice(rnet_impersonate_fingerprints)
            return self.rnet_handler.download_request(request, spider)
