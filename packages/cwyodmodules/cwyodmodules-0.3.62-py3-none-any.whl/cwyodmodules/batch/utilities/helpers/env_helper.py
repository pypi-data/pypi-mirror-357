...
class EnvHelper:
    ...
    def __load_config(self, **kwargs) -> None:
        ...
        self.LOGLEVEL = self.keyvault.get_secret("logging-level")
        ...
        # Azure
        self.AZURE_SUBSCRIPTION_ID = self.keyvault.get_secret("subscription-id")
        self.AZURE_RESOURCE_GROUP = self.keyvault.get_secret("resource-group-name")
        self.AZURE_HEAD_RESOURCE_GROUP = self.head_keyvault.get_secret(
            "resource-group-name"
        )
        self.AZURE_RESOURCE_ENVIRONMENT = self.keyvault.get_secret(
            "resource-group-environment"
        )
        self.AZURE_RESOURCE_PRIVATE = (
            self.keyvault.get_secret("run-private-endpoint").lower() == "true"
        )
        self.PROJECT_CODE = self.keyvault.get_secret("project-code")
        ...
        self.AZURE_FUNCTION_APP_ENDPOINT = self.keyvault.get_secret(
        ...
        if self.DATABASE_TYPE == DatabaseType.COSMOSDB.value:
            ...
            self.AZURE_COSMOSDB_ACCOUNT_KEY = self.keyvault.get_secret(
        ...
        # PostgreSQL configuration
            ...
            if azure_postgresql_info:
                ...
                self.POSTGRESQL_DATABASE = self.keyvault.get_secret(
                    f"{self.POSTGRESQL_NAME}-default-database-name"
                )
                self.POSTGRESQL_HOST = self.keyvault.get_secret(
        ...
        # self.AZURE_COMPUTER_VISION_NAME = self.head_keyvault.get_secret(
        #     "cognitive-kind-ComputerVision"
        # )
        self.AZURE_COMPUTER_VISION_NAME = ""
        # self.AZURE_COMPUTER_VISION_ENDPOINT = self.head_keyvault.get_secret(
        ...
        if self.AZURE_AUTH_TYPE == "rbac":
            ...
            self.AZURE_SEARCH_KEY = self.keyvault.get_secret("AZURE_SEARCH_KEY")
            self.AZURE_OPENAI_API_KEY = self.keyvault.get_secret(
                "AZURE_OPENAI_API_KEY"
            )
            self.AZURE_COMPUTER_VISION_KEY = self.keyvault.get_secret(
                "AZURE_COMPUTER_VISION_KEY"
            )

        # Set env for Azure OpenAI
        self.AZURE_AI_SERVICES_NAME = self.head_keyvault.get_secret(
        ...
        self.AZURE_FORM_RECOGNIZER_NAME = self.keyvault.get_secret(
            "cognitive-kind-FormRecognizer"
        )
        self.AZURE_FORM_RECOGNIZER_ENDPOINT = self.keyvault.get_secret(
        ...
        self.AZURE_CONTENT_SAFETY_NAME = self.head_keyvault.get_secret(
            "cognitive-kind-ContentSafety"
        )
        self.AZURE_CONTENT_SAFETY_ENDPOINT = self.head_keyvault.get_secret(
            f"{self.AZURE_CONTENT_SAFETY_NAME}-endpoint"
        )

        # Speech Service
        self.AZURE_SPEECH_SERVICE_NAME = self.head_keyvault.get_secret(
            "cognitive-kind-SpeechServices"
        )
        self.AZURE_SPEECH_ENDPOINT = self.head_keyvault.get_secret(
        ...
        self.AZURE_SPEECH_KEY = self.head_keyvault.get_secret(f"{self.AZURE_SPEECH_SERVICE_NAME}-key") # changed from None to allow private endpoint communication
