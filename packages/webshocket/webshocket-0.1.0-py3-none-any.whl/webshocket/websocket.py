...
from .exceptions import ConnectionFailedError, WebSocketError
        ...
        try:
            if isinstance(self.handler, DefaultWebSocketHandler):
                ...
                async for data in websocket_protocol:
                    await self.handler.on_receive(
                        _websocket,
                        self._to_packet(data),
                    )
    ...
    async def recv(self, timeout: int | None = 30) -> Packet:
        ...
        try:
            ...
            try:
                ...
                # raise MessageError("Received malformed or invalid packet data.") from e

                packet: Packet = Packet(data=data, source=PacketSource.CUSTOM)
                return packet
