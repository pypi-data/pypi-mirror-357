...
def start(tag, duotag): #REAL   is calledby run_job
    ...
    #####################################################################################
    # encapsulate to ''
    #CODE = f"bash -c '{CODE}'" # REPLACED 20250625
    ##### I am replacing by exec to replace bash
    CODE = f"exec '{CODE}'" # this should replace shell and sojhuld solve the ppid 1 adter killing screen
    CMD = f"{ENV} {SCR} {CODE}"
    args = shlex.split(CMD)
    #print(args)
    with open("/tmp/cronvice.log", "a") as f:
