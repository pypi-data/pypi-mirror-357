...
class DovchaApp:
    def __init__(self, tamplate_dir='templates', static_dir='static'):
        self.routes = dict()

        self.template_env = Environment(
            loader=FileSystemLoader(os.path.abspath(tamplate_dir))
        )
    ...
    def add_route(self, path, handler, allowed_methods=None):
        assert path not in self.routes, f"Duplicate route. Please change URL: {path}"     
        
       
        
        if allowed_methods is None:
            allowed_methods = ['get', 'post', 'put', 'delete', 'patch', 'options', 'head', 'trace', 'connect']
