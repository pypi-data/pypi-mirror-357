...
class HypixelClient:
    ...
    def get_player(self, uuid_or_name: str):
        if "-" in uuid_or_name or len(uuid_or_name) == 32:
            return self._request("player", {"uuid": uuid_or_name})
        uuid = self.get_uuid(uuid_or_name)
        return self._request("player", {"uuid": uuid})

    def get_guild_by_player(self, uuid: str):
        return self._request("guild", {"player": uuid})

    def get_guild_by_name(self, name: str):
        return self._request("guild", {"name": name})

    def get_friends(self, uuid: str):
        return self._request("friends", {"uuid": uuid})

    def get_status(self, uuid: str):
        return self._request("status", {"uuid": uuid})

    def get_recent_games(self, uuid: str):
        return self._request("recentgames", {"uuid": uuid})
