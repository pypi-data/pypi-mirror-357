...
class AssertUtils(AssertExcel, metaclass=SingletonMeta):
    ...
    def assert_length(actual, expected, assert_method):
        ...
        if type(actual) != type(expected):
            actual_len = len(actual) if isinstance(actual, list) else int(actual)
            expected_len = len(expected) if isinstance(expected, list) else int(expected)
        else:
    ...
    def diff_data(self, actual, expect, path='$', diff_key=None):
        if diff_key is None: diff_key = []
        if isinstance(expect, list):
            if type(actual) != type(expect):
                diff_key.append(f'{path}该路径类型不一致')
            elif type(expect[0]) != type(actual[0]):
                diff_key.append(f'{path + ".0"}该路径类型不一致')
            elif isinstance(expect[0], list):
                self.diff_data(actual[0], expect[0], path=path + '.0', diff_key=diff_key)
            ...
            for k, v in expect.items():
                ...
                if k not in actual:
                    ...
                    self.diff_data(actual[k], v, new_path, diff_key)
        ...
        for val in validates:
            for k, v in val.items():
                ...
                if isinstance(methods, tuple):
                    methods[1](actual, expected, methods[0])
                else:
                    methods(actual, expected)
