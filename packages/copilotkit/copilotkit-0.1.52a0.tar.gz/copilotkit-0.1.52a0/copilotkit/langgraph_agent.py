...
class LangGraphAgent(Agent):
    ...
    async def _stream_events( # pylint: disable=too-many-locals
        ...
        thread_id = thread_id or config.get("configurable", { "thread_id": str(uuid.uuid4()) }).get('thread_id')
        ...
        time_travel_last_user_message = None
        if len(agent_state.values.get("messages", [])) > len(non_system_messages):
            langchain_messages_from_arg_messages = self.convert_messages(messages)

            # Find the last user message by working backwards from the last message
            last_user_message_id = None
            for i in range(len(langchain_messages_from_arg_messages) - 1, -1, -1):
                if isinstance(langchain_messages_from_arg_messages[i], HumanMessage):
                    time_travel_last_user_message = langchain_messages_from_arg_messages[i]
                    last_user_message_id = langchain_messages_from_arg_messages[i].id
                    break

            if last_user_message_id:
                time_travel_checkpoint = await self.get_checkpoint_by_message(last_user_message_id, config)

        if time_travel_checkpoint:
            # all used state values are now derived from the checkpoint.
            agent_state = time_travel_checkpoint
            initial_state = time_travel_checkpoint.values
            # Act as if no new messages were sent by the user
            langchain_messages = [time_travel_last_user_message]
        active_interrupts = agent_state.tasks[0].interrupts if agent_state.tasks and agent_state.tasks[0].interrupts else None
        initial_state["messages"] = agent_state.values.get("messages", [])
        ...
        if mode == "continue" and not has_active_interrupts and time_travel_checkpoint is None:
            await self.graph.aupdate_state(config, state, as_node=node_name)
        elif time_travel_checkpoint is not None:
            await self.graph.aupdate_state(time_travel_checkpoint.config, time_travel_checkpoint.values, as_node=time_travel_checkpoint.next[0] or "__start__")
        ...
        if time_travel_checkpoint:
            stream_input = state if mode == "start" else None
        elif resume_input:
    ...
    async def get_checkpoint_by_message(self, message_id: str, config: RunnableConfig):
        target_state = None
        history_list = []
        ...
        async for entry in self.graph.aget_state_history({"configurable": {"thread_id": thread_id}}):
        ...
        for idx, entry in enumerate(history_list):
            msgs = entry.values.get("messages", [])
            if any(m.id == message_id for m in msgs):
                target_state = history_list[idx - 1]
                break
