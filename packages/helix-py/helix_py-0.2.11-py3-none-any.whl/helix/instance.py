...
class Instance:
    def __init__(self, config_path=None, port=None, verbose=False):
        ...
        output = [self.process_line.sub('', line).strip() for line in process.stdout]
    ...
    def stop(self):
        if not self.instance_id or self.instance_id not in self.ids_running: # Instance ID not found
