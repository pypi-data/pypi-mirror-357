...
class Plot(abc.ABC):
    ...
    :param \\**kwargs: Extra arguments to pass to `pyplot.subplots`_.
    ...
    def legend(self, **kwargs):
        ...
        :param \\**kwargs: Extra arguments to pass to `Figure.legend`_.
        """
    ...
    def save(self, filename, format, width, height, **kwargs):
        """
        Save the plot to disk (a thin wrapper for `Figure.savefig`_).
        ...
        :param \\**kwargs: Extra arguments for ``savefig``; the defaults are
            ``transparent=False`` and ``bbox_inches='tight'``.
        """
        save_args = {'transparent': False, 'bbox_inches': 'tight'}
        # Do not save matplotlib version in the image metadata.
        ...
        if format == 'pdf':
            ...
            save_args['metadata'] = {'Software': None}
        ...
        for rix, yf in enumerate(self.__yfac_lvls):
            for cix, xf in enumerate(self.__xfac_lvls):
                ...
                if subplot.is_last_col():
                    ax2 = ax.twinx()
                    ax2.set_ylabel(
                        _label(self.__yfac[1], yf, 'yfac'), rotation=270
                    )
...
def cred_ints(ax, data, x, ci, palette='Blues', **kwargs):
    ...
    :param palette: The colour map name or a list of colours.
    :param \\**kwargs: Extra arguments to pass to `Axes.plot`_ and
        `Axes.fill_between`_.
    :returns: A list of the series that were plotted.
    """
    hs = []
    for cint, colour in colour_iter(data[ci], palette=palette, reverse=True):
        if cint > 0:
            label = '{:d}% CI'.format(cint)
        else:
            label = 'Median'
        df = data[data['prob'] == cint]
        y_min = df['ymin']
        y_max = df['ymax']
        args = {'edgecolor': 'None', 'facecolor': colour}
        args.update(kwargs)
...
def observations(ax, data, label='Observations', future=False, **kwargs):
    ...
    :param \\**kwargs: Extra arguments to pass to `Axes.plot`_.
    :returns: A list of the series that were plotted.
    """
    if future:
        args = {'color': '#eee8d5', 'edgecolors': 'black', 'linewidths': 1}
    else:
        args = {'color': 'black'}
    args.update(kwargs)
...
def series(ax, data, x, y, scales, legend_cols=True, **kwargs):
    ...
    for ix, col in enumerate(cols):
        ...
        for lix, val in enumerate(levels[ix]):
            ...
            kw['label'] = _label(fmts[ix], val, col)
            if names[ix] != 'color':
                kw['color'] = 'black'
