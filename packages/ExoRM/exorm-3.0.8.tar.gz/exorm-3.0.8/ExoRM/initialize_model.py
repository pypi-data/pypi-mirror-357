def initialize_model(s_filepath = None, degree = 1, iterations = 500, n_s_values = 100, s_value_range_factor = [1 / 12, 1 / 6], *, weight_conv_window = 101, val_split = 0.1):
    ...
    if s_filepath is not None:
        pandas.DataFrame(s_residuals, columns = ['s_value', 'val_mape']).to_csv(s_filepath, index = False)
