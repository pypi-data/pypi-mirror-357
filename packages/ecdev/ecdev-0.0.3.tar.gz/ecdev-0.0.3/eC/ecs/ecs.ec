...
static void WriteMain(const char * fileName)
{
   ...
   if(f)
   {
      ...
      if(dcomSymbols)
         f.Printf("void __eCRegisterModule_%s(Module module);\n\n", mainModuleName);
      ...
      for(module = _imports.first; module; module = module.next)
      {
         ...
         if(module.importType == staticImport)
         {
            ...
            {
               f.Printf("bool __stdcall __eCDll_Load_%s(Module module);\n", module.name);
               f.Printf("bool __stdcall __eCDll_Unload_%s(Module module);\n", module.name);
            }
            else*/
            {
               f.Printf("bool __eCDll_Load_%s(Module module);\n", module.name);
               f.Printf("bool __eCDll_Unload_%s(Module module);\n", module.name);
            }
         }
         for(_class = module.classes.first; _class; _class = _class.next)
         {
            ...
            //if(!_class.isRemote)
            {
               //if(strcmp(_class.name, "SerialBuffer"))
               {
                  for(method = _class.methods.first; method; method = method.next)
                  {
                     ...
                     else if((!strcmp(_class.name, "float") || !strcmp(_class.name, "double") || module.name) && module.importType != staticImport && (!meth || !meth.dataType.dllExport))
                     {
                        ...
                        type.refCount++;
                        sprintf(name, "__eCMethod_%s_%s", className, method.name);
                        PrintType(type, name, true);
                        f.Printf("%s;\n", name);
                        delete type;
                        */
                        //f.Printf("void * __eCMethod_%s_%s;\n", className, method.name);

                        f.Printf("%sint (*__eCMethod_%s_%s)();\n", attributeCommon, className, method.name);
                     }
               ...
               for(prop = _class.properties.first; prop; prop = prop.next)
               {
                  ...
                  {
                     if(prop.hasSet)
                        f.Printf("%svoid * __eCProp_%s_Set_%s;\n", attributeCommon, className, propName);
                     if(prop.hasGet)
                        f.Printf("%svoid * __eCProp_%s_Get_%s;\n", attributeCommon, className, propName);
                  }
                  f.Printf("%sProperty __eCProp_%s_%s;\n", attributeCommon, className, propName);
                  anyProp = true;
               }
            }
         }
         for(function = module.functions.first; function; function = function.next)
         {
            ...
            if(module.name && module.importType != staticImport && (!func || !func.dataType || !func.dataType.dllExport))
            {
               ...
               f.Printf("%svoid * __eCFunction_%s;\n", attributeCommon, functionName);
               anyFunction = true;
            }
         }
      }
      ...
      for(defModule = ::modules.first; defModule; defModule = defModule.next)
      {
         ...
         f.Printf("void __eCRegisterModule_%s(Module module);\n", moduleName);
         f.Printf("void __eCUnregisterModule_%s(Module module);\n", moduleName);
         if(defModule.globalInstance)
         {
            f.Printf("void __eCCreateModuleInstances_%s();\n", moduleName);
            f.Printf("void __eCDestroyModuleInstances_%s();\n", moduleName);
         }
      }
      ...
      {
         /*if(targetPlatform == win32)
            f.Printf("\nbool __stdcall __eCDll_Load_%s(Module module)\n{\n", projectName);
         else*/
      ...
      else
      {
         ...
         if(targetPlatform == win32 && !isConsole)
            f.Puts("      __thisModule = eCrt_Initialize(1, 0, null);\n\n");
         else
            f.Puts("      __thisModule = eCrt_Initialize(1, _argc, (void *)_argv);\n\n");
      ...
      if(::modules.count)
      {
         for(defModule = ::modules.first; defModule; defModule = defModule.next)
         {
            ...
            strcpy(moduleName, defModule.name);
            FixModuleName(moduleName);
            f.Printf("   __eCRegisterModule_%s(module);\n", moduleName);
         }
         f.Printf("\n");
      }
            ...
            for(_class = module.classes.first; _class; _class = _class.next)
            {
               ...
               //if(!_class.isRemote)
               {
                     ...
                     if(!meth || !meth.dataType.dllExport)
                     {
                        if(method.isVirtual || ((!strcmp(_class.name, "float") || !strcmp(_class.name, "double") || module.name) && module.importType != staticImport))
                        {
                           f.Printf("   method = eClass_FindMethod(%s, \"%s\", module);\n",
                              classID, method.name);
                           if(method.isVirtual)
                              f.Printf("   if(method) __eCVMethodID_%s_%s = method.vid;\n", className, method.name);
                           else
                              f.Printf("   if(method) __eCMethod_%s_%s = method.function;\n", className, method.name);
                        }
                     }
                  }
                  ...
                  for(prop = _class.properties.first; prop; prop = prop.next)
                  {
                     ...
                     f.Printf("   __eCProp_%s_%s = _property = eClass_FindProperty(%s, \"%s\", module);\n",
                        className, propName, classID, prop.name);

                     if((!strcmp(_class.name, "float") || !strcmp(_class.name, "double") || module.name) && module.importType != staticImport)
                     {
                        if(prop.hasSet)
                           f.Printf("   if(_property) __eCProp_%s_Set_%s = _property.Set;\n", className, propName);
                        if(prop.hasGet)
                           f.Printf("   if(_property) __eCProp_%s_Get_%s = _property.Get;\n", className, propName);
                     }
                  }
                  f.Printf("\n");
               }
         ...
         if(module.functions.count)
         {
            for(function = module.functions.first; function; function = function.next)
            {
               ...
               if(module.name && module.importType != staticImport && (!func || !func.dataType || !func.dataType.dllExport))
               {
                  ...
                  f.Printf("   if(function) __eCFunction_%s = function.function;\n", functionName);
                  f.Printf("\n");
               }
            }
            f.Printf("\n");
      ...
      for(defModule = ::modules.first; defModule; defModule = defModule.next)
         if(defModule.globalInstance)
         {
            if(!strcmp(defModule.name, "i18n"))
               f.Printf("   __eCCreateModuleInstances_i18n();\n");
         }
      if(i18n)
      {
         if(isDynamicLibrary)
      ...
      if(::modules.count)
      {
         for(defModule = ::modules.first; defModule; defModule = defModule.next)
            if(defModule.globalInstance)
            {
               ...
               strcpy(moduleName, defModule.name);
               FixModuleName(moduleName);
               f.Printf("   __eCCreateModuleInstances_%s();\n", moduleName);
            }
      ...
      if(isDynamicLibrary)
      {
         ...
         {
            /*if(targetPlatform == win32)
               f.Printf("\nbool __stdcall __eCDll_Unload_%s(Module module)\n{\n", projectName);
            else*/
      ...
      // Then check if there's any global instances to destroy
      {
         ...
         if(::modules.count)
         {
            for(defModule = ::modules.last; defModule; defModule = defModule.prev)
               if(defModule.globalInstance)
               {
                  ...
                  strcpy(moduleName, defModule.name);
                  FixModuleName(moduleName);
                  f.Printf("   __eCDestroyModuleInstances_%s();\n", moduleName);
               }
         ...
         if(destroyI18n)
            f.Printf("   __eCDestroyModuleInstances_i18n();\n");
      }
      if(isDynamicLibrary)
      {
         f.Puts("   }\n");
         ...
         if(::modules.count)
         {
            for(defModule = ::modules.first; defModule; defModule = defModule.next)
            {
               ...
               strcpy(moduleName, defModule.name);
               FixModuleName(moduleName);
               f.Printf("   __eCUnregisterModule_%s(module);\n", moduleName);
            }
            f.Printf("\n");
         }
         f.Puts("   if(__currentModule == module)\n");
...
{
   Class dcomClientObjectClass = eSystem_FindClass(privateModule, "ecereNet::DCOMClientObject");
   OldLink deriv;
   ...
   if(dcomClientObjectClass && dcomClientObjectClass.derivatives.first)
   {
      ...
      for(deriv = dcomClientObjectClass.derivatives.first; deriv; deriv = deriv.next)
      {
         ...
         DeclareClass(null, FindClass("ecereNet::DCOMClientObject"), "__eCClass___eCNameSpace__eC__net__DCOMClientObject");
         f.Printf("class %s : ecereNet::DCOMClientObject\n", _class.fullName);
         f.Printf("{\n");
         ...
         {
            int vid;
            f.Printf("   virtual void CallVirtualMethod(uint __eCMethodID, SerialBuffer __eCBuffer)\n");
            f.Printf("   {\n");
            f.Printf("      switch(__eCMethodID)\n");
            f.Printf("      {\n");
                     ...
                     f.Printf("         {\n");
                     {
                        ...
                        for(param = method.dataType.params.first; param; param = param.next)
                        {
                           if(param.kind == classType && !strcmp(param._class.string, "String"))
                           {
                              ...
                              DeclareClass(null, FindClass("StaticString"), "__eCClass_StaticString");
                              DeclareClass(null, FindClass("String"), "__eCClass_String");
                           }
                           else
                           {
                              ...
                              decl = SpecDeclFromString(type, specs, MkDeclaratorIdentifier(MkIdentifier(param.name /*"__eCResult"*/)));
                        ...
                        for(param = method.dataType.params.first; param; param = param.next)
                        {
                           f.Printf("            __eCBuffer.Unserialize(");
                           if(param.kind == classType && !strcmp(param._class.string, "String"))
                           {
                              DeclareClass(null, FindClass("StaticString"), "__eCClass_StaticString");
                        ...
                        for(param = method.dataType.params.first; param; param = param.next)
                        {
                           if(param.kind == classType && ((param._class && param._class.registered && param._class.registered.type == structClass) || !strcmp(param._class.string, "String")) && !param.constant)
                           {
                              ...
                              {
                                 DeclareClass(null, FindClass("StaticString"), "__eCClass_StaticString");
                                 f.Printf("            __eCBuffer.Serialize((StaticString)%s);\n", param.name);
                              }
                              else
                                 f.Printf("            __eCBuffer.Serialize(%s);\n", param.name);
                           }
                        }
                        if(method.dataType.returnType.kind != voidType)
                        {
                           f.Printf("            __eCBuffer.Serialize(__eCResult);\n");
                        }
         ...
         for(method = next; method; method = next)
         {
            ...
            if(method.dataType.name)
            {
               ...
               if(method.dataType)
               {
                  ...
                  DeclareMethod(null,
                     eClass_FindMethod(
                        eSystem_FindClass(privateModule, "ecereNet::DCOMClientObject"), "CallMethod", privateModule),
                     "__eCMethod___eCNameSpace__eC__net__DCOMClientObject_CallMethod");
                  ...
                  for(param = method.dataType.params.first; param; param = param.next)
                  {
                     if(param.kind == classType && ((param._class && param._class.registered && param._class.registered.type == structClass) || !strcmp(param._class.string, "String")) && !param.constant)
                     {
                        ...
                        {
                           DeclareClass(null, FindClass("StaticString"), "__eCClass_StaticString");
                           f.Printf("         __eCBuffer.Unserialize((StaticString)%s);\n", param.name);
                        }
                        else
                           f.Printf("         __eCBuffer.Unserialize(%s);\n", param.name);
                     }
                  }
                  if(method.dataType.returnType.kind != voidType)
                  {
                     ...
                     {
                        DeclareClass(null, FindClass("StaticString"), "__eCClass_StaticString");
                        f.Printf("         __eCBuffer.Unserialize((StaticString)__eCResult);\n");
                     }
                     else
                        f.Printf("         __eCBuffer.Unserialize(__eCResult);\n");
                  }
                  f.Printf("      }\n");
                  f.Printf("      __eCBuffer.Free();\n");
                  //f.Printf("      delete this;\n");
                  f.Printf("      safeDecRef();\n");
                  if(method.dataType.returnType.kind != voidType)
                  {
                     f.Printf("      return __eCResult;\n");
                  }
               }
...
static void BindDCOMServer()
{
   ...
   if(_class)
   {
      ...
      DeclareClass(null, FindClass("ecereNet::DCOMServerObject"), "__eCClass___eCNameSpace__eC__net__DCOMServerObject");
      ...
      for(_class = privateModule.classes.first; _class; _class = _class.next)
      {
         if(_class.isRemote == 3) //2 && !strncmp(_class.fullName, "DCOMServer_", strlen("DCOMServer_")))
         {
            ...
            f.Printf("class DCOM%s : ecereNet::DCOMServerObject\n", _class.fullName);
            ...
            */
            // f.Printf("\n");
            f.Printf("   virtual void CallMethod(uint __eCMethodID, SerialBuffer __eCBuffer)\n");
            f.Printf("   {\n");
            ...
            for(vid = _class.base.vTblSize; vid < _class.vTblSize; vid++)
            {
               ...
               if(method)
               {
                  if(!mutexDeclared)
                  {
                     DeclareClass(null, FindClass("eC::mt::Mutex"), "__eCClass___eCNameSpace__eC__mt__Mutex");
                     DeclareMethod(null,
                        eClass_FindMethod(
                           eSystem_FindClass(privateModule, "eC::mt::Mutex"), "Wait", privateModule),
                              "__eCMethod___eCNameSpace__eC__mt__Mutex_Wait");
                     DeclareMethod(null,
                        eClass_FindMethod(
                           eSystem_FindClass(privateModule, "eC::mt::Mutex"), "Release", privateModule),
                              "__eCMethod___eCNameSpace__eC__mt__Mutex_Release");
                     mutexDeclared = true;
                  }
                  ...
                  if(method.dataType.name)
                  {
                     ...
                     if(method.dataType)
                     {
                        f.Printf("      DCOM%s __eCObject_ = (void *)_vTbl[-1];\n", _class.fullName);
                        if(method.dataType.returnType.kind != voidType)
                        {
                           TypeName resultType;
                           OldList * specs = MkList();
                        ...
                        f.Printf("      incref __eCObject_;\n");
                        f.Printf("      __eCMethod___eCNameSpace__eC__mt__Mutex_Wait(__eCObject_.mutex);\n");
                        ...
                        for(param = method.dataType.params.first; param; param = param.next)
                        {
                           ...
                           {
                              DeclareClass(null, FindClass("StaticString"), "__eCClass_StaticString");
                              f.Printf("      __eCObject_.argsBuffer.Serialize((StaticString)%s);\n", param.name);
                           }
                           else
                              f.Printf("      __eCObject_.argsBuffer.Serialize(%s);\n", param.name);
                        }

                        DeclareMethod(null,
                           eClass_FindMethod(
                              eSystem_FindClass(privateModule, "ecereNet::DCOMServerObject"), "CallVirtualMethod", privateModule),
                           "__eCMethod___eCNameSpace__eC__net__DCOMServerObject_CallVirtualMethod");
                        ...
                        // Check if this method needs to return anything (hasReturnValue)
                        {
                           ...
                           f.Printf("      if(__eCObject_.CallVirtualMethod(%d, %s))\n", vid - _class.base.vTblSize,
                        ...
                        for(param = method.dataType.params.first; param; param = param.next)
                        {
                           if(param.kind == classType && ((param._class && param._class.registered && param._class.registered.type == structClass) || !strcmp(param._class.string, "String")) && !param.constant)
                           {
                              ...
                              {
                                 DeclareClass(null, FindClass("StaticString"), "__eCClass_StaticString");
                                 f.Printf("         __eCObject_.returnBuffer.Unserialize((StaticString)%s);\n", param.name);
                              }
                              else
                                 f.Printf("         __eCObject_.returnBuffer.Unserialize(%s);\n", param.name);
                           }
                        }
                        if(method.dataType.returnType.kind != voidType)
                        {
                           ...
                           {
                              DeclareClass(null, FindClass("StaticString"), "__eCClass_StaticString");
                              f.Printf("         __eCObject_.returnBuffer.Unserialize((StaticString)__eCResult);\n");
                           }
                           else
                              f.Printf("         __eCObject_.returnBuffer.Unserialize(__eCResult);\n");
                        }
                        f.Printf("      }\n");
                        f.Printf("      else\n");
                        ...
                        f.Printf("      __eCObject_.returnBuffer.Free();\n");
                        f.Printf("      __eCMethod___eCNameSpace__eC__mt__Mutex_Release(__eCObject_.mutex);\n");
                        //f.Printf("      delete this;\n");
                        f.Printf("      delete __eCObject_;\n");
                        if(method.dataType.returnType.kind != voidType)
                        {
                           f.Printf("      return __eCResult;\n");
                        }
