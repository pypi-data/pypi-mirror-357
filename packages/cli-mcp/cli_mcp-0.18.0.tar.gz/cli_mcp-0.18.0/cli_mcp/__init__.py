...
from .config import (
    ...
    is_mcp_remote_proxy, extract_env_headers, prepare_env_vars
)
...
def _get_version() -> str:
    ...
    # by looking for pyproject.toml in the parent directory
    current_dir = Path(__file__).parent.parent
    pyproject_path = current_dir / "pyproject.toml"

    # If pyproject.toml exists and contains our package, prefer it (development mode)
    ...
    try:
        return importlib_metadata.version("cli-mcp")
...
def _example_command(
    ...
    if options:
        options = " " + options
    return f"uvx cli-mcp direct call --url {server}{options} {name} '{args}'"
...
def _example_simplified_command(tool: dict, server_name: str) -> str:
    ...
    name = tool.get("name", "tool")
    args = _example_args(tool.get("inputSchema", {}))
    return f"uvx cli-mcp call {server_name} {name} '{args}'"
...
def _echo_result(result: _t.Any, raw: bool) -> None:
    """Pretty-print tool result as JSON.

    If the result is a JSON string, attempt to parse it before printing so that
    it is formatted nicely. The ``raw`` flag controls whether only the result
    body is printed or wrapped in a ``{"result": ...}`` object.
    """
    ...
    if isinstance(result, str):
        try:
            ...
            # Leave data as the original string if it's not valid JSON
            data = result

    payload = data if raw else {"result": data}
    click.echo(json.dumps(payload, indent=2, ensure_ascii=False))


@click.group(context_settings={"help_option_names": ["-h", "--help"]})
@click.version_option(version=_get_version(), prog_name="cli-mcp")
def cli():
    """CLI MCP - Connect to MCP servers locally and remotely."""
    pass
...
async def _list_spawned_tools(server_name: str, server_config: dict):
    ...
    env = prepare_env_vars(env_config)
...
def _call_remote_tool(server_name: str, server_config: dict, tool_name: str, tool_args: dict):
    ...
    # Per docs, arguments must be a JSON string
...
def direct_call_tool(url: str, token: str | None, header: tuple[str, ...], tool_name: str, tool_args: str, raw: bool):
    ...
    _echo_result(resp, raw)


def main(argv: _t.Sequence[str] | None = None) -> None:
    """Entry point for the cli-mcp."""
    if argv is None:
        argv = sys.argv[1:]

    cli.main(args=list(argv), prog_name="cli-mcp")
