...
class HeadBar(klovve.builtin.drivers.gtk.ViewMaterialization[klovve.views.HeadBar]):
    ...
    def create_native(self):
        gtk_outer_box = self.new_native(
            Gtk.Box, self.piece, css_classes=self.bind.__css_classes, orientation=Gtk.Orientation.VERTICAL)
        ...
        gtk_inner_box.append(gtk_inner_center_box := self.new_native(Gtk.Box, halign=Gtk.Align.CENTER))
        gtk_inner_center_box.append(self.new_native(Gtk.Spinner, visible=self.bind.__is_spinner_visible, spinning=True))
        ...
        gtk_outer_box.append(self.new_native(Gtk.ProgressBar, fraction=self.bind.__progress_float,
                                             opacity=self.bind.__progress_bar_opacity, margin_top=3))
    ...
    __is_spinner_visible: bool = klovve.ui.computed_property(_)

    def _(self):
        return self.piece.progress or 0
    __progress_float: float = klovve.ui.computed_property(_)

    def _(self):
        return 0 if self.piece.progress is None else 1
    __progress_bar_opacity: float = klovve.ui.computed_property(_)
    ...
    def _(self):
        ...
        return tuple(result)
    __css_classes = klovve.ui.computed_property(_)
