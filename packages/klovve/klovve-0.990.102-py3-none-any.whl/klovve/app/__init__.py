...
class Application(klovve.object.Object, klovve.event._EventHandlingObject, klovve.timer._TimingObject,
    ...
    async def dialog(self, dialog, dialog_args=None, dialog_kwargs=None, *, view_anchor):
        if isinstance(dialog, klovve.ui.View):
            dialog = klovve.ui.dialog._InteractDialog[dialog]

        return await klovve.driver.Driver.get().dialog_host.show_dialog(self, view_anchor,
                                                                        dialog, dialog_args, dialog_kwargs)
