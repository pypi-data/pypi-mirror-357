...
class HeadBar(klovve.builtin.drivers.viwid.ViewMaterialization[klovve.views.HeadBar]):
    ...
    def create_native(self):
        viwid_outer_box = self.new_native(
            viwid.widgets.box.Box, self.piece, background=self.bind.__background_color,
        ...
        viwid_inner_box.children.append(viwid_inner_center_box := self.new_native(
            viwid.widgets.box.Box, horizontal_alignment=viwid.Alignment.CENTER))
        viwid_inner_center_box.children.append(self.new_native(
            viwid.widgets.busy_animation.BusyAnimation, is_visible=self.bind.__is_spinner_visible))
        ...
        viwid_outer_box.children.append(self.new_native(
            viwid.widgets.progress_bar.ProgressBar, value=self.bind.__progress_float,
            is_visible=self.bind.__is_progress_bar_visible))
    ...
    __is_progress_bar_visible: float = klovve.ui.computed_property(_)
    ...
    def _(self):
        match self.piece.style:
            ...
            case klovve.views.HeadBar.Style.FAILED:
                return "#f00"
    __background_color = klovve.ui.computed_property(_)
