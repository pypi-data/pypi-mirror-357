...
- 方便集成到 Trae、Claude Desktop、Cursor 及其他兼容 MCP 的客户端
- 支持 uvx 安装
...
```bash
uvx pip install szetop-backend-code-gen-mcp
```
...
```bash
pip install szetop-backend-code-gen-mcp
```
...
```json
{
  "mcpServers": {
    "image-recognition": {
      ...
      "args": [
        "-m",
        "backend_gen_server.szetop_backend_code_gen_mcp"
      ],
