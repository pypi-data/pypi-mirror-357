...
class Prompt:
    ...
    def generate_general_api_prompt(page_type, request_params, response_fields, business_logic, title,
        ...
        other_demand = " "
        api_type = "列表"
        service_impl_patch = "（`涉及数据库表结构`的表都要查询）"
        if page_type == "编辑":
            response_fields = "-返回主表的主键id"
            api_type = "新增\编辑"
            title = re.sub(r"编辑|新增", "", title)
            other_demand = """- Controller层新增\编辑可以合并成一个接口，Service层可以分两个方法，根据传入的主表主键Id判断新增或编辑，为空则新增，否则编辑。
- 代码生成需要结合DTO传参和数据表字段，依据表\字段与DTO属性关系，赋值DTO属性到Entity实体对象（最好一次性赋值(copyProperties)，不要一个个字段），一般先保存主表，再保存其他表数据（参考`涉及数据库表结构`）。
"""
            service_impl_patch = "（`涉及数据库表结构`的表都要更新、字段赋值不要省略代码）"
        elif page_type == "详情":
        ...
        model = {
            "request_params": request_params,
            "response_fields": response_fields,
            "business_rules": business_logic if business_logic else " ",
            "other_demand": other_demand,
            "service_impl_patch": service_impl_patch,
            "page_title": title,
            "api_type": api_type,
            "tables_structure": prompt.get_table_structures(),
            "author": GIT_USERNAME,
            "date": str(datetime.date.today()),
        }
    ...
    def get_table_structures(self) -> str:
        ...
        for table in self.tables:
            table_names.append(f"{table.table_name}({table.description})")
            table_structures += f"""
...
| ---- | ---- | ---- | ---- |        
        ...
        table_structures = "涉及数据库表：" + "、".join(table_names) + "，具体表结构如下：\n" + table_structures
