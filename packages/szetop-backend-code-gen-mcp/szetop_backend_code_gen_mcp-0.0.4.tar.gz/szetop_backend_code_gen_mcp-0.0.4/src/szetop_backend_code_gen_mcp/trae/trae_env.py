...
class TraeEnv:
    def __init__(self, project_root: str, entity_paths: list[str]):
        """
        初始化对象
    ...
    @staticmethod
    def set_rules(project_root: str, entity_paths: list[str]) -> None:
        """
        设置Trae的规则
        :param project_root: 项目根目录
        :param entity_paths: 实体类路径
        """
        trae = TraeEnv(project_root, entity_paths)
    ...
    def create_rules(self):
        """
        判断 project_root/.trae/rules/project_rules.doc 是否存在，如果存在则返回；
        否则读取 trae/springboot3/project_rules.doc 文件内容，写入目标路径。
        """
        ...
        if major_version == '2':
            source_rules = this_dir / "ide_template/springboot2/project_rules.doc"
        elif major_version == '3':
            source_rules = this_dir / "ide_template/springboot3/project_rules.doc"
        else:
            raise NotImplementedError
        if not source_rules.exists():
            raise FileNotFoundError(f"缺少源规则文件：{source_rules}")

        rules_target = self.rules_dir / "project_rules.doc"
        ...
        entity_path = self.entity_paths[0]
        entity_dir = Path(entity_path).parent.parent
        relative_entity_path = entity_dir.relative_to(self.project_root)
        relative_entity_path = str(relative_entity_path).replace("\\", "/")
        if not relative_entity_path.startswith("/"):
            relative_entity_path = "/" + relative_entity_path
        java_root_path = relative_entity_path
        resource_root_path = relative_entity_path
        match = re.match(r"^(.*?/java)", relative_entity_path)
        if match:
            resource_root_path = match.group(1)
            resource_root_path = resource_root_path.replace("java", "resources")
        ...
        logger.info(f"已创建 project_rules.doc 文件")
