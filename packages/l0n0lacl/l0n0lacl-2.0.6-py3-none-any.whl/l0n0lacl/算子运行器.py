...
def 根据算子名称获取include文件(算子名称: str):
    下划线名称 = 驼峰转下划线(算子名称)
...
def 创建算子函数(算子名称: str, 目标目录:str):
    include文件内容 = 根据算子名称获取include文件(算子名称)
    # 匹配出GetWorkspaceSize
    结果 = re.search(fr'aclnn{算子名称}GetWorkspaceSize\(([\s\S]+?)\)', include文件内容)
    if 结果 is None:
        记录错误日志并抛出异常(f'未找到aclnn{算子名称}GetWorkspaceSize')
        return
    结果 = 结果.group(1).strip()
    # 替换调无用的信息
    结果 = re.sub(r'uint64_t[\s\S]*?workspaceSize,', '', 结果)
    结果 = re.sub(r'aclOpExecutor[\s\S]*?executor', '', 结果)
    ...
    for 参数字符串 in 结果:
        ...
        参数名 = ret.group(2).strip()
        if len(参数名) == 0:
            记录错误日志并抛出异常(f'{参数字符串} 参数名为空')
            return
        ctypes类型 = 根据include声明参数类型转换为ctypes类型(参数类型)
        GetWorkspaceSize参数声明列表.append(f'{ctypes类型}, # {参数名}')
        ...
        参数名列表.append(参数名)
    GetWorkspaceSize参数声明列表.append('ctypes.c_void_p, # workspaceSize')
    GetWorkspaceSize参数声明列表.append('ctypes.c_void_p  # executor')
