...
class SharePointSync:
    ...
    def sync(self) -> Dict[str, Union[List[str], int, str, Dict[str, Any]]]:
        ...
        # Convert old permission keys to include data/ prefix if needed
        if permissions and not any(k.startswith("data/") for k in permissions.keys()):
            permissions = {f"data/{k}": v for k, v in permissions.items()}
        
        # Create a modified sp_files dict with data/ prefix for permission comparison
        sp_files_with_prefix = {}
        for rel_path, file_info in sp_files.items():
            prefixed_path = f"data/{rel_path}"
            sp_files_with_prefix[prefixed_path] = file_info
        
        permission_changes = self.compare_permissions(sp_files_with_prefix, permissions)
        ...
        for rel_path in added:
            ...
            # Update permissions for this file with data/ prefix
            prefixed_path = f"data/{rel_path}"
            permissions[prefixed_path] = sp_files[rel_path]["permissions"]
            # For added files, all permissions are new
            rag_changes["added"][prefixed_path] = {
        ...
        for rel_path in modified:
            ...
            perm_info = permission_changes.get(prefixed_path)
            rag_changes["modified"][prefixed_path] = {
                "content_changed": True,
                "permissions_changed": perm_info is not None,
                "file_path": str(self.data_dir / rel_path),
            }
            if perm_info:
                rag_changes["modified"][prefixed_path]["permission_changes"] = perm_info

        # Process permission-only changes
        for prefixed_path, perm_info in permission_changes.items():
            # Extract the original path without data/ prefix
            rel_path = prefixed_path.removeprefix("data/")
            if rel_path not in added and rel_path not in modified:
                self.logger.info(f"Permission updated: {prefixed_path}")
                permissions[prefixed_path] = sp_files[rel_path]["permissions"]
                rag_changes["permission_only_changes"][prefixed_path] = {
        ...
        for rel_path in deleted:
            ...
            deleted_permissions = permissions.get(prefixed_path, [])
            ...
            if prefixed_path in permissions:
                del permissions[prefixed_path]

            # Record deletion with permissions for RAG
            rag_changes["deleted"][prefixed_path] = {"permissions": deleted_permissions}
