...
2. Pure text content ZestRAG insertion
...
class RAGAnything:
    ...
    def __init__(
        ...
        Args:
            zestrag: Optional pre-initialized ZestRAG instance
        ...
        # Use provided ZestRAG or mark for later initialization
        self.zestrag = zestrag
        self.modal_processors = {}

        # If ZestRAG is provided, initialize processors immediately
        if self.zestrag is not None:
            self._initialize_processors()

    def _initialize_processors(self):
        """Initialize multimodal processors with appropriate model functions"""
        if self.zestrag is None:
            raise ValueError(
                "ZestRAG instance must be initialized before creating processors"
            )

        # Create different multimodal processors
        self.modal_processors = {
            "image": ImageModalProcessor(
                zestrag=self.zestrag,
                modal_caption_func=self.vision_model_func or self.llm_model_func,
            ),
            "table": TableModalProcessor(
                zestrag=self.zestrag, modal_caption_func=self.llm_model_func
            ),
    ...
    async def _ensure_zestrag_initialized(self):
        """Ensure ZestRAG instance is initialized, create if necessary"""
        if self.zestrag is not None:
            return
        ...
        if self.llm_model_func is None:
            raise ValueError(
                "llm_model_func must be provided when ZestRAG is not pre-initialized"
            )
        if self.embedding_func is None:
            raise ValueError(
                "embedding_func must be provided when ZestRAG is not pre-initialized"
            )

        from zestrag.kg.shared_storage import initialize_pipeline_status

        # Create ZestRAG instance with provided functions
        self.zestrag = ZestRAG(
        ...
        await self.zestrag.initialize_storages()
        await initialize_pipeline_status()

        # Initialize processors after ZestRAG is ready
        self._initialize_processors()

        self.logger.info("ZestRAG and multimodal processors initialized")
    ...
    async def _insert_text_content(
        ...
        """
        Insert pure text content into ZestRAG
        ...
        """
        self.logger.info("Starting text content insertion into ZestRAG...")

        # Use ZestRAG's insert method with all parameters
        await self.zestrag.ainsert(
            input=input,
    ...
    async def process_document_complete(
        ...
        """
        # Ensure ZestRAG is initialized
        await self._ensure_zestrag_initialized()
    ...
    async def query_with_multimodal(self, query: str, mode: str = "hybrid") -> str:
        ...
        if self.zestrag is None:
            raise ValueError(
                "No ZestRAG instance available. "
                "Please either:\n"
                "1. Provide a pre-initialized ZestRAG instance when creating RAGAnything, or\n"
                "2. Process documents first using process_document_complete() or process_folder_complete() "
                "to create and populate the ZestRAG instance."
            )

        result = await self.zestrag.aquery(query, param=QueryParam(mode=mode))
