...
import os
import logging
from zestrag.kg.shared_storage import finalize_share_data
from zestrag.utils import setup_logger, get_env_value
from zestrag.constants import (
...
errorlog = os.getenv("ERROR_LOG", log_file_path)  # Default write to zestrag.log
accesslog = os.getenv("ACCESS_LOG", log_file_path)  # Default write to zestrag.log
...
logconfig_dict = {
    ...
    "filters": {
        "path_filter": {
            "()": "zestrag.utils.LightragPathFilter",
        },
    },
    "loggers": {
        "zestrag": {
...
def on_starting(server):
    ...
    # Log the location of the ZestRAG log file
    print(f"ZestRAG log file: {log_file_path}\n")
...
def post_fork(server, worker):
    ...
    setup_logger("zestrag", log_level, add_filter=True, log_file_path=log_file_path)

    # Set up zestrag submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith("zestrag."):
