...
from zestrag.base import StorageNameSpace
from zestrag.utils import (
    logger,
    compute_mdhash_id,
)
from zestrag.zestrag import ZestRAG
from dataclasses import asdict
from zestrag.kg.shared_storage import get_namespace_data, get_pipeline_status_lock


class BaseModalProcessor:
    """Base class for modal processors"""

    def __init__(self, zestrag: ZestRAG, modal_caption_func):
        """Initialize base processor

        Args:
            zestrag: ZestRAG instance
            modal_caption_func: Function for generating descriptions
        """
        self.zestrag = zestrag
        self.modal_caption_func = modal_caption_func

        # Use ZestRAG's storage instances
        self.text_chunks_db = zestrag.text_chunks
        self.chunks_vdb = zestrag.chunks_vdb
        self.entities_vdb = zestrag.entities_vdb
        self.relationships_vdb = zestrag.relationships_vdb
        self.knowledge_graph_inst = zestrag.chunk_entity_relation_graph

        # Use ZestRAG's configuration and functions
        self.embedding_func = zestrag.embedding_func
        self.llm_model_func = zestrag.llm_model_func
        self.global_config = asdict(zestrag)
        self.hashing_kv = zestrag.llm_response_cache
        self.tokenizer = zestrag.tokenizer
    ...
    async def _process_chunk_for_extraction(
        ...
        from zestrag.operate import extract_entities, merge_nodes_and_edges
...
class ImageModalProcessor(BaseModalProcessor):
    ...
    def __init__(self, zestrag: ZestRAG, modal_caption_func):
        ...
        """
        super().__init__(zestrag, modal_caption_func)
