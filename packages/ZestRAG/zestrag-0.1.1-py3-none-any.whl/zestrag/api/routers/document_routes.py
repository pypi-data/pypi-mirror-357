"""
This module contains all document-related routes for the ZestRAG API.
"""
...
from zestrag.base import DocProcessingStatus, DocStatus
from zestrag.api.utils_api import get_combined_auth_dependency
...
async def pipeline_enqueue_file(rag: ZestRAG, file_path: Path) -> bool:
    """Add a file to the queue for processing

    Args:
        rag: ZestRAG instance
...
async def pipeline_index_file(rag: ZestRAG, file_path: Path):
    """Index a file
...
async def pipeline_index_files(rag: ZestRAG, file_paths: List[Path]):
...
async def pipeline_index_texts(
    rag: ZestRAG, texts: List[str], file_sources: List[str] = None
):
...
async def run_scanning_process(rag: ZestRAG, doc_manager: DocumentManager):
...
def create_document_routes(
    rag: ZestRAG, doc_manager: DocumentManager, api_key: Optional[str] = None
):
