...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        self.window = window
        self.strategy = strategy if isinstance(strategy, LimitStrategy) else LimitStrategy(strategy)
    ...
    def _setup_redis(self):
        try:
            ...
            logger.warning("redis-py package not installed. Falling back to in-memory storage.")
            self.redis = None
    ...
    def _setup_cleanup_task(self):
        ...
        async def cleanup_task():
            while True:
                ...
                if self.enable_statistics and time.time() - self.statistics["last_reset"] > 3600:
                    self.statistics = {
                        "total_requests": 0,
                        "limited_requests": 0,
                        "last_reset": time.time(),
    ...
    def _cleanup_expired_records(self):
        ...
        for key, timestamps in self.requests.items():
            valid_timestamps = [ts for ts in timestamps if ts > current_time - self.window]
        ...
        logger.debug(f"Cleaned up {len(expired_keys)} expired records. Current records: {len(self.requests)}")
    ...
    def _get_request_key(self, request: Request) -> str:
        ...
        if self.strategy == LimitStrategy.IP:
            ...
            user_id = self.user_identifier(request) if self.user_identifier else "anonymous"
        ...
        if self.redis:
            try:
                ...
                if key not in self.requests:
                    self.requests[key] = []
                self.requests[key] = [ts for ts in self.requests[key] if ts > current_time - self.window]
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if client_ip in self.blacklist:
            return JSONResponse(
                status_code=403,
                content={"error": "Forbidden", "detail": "Your IP address is blacklisted."}
            )
        ...
        if request_count >= self.max_requests:
            oldest_timestamp = min(
                self.requests.get(key, [current_time])) if not self.redis else current_time - self.window
