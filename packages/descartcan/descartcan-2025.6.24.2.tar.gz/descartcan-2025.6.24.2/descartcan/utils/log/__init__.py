...
JSON_FORMAT_PRESETS = {
    ...
    "production": "{time:YYYY-MM-DD HH:mm:ss.SSS} | {extra[hostname]} | {extra[app_name]} | {level: <8} | {name}:{function} | {message}",
}
...
CONSOLE_FORMAT_PRESETS = {
    ...
    "production": "<green>{time:YYYY-MM-DD HH:mm:ss.SSS}</green> | {extra[hostname]} | {extra[app_name]} | <level>{level: <8}</level> | <cyan>{name}</cyan>:<cyan>{function}</cyan> | <level>{message}</level>",
}
