...
OAuth2Scheme = OAuth2PasswordBearer(
    tokenUrl=config.OAUTH2_REDIRECT_URL,
    auto_error=False
)
...
class AuthDependency:
    ...
    @staticmethod
    async def check_permissions(request: Request, token: str = Depends(OAuth2Scheme)) -> None:
    ...
    @staticmethod
    async def open_api_check_permissions(request: Request, token: str = Depends(OAuth2Scheme)) -> None:
        token_data = JWTHandler.verify_token(token)
