...
class TestSubmitToBatch:
    @pytest.fixture
    def customise_fake_processes(self, fake_process):
        def _customise_fake_processes(batch_submit_returncode=0, llsubmit_returncode=0):
            fake_process.register_subprocess(['llsubmit', fake_process.any()],
                                             stdout='',
                                             returncode=llsubmit_returncode,
                                             occurrences=100)
            fake_process.register_subprocess(['batch_submit'],
                                             stdout='llsubmit',
                                             returncode=batch_submit_returncode,
                                             occurrences=100)
            return fake_process

        return _customise_fake_processes

    @pytest.fixture
    def fake_processes(self, customise_fake_processes):
        return customise_fake_processes()
    ...
    def test_raises_if_batch_submit_command_fails(self, manager, mock_config, jintrac_env,
                                                  rundir, customise_fake_processes, run_root, jetto_sources):
        _ = customise_fake_processes(batch_submit_returncode=1)

        with pytest.raises(job.JobManagerError):
            manager.submit_job_to_batch(mock_config, rundir, run=True)

    def test_raises_if_batch_submit_fails(self, manager, mock_config, jintrac_env,
                                          customise_fake_processes, rundir, run_root, jetto_sources):
        _ = customise_fake_processes(llsubmit_returncode=1)
