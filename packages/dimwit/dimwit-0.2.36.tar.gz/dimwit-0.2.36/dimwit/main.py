...
def get_notion_pages_from_db_sync_with_chunked(
    ...
    chunked_page_ts, chunked_page_events = decompress_func(
        [],
        [],
        len(chunked_pages) - 1,
        chunked_pages[-1],
    )
    latest_chunked_date = chunked_page_ts[-1]
    print(f"Earliest unchunked timestamp: {chunked_page_ts[0]}")
    print(f"Latest unchunked timestamp: {chunked_page_ts[-1]}")
    ...
    unchunked_pages = get_notion_pages_from_db(
        ...
        filter_by={"property": "date", "date": {"after": latest_chunked_date}},
    )
    unchunked_ts, unchunked_events = get_all_entries(
        unchunked_pages,
        extract_func,
    )
    print(f"Earliest unchunked timestamp: {unchunked_ts[0]}")
    print(f"Latest unchunked timestamp: {unchunked_ts[-1]}")
    ...
    new_chunked_pages = compress_func(ts, events)
    # PATCH/POST the latest chunked page and any new chunked pages.
    replace_page = new_chunked_pages[0]
    patch_chunked_page(chunked_db_id, len(chunked_pages) - 1, replace_page)
    for idx, chunked_page in enumerate(new_chunked_pages[1:]):
        post_new_chunked_page(
            chunked_db_id,
            len(chunked_pages) + idx,
            chunked_page,
        )
