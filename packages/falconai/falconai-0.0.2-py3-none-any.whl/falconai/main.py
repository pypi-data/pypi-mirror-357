...
class ai:
    ...
    def chat(prompt, **kwargs):
        ...
        valid_keywords = ['output_type', 'voice_name', 'voice_speed', 'voice_volume', 'voice_type', 'web', 'model', 'test_output', 'image', 'csv', 'ipynb', 'website', 'pdf', 'text', 'markdown', 'document', 'youtube',
                          'html', 'browser', 'browser_headless', 'browser_path','browser_system_prompt', 'anonymous', 'MCP', 'MCP_builtin_server', 'MCP_custom_server', 'MCP_agent_max_steps', 'MCP_agent_system_prompt', 'MCP_agent_chat', 'MCP_user_reply']
...
2. Query: A specific question or requirement stated as "{prompt}".
        ...
        # Coming soon!
        # if anonymous and model.startswith("anonymous/"):
        ...
        #     anonymous_model = next((m for m in candidates if m in model), None)

        #     if anonymous_model:
        #         results = DuckAI().chat(instance.final_prompt)
        #         return results
        #     else:
        #         raise ValueError(
        #             "The supported models for anonymous inference are "
        #             "gpt-4o-mini, llama-3.3-70b, claude-3-haiku, o3-mini and mistral-small-3"
        #         )
