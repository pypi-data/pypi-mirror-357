...
def configure_thunder_virtualization(ssh, token, metadata, instance_id, latest_binary_hash=None):
    """
    Configure or remove Thunder Compute virtualization based on GPU type.
    
    For native GPU instances (e.g., t4_native, a100xl_native), removes Thunder virtualization.
    For virtual GPU instances, sets up Thunder virtualization with binary and config.
    
    Args:
        ssh: SSH client connection
        token: API token
        metadata: Instance metadata containing GPU type info
    ...
    try:
        ...
        is_native_gpu = current_gpu_type.endswith('_native')
        
        if is_native_gpu:
            # For native GPU instances, remove Thunder Compute virtualization
            ...
            return True, "Successfully cleaned up Thunder virtualization for native GPU"
            
        else:
            # For non-native GPU instances, set up Thunder Compute virtualization
