...
def aggregate_data(experiment_array):
    ...
    aggregate["jupiter"] = MachineData("jupiter")
    ...
    for experiment in experiment_array:
        ...
        elif experiment.json_data()["experiment"]["system"] == "jupiter":
            aggregate["jupiter"].add_data(data["nodes"], data["runtime"])
...
def generate_plot_pdf_file(plotting_data, output_dir):
    ...
    ax = plot.subplots()
    ax.set_xlabel("Number of JUPITER/JEDI Nodes  (log-scale)")
...
def generate_author_tex_file(output_dir):
    ...
    with open(authorfilename, "w") as authorfile:
        ...
        authorfile.write("\\title{JUPITER Evaluation Report}\n")
...
def prepare_report_dir(output_dir, share_dir):
    texdir = os.path.join(share_dir, "jureap/tex/jupiter")
...
def generate_energy_plot_file(experiment_energy_info_array, output_dir):
    ...
    ax = plot.subplots()
    ax.set_xlabel("Number of JEDI/JUPITER Nodes  (log-scale)")
    ax.set_ylabel("Runtime / s   (log-scale)")
    ax.set_yscale("log")
    ax.set_xscale("log")
    ax.margins(2)
    ...
    for experiment in experiment_energy_info_array:
        if experiment["system"] == "jedi" or experiment["system"] == "jupiter":
