...
class ChunkMetadataBuilder:
    ...
    def build_flat_metadata(
        ...
        # tags and chunks should be lists of strings
        ...
        if chunks is not None and not isinstance(chunks, list):
            raise ValueError(f"chunks must be a list of strings or None, got: {chunks.__class__.__name__}")
        if chunks:
            chunks = list_to_str(chunks, separator=',', allow_none=True)
    ...
    def build_semantic_chunk(
        ...
        Args:
            ...
            tags: List of tags
            chunks: List of text chunks returned after vectorization alongside embedding
            status: Processing status of the chunk (default: RAW for initial data ingestion)
        ...
        if chunks is None:
            ...
            raise ValueError(f"chunks must be a list of strings, got: {type(chunks)}")
