...
def test_example_vectorization_chunks():
    """Test example_vectorization_chunks functionality."""
    chunk = example_vectorization_chunks()
    
    # Verify chunks field exists and has correct content
    assert chunk.chunks is not None
    assert isinstance(chunk.chunks, list)
    assert len(chunk.chunks) == 2
    assert chunk.chunks[0] == "Machine learning is a subset of artificial intelligence."
    assert chunk.chunks[1] == "It enables computers to learn without being explicitly programmed."
    
    # Verify other fields
    assert chunk.body == chunk.text  # Should be the same
    assert "Machine learning" in chunk.body
    assert chunk.tags is not None
    assert "machine-learning" in chunk.tags
    
    print("✓ example_vectorization_chunks works correctly") 