...
class TestRoundTripRedis:
    ...
    def test_tags_and_links_always_lists(self, valid_uuid, sample_text):
        """Verify tags, links and chunks are always List[str] regardless of format"""
        # Test with various tag/link/chunks formats
        test_cases = [
            {"tags": ["a", "b", "c"], "links": ["rel:uuid1", "rel:uuid2"], "chunks": ["chunk1", "chunk2"]},
            {"tags": [], "links": [], "chunks": []},
            {"tags": None, "links": None, "chunks": None},
        ]
        ...
        for case in test_cases:
            ...
            # Note: SemanticChunk may initialize None tags/links/chunks as None, but from_flat_dict converts to []
            ...
            orig_chunks = original_chunk.chunks if original_chunk.chunks is not None else []
            ...
            rest_chunks = restored_chunk.chunks if restored_chunk.chunks is not None else []
            
            assert isinstance(rest_tags, list)
            assert isinstance(rest_links, list)
            assert isinstance(rest_chunks, list)
            
            # Content should match
            assert orig_tags == rest_tags
            assert orig_links == rest_links
            assert orig_chunks == rest_chunks
    ...
    def _compare_chunks(self, original: SemanticChunk, restored: SemanticChunk):
        ...
        for key in original_dict:
            ...
            # Special handling for list fields (tags, links, chunks, embedding)
            if key in ["tags", "links", "chunks", "embedding"]:
