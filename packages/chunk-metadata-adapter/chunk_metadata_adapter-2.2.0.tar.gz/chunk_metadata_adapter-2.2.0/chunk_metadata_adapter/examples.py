...
def example_structured_chunk():
    ...
    chunk = builder.build_semantic_chunk(
        ...
        chunks=["# Introduction", "This is the documentation", "for the system."],
        start=0,
        end=56,
...
def example_vectorization_chunks():
    """Example showing how chunks field is used after vectorization."""
    builder = ChunkMetadataBuilder(project="VectorizationExample")
    
    # Original text before vectorization
    original_text = "Machine learning is a subset of artificial intelligence. It enables computers to learn without being explicitly programmed."
    
    # After vectorization, the text is split into chunks
    text_chunks = [
        "Machine learning is a subset of artificial intelligence.",
        "It enables computers to learn without being explicitly programmed."
    ]
    
    # Create chunk with vectorization results
    chunk = builder.build_semantic_chunk(
        body=original_text,
        text=original_text,
        language=LanguageEnum.EN,
        ...
        chunks=text_chunks,  # Text chunks returned after vectorization
        tags=["machine-learning", "ai", "definition"],
        start=0,
        end=len(original_text)
    )
    
    # The chunks field contains the text pieces alongside the embedding vector
    print(f"Original text length: {len(original_text)} characters")
    print(f"Split into {len(chunk.chunks)} chunks:")
    for i, text_chunk in enumerate(chunk.chunks):
