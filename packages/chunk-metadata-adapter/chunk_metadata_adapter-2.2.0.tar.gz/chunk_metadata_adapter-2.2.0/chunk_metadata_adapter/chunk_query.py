...
class ChunkQuery(BaseModel):
    ...
    chunks: Optional[List[str]] = Field(default=None, description="List of text chunks returned after vectorization (search by chunk content).")
    ...
    def from_dict_with_validation(cls, data: dict) -> ("ChunkQuery", Optional[dict]):
        ...
        for field, value in data.items():
            ...
            if isinstance(value, (list, dict)):
                # Для фильтра допускается list только для tags, links, chunks и dict для block_meta
                if field not in ["tags", "links", "chunks", "block_meta"]:
