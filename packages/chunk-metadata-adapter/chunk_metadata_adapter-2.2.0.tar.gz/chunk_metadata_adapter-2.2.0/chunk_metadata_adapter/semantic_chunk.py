...
class SemanticChunk(BaseModel):
    ...
    chunks: Optional[List[str]] = Field(default=None, min_length=0, max_length=100, description="List of text chunks returned after vectorization alongside the embedding vector.")
    ...
    def from_flat_dict(cls, data: dict) -> "SemanticChunk":
        ...
        for field in ["tags", "links", "chunks", "embedding"]:
    ...
    def validate_metadata(self) -> None:
        ...
        if not isinstance(self.chunks, list):
            raise ValueError("chunks must be a list for structured metadata")
    ...
    def validate_and_fill(cls, data: dict):
        ...
        for field in ["tags", "links", "chunks", "embedding", "source_lines"]:
            val = data.get(field)
            if isinstance(val, str):
                if val.strip() == "" or val.strip() == "null":
