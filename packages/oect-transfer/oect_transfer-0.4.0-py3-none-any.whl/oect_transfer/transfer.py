...
class Transfer:
    def __init__(
        ...
        self.tp_idx = np.where(np.abs(np.diff(np.sign(np.diff(x)))) == 2)[0][0] + 1
    ...
    def _compute_absgm_max(self) -> Point:
        ...
        return Point(
            ...
            forward=np.abs(self.gm.forward).max(),
            reverse=np.abs(self.gm.reverse).max(),
        )
    ...
    def _compute_gm_min(self) -> Point:
        ...
        return Point(
            raw=self.gm.raw.min(),
            where=gm_min_where,
            forward=self.gm.forward.min(),
            reverse=self.gm.reverse.min(),
        )
    ...
    def _compute_absI_max(self) -> Point:
        ...
        return Point(
            raw=np.abs(self.I.raw).max(),
            where=absI_max_where,
            forward=np.abs(self.I.forward).max(),
            reverse=np.abs(self.I.reverse).max(),
        )
    ...
    def _compute_I_max(self) -> Point:
        ...
        return Point(
            raw=self.I.raw.max(),
            where=I_max_where,
            forward=self.I.forward.max(),
            reverse=self.I.reverse.max(),
        )
    ...
    def _compute_absI_min(self) -> Point:
        ...
        return Point(
            raw=np.abs(self.I.raw).min(),
            where=absI_min_where,
            forward=np.abs(self.I.forward).min(),
            reverse=np.abs(self.I.reverse).min(),
        )
    ...
    def _compute_I_min(self) -> Point:
        ...
        return Point(
            raw=self.I.raw.min(),
            where=I_min_where,
            forward=self.I.forward.min(),
            reverse=self.I.reverse.min(),
        )
