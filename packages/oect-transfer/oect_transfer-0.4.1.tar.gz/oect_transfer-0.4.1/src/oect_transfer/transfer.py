...
class Transfer:
    def __init__(
        ...
        # 修复转折点查找逻辑，添加错误处理
        self.tp_idx = self._find_turning_point(x)
    ...
    def _find_turning_point(self, x: NDArray[np.float64]) -> int:
        """
        寻找转折点索引，带有健壮的错误处理
        ...
        try:
            ...
            # 寻找二阶差分符号变化最大的点
            sign_changes = np.abs(np.diff(np.sign(diff2)))
            turning_points = np.where(sign_changes >= 1.5)[0]  # 降低阈值，提高容错性
            
            if len(turning_points) > 0:
                # 如果找到转折点，选择第一个
                return turning_points[0] + 1
            else:
                # 如果没有找到明显转折点，尝试其他方法
                return self._find_turning_point_alternative(x)
                
        except Exception as e:
            print(f"警告：转折点检测失败，使用默认中点: {e}")
            return len(x) // 2

    def _find_turning_point_alternative(self, x: NDArray[np.float64]) -> int:
        """
        备用转折点查找方法
        ...
        try:
            ...
            if len(x) >= 3:
                ...
                try:
                    peaks_max, _ = find_peaks(x)
                    peaks_min, _ = find_peaks(-x)
                    
                    all_peaks = np.concatenate([peaks_max, peaks_min])
                    if len(all_peaks) > 0:
                        # 选择最靠近中间的峰值
                        center = len(x) // 2
                        closest_peak = all_peaks[np.argmin(np.abs(all_peaks - center))]
                        return int(closest_peak)
                except ImportError:
                    # 如果没有scipy，使用简单的峰值检测
