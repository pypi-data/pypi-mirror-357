...
class RuleManager:
    ...
    def assign_tags_to_rule(self, rule: Dict[str, Any]) -> Dict[str, Any]:
        tags_blocks = {}
        engine_type = rule.get("ENGINE_TYPE", "postgres")
        tags = rule.get("tags")
        blocks = self.extract_snippet_blocks(tags)

       
        for tag, block_info in blocks.items():
            resolved_code = self.replace_common_variables(block_info["code"], rule)
            if block_info["exec"]:
                sql_to_run = resolved_code
                result = self.execute_sql_scalar(sql_to_run, engine_type)
                tags_blocks[tag] = result.strip() if isinstance(result, str) else (str(result).strip() if result else "")
            else:
                tags_blocks[tag] = resolved_code

        rule.update(tags_blocks)
        return rule
    ...
    def business_rule_check(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        if self.template.get("RULE_CATEGORY_NAME") == 'BUSINESS_RULE_CHECK':
            for idx, rule in enumerate(self.rules.get("RULES", [])):
                ...
                rule = self.assign_tags_to_rule(rule)
