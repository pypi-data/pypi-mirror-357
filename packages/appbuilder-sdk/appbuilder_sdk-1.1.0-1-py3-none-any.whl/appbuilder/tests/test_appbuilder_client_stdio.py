...
class TestGetAppList(unittest.TestCase):
    ...
    def test_ai_search_mcp_stdio(self):
        async def process():
            ...
            assert len(tools) > 0
            result = await mcp_client.call_tool("search", {"query": "今天的头条新闻"})
            assert result.content[0] != ""

        subprocess.check_call([sys.executable, "-m", "pip", "install", "mcp"])
        from appbuilder.modelpore.client import MCPClient
        from appbuilder.mcp.ai_search import ai_search_server
