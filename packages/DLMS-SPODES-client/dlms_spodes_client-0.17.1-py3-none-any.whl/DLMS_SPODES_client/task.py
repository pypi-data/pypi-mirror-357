    ...
    async def run(self, c: Client) -> result.Simple[bool] | result.Error:
        ...
        while True:
            ...
            for n in range(self.number_of_retries):
                ...
                if res.append(await super(Scheduler, self).run(c)):
                    break
    ...
    async def exchange(self, c: Client) -> result.List:
        ...
        for t in self.tasks:
            try:
                ...
                res.append(result.Simple().append_err(e))
        return res
...
class ExecuteByDesc(Base):
    ...
    async def exchange(self, c: Client) -> result.Result:
        try:
            ...
            return result.Simple(value=pdu_enums.ActionResult.SUCCESS)
        except Exception as e:
...
class Execute(Base):
    ...
    async def exchange(self, c: Client) -> result.Simple[pdu_enums.ActionResult.SUCCESS] | result.Error:
        obj = c.objects.get_object(self.ln)
        try:
