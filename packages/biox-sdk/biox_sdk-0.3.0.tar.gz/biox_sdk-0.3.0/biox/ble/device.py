...
class BluetoothDevice:
    ...
    async def read_gatt_char(self, char_specifier: str) -> bytes:
        ...
        """
        try:
            data = await (await self.get_client()).read_gatt_char(char_specifier)
        except Exception as e:
    ...
    async def check_device_work_status(self) -> str:
        ...
        resp = await self.read_gatt_char(character1)
    ...
    async def query_sampling_rate_EEG(self) -> str:
        """查询EEG数据采样率

        Returns:
            str: EEG数据采样率
        """
        send_command = command.Set_Query_Sampling_Rate_EEG_Command + "\r"
        command_bytes = send_command.encode('utf-8')
...
def extract_data(data: bytes) -> str:
    ...
    if end_index == -1:
        ...
        return data.decode('ascii', errors='ignore').rstrip('\r')
    else:
        # 解码从开始到第一个 '\r' 之间的字节为字符串
        version_info = data[:end_index].decode('ascii', errors='ignore')
        return version_info
