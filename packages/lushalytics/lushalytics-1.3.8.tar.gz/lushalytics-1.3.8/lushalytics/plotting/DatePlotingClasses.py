...
class DateLinePlotter(DatePlotter):
    ...
    def __init__(self, df, title, all_possible_labels=None):

        super().__init__(df, title)
        self.all_possible_labels = all_possible_labels
        
        if self.all_possible_labels:
            self.color_map = {
                str(label): color for label, color in zip(self.all_possible_labels, self.colors)
            }
        else:
            self.color_map = None
    ...
    def plot(self, 
        ...
        if segment_col:
            if not self.color_map:
                raise ValueError("`all_possible_labels` must be provided when using `segment_col`.")
            
            unique_segments = agg_df[segment_col].unique()
            sorted_segments = sorted(
                unique_segments, 
                key=lambda x: self.all_possible_labels.index(x) if x in self.all_possible_labels else -1
            )
            
            for segment in sorted_segments:
                seg_data = agg_df[agg_df[segment_col] == segment].copy()
                color = self.color_map.get(str(segment), "#000000")
...
class LegendPlotter:
    def __init__(self, selected_labels, all_possible_labels):
        """
        Initializes the plotter with selected labels and all possible labels
        to create a consistent color mapping.
        """
        ...
        # Create a stable map between every possible label and a color.
        # This ensures colors are consistent.
        self.color_map = {
            str(label): color for label, color in zip(self.all_possible_labels, self.colors)
        }
    ...
    def get_legend_figure(self):
        ...
        # Iterate through the newly SORTED list of selected labels
        for lbl in sorted_labels:
            # Look up the correct, consistent color from the map
            clr = self.color_map.get(str(lbl), "#000000") # Default to black if not found
            fig.add_trace(go.Scatter(x=[None], y=[None], mode='markers',
                                     name=str(lbl), marker=dict(color=clr, size=10)))
