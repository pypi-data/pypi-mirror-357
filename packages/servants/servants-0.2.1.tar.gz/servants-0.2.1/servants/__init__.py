...
class Servant:
    def __init__(self, tools: list = None, show_memory: bool = False, max_iterations: int = 5):
    ...
    def thinking_loop(self, problem, chat_completion_func, system_message=None, callback=None):
        """Internal thinking loop that runs in a separate thread."""
        self._reset_events()
        def _loop():
            try:
                self.status = "running"
                tools = self.tools  # Use servant's own tools
                
                # Generate default system message if none provided
                if system_message is None:
                    if tools:
                        default_system_message = "You are a helpful AI assistant that can use tools to solve problems. Analyze the problem step by step and use the available tools when needed to complete the task."
                    else:
                        default_system_message = "You are a helpful AI assistant. Analyze the problem step by step and provide a clear solution."
                    sys_msg_base = default_system_message
                else:
                    sys_msg_base = system_message
    ...
    def execute(self, problem, chat_completion_func, system_message=None, callback=None):
        """Execute the servant independently and wait for completion."""
        # Start the thinking loop
        self.thinking_loop(problem, chat_completion_func, system_message, callback)
        ...
        # Return the result
        return self.get_thinking_result()
    def pause(self):
        if self._thinking_thread and self._thinking_thread.is_alive():
...
class Master:
    ...
    def run(self, problem, chat_completion_func, system_message=None, callback=None):
        """Run all servants with the given problem. Each servant uses its own tools and max_iterations."""
...
def extract_json_from_text(text):
    ...
    # If no valid JSON found, try a simpler approach
    # Look for anything that looks like JSON between braces
    start_idx = text.find('{')
    if start_idx != -1:
        brace_count = 0
