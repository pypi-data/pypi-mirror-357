...
from splunk_appinspect.common.string_utils import is_printable
...
class Reporter(object):
    def __init__(self, record_filter: Optional[Callable[[ReportRecord], bool]] = None):
        self._report_records = []
        self._record_filter: Callable[[ReportRecord], bool] = record_filter or (lambda x: True)
    ...
    def report_records(self, max_records=MAX_MESSAGES_PER_CHECK, status_types_to_return=STATUS_TYPES):
        ...
        if len(filtered_records) > max_records:
            ...
            check_code_fine_name = Path(check_file_path).name
            records.append(
                ReportRecord(
                    "warning",
                    "Suppressed " + text,
    ...
    def __save_result_message(
        ...
        check_code_file_name = Path(check_file_path).name
        message_stripped_of_unprintables = "".join(s for s in message if is_printable(s))
        ...
        if self._record_filter(report_record):
    ...
    def not_applicable(self, message, file_name=None, line_number=None):
