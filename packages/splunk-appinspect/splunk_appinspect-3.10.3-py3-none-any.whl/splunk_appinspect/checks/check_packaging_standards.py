...
class CheckPackageCompression(Check):
    ...
    def check(self, app: "App") -> Generator[CheckMessage, Any, None]:
        ...
        try:
            ...
            # TODO: change OSError to gzip.BadGzipFile once python3.7 is deprecated
            yield FailMessage(
...
class CheckPackageTypeIsNotZip(Check):
    def __init__(self) -> None:
        super().__init__(
            config=CheckConfig(
                name="check_that_splunk_app_package_type_is_not_zip_type",
                description="Check that the provided app package is not .zip type for SSAI purpose.",
                tags=(
                    Tags.SPLUNK_APPINSPECT,
                    Tags.CLOUD,
                    Tags.PRIVATE_APP,
    ...
    def check(self, app: "App") -> Generator[CheckMessage, Any, None]:
        if not app.package.check_valid_package_for_SSAI():
            yield WarningMessage(
                "`.zip` format is not supported on Splunk Cloud. Please repackage it to `.tgz` or `.spl` file."
            )
