...
class CrowdSegDataset(tfds.core.GeneratorBasedBuilder):
    ...
    def _split_generators(
        ...
        # Count examples for each split to provide meaningful progress bar
        train_examples = len(self._get_image_filenames(patches_dir / "Train"))
        validation_examples = len(self._get_image_filenames(patches_dir / "Valid"))
        test_examples = len(self._get_image_filenames(patches_dir / "Test"))

        return {
            "train": tfds.core.SplitGenerator(
                name=tfds.Split.TRAIN,
                gen_kwargs={
                    "image_dir": patches_dir / "Train",
                    "mask_dir": masks_dir / "Train",
                },
                num_examples=train_examples,
            ),
            "validation": tfds.core.SplitGenerator(
                name=tfds.Split.VALIDATION,
                gen_kwargs={
                    "image_dir": patches_dir / "Valid",
                    "mask_dir": masks_dir / "Valid",
                },
                num_examples=validation_examples,
            ),
            "test": tfds.core.SplitGenerator(
                name=tfds.Split.TEST,
                gen_kwargs={
                    "image_dir": patches_dir / "Test",
                    "mask_dir": masks_dir / "Test",
                },
    ...
    def _generate_examples(
        ...
        image_dir = gen_kwargs["image_dir"]
        mask_dir = gen_kwargs["mask_dir"]
...
def test_progress_bar():
    """Test function to verify progress bar shows meaningful information."""
    print("Testing progress bar with meaningful ETA...")
    train, validation, test = get_crowd_seg_dataset_tfds(
        image_size=(64, 64),
    )
