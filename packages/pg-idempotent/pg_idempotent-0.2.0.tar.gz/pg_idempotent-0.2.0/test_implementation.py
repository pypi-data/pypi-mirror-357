...
"""
    ...
    if result.success:
        console.print(f"[green]✓[/green] Transformation successful!")
        ...
        console.print(f"[red]✗[/red] Transformation failed: {result.errors}")
...
CREATE FUNCTION update_timestamp() RETURNS TRIGGER AS $func$
BEGIN
...
"""
    ...
    if result.success:
        console.print(f"[green]✓[/green] Complex features handled!")
        console.print(f"  Statements: {result.statement_count}")
        console.print(f"  Transformed: {result.transformed_count}")
        console.print("\n[bold]Transformed SQL:[/bold]")
        syntax = Syntax(result.transformed_sql, "sql", theme="monokai", line_numbers=True)
        ...
        console.print(f"[red]✗[/red] Failed: {result.errors}")
...
def test_file_transformation():
    ...
    example_file = Path("examples/simple.sql")
    if not example_file.exists():
        console.print(f"[yellow]![/yellow] Example file not found: {example_file}")
        return
    
    transformer = SQLTransformer()
    result = transformer.transform_file(str(example_file))
    
    if result.success:
        console.print(f"[green]✓[/green] File transformation successful!")
        console.print(f"  Statements: {result.statement_count}")
        console.print(f"  Transformed: {result.transformed_count}")
        
        # Show first 1000 chars of result
        console.print("\n[bold]Preview (first 1000 chars):[/bold]")
        preview = result.transformed_sql[:1000] + "..." if len(result.transformed_sql) > 1000 else result.transformed_sql
        syntax = Syntax(preview, "sql", theme="monokai")
        console.print(syntax)
    else:
        console.print(f"[red]✗[/red] File transformation failed: {result.errors}")
