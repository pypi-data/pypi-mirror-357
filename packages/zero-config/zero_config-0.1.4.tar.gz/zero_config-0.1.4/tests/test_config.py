...
class TestEnvironmentVariableConsistency:
    """Test the consistency between environment variables and env file variables."""
    ...
    def test_env_var_vs_env_file_consistency(self):
        """Test that environment variables and env file variables behave consistently."""
        with tempfile.TemporaryDirectory() as tmpdir:
            tmpdir_path = Path(tmpdir).resolve()

            # Create .env file with new variable not in defaults
            env_file = tmpdir_path / ".env.zero_config"
            env_file.write_text("database.test_url=file://test.db\nnew_key_from_file=file_value")

            with patch('zero_config.config.find_project_root', return_value=tmpdir_path):
                with patch.dict(os.environ, {
                    'DATABASE__TEST_URL': 'env://test.db',  # Same key as in env file
                    'NEW_KEY_FROM_ENV': 'env_value'         # New key from env var
                }, clear=True):
                    # Setup with minimal defaults (no database.test_url or new keys)
                    setup_environment(default_config={
                        'app_name': 'test_app'
                    })
                    ...
                    # Both environment variables and env file variables should be available
                    # Env files should win over environment variables (higher priority per README)
                    assert config.get('database.test_url') == 'file://test.db'  # Env file wins
                    assert config.get('new_key_from_env') == 'env_value'       # New from env var
                    assert config.get('new_key_from_file') == 'file_value'     # New from env file
                    assert config.get('app_name') == 'test_app'                # From defaults

    def test_env_var_new_keys_added(self):
        """Test that environment variables can add new keys not in defaults."""
        with tempfile.TemporaryDirectory() as tmpdir:
            tmpdir_path = Path(tmpdir).resolve()

            with patch('zero_config.config.find_project_root', return_value=tmpdir_path):
                with patch.dict(os.environ, {
                    'DATABASE__HOST': 'env.db.com',
                    'DATABASE__PORT': '5432',
                    'LLM__API_KEY': 'sk-env-key',
                    ...
                    # All environment variables should be available as new keys
                    assert config.get('database.host') == 'env.db.com'
                    assert config.get('database.port') == '5432'  # String (no type hint)
                    assert config.get('llm.api_key') == 'sk-env-key'
                    assert config.get('llm.temperature') == '0.8'  # String (no type hint)
                    assert config.get('new_feature_enabled') == 'true'

    def test_env_var_type_conversion_with_defaults(self):
        """Test that environment variables still get type conversion when defaults exist."""
        with tempfile.TemporaryDirectory() as tmpdir:
            tmpdir_path = Path(tmpdir).resolve()

            with patch('zero_config.config.find_project_root', return_value=tmpdir_path):
                with patch.dict(os.environ, {
                    'DATABASE__PORT': '3306',      # Should convert to int
                    'LLM__TEMPERATURE': '0.9',     # Should convert to float
                    'DEBUG': 'true',               # Should convert to bool
                    'NEW_PORT': '8080'             # Should stay as string (no default)
                }, clear=True):
                    # Setup with typed defaults for some keys
                    setup_environment(default_config={
                    ...
                    # Variables with defaults should get type conversion
                    assert config.get('database.port') == 3306      # int
                    assert config.get('llm.temperature') == 0.9     # float
                    assert config.get('debug') is True              # bool

                    # Variables without defaults should stay as strings
                    assert config.get('new_port') == '8080'         # string

    def test_env_var_section_access_new_keys(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('zero_config.config.find_project_root', return_value=tmpdir_path):
                with patch.dict(os.environ, {
                    ...
                    # Section access should work with new keys
    ...
    def test_priority_order_with_new_keys(self):
        """Test priority order: defaults < env vars < env files (per README)."""
        with tempfile.TemporaryDirectory() as tmpdir:
            tmpdir_path = Path(tmpdir).resolve()
            ...
            env_file = tmpdir_path / ".env.zero_config"
            env_file.write_text("""
database.host=file.db.com
database.new_setting=from_file
completely_new_key=file_value
""".strip())

            with patch('zero_config.config.find_project_root', return_value=tmpdir_path):
                with patch.dict(os.environ, {
                    'DATABASE__HOST': 'env.db.com',        # Should be overridden by env file
                    'DATABASE__NEW_SETTING': 'from_env',   # Should be overridden by env file
                    'ENV_ONLY_KEY': 'env_only'             # Should remain from env
                }, clear=True):
                    setup_environment(default_config={
                        'database.host': 'default.db.com'  # Should be overridden
                    })

                    config = get_config()

                    # Priority: env file > env var > default
                    assert config.get('database.host') == 'file.db.com'        # env file wins
                    assert config.get('database.new_setting') == 'from_file'   # env file wins
                    assert config.get('env_only_key') == 'env_only'            # env var only
                    assert config.get('completely_new_key') == 'file_value'    # env file only
