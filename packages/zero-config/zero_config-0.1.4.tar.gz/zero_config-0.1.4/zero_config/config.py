...
def apply_environment_variables(config: Dict[str, Any]) -> None:
    ...
    for env_var, env_value in os.environ.items():
        if env_var.isupper():
            ...
            # Apply environment variable (consistent with env file behavior)
            if config_key in config:
                ...
                # Add new key as string (same behavior as env files)
                config[config_key] = env_value
                logging.debug(f"Environment addition: {env_var} -> {config_key} = {env_value}")
