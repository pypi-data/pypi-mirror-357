...
class ChunkSplitterFactory(BaseModel):
    ...
    def get(self, file_path: str, spilt_type: Literal["title", "recursive"], **kwargs) -> ChunkSplitter:
        if spilt_type == "recursive":
            return RecursiveChunkSplitter(**kwargs)
        else:
            ext = get_file_ext(file_path)
...
class SentenceSplitterFactory(BaseModel):
    ...
    def get(self, file_path: str, spilt_type: Literal["title", "recursive"]) -> SentenceSplitter:
        if spilt_type == "recursive":
            return NoItemSentenceSplitter()
        else:
            ext = get_file_ext(file_path)
            for splitter in self.splitters:
                if splitter.support(ext):
...
class PanshiFileIntelligentProcess(BaseModel):
    ...
    def split(self, file_path: str, result: ParseResult, spilt_type: Literal["title", "recursive"], **kwargs
    ) -> SplitResult:
        chunk_splitter = self.chunk_splitter_factory.get(file_path, spilt_type, **kwargs)
        sentence_splitter = self.sentence_splitter_factory.get(file_path, spilt_type)
    ...
    def process(self, file_path: str, spilt_type: Literal["title", "recursive"] = "title", **kwargs) -> ProcessResult:
        """
        word:
            converted_pdf_path : 当文档是word时,需要传递word对应的pdf文件,用于修复页码信息

        if spilt_type= recursive,**kwargs可以设置以下参数：
            keep_separator：是否保留分隔符在生成的文本块中。
            strip_whitespace：是否移除文本块首尾的空白字符
            separators： 用于分割文本的分隔符优先级列表。
            chunk_size：单个文本块的最大字符数
            chunk_overlap：相邻文本块的重叠字符数

        """
        parse_result = self.parse(file_path)
        split_result = self.split(file_path, parse_result, spilt_type, **kwargs)
