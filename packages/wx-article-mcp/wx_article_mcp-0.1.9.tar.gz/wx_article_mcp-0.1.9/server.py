...
def get_default_image_path() -> str:
    """
    获取默认图片路径，支持开发环境和打包环境
    
    Returns:
        默认图片的文件路径
    """
    try:
        # 尝试使用importlib.resources（适用于打包后的环境）
        import sys
        if hasattr(sys, '_MEIPASS'):  # PyInstaller打包环境
            return os.path.join(sys._MEIPASS, "img", "jewei-cat.jpg")
        
        # 尝试从包资源中获取
        try:
            package_files = files(__package__ or __name__.split('.')[0])
            img_path = package_files / "img" / "jewei-cat.jpg"
            if img_path.is_file():
                return str(img_path)
        except Exception:
            pass
        ...
        default_path = os.path.join(current_dir, "img", "jewei-cat.jpg")
        if os.path.exists(default_path):
            return default_path
        
        # 最后的回退选项：在当前工作目录查找
        fallback_path = os.path.join(os.getcwd(), "img", "jewei-cat.jpg")
        if os.path.exists(fallback_path):
            return fallback_path
            
        raise FileNotFoundError("无法找到默认图片文件 jewei-cat.jpg")
