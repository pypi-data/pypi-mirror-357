...
# 导入图片数据
from image_data import DEFAULT_IMAGE_BASE64

# 移除了importlib.resources相关导入，因为现在使用base64方案

# 加载环境变量
load_dotenv()

def create_temp_image_from_base64(base64_data: str, filename: str = "default_image.jpg") -> str:
    """
    从base64数据创建临时图片文件
    ...
    try:
        ...
        raise Exception(f"创建临时图片文件失败: {str(e)}")

# 获取默认图片路径的函数
def get_default_image_path() -> str:
    """
    获取默认图片路径，使用base64数据创建临时文件
    ...
    try:
        # 使用base64数据创建临时文件
        if DEFAULT_IMAGE_BASE64 and DEFAULT_IMAGE_BASE64 != "xxx":
            return create_temp_image_from_base64(DEFAULT_IMAGE_BASE64, "jewei-cat.jpg")
        else:
            raise FileNotFoundError("默认图片的base64数据不可用")
        
    except Exception as e:
        raise FileNotFoundError(f"获取默认图片路径失败: {str(e)}")
    ...
    try:
        ...
        if not thumb_image_url or not thumb_image_url.strip():
            ...
            try:
                ...
                await ctx.error(f"默认图片不存在: {str(e)}")
                raise Exception(f"无法找到默认封面图片，请提供thumb_image_url参数或确保默认图片存在: {str(e)}")
        else:
