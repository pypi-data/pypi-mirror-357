...
from src.utils.compute_moving_average import compute_moving_average
...
class DQN:
    ...
    def __init__(
        ...
        self.observation_dim = 12  # dimensione dello stato (3 achieved_goal + 3 desired_goal + 6 observation)
    ...
    def preprocess_obs(self, observation):
        ...
        return np.concatenate([achieved_goal, desired_goal, state])
    ...
    def build_model(self):
        model = tf.keras.Sequential(
            [
                tf.keras.layers.Input(shape=(self.observation_dim,)),
    ...
    def train(self):
        ...
        for i in range(self.num_episodes):
            ...
            while not done:
                ...
                next_state = self.preprocess_obs(next_state)  # concateno il nuovo stato
            ...
            if i % self.print_every == 0 and i > 0:
                ...
                print(
                    f"Episode {i}/{self.num_episodes}\tAverage Score: {avg_score:.2f}\tSuccess Rate: {recent_success_rate*100:.1f}%\tEpsilon: {self.epsilon:.3f}"
                )
                if self.store_data:
                    self.plot_results()
                    self.save_metrics()
    ...
    def save_metrics(self):
        results_df = pd.DataFrame(
            {
                "episode": range(1, len(dqn.reward_track) + 1),
                "reward": dqn.reward_track,
                "success_rate": dqn.success_track,
            }
        )
...
if __name__ == "__main__":
    ...
    dqn = DQN(env, num_episodes=10000, print_every=25)
    dqn.train()
