...
def visualize(results_path,color="blue", title="", window_size=1000,save_path="", save=False):
    ...
    success_rate = np.array([int(row[2]) for row in results])
...
if __name__ == "__main__":
    ...
    results_path = "results/train/panda_reach_results/dqn_target_net_training_results.csv"
    save_path = "plots/panda_reach_plots/reward_successrate_DQN_target_network.png"
    window_size = 1000
    color = sns.color_palette("colorblind")[3]
    
    visualize(results_path=results_path,save_path=save_path, window_size=window_size, title=title, color=color, save=True)