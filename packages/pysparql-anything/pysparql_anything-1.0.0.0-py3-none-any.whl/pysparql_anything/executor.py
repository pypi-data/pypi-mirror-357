...
class SPARQLAnythingException(Exception):
    pass


class Executor:
    def __init__(self, receiver: SPARQLAnythingReflection) -> None:
        self.receiver = receiver

    # Helpers for the command execution functions.
    def to_pandas_df(self, select_dict: dict) -> pd.DataFrame:
        """
        Helper method to convert the sa.SparqlAnything.select method's
        dictonary result of a SELECT query into a corresponding pandas
        DataFrame object.\n
        Args:\n
    ...
    def execute_ask(
        self, kwargs: dict[str, str | dict[str, str]]
    ) -> bool:
        """
        ...
        query_output = self.receiver.call_main(args)
        # Inspect stderr output for ERROR:
        err_messages = query_output.err.split('\n')
        for msg in err_messages:
            if 'ERROR' in msg:
                err_msg = msg.split('cli.SPARQLAnything -')[1].lstrip()
                raise SPARQLAnythingException(err_msg)
        return bool('<boolean>true</boolean>' in query_output.out)
    ...
    def execute_construct(
        ...
        Args: \n
            kwargs: A dictionary containing the SPARQL Anything CONSTRUCT
                request parameters.\n
        ...
        for msg in err_messages:
            if 'ERROR' in msg:
                err_msg = msg.split('cli.SPARQLAnything -')[1].lstrip()
                raise SPARQLAnythingException(err_msg)
        graph = Graph().parse(data=query_output.out)
    ...
    def execute_select(
        ...
        for msg in err_messages:
            if 'ERROR' in msg:
                err_msg = msg.split('cli.SPARQLAnything -')[1].lstrip()
                raise SPARQLAnythingException(err_msg)
        results_dict = json.loads(query_output.out)
        if output_type == dict:
            return results_dict
        return self.to_pandas_df(results_dict)
    ...
    def execute_run(
        ...
        args = transform_args(kwargs)
        query_output = self.receiver.call_main(args)
        print(query_output.err)
        print(query_output.out)
