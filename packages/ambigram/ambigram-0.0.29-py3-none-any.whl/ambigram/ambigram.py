...
class Ambigram:
    ...
    def add_base_rectangle_minimize(
        self,
        height: float,
        padding: float = 0.0
    ) -> Self:
        ...
        """
        bb = self.assembly.toCompound().BoundingBox()
        xmax, ymax, zmax = self.max_column

        # Bottom Line | Point that goes TOP LEFT
        bottom_x1, bottom_y1 = bb.xmin - ymax, bb.ymin + xmax

        # Bottom Line | Point that goes BOTTOM RIGHT
        bottom_x2, bottom_y2 = bb.xmin + ymax, bb.ymin - xmax

        # Top Line | Point that goes TOP LEFT
        top_x1, top_y1 = bb.xmax - ymax, bb.ymax + xmax

        # Top Line | Point that goes BOTTOM RIGHT
        top_x2, top_y2 = bb.xmax + ymax, bb.ymax - xmax

        # Left Line | Point that goes BOTTOM LEFT
        left_x1, left_y1 = bb.xmin - xmax, bb.ymin

        # Left Line | Point that goes TOP RIGHT
        left_x2, left_y2 = bb.xmax, bb.ymax + ymax

        # Right Line | Point that goes BOTTOM LEFT
        right_x1, right_y1 = bb.xmin, bb.ymin - ymax

        # Right Line | Point that goes TOP RIGHT
        right_x2, right_y2 = bb.xmax + xmax, bb.ymax


        horizontal_base = (cq.Workplane("XY").polyline([
            [bottom_x1 - padding, bottom_y1 - padding],
            [bottom_x2 + padding, bottom_y2 - padding],
            [top_x2 + padding, top_y2 + padding],
            [top_x1 - padding, top_y1 + padding]
        ])
            .close()
            .extrude(height)
            .translate([bb.xmin, bb.ymin, bb.zmin - height])
        ...
        vertical_base = (cq.Workplane("XY").polyline([
            [left_x1 - padding, left_y1 - padding],
            [left_x2 - padding, left_y2 + padding],
            [right_x2 + padding, right_y2 + padding],
            [right_x1 + padding, right_y1 - padding]
        ])
            .close()
            .extrude(height)
            .translate([bb.xmin, bb.ymin, bb.zmin - height])
        ...
        self.base = horizontal_base.intersect(vertical_base)
