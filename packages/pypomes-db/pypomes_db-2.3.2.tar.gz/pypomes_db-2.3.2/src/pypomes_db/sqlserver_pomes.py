...
def select(errors: list[str] | None,
    ...
    have been specifed, otherwise *offset_count* is ignored. If the search is empty, an empty list is returned.
    ...
    curr_conn: Connection = conn or connect(errors=errors,
                                            ...
                                            logger=logger)
    if curr_conn:
        # OFFSET clause requires ORDER BY to have been defined
        if sel_stmt.find("ORDER BY") > 0 and \
                isinstance(offset_count, int) and offset_count > 0:
        ...
        # TOP and OFFSET clauses cannot be used together
