...
# TODO PASSWORD_CLEAR_TEXT_COLUMN_NAME = 
PASSWORD_COLUMN_NAME = "password_clear_text"
...
class UserExternalsLocal(GenericCrudMysql, metaclass=MetaLogger, object=object_init):
    def __init__(self, is_test_data: bool = False):
        ...
        self.user_external_pii = GenericCrudMysql(
            ...
            default_column_name=PASSWORD_COLUMN_NAME,
            is_test_data=is_test_data,
        )
    ...
    def update_user_external_access_token(
        ...
        *,
        ...
        if user_external_id is None:
            user_external_id = self.__get_external_user_ids_by_profile_id_system_id(
                system_id=system_id, profile_id=profile_id
            )
        
        if isinstance(user_external_id, (list, tuple)):
            # If there are multiple user_external_ids, we will update all of them
            updated_rows = 0
            for user_external_id_single in user_external_id:
                updated_rows += (
                    self.token__user_externals.update_access_token(
                        user_external_id=user_external_id_single,
                        name=username,
                        access_token=access_token,
                        expiry=expiry,
                    )
    ...
    def delete_access_token(
        ...
        if isinstance (user_external_id, (list, tuple)):
            # If there are multiple user_external_ids, we will delete all of them
            deleted_rows = 0
            for user_external_id_single in user_external_id:
                deleted_rows += (
            ...
            return deleted_rows
        # If there is only one user_external_id, we will delete it
        else:
            deleted_rows = (
            self.token__user_externals.delete_access_token_by_user_external_id(
                user_external_id=user_external_id,
    ...
    def get_auth_details(
        ...
        Return: dict or a list - a dictionary/list containing user_external_id, access_token, refresh_token, expiry, is_refresh_token_valid, and oauth_state
        """
        ...
        if isinstance(user_external_id, (list, tuple)):
            # If there are multiple user_external_ids, we will get the auth details for all of them
            auth_details_list = []
            for id in user_external_id:
    ...
    # Supposedly this method was supposed to retain it' functionallity of returing a single user_external_id
    # def __get_user_external_id_by_system_id_and_profile_id(
    #     self, *, system_id: int, profile_id: int
    # ):
    #     self.logger.deprecation_warning(
    #         old_name="__get_user_external_id_by_system_id_and_profile_id",
    #         new_name="get_external_user_ids_by_profile_id_system_id"
    #     )

    #     return self.__get_external_user_ids_by_profile_id_system_id( 
    #         system_id=system_id, 
    #         profile_id=profile_id
    #         )
    
    def __get_external_user_ids_by_profile_id_system_id(self, *, system_id: int, profile_id: int) -> tuple | int:
        user_external_ids = self.select_multi_value_by_where(
            schema_name=USER_EXTERNAL_SCHEMA_NAME,
            view_table_name=USER_EXTERNAL_VIEW_NAME,
            select_clause_value=USER_EXTERNAL_ID_COLUMN_NAME,
        ...
        if len(user_external_ids) == 1:
            # If there is only one user_external_id, we will return it as an int
            user_external_ids = user_external_ids[0]
        elif len(user_external_ids) > 1:
            # If there are multiple user_external_ids, we will return them as a tuple
            user_external_ids = tuple(user_external_ids)
