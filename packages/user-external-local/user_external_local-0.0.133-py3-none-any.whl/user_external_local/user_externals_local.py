...
class UserExternalsLocal(GenericCRUD, metaclass=MetaLogger, object=object_init):
    ...
    def get_auth_details(
        ...
        user_external_ids = self.__get_external_user_ids_by_profile_id_system_id(
            system_id=system_id, profile_id=profile_id
        )

        if isinstance(user_external_ids, (list, tuple)):
            # If there are multiple user_external_ids, we will get the auth details for all of them
            auth_details_list = []
            for id in user_external_ids:
            ...
            auth_details = self.token__user_externals.get_auth_details(
            user_external_id=user_external_ids,
        )
            return auth_details
    ...
    def __get_external_user_ids_by_profile_id_system_id(self, *, system_id: int, profile_id: int):
