...
async def add_function_to_capability_group(name: str, 
                                           source_code: str,
                                           function: Dict) -> Dict:
    try:
        return await add_function_to_capability_group_async(name, source_code, function)
    except Exception as exc:
        return "".join(format_exception_only(exc))


async def delete_function_from_capability_group(name: str, function: str) -> Dict:
    try:
        return await delete_function_from_capability_group_async(name, function)
    except Exception as exc:
        return "".join(format_exception_only(exc))


async def update_function_in_capability_group(name: str, 
                                              old_function_name: str,
                                              source_code: str,
                                              new_function: Dict) -> Dict:
    try:
        return await update_function_in_capability_group_async(name, old_function_name, source_code, new_function)
    except Exception as exc:
        return "".join(format_exception_only(exc))