...
class HIPObject(BaseObject):
    """Manages HIP objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    def create(
        ...
        payload = hip_object.model_dump(exclude_unset=True)
        ...
        return HIPObjectResponseModel(**response)
    ...
    @staticmethod
    def _apply_filters(
        hip_objects: List[HIPObjectResponseModel],
        filters: Dict[str, Any],
    ) -> List[HIPObjectResponseModel]:
        """Apply client-side filtering to the list of HIP objects.

        Args:
            hip_objects: List of HIPObjectResponseModel objects
        ...
        if "criteria_types" in filters:
            if not isinstance(filters["criteria_types"], list):
                raise InvalidObjectError(
                    message="'criteria_types' filter must be a list",
                    error_code="E003",
                    http_status_code=400,
                    details={"errorType": "Invalid Object"},
                )
            criteria_types = filters["criteria_types"]
            filter_criteria = [
                obj
                for obj in filter_criteria
                if any(
        ...
        **filters,
        ...
        Args:
            ...
            **filters: Additional filters including:
                - criteria_types: List[str] - Filter by criteria types (e.g., ['host_info', 'network_info'])
        ...
        while True:
            ...
            data = response["data"]
            object_instances = [HIPObjectResponseModel(**item) for item in data]
