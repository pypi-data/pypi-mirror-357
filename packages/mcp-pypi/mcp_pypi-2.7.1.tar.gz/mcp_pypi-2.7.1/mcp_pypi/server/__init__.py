...
from mcp_pypi.core.models import (DependenciesResult, DependencyTreeResult,
                                  DocumentationResult, ErrorResult,
                                  ExistsResult, MetadataResult, PackageInfo,
                                  PackageRequirementsResult, PackagesFeed,
                                  PyPIClientConfig, ReleasesFeed, ReleasesInfo,
                                  SearchResult, StatsResult, UpdatesFeed,
...
• 🚀 Safe version recommendations""",
    ...
    def _register_tools(self):
        ...
        async def get_package_releases(
            package_name: str, limit: Optional[int] = 10
        ) -> Dict[str, Any]:
            ...
            try:
                ...
                limited_releases = {
                    version: releases[version] for version in sorted_versions
                }
        ...
        async def get_dependency_tree(
            package_name: str, version: Optional[str] = None, max_depth: int = 3
            ...
            """
            try:
                return await self.client.get_dependency_tree(
        ...
        async def compare_versions(
            ...
            """
            try:
                return await self.client.compare_versions(
        ...
        async def scan_dependency_vulnerabilities(
            ...
            try:
                ...
                tree_result = await self.client.get_dependency_tree(
                    package_name, version, max_depth
                )
                ...
                for pkg_name, pkg_version in packages_to_scan:
                    ...
                    if vuln_result.get("vulnerable", False):
                        ...
                        severity_counts["critical"] += vuln_result.get(
                            "critical_count", 0
                        )
            ...
            try:
                ...
                for pkg in installed_packages:
                    ...
                    if vuln_result.get("vulnerable", False):
                        ...
                        latest_version_info = await self.client.get_latest_version(
                            pkg_name
                        )
                    ...
                    if env_type in ["virtualenv", "system"]:
                        ...
                        if critical_updates:
                            ...
                            update_commands.append(
                                f"{pip_cmd} install --upgrade {pkgs}"
                            )
        ...
        async def security_audit_project(
            ...
            try:
                ...
                if check_files:
                    ...
                    for req_file in req_files:
                        ...
                        if not req_result.get("error"):
                            ...
                            all_packages = req_result.get(
                                "outdated", []
                            ) + req_result.get("up_to_date", [])
                            for req in all_packages:
                                ...
                                if pkg_name and current_version:
                                    vuln_check = (
                                        await self.client.check_vulnerabilities(
                                    ...
                                    if vuln_check.get("vulnerable"):
                                        ...
                                        if (
                                            ...
                                            results["all_vulnerable_packages"][
                                                pkg_name
                                            ] = {
                ...
                if check_files:
                    conda_files = (
                        list(project_root.glob("**/environment.yml"))
                ...
                results["total_vulnerabilities"] = sum(
                    results["severity_breakdown"].values()
                )
                ...
                if total_high > 0:
                    remediation_steps.append(
                        "2. ⚠️ Update packages with HIGH vulnerabilities within 24 hours"
                    )
                if total_medium > 0:
                    remediation_steps.append(
                ...
                if total_low > 0:
                    remediation_steps.append(
                        "4. ℹ️ Review LOW vulnerabilities in next maintenance window"
                    )
                ...
                priority_fixes.sort(
                    key=lambda x: (
                        x["critical"]
                        * 1000  # Critical vulnerabilities are highest priority
                        + x["high"] * 100  # Then high
                        + x["medium"] * 10  # Then medium
                        + x["low"]  # Then low
                        + len(x["found_in"])
                        * 0.1  # Tie-breaker: packages used in multiple places
                    ),
                    reverse=True,
                )
                ...
                audit_data["formatted_report"] = self._format_security_report(
                    audit_data
                )
                ...
                for req in requirements:
                    ...
                    if pkg_name:
                        ...
                        if latest_info and not latest_info.get("error"):
                            ...
                            vuln_check = await self.client.check_vulnerabilities(
                                pkg_name, version
                            )
                            if vuln_check.get("vulnerable"):
                                vulns.append(
                ...
                for pkg in data["package"]:
                    ...
                    if pkg_name and version:
                        ...
                        if vuln_check.get("vulnerable"):
                            ...
                            results["all_vulnerable_packages"][pkg_name][
                                "sources"
                            ].append(f"poetry.lock:{poetry_lock.name}")
        except Exception as e:
                ...
                for dep in data["dependencies"]:
                    if isinstance(dep, str):
                        ...
                        if vuln_check.get("vulnerable"):
                            ...
                            results["all_vulnerable_packages"][pkg_name][
                                "sources"
                            ].append(f"conda:{conda_file.name}")
                        ...
                        for pip_dep in dep["pip"]:
                            ...
                            if pkg_name:
                                latest_info = await self.client.get_latest_version(
                                    pkg_name
                                )
                                if latest_info and not latest_info.get("error"):
                                    version = latest_info.get("version")
    ...
    def _format_security_report(self, audit_result: Dict[str, Any]) -> str:
        ...
        report_lines.append(
            f"📁 Project: {audit_result.get('project_path', 'Unknown')}"
        )
        report_lines.append(
            f"🕒 Scan Date: {audit_result.get('scan_timestamp', 'Unknown')}"
        )
        report_lines.append("")
        ...
        report_lines.append(
            f"{V_LINE} Security Score: {self._color_score(security_score)}{BOLD}{security_score}/100{RESET}{'':>28}{V_LINE}"
        )
        report_lines.append(
        ...
        report_lines.append(
            f"{V_LINE} {RED}🚨 CRITICAL{RESET}{'':>8} {V_LINE} {RED}{critical:>8}{RESET} {V_LINE} {bar:<25} {V_LINE}"
        )

        # High
        bar = self._make_bar(high, max(total_vulns, 1), 20, ORANGE)
        report_lines.append(
            f"{V_LINE} {ORANGE}⚠️  HIGH{RESET}{'':>11} {V_LINE} {ORANGE}{high:>8}{RESET} {V_LINE} {bar:<25} {V_LINE}"
        )

        # Medium
        bar = self._make_bar(medium, max(total_vulns, 1), 20, YELLOW)
        report_lines.append(
            f"{V_LINE} {YELLOW}⚡ MEDIUM{RESET}{'':>10} {V_LINE} {YELLOW}{medium:>8}{RESET} {V_LINE} {bar:<25} {V_LINE}"
        )

        # Low
        bar = self._make_bar(low, max(total_vulns, 1), 20, BLUE)
        report_lines.append(
            f"{V_LINE} {BLUE}ℹ️  LOW{RESET}{'':>13} {V_LINE} {BLUE}{low:>8}{RESET} {V_LINE} {bar:<25} {V_LINE}"
        )
    ...
    def run(self, transport: Literal["stdio", "http"] = "stdio"):
        ...
        if transport == "stdio":
            ...
            logger.info(
                f"Starting HTTP server on {self.mcp_server.settings.host}:{self.mcp_server.settings.port}"
            )
            logger.info(
                f"SSE endpoint: http://{self.mcp_server.settings.host}:{self.mcp_server.settings.port}/sse"
            )
            logger.info(
                f"Streamable-HTTP endpoint: http://{self.mcp_server.settings.host}:{self.mcp_server.settings.port}/mcp"
            )
            self.mcp_server.run(
                transport="sse"
            )  # This actually starts the full HTTP server
        else:
