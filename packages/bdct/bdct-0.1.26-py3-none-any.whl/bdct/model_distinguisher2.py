...
def ct_test(forest):
    ...
    # n_less = (random_diffs < real_diffs).sum()
...
def bdei_test(forest):
    ...
    if len(all_cherries) > 2:
        all_cherries = sorted(all_cherries, key=lambda _: getattr(_.root, TIME))
        n_l, n_t = get_real_vs_reshuffled_diffs_less(all_cherries)
        n_cherries += n_t
        n_less += n_l
...
def get_real_vs_reshuffled_diffs_less(all_couples):
    ...
    first_vs_mean = np.abs(first_dists[:-1] - (first_dists[1:] + other_dists[1:]) / 2)

    other_vs_other_diffs = np.abs(other_dists[:-1] - other_dists[1:])
    other_vs_first_diffs = np.abs(other_dists[:-1] - first_dists[1:])

    other_vs_mean = np.abs(other_dists[:-1] - (first_dists[1:] + other_dists[1:]) / 2)

    mean_vs_first = np.abs(first_dists[1:] - (first_dists[:-1] + other_dists[:-1]) / 2)
    mean_vs_other = np.abs(other_dists[1:] - (first_dists[:-1] + other_dists[:-1]) / 2)

    n_less = (first_vs_mean < real_diffs[:-1]).sum() + (other_vs_mean < real_diffs[:-1]).sum() \
             + (mean_vs_first < real_diffs[1:]).sum() + (mean_vs_other < real_diffs[1:]).sum()
    n_total = len(first_vs_first_diffs) * 4

    # n_less = (first_vs_first_diffs[1:] < real_diffs[1:-1]).sum() + (first_vs_first_diffs[:-1] < real_diffs[1:-1]).sum() \
    #  + (first_vs_other_diffs[1:] < real_diffs[1:-1]).sum() + (first_vs_other_diffs[:-1] < real_diffs[1:-1]).sum() \
    #  + (other_vs_other_diffs[1:] < real_diffs[1:-1]).sum() + (other_vs_other_diffs[:-1] < real_diffs[1:-1]).sum() \
    #  + (other_vs_first_diffs[1:] < real_diffs[1:-1]).sum() + (other_vs_first_diffs[:-1] < real_diffs[1:-1]).sum()
    #
    #
    # n_total = (len(first_vs_first_diffs) - 1) * 8
...
The test therefore reports a probability of partner notification being present in the tree.""")
    ...
    parser.add_argument('--nwk', default='/home/azhukova/projects/bdct/simulations/BDEICT1000/tree.5.nwk', type=str, help="input forest file in newick or nexus format")
    params = parser.parse_args()

    forest = read_forest(params.nwk)
    pval, n_cherries = ct_test(forest)
    print(f"CT test {pval} on {n_cherries} cherries.")
    pval, n_cherries = bdei_test(forest)
    print(f"BDEI test {pval} on {n_cherries} cherries.")
