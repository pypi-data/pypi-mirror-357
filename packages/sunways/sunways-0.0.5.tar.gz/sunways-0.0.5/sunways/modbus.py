...
def _modbus_checksum(data: bytearray | bytes) -> int:
...
def validate_modbus_rtu_response(
    ...
    data[0] is source address
    data[1] is command return type
    data[2] is response payload length (for read commands)
    data[3:-2] is response payload
    data[-2:] is crc-16 checksum
    """
    if len(data) <= 4:
        logger.debug("Response is too short.")
        return False
    if data[1] == MODBUS_READ_CMD:
        if data[2] != value * 2:
            logger.debug(
                "Response has unexpected length: %d, expected %d.", data[2], value * 2
            )
            return False
        expected_length = len(data)
    elif data[1] in (MODBUS_WRITE_CMD, MODBUS_WRITE_MULTI_CMD):
        if len(data) < 8:
            logger.debug(
                "Response has unexpected length: %d, expected %d.", len(data), 8
            )
            return False
        expected_length = 8
        response_offset = int.from_bytes(data[2:4], byteorder="big", signed=False)
        if response_offset != offset:
            logger.debug(
                "Response has wrong offset: %X, expected %X.", response_offset, offset
            )
            return False
        response_value = int.from_bytes(data[4:6], byteorder="big", signed=True)
    ...
    if _modbus_checksum(data[0:checksum_offset]) != (
    ...
    if data[1] != cmd:
        failure_code = FAILURE_CODES.get(data[2], "UNKNOWN")
        logger.debug(
            "Response is command failure: %s.", FAILURE_CODES.get(data[2], "UNKNOWN")
        )
