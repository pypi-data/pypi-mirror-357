...
class TkeBase(Model):
    ...
    @retry_with_conditions(3, 10)
    def create_cluster_endpoint(self, cluster_id=None, subnet_id=None, is_extranet=None, domain=None, security_group=None, extensive_parameters=None, existed_load_balancer_id=None, **kwargs):
        """创建集群访问端口
        :param cluster_id: 集群ID
        :type cluster_id: str
...
BandwidthPackageId含义：带宽包ID，指定此参数时，网络计费方式InternetAccessible.InternetChargeType只支持按带宽包计费BANDWIDTH_PACKAGE。
        ...
        :param existed_load_balancer_id: 使用已有clb开启内网或外网访问
        :type existed_load_balancer_id: str
        ...
        req.ExistedLoadBalancerId = existed_load_balancer_id
        if kwargs:
            req = self.convert_request(req, **kwargs)
...
类型：String
    ...
    @retry_with_conditions(3, 10)
    def describe_master_component(self, cluster_id=None, component=None, **kwargs):
        ...
        """
        req = self.model_v20180525.DescribeMasterComponentRequest()
        req.ClusterId = cluster_id
        req.Component = component
        if kwargs:
            req = self.convert_request(req, **kwargs)
        response = self.client_v20180525.DescribeMasterComponent(req)
        return response
...
表示获取账号下所有集群)
    ...
    @retry_with_conditions(3, 10)
    def modify_master_component(self, cluster_id=None, component=None, operation=None, dry_run=None, **kwargs):
        ...
        :type operation: str
        :param dry_run: 为true时，不真正执行停机或恢复操作	
        :type dry_run: bool
        
        """
        req = self.model_v20180525.ModifyMasterComponentRequest()
        req.ClusterId = cluster_id
        req.Component = component
        ...
        if kwargs:
            req = self.convert_request(req, **kwargs)
        response = self.client_v20180525.ModifyMasterComponent(req)
        return response
...
major 大版本原地升级
    ...
    @retry_with_conditions(3, 10)
    def delete_cluster_machines(self, cluster_id=None, machine_names=None, enable_scale_down=None, instance_delete_mode=None, **kwargs):
        """删除原生节点池节点
        :param cluster_id: 集群 ID
        :type cluster_id: str
        :param machine_names: 节点名列表
        :type machine_names: list of str
        :param enable_scale_down: 删除节点时是否缩容节点池，true为缩容
        :type enable_scale_down: bool
        :param instance_delete_mode: 集群实例删除时的策略：terminate（销毁实例，仅支持按量计费云主机实例）retain（仅移除，保留实例）
        :type instance_delete_mode: str
        
        """
        req = self.model_v20220501.DeleteClusterMachinesRequest()
        req.ClusterId = cluster_id
        req.MachineNames = machine_names
        req.EnableScaleDown = enable_scale_down
        req.InstanceDeleteMode = instance_delete_mode
        if kwargs:
            req = self.convert_request(req, **kwargs)
        response = self.client_v20220501.DeleteClusterMachines(req)
        return response
