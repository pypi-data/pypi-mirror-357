...
class IQ_Option:
    ...
    def connect(self, timeout=30):
        """
        Conecta al API de IQ Option con timeout
        Args:
            timeout (int): Tiempo máximo en segundos para esperar la conexión
        Returns:
            bool: True si la conexión fue exitosa, False si falló
        """
        try:
            logging.info(f'Iniciando conexión con timeout={timeout}s')
            
            if hasattr(self, 'api'):
                try:
                    logging.info('Cerrando conexión anterior...')
                    self.api.close()
                except Exception as e:
                    logging.error(f'Error al cerrar conexión anterior: {str(e)}')

            logging.info('Creando nueva instancia de IQOptionAPI...')
            ...
            logging.info(f'Resultado de la conexión: {check}, Razón: {reason}')

            if not check:
                raise ConnectionError(f'Failed to connect: {reason}')

            logging.info('Esperando balance_id con timeout...')
            ...
            while global_value.balance_id is None:
                ...
                if elapsed > timeout:
                    if retry_count < max_retries:
                        retry_count += 1
                        logging.warning(f'Timeout after {timeout} seconds waiting for balance_id. Retry {retry_count}/{max_retries}')
                        start_time = time.time()  # Reset timer for new attempt
                        continue
                    else:
                        logging.error(f'Failed to get balance_id after {max_retries} retries')
                        raise TimeoutError(f'Timeout after {timeout} seconds waiting for balance_id')
                
                logging.debug(f'Esperando balance_id... Tiempo transcurrido: {elapsed:.1f}s')
                time.sleep(0.1)

            logging.info(f'Balance_id obtenido: {global_value.balance_id}')
            
            # Si llegamos aquí, el balance_id se obtuvo exitosamente
            ...
            logging.error(f'Error connecting to IQ Option: {str(e)}', exc_info=True)
            raise
