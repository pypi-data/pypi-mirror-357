...
class Inputs(InputsWithOverwrite):
    ...
    intensity_fine_filtered_data_url: str = Field(
        default_factory=lambda data: data["intensity_filtered_data_url"],
...
class MakeGrainMap(Task, input_model=Inputs, output_names=["make_map_data_url"]):
    ...
    def run(self):
        ...
        lat_par_url = get_parameter(inputs.intensity_filtered_data_url, key="data_from")
        if not isinstance(lat_par_url, str):
            raise TypeError(
                f'Expected a str dataset at "{inputs.intensity_filtered_data_url}/data_from". Got {lat_par_url}'
            )

        omega_slop = find_omega_slope(inputs.folder_file_config)
        with tmp_grain_processing_files(
            ubi_init_data_url=inputs.indexed_grain_data_url,
            geo_init_data_url=geo_par_url,
            lat_init_data_url=lat_par_url,
