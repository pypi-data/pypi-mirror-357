...
from .models import UnitCellParameters, SampleConfig
...
def get_omega_slope(filepath: str | Path, scan_number: str | int, omega_motor: str):
...
def find_omega_slope(cfg: SampleConfig):
    _, sample_name, dset_name, scan_number = extract_sample_info(
        path_str=cfg.scan_folder
    )
    master_file = Path(cfg.scan_folder).parent / f"{sample_name}_{dset_name}.h5"
    if not master_file.exists():
        raise FileNotFoundError(
            f"""Could not find HDF5 master file at {master_file}.
                Check `scan_folder`, `sample_name` and `dset_name` in {cfg.model_dump()}.
            """
        )

    return get_omega_slope(
        filepath=master_file, scan_number=scan_number, omega_motor=cfg.omega_motor
    )
...
def copy_par_file(original_par_file: str, dest_folder: Path) -> Path:
    ...
    shutil.copy2(par_file, output_file)
