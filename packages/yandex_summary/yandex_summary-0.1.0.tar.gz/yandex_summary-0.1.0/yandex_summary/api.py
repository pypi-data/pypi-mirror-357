...
class SummaryResult:
    ...
    def to_html(self) -> str:
        ...
        if self.error:
            ...
            if self.chapters:
                html.append("<h2>Detailed Summary</h2>")
                for i, chapter in enumerate(self.chapters, 1):
                    ...
                    html.append("</ul>")
            if self.summary_points:
                html.append("<h2>Short Summary</h2>")
                html.append("<ul>")
                for i, point in enumerate(self.summary_points, 1):
...
class YandexSummaryAPI:
    """Client for interacting with the Yandex Summary API.

    This class provides methods to summarize articles using the Yandex 300 API,
    supporting both detailed and short summary formats.
    """
    ...
    def __init__(
        ...
        """Initialize Yandex Summary API client.

        Args:
            api_key: API key for Yandex API. If None, retrieved from YANDEX_API_KEY environment variable.
            base_url: Base URL for API requests (default: https://300.ya.ru/api).
            max_retries: Maximum number of retry attempts for requests.
            retry_delay: Initial delay between retries in seconds.
            timeout: Request timeout in seconds.

        Raises:
            ValueError: If API key or base_url is invalid.
        """
