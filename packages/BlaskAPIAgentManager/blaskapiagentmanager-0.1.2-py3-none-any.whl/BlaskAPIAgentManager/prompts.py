...
- For sorting parameters (sortBy, sort):
  * For brand performance analysis, the primary metric is the Blask Index. If the user query is about brand performance but does not specify a metric, DEFAULT to sorting by "blask_index".
...
Follow these guidelines:
- **BRAND PERFORMANCE METRIC:** The main metric for analyzing brand performance is the Blask Index. If the query is about brand performance and does not explicitly specify another metric, prioritize endpoints that provide the Blask Index.
- **BROAD ANALYSIS:** If a broad, detailed analysis is required, plan to call endpoints that provide a set of key metrics (e.g., Blask Index, GGR, FTD, Market Share).
...
fetch_ids_prompt = PromptTemplate.from_template(
    """You are an AI assistant tasked with extracting country and brand information from a user query.
...
Your task:
1. Analyze the user query to identify if any specific countries, regions, locations, or brands are mentioned.
2. If specific countries or regions are mentioned, identify them and match them to the list of available countries.
3. If specific brands are mentioned, identify them and match them to the list of available brands.
4. Return a JSON object with "country_mapping" and "brand_mapping" keys.

Follow these guidelines:
- If the query explicitly mentions specific countries, return a dictionary mapping those country names to their IDs in "country_mapping".
...
- If the query does not specify any location, return an empty dictionary for "country_mapping".
- If the query explicitly mentions specific brands, return a dictionary mapping those brand names to their IDs in "brand_mapping".
- If the query does not specify any brand, return an empty dictionary for "brand_mapping".
- Only include countries and brands that are explicitly mentioned or clearly implied in the query.
- Be exact in matching country and brand names - do not assume unless clearly specified.
- Return ONLY names and IDs that match exactly the items in the AVAILABLE COUNTRIES and AVAILABLE BRANDS lists.
...
Example outputs:
- For "What is the top 5 Asian brands now?": {{"country_mapping": {{"Bangladesh": 3, "India": 90, "Philippines": 137, "Japan": 141, "Malaysia": 143, "Azerbaijan": 145, "Vietnam": 146}}, "brand_mapping": {{}} }}
- For "Show top performing brands for Betano": {{"country_mapping": {{}}, "brand_mapping": {{"Betano": 123}} }}
- For "Show top performing brands": {{"country_mapping": {{}}, "brand_mapping": {{}} }}

The output should be a valid JSON object with "country_mapping" and "brand_mapping" keys.
- "country_mapping" contains a dictionary mapping country names to their IDs for mentioned countries/regions.
- "brand_mapping" contains a dictionary mapping brand names to their IDs for mentioned brands.
- An empty dictionary {{}} should be used if no location or brand is specified.
