...
class APICallConfig(BaseModel):
    ...
    granularity: Optional[str] = Field(
        None, description="Optional granularity for time-based data"
    )
    sort_by: Optional[str] = Field(None, description="Optional sorting field")
    sort_order: Optional[str] = Field(None, description="Optional sort order")
    sort: Optional[Literal["ggr", "ftd", "yoy", "mom", "market_share"]] = Field(
        None, description="Optional sort parameter with predefined values"
    )
...
class ParameterGenerator:
    ...
    def generate_parameters(
        ...
        try:
            ...
            result_params = self._process_parameters(params, config)
    ...
    def _process_parameters(self, params: Any, config: APICallConfig) -> APIParams:
        ...
        if config.time_range and not any(
            p.name in ["date_from", "date_to", "dateFrom", "dateTo"]
            for p in result_params.query_params
        ):
            start_date = (
            ...
            if start_date:
                ...
                result_params.query_params.append(
                    APIParam(name="dateFrom", value=start_datetime)
                )
            ...
            if end_date:
                ...
                result_params.query_params.append(
                    APIParam(name="dateTo", value=end_datetime)
                )
