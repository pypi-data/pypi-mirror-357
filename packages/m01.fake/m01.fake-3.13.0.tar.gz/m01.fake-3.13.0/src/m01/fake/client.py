...
class DocumentIndex(object):
    ...
    def __init__(self, storage, idx):
        ...
        self.name = idx['name']
        self.key = idx['key']
        self.drop_dups = idx.get('drop_dups')
        self.cache_for = idx.get('cache_for')
        self.clear()
...
class DocumentIndexes(collections.OrderedDict):
    ...
    def reIndex(self):
        ...
        for idx in list(self.values()):
            ...
            drop_dups = doc.get('drop_dups')
            if drop_dups is not None:
                data['drop_dups'] = drop_dups
            cache_for = doc.get('cache_for')
            if cache_for is not None:
                data['cache_for'] = cache_for
            append(data)
        return {
    ...
    def listIndexes(self):
        ...
        for idx in list(self.values()):
            ...
            drop_dups = doc.get('drop_dups')
            if drop_dups is not None:
                data = data + ((u'drop_dups', drop_dups),)
            cache_for = doc.get('cache_for')
            if cache_for is not None:
                data = data + ((u'cache_for', cache_for),)
...
class FakeMongoClient(pymongo.common.BaseObject):
    ...
    def __init__(self, host=None, port=None, document_class=dict,
        ...
        self._event_listeners = getattr(getattr(options, 'pool_options', None), 'event_listeners', None)
