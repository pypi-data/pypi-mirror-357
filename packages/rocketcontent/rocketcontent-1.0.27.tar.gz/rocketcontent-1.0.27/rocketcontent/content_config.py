...
class ContentConfig:
    ...
    def __init__(self, yaml_file):
        ...
        try:
            self.repo_server_user = content_config.get('repo_server_user')
        except:
            self.repo_server_user = "ADMIN"
        try:
            self.repo_server_pass = content_config.get('repo_server_pass')
        except:
            self.repo_server_pass = ""         
        
        self.encoded_repo_credentials = self._encode_basic_repo_auth(self.repo_server_user, self.repo_server_pass, self.repo_name)
    ...
    def get_vdr_content_sources(self):
        try:
            ...
            self.logger.debug(f"VDR Content Sources URL : {content_sources_url}")
            ...
            self.logger.info("--------------------------------")
            self.logger.info("Method : get_vdr_content_sources")
            self.logger.debug(f"VDR Repositories URL : {content_sources_url}")
            self.logger.debug(f"Headers : {json.dumps(headers)}")
    ...
    def get_repo_capabilities(self):
        ...
        try:
            ...
            self.logger.info("--------------------------------")
            self.logger.info("Method : get_repo_capabilities")
            self.logger.info("Checking if Content Repository is active ...")
            self.logger.debug (f"Request URL: {capabilities_url}")
            self.logger.debug (f"Repository ID: {self.repo_id}")  
    ...
    def _encode_basic_repo_auth(username, password, repo_name):
        credentials = f"{repo_name}:{username}:{password}"
