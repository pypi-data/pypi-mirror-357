...
class DatabricksSession:
    ...
    _client_image_version = "2"
    _py312_client_image_version = "3"
    ...
    class Builder:
        ...
        def _from_sdkconfig(
            ...
            if (
                ...
                def _get_client_image_version() -> str:
                    """Get client image version for serverless UDF execution based on the local python version.

                    Python 3.12 maps to client image version 3.
                    All other Python versions map to client image version 2.
                    """
                    v = sys.version_info
                    version_str = f"{v.major}.{v.minor}"
                    if version_str == "3.12":
                        client_image_version = DatabricksSession._py312_client_image_version
                    else:
                        client_image_version = DatabricksSession._client_image_version
                    return client_image_version

                headers["x-databricks-client-image-version"] = _get_client_image_version()
