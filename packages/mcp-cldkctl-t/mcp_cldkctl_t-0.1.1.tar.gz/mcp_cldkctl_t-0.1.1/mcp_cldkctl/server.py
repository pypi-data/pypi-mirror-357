...
async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> list[Content]:
    ...
    try:
        ...
        endpoint_map = {
            ...
            # --- Kubernetes ---
            "k8s_pods": "/core/pods",  # GET, project_id as query param
            "k8s_deployments": "/core/deployment",  # GET, project_id as query param
            "k8s_services": "/core/kubernetes/services",  # GET, project_id as query param
            "k8s_configmaps": "/core/kubernetes/configmap",  # GET, project_id as query param
            "k8s_secrets": "/core/kubernetes/secret",  # GET, project_id as query param
            ...
            # --- Virtual Machines ---
            "vm_list": "/core/virtual-machine/list/all",  # GET, project_id as query param
            "vm_detail": ("/core/virtual-machine/detail-vm", "POST"),  # POST, expects {project_id, vm_id} in body

            # --- Registry ---
            "registry_list": "/core/dekaregistry/v2/registry",  # GET, project_id as query param
            ...
            # --- Vouchers ---
            "voucher_list": "/core/user/voucher/claimed",  # GET
        ...
        if name in endpoint_map:
            ...
            # --- Kubernetes and VM tools: use project_id as query param if needed ---
            if name in ["k8s_pods", "k8s_deployments", "k8s_services", "k8s_configmaps", "k8s_secrets", "vm_list", "registry_list"]:
                params = {k: v for k, v in arguments.items() if k in ["project_id", "namespace"]}
                data = make_authenticated_request(method, endpoint_template, params=params)
                return [TextContent(type="text", text=format_response(data))]

            # --- vm_detail: POST with project_id and vm_id in body ---
            if name == "vm_detail":
                data = make_authenticated_request(method, endpoint_template, data=arguments)
                return [TextContent(type="text", text=format_response(data))]

            # --- registry_repositories: path param registry_id ---
            if name == "registry_repositories":
                endpoint = endpoint_template.format(**arguments)
                data = make_authenticated_request(method, endpoint)
                return [TextContent(type="text", text=format_response(data))]

            # --- voucher_list: no params ---
            if name == "voucher_list":
                data = make_authenticated_request(method, endpoint_template)
                return [TextContent(type="text", text=format_response(data))]

            # --- Default: path param substitution, body/query param split ---
            endpoint = endpoint_template.format(**arguments)
