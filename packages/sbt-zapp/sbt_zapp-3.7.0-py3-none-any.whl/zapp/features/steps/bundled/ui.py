...
@when(
    'Я перетащил элемент "{target}" на элемент "{other_element}" за {steps_count:d} шагов с задержкой в {step_delys_in_millis:d} мс между ними'
)
def drag_element_to_other_by_steps(
    context, target, other_element, steps_count: int = 1, step_delys_in_millis: int = 100
):
    """
...
@when(
    'Я взял элемент "{target}" и поместил над элементом "{other_element}" за {steps_count:d} шагов с задержкой в {step_delys_in_millis:d} мс между ними'
)
...
@when(
    'Я переместил курсор с элементом "{target}" на элемент "{other_element}" и отпустил за {steps_count:d} шагов с задержкой в {step_delys_in_millis:d} мс между ними'
)
def drag_and_hover_on_other_by_steps(
    context, target, other_element, steps_count: int = 1, step_delys_in_millis: int = 100
):
    """
    Поместить элемент (который был ранее нажат и перенесен курсором) над другим и отпустить. Выполняется несколькими шагами с ожиданием
    """
    drop = context.browser.element(locator(other_element)).highlight()
    (
        context.browser.element(locator(target))
