...
class semantic:
   ...
   def set_keys_for_type(self, type, *predicate_names):
      #greenMsg(f'set_keys_for_type: {type} URI keys {json.dumps(predicate_names)}')
  ...
  '''
   ...
   def update_map(self, obj_thunk, *arg):
      ...
      if obj.is_right():
        ...
        for key in json_ld_object.keys():
            msg(f'key  {key} value {json_ld_object[key]}')
        '''
   ...
   async def get_usd_path_for_segment_in_map(self, segment):
       result = self.usdPathMap.get(segment)
       if result is None:
          result = await self.set_usd_path_for_segment(segment)

       #greenMsg(f'get_usd_path_for_segment_in_map: {result}')
       return result
    
   async def ref_for_usd_object(self,  segmentName: str):

      path = await self.get_usd_path_for_segment_in_map( segmentName)
      if path is None:
          
          redMsg(f'ref_for_usd_object: path is None for segmentName: {segmentName}')
   ...
   async def get_usd_path_for_segment(self, segmentName: str) -> Either:
          ...
          if (prim_either is None or prim_either.is_left()):
            txt = f'get_usd_path_for_segment: failure in query (not empty query return) for frame @type: UsdPrim segmentName: {segmentName} '
            redMsg(txt)
            result = Left(txt)
          else:
            prim = prim_either.either(lambda e: f'get_usd_path_for_segment: Error: {e}', lambda x: x)
            '''
            ...
            if name is not None:
              ...
              txt = f'get_usd_path_for_segment: frame @type: UsdPrim segmentName: {segmentName} returned empty'
              redMsg(txt)
              result = Left(txt)
