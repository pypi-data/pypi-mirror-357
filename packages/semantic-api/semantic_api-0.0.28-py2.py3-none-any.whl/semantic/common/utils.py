...
def to_json_ld_type(obj):
    '''
        nb:  no copies, obj is modified!
             type -> @type

        narration will modify objects passed into it
        precondition: client copies if unmodified (type version) is required post narration
    '''
    for k,v in obj.items():
        if isinstance(v, dict):
          greenMsg(f'to_json_ld_type  k {k} obj.k {json.dumps(obj[k], indent=6)}')
          obj[k]['@type'] =  obj[k].pop('type')
        elif type(v) is list or type(v) is tuple:
          for o in v:
            if isinstance(o, dict):
                to_json_ld_type(o)
    obj['@type'] = obj.pop('type')
    #greenMsg('to_json_ld_type '+json.dumps(obj, indent=6))
    return obj
