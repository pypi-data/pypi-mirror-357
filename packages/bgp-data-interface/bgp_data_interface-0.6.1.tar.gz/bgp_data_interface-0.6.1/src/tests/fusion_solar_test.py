...
def test_historical_empty_params() -> None:
    ...
    assert df['date_time'].iloc[0].day == yesterday.day
    assert df['date_time'].iloc[-1].day == yesterday.day
    assert df['date_time'].iloc[0].hour == 0
    assert df['date_time'].iloc[-1].hour == 23
...
def test_historical_last3days() -> None:
    ...
    assert df['date_time'].iloc[0].day == last3days.day
    assert df['date_time'].iloc[-1].day == yesterday.day
    assert df['date_time'].iloc[0].hour == 0
    assert df['date_time'].iloc[-1].hour == 23
...
def test_historical_vhh() -> None:
    ...
    df = api.historical({
        'location': loc['fusion_solar_location'],
    })
