...
from interface_expander.I2cInterface import (I2cInterface, I2cConfig, ClockFreq, AddressWidth, I2cId, I2cMasterRequest,
                                             I2cStatusCode, I2C_SLAVE_BUFFER_SPACE, I2cSlaveRequest)
...
class TestI2cErrorNotifications:
    ...
    def test_i2c_error_notify_invalid_write_read_size(self):
        ...
        write_request = I2cMasterRequest(slave_addr=TestI2cErrorNotifications.FRAM_SLAVE_ADDR,
                                         write_data=bytes(), read_size=0)
        with pytest.raises(ValueError):
            i2c0.send_request(request=write_request)

        write_request = I2cMasterRequest(slave_addr=TestI2cErrorNotifications.FRAM_SLAVE_ADDR,
                                         write_data=bytes(), read_size=1024)
        with pytest.raises(ValueError):
            i2c0.send_request(request=write_request)
