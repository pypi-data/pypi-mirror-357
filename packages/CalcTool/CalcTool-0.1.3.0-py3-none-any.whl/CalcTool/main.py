...
class LaTeXCalculate(float):
    ...
    def Check(self, tag):
        ...
        have_bracket = False
        for i in range(len(checked_tag)*2):
            try:
                checked_tag[i]
            except IndexError:
                break
            ...
            # 如果是分数
            if item == r'\frac':
                checked_tag.insert(i+1, ["{", "bracket"])  # 在\frac第一个参数前面插入括号
                checked_tag.insert(i+3, ["/", "operator"]) # 在\frac后两个参数之间插入"/"
                checked_tag.insert(i+5, ["}", "bracket"])  # 在\frac第二个参数后面插入括号
                checked_tag.remove(checked_tag[i])         # 将当前的\frac删除
