...
@mcp.tool()
def create_powerpoint_presentation(filename: str = "new_presentation") -> str:
    """
...
@mcp.tool()
def save_presentation_as(prs_id: str, file_path: str, new_filename: str = None) -> str:
    """
    保存PowerPoint演示文稿到磁盘。
    
    Args:
        prs_id: 该演示文稿在内存中的唯一标识符
        file_path: 保存到文件系统中的路径
    ...
    try:
        ...
        # 创建新文件路径
        new_file_path = os.path.join(file_dir, f"{file_name}.pptx")
...
@mcp.tool()
def get_presentation_download_link(prs_id: str, file_path: str) -> str:
    """
    Get a download link or base64 representation for the presentation.

    Args:
        prs_id: 演示文稿在内存中的唯一标识符
        file_path: The presentation ID
...
@mcp.tool()
def add_slides_bulk(prs_id: str, slides: List[Dict[str, Any]]) -> str:
    """
    批量添加幻灯片并为每页填充占位符内容。
    ...
    Args:
        ...
        slides: 每一项为 dict，包含：
                - layout: 幻灯片布局名
                - placeholders: 按序对应布局内的占位符内容（字符串列表）
    ...
    #     result = add_slide(prs_id, layout_name)
    #     if result.startswith("已成功添加"):
    #         added += 1

    for idx, slide_info in enumerate(slides):
        layout_name = slide_info.get("layout", "Title and Content")
        placeholder_texts = slide_info.get("placeholders", [])

        res.append(add_slide(prs_id, layout_name, placeholder_texts))
...
Generate an SVG image from a natural‑language description using chain‑of‑thought:
1) Break down your approach in numbered "Thought 1: …", "Thought 2: …" steps.
2) Output only the final SVG markup wrapped in <svg>…</svg> tags.
""")
def svg_generator_prompt(description: str) -> list[base.Message]:
    return [
        base.UserMessage(
...
def generate_svg(prs_id: str, svg_markup: str, title: str = None, width: float = 6.0) -> str:
    """
    Takes final <svg>…</svg> text, writes to /tmp/, rasterize via rsvg-convert,
