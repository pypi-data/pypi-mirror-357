...
@pytest.mark.parametrize("dataset_name", ["dlnirsp", "visp", "dlnirsp-mosaic", "vbi", "vbi-mosaic"])
def test_pixel_axis_type_map(dataset_name, simulated_dataset):
    ...
    expected_map_ordering["vbi-mosaic"] = expected_map_ordering["vbi"]
...
def test_time_bug():
    """
    There is a bug either in astropy's usage of numpy or numpy itself that causes
    a SIGSEGV when creating a Time object with a large number of dates when using numpy >= 2.3.0.
    """
    some_dates = [datetime.now().isoformat("T") for _ in range(1000)]
    Time(some_dates)  # errors for numpy >= 2.3.0
