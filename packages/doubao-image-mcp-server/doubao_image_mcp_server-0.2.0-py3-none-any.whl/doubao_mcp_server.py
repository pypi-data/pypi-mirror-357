...
async def doubao_generate_image(
    ...
    seed: Annotated[int, Field(description="Random seed for controlling model generation randomness, -1 means auto-generate", ge=-1, le=2147483647)] = -1,
    ...
    Args:
        ...
        seed (int): Random seed for controlling model generation randomness, -1 means auto-generate, range -1 to 2147483647
                   随机数种子，用于控制模型生成内容的随机性，-1表示自动生成，范围-1到2147483647
...
- **Prompt**: {prompt}
- **Resolution**: {size}
- **Random Seed**: {seed if seed != -1 else 'Auto-generate'}
...
- **seed**: Random seed, -1 means auto-generate, same seed can reproduce results
