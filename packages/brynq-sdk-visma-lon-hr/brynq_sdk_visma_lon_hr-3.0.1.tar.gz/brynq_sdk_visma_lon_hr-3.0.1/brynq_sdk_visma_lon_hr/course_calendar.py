...
import pandas as pd
from .schemas.course_calendar import CourseCalendarSchema

class CourseCalendar:
    """
    Handles course calendar operations for Visma Lon HR API.
    CourseCalendar holds information about the course calendar from the Visma HR table CourseCalendar.
    """

    def __init__(self, visma_instance):
        """
        Initialize the CourseCalendar class.
    ...
    def get(self, 
        ...
        """
        Get course calendar data from Visma.
        ...
        Returns:
            pd.DataFrame: DataFrame with course calendar data
        """
        return self.visma.get(
            entity_type="CourseCalendar",
            filter=filter,
            select=select,
            orderby=orderby,
            top=top,
