...
from raphson_mp.playlist import Playlist, get_playlists
...
async def route_download(_request: web.Request, conn: Connection, user: User):
    """Download page"""
    # TODO pass playlist objects directly to template
    playlists = [
        (playlist.name, playlist.is_writable())
        for playlist in get_playlists(conn, user)
    ]
...
async def route_ytdl(request: web.Request, conn: Connection, user: User):
    ...
    playlist_name: str = cast(str, json["directory"])
