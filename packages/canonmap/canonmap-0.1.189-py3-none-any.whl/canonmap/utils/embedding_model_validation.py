...
def _handle_gcp_strategy(state: Dict[str, Any], config: CanonMapEmbeddingConfig) -> bool:
    ...
    # Check GCP prefix - if it doesn't exist but local files do, try to upload
    if not gcp_status.get("prefix_exists", False):
        if local_exists and local_complete:
            logger.info("GCP prefix doesn't exist but local model is complete, uploading to GCP...")
            try:
                uploaded_files = config.upload_local_model_to_gcp()
                if uploaded_files:
                    logger.info(f"✅ Successfully uploaded {len(uploaded_files)} model files to GCP")
                    # Re-check GCP status after upload
                    gcp_status = validate_gcp_resources(config)
                    if gcp_status.get("prefix_exists", False):
                        logger.info("GCP prefix now exists after upload")
                        gcp_available = gcp_status.get("gcp_configured", False) and gcp_status.get("bucket_exists", False)
                    else:
                        logger.warning("GCP prefix still doesn't exist after upload attempt")
                else:
                    logger.warning("Upload to GCP returned no files")
            except Exception as e:
