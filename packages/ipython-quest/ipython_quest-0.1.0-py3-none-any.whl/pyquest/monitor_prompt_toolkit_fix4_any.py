...
from pyquest.util import setup_logging
import string
import sys
import time
    ...
    if not allow_paste:
        for k in (Keys.ControlV, Keys.Insert, Keys.ControlInsert, Keys.ShiftInsert, Keys.ControlShiftInsert, Keys.BracketedPaste):
            ...
            def _monitor_paste(event):
                if allow_paste:
                    print("\n🚨 PASTE DETECTED: Try typing the code yourself!")
                    pastedtext = event.app.clipboard.get_data()
                    event.app.current_buffer.paste_clipboard_data(
                        pastedtext
                    )
                    print(pastedtext)
                    # event.app.current_buffer.insert_text(pastedtext)
                else:
                    print("\n🚨 PASTE BLOCKED: Try typing the code yourself!")
                    monitor.log_keystroke(f'{str(k)}(PASTE?)')
