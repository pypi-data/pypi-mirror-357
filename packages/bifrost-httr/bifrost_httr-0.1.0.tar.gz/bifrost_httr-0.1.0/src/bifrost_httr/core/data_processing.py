...
def validate_substances_cell_types(substances_cell_types_dict: dict[str, Any]) -> None:
    """Validate substances and cell types dictionary.

    Args:
        substances_cell_types_dict: Dictionary containing test substances and cell types
    ...
    required_keys = [
        "Test substances",
        "Cell types",
        # 'Additional divider' and 'Specific filters' are optional in the old package
    ]

    # Check for required keys
    missing_keys = [
        key for key in required_keys if key not in substances_cell_types_dict
    ]
    if missing_keys:
        error_msg = f"Missing required keys in substances and cell types dictionary: {missing_keys}"
        raise ValueError(
            error_msg,
        )

    # Validate test substances
    if not isinstance(substances_cell_types_dict["Test substances"], list) or not all(
        isinstance(x, str) for x in substances_cell_types_dict["Test substances"]
    ):
        error_msg = "Test substances must be a list of strings"
        raise ValueError(error_msg)

    # Validate cell types
    if not isinstance(substances_cell_types_dict["Cell types"], list) or not all(
        isinstance(x, str) for x in substances_cell_types_dict["Cell types"]
    ):
        error_msg = "Cell types must be a list of strings"
        raise ValueError(error_msg)

    # Check if Additional divider is present and has correct type
    if "Additional divider" in substances_cell_types_dict and not isinstance(
        substances_cell_types_dict["Additional divider"],
        str,
    ):
    ...
    if (
        "Specific filters" in substances_cell_types_dict
        and substances_cell_types_dict["Specific filters"] is not None
        and not isinstance(substances_cell_types_dict["Specific filters"], dict)
    ):
...
def write_bifrost_input(
    ...
    batch_index = np.array(
        [unique_batches.index(k) + 1 for k in df[config_dict["Batch key"]]],
        dtype="int",
    )
