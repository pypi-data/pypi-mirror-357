...
@click.option(
    ...
    required=True,
    type=click.Path(exists=True),
    help="Path to meta data mapper YAML file",
)
@click.option(
    "--counts",
    "-c",
...
@click.option(
    ...
    required=True,
    type=click.Path(exists=True),
    help="Path to substances and cell types YAML file",
)
@click.option(
    "--batch-key",
    default="Batch",
    help="Field to use as batch key in the BIFROST model",
)
@click.option(
    "--min-percent-mapped-reads",
    type=float,
    default=80.0,
    help="Minimum percentage of mapped reads required",
)
@click.option(
    "--min-num-mapped-reads",
    type=int,
    default=1000000,
    help="Minimum number of mapped reads required",
)
@click.option(
    "--min-avg-treatment-count",
    type=int,
    default=10,
    help="Minimum average treatment count required",
)
@click.option(
    "--output-dir",
    "-o",
...
def prepare_inputs(
    ...
    meta_mapper: str,
    counts: str,
    substances_cell_types: str,
    batch_key: str,
    min_percent_mapped_reads: float,
    min_num_mapped_reads: int,
    min_avg_treatment_count: int,
    output_dir: str,
    ...
    substances_cell_types_dict = load_yaml_file(substances_cell_types)
    validate_substances_cell_types(substances_cell_types_dict)

    # Create config dictionary from arguments (CLI overrides YAML)
    config_dict = {
        "Test substances": substances_cell_types_dict["Test substances"],
        "Cell types": substances_cell_types_dict["Cell types"],
        "Additional divider": substances_cell_types_dict.get(
            "Additional divider",
            "N/A",
        ),
        "Specific filters": substances_cell_types_dict.get("Specific filters", None),
        "Batch key": batch_key,
    ...
    generate_bifrost_inputs(meta, counts_table, config_dict, output_dir)
...
def split_data(
    ...
    # Validate and create analysis directory
    analysis_dir = Path(analysis_dir)
    validate_output_directory(analysis_dir)
    analysis_dir.mkdir(parents=True, exist_ok=True)

    # Process the data into pickle files
    process_data(
        input_file_path=input_file,
        path_to_output=analysis_dir,
        testing_mode=False,
    )
