...
class Task:
    ...
    def bulk_register_periodic_task(self, tasks: Sequence[Tuple[str, crontab | timedelta, Sequence]], queue: str | None = None):
        for route, schedule, args in tasks:
            self.register_periodic_task(route=route, schedule=schedule, queue=queue, *args)
        return self
