...
from .util.core_helpers import generate_random_code, get_site_preferences, has_person
...
class Person(ContactMixin, ExtensibleModel):
    ...
    def as_vcard(self, request, params) -> str:
        ...
        # Related persons
        if not self._is_unrequested_prop("RELATED", params) and request.user.has_perm(
            "core.view_personal_details", self
        ):
...
class FreeBusy(CalendarEvent):
        ...
        if request:
            for person in Person.objects.filter(id__in=persons):
                ...
                for feed in CalendarEventMixin.valid_feeds:
                    feed_events = feed.get_availability_objects(request=request, obj=person)
                    if feed_events:
            ...
            for group in Group.objects.filter(id__in=groups):
                if not request.user.has_perm("core.view_group_free_busy_feed_rule", group):
                    continue
                for feed in CalendarEventMixin.valid_feeds:
                    feed_events = feed.get_availability_objects(request=request, obj=group)
                    if feed_events:
                        events.update(feed_events)
    ...
    def value_description(cls, reference_object, request: HttpRequest | None = None) -> str:
        return _("Free/busy status generated by events in feed: {}").format(
            reference_object.dav_verbose_name
        )
...
class AvailabilityEvent(CalendarEvent):
    ...
    @classmethod
    def get_availability_objects(cls, request, obj: Union[Person, Group] = None) -> QuerySet:
        if isinstance(obj, Person):
            return cls.objects.for_persons([obj])
        elif isinstance(obj, Group):
            return cls.objects.for_groups([obj])
...
class PersonalEvent(CalendarEvent):
    ...
    @classmethod
    def get_availability_objects(cls, request, obj: Union[Person, Group] = None) -> QuerySet:
        qs = super().get_objects(request=request)
        if isinstance(obj, Person):
            persons = [obj]
        elif isinstance(obj, Group):
            persons = obj.members.all()
        if persons:
            return qs.for_persons(persons)
        return None
