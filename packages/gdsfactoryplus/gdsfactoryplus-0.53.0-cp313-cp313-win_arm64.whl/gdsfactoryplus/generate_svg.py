...
def generate_svg(comp: gf.Component, width: int, height: int) -> str:
    """Generate the svg for a cell."""
    mp = _generate_multipolygon(comp)
    ...
    return _svg_from_svg_strings(ds, width, height)


@lru_cache
def get_svg(comp: gf.Component, width: int, height: int) -> str:
    """Get the (possible cached) svg for a cell."""
    return generate_svg(comp, width, height)


def _generate_multipolygon(comp: gf.Component) -> sg.MultiPolygon:
...
def _rdp(poly: np.ndarray, eps: float) -> np.ndarray:
    ...
    if not poly.shape:
        return np.array([])
    if poly.shape[0] < 3:  # noqa: PLR2004
        return poly
...
def _path_from_svg_string(path_string: str, path_id: str) -> str:
    template = (
        '<path d="{path_string}" id="{path_id}" '
        'style="fill:#e5e5e5;fill-opacity:1;stroke:#000000;stroke-width:2;'
        'stroke-dasharray:none;stroke-opacity:1" />'
    )
    return template.format(path_string=path_string, path_id=path_id)


def _svg_from_svg_strings(path_strings: list[str], width: int, height: int) -> str:
...
<svg
  ...
  <g
     id="icon">
    <rect
       style="fill:#cccccc;fill-opacity:1;stroke:#808080;stroke-width:1;stroke-dasharray:none;stroke-opacity:1"
       id="frame"
       width="{width1}"
       height="{height1}"
       x="0.5"
    ...
    for i, path_string in enumerate(path_strings):
        paths += _path_from_svg_string(path_string, f"poly{i}") + r"\n"
