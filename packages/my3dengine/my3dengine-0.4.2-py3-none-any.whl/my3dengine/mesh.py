...
class Mesh:
    ...
    def is_in_fov(self, cam_pos, cam_dir, fov_deg):
        ...
        cos_angle = np.dot(cam_dir_norm, to_obj_norm)

        return cos_angle >= cos_half_fov

    def is_visible(self, view_proj_matrix):
        # Tutaj prosta implementacja - sprawdzamy tylko czy pozycja mesh jest w zasięgu [-1,1] po przekształceniu
        pos = np.append(self.position, 1.0)
        clip_space_pos = view_proj_matrix @ pos

        # Normalizacja (perspektywiczna)
        if clip_space_pos[3] == 0:
            return False
        ndc = clip_space_pos[:3] / clip_space_pos[3]

        # Sprawdź, czy w NDC (Normalized Device Coordinates) jest wewnątrz sześcianu -1..1
        return np.all(ndc >= -1) and np.all(ndc <= 1)
    ...
    def draw(self, cam_pos=None, cam_dir=None, fov_deg=70, view_proj_matrix=None, debug=False):
        ...
        # Usuwamy warunek z is_visible i zostawiamy tylko FOV culling
        if cam_pos is not None and cam_dir is not None:
            if not self.is_in_fov(cam_pos, cam_dir, fov_deg):
                if debug:
                    print(f"Mesh at {self.position} skipped by FOV cull")
                return
