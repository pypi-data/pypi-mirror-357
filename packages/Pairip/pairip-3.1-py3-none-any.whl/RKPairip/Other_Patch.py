...
def Application_Name(L_S_F):
    pattern = C.re.compile(r'\.class\s+public\s+Lcom/pairip/application/Application;\s+\.super\s+L([^;\s]+)', C.re.DOTALL)
    super_value = None
    for root, _, files in C.os.walk(L_S_F):
        for file in files:
            if file == 'Application.smali':
                ...
                if match:
                    super_value = match.group(1).replace(C.os.sep, ".")
                    break
        if super_value:
            break
    return super_value
...
def Merge_Smali_Folders(decompile_dir, isAPKTool, L_S_F):
    ...
    #smali_folder = sorted([f for f in C.os.listdir(smali_path) if f == "smali" or f.startswith(prefix)], key=lambda x: int(x[len(prefix):]) if x[len(prefix):].isdigit() else 0)
