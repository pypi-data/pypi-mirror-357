...
from diffstarpop.loss_kernels.mstar_ssfr_loss_tpeak_sepms_satfrac import (
    ...
    loss_mstar_ssfr_sat_kern_tobs_grad_wrapper,
...
if __name__ == "__main__":
    ...
    # Define loss kernel ---------------------------------------------
    if loss_type == "mstar":
        loss_kernels = [loss_mstar_kern_tobs_grad_wrapper]
    elif loss_type == "mstar_ssfr_cen":
        loss_kernels = [
            loss_mstar_kern_tobs_grad_wrapper,
            loss_mstar_ssfr_kern_tobs_grad_wrapper,
    ...
    for i in range(Nloss):
        loss_kernels[i](params_init, loss_data[i])
    ...
    for istep in range(n_step):
        ...
        loss, grads = loss_kernels[loss_alt_id](p, loss_data[loss_alt_id])
