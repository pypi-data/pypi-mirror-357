...
from .. import sfh_pdf_tpeak_line_sepms_satfrac_sigslope as qseq
...
def test_frac_quench_cen_vs_logmp0():
    lgmarr = np.linspace(1, 20, 100)
    fqarr = qseq._frac_quench_cen_vs_logmp0(qseq.SFH_PDF_QUENCH_PARAMS, lgmarr)
    assert np.all(fqarr >= 0.0)
    assert np.all(fqarr <= 1.0)


def test_frac_quench_sat_vs_logmp0():
    lgmarr = np.linspace(1, 20, 100)
    fqarr = qseq._frac_quench_sat_vs_logmp0(qseq.SFH_PDF_QUENCH_PARAMS, lgmarr)
    assert np.all(fqarr >= 0.0)
    assert np.all(fqarr <= 1.0)
...
def test_get_mean_u_params_qseq_block():
    ...
    lgmarr = np.linspace(10, 15, n_gals)
    params = qseq.SFH_PDF_QUENCH_PARAMS
    _mean_pars_ms_block = qseq._get_mean_u_params_qseq(params, lgmarr)
