...
class ApiModelWrapper:
    ...
    def _cache_id(self) -> str:
        raise NotImplementedError("Subclasses must implement the cache_id property")

    def __init__(self, *, client: "KickClient", data: dict, cache_strategy = CacheStrategy.UPDATE):
        """ Initialize an ApiModelWrapper instance.
        ...
        if self.__cache__:
            cached_data = self.save_to_cache(
                cache_id=self.get_cache_id(self._cache_id),
                data=self._data,
...
class Token(ApiModelWrapper):
    ...
    def __init__(self, *, client: "KickClient", data: dict, cache_strategy = CacheStrategy.UPDATE):
        super().__init__(client=client, data=data, cache_strategy=cache_strategy)
...
class Broadcaster(User):
    ...
    def __init__(self, *, client, data, cache_strategy=CacheStrategy.UPDATE):
        super().__init__(client=client, data=data, cache_strategy=cache_strategy)
...
class Channel(ApiModelWrapper):
    ...
    def __init__(self, *, client: "KickClient", data: dict, **kwargs):
...
class KickClient:
    ...
    async def __run_auth_handler(self):
        ...
        if token is None:
            if not self.scopes:
                ...
                logging.info(f"Visit this URL to authorize:\n{str(oauth_url)}")
                code = await fut
    ...
    async def __run(self):
        ...
        async with KickHttpClient(loop=self._loop) as self._kick_http_client:
