...
def test_strings(test_env):
    ...
    addr, content, length, str_type = string_info
    assert addr == 0x3D4
    assert content == 'Hello, IDA!\n'
...
def test_flirt(test_env):
    ...
    sig_files = db.flirt.create(pat_only=True)
...
def test_bytes(test_env):
    ...
    assert isinstance(db.bytes.is_code(0x0), bool) and db.bytes.is_code(0x0)
    assert isinstance(db.bytes.is_data(0x330), bool) and db.bytes.is_data(0x330)
    assert isinstance(db.bytes.is_unknown(0x330), bool) and not db.bytes.is_unknown(0x330)
    assert isinstance(db.bytes.is_head(0x330), bool) and db.bytes.is_head(0x330)
    assert isinstance(db.bytes.is_tail(0x331), bool) and not db.bytes.is_tail(0x331)
