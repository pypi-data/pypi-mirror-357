...
import argparse
import ida_domain
from ida_domain.bytes import StringType, SearchFlags, ByteFlags
...
def analyze_bytes(db_path, search_pattern=None, patch_demo=False, max_results=20):
    ...
    if db.open(db_path, ida_options):
        ...
        byte_val = bytes_handler.get_byte(entry_point)
        word_val = bytes_handler.get_word(entry_point)
        dword_val = bytes_handler.get_dword(entry_point)
        ...
        # Get disassembly
        disasm = bytes_handler.get_disassembly(entry_point)
        ...
        for addr in test_addresses:
            if addr <= db.maximum_ea:
                flags = bytes_handler.get_flags(addr)
                data_type = bytes_handler.get_data_type(addr)
                data_size = bytes_handler.get_data_size(addr)

                # Use new flag checking methods
                is_code = bytes_handler.check_flags(addr, ByteFlags.CODE)
                is_data = bytes_handler.check_flags(addr, ByteFlags.DATA)
                has_any_data_flags = bytes_handler.has_any_flags(
                    addr, ByteFlags.BYTE | ByteFlags.WORD | ByteFlags.DWORD
                )

                print(f'  Address {hex(addr)}:')
                print(f'    Type: {data_type}, Size: {data_size}')
        ...
        for pattern, description in patterns_to_search:
            found_addr = bytes_handler.find_bytes(pattern)
            if found_addr:
        ...
        if search_pattern:
            ...
            addr_nocase = bytes_handler.find_text(search_pattern, flags=SearchFlags.DOWN)
        ...
        immediate_addr = bytes_handler.find_immediate(1)
        ...
        string_count = 0
        for addr, string_val in db.strings.get_all():
            ...
            success, cstring = bytes_handler.get_cstring(addr)
            if success:
                print(f'    C-string: {repr(cstring)}')

            success, unicode_str = bytes_handler.get_unicode_string(addr)
            if success:
                print(f'    Unicode: {repr(unicode_str)}')
        ...
        for addr in range(db.minimum_ea, min(db.minimum_ea + 0x100, db.maximum_ea), 4):
            if bytes_handler.is_data(addr) or bytes_handler.is_unknown(addr):
                data_addr = addr
                break
        ...
        if data_addr:
            ...
            original_type = bytes_handler.get_data_type(data_addr)
            print(f'    Original type: {original_type}')

            # Make it a byte
            if bytes_handler.make_byte(data_addr):
        ...
        if patch_demo:
            ...
            if patch_addr:
                ...
                # Get original values
                orig_byte = bytes_handler.get_byte(patch_addr)
                orig_word = bytes_handler.get_word(patch_addr)

                print(f'    Original byte: 0x{orig_byte:02x}')
                print(f'    Original word: 0x{orig_word:04x}')

                # Patch byte
                if bytes_handler.patch_byte(patch_addr, 0xAB):
                    new_byte = bytes_handler.get_byte(patch_addr)
                    print(f'    Patched byte: 0x{new_byte:02x}')

                    # Get original value
                    retrieved_orig = bytes_handler.get_original_byte(patch_addr)
                    print(f'    Retrieved original: 0x{retrieved_orig:02x}')

                    # Revert patch
                    if bytes_handler.revert_byte(patch_addr):
                        reverted_byte = bytes_handler.get_byte(patch_addr)
                ...
                if bytes_handler.patch_bytes(patch_addr, test_data):
                    ...
                    # Get original bytes
                    success, orig_bytes = bytes_handler.get_original_bytes(
        ...
        if test_addr <= db.maximum_ea:
            next_head = bytes_handler.next_head(test_addr)
            prev_head = bytes_handler.prev_head(test_addr)
            next_addr = bytes_handler.next_addr(test_addr)
            prev_addr = bytes_handler.prev_addr(test_addr)
        ...
        for addr in range(db.minimum_ea, db.minimum_ea + sample_size):
            if bytes_handler.is_code(addr):
                code_count += 1
            elif bytes_handler.is_data(addr):
                data_count += 1
            elif bytes_handler.is_unknown(addr):
                unknown_count += 1
