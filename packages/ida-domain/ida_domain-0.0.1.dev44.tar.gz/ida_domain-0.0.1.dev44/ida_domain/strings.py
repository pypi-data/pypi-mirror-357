...
class Strings:
    ...
    def get_at(self, ea: ea_t) -> Tuple[ea_t, str, int, StringType] | None:
        """
        ...
        Returns:
            A tuple (address, content, length, type) if found, None otherwise.
        """
        ...
        for index in range(ida_strlist.get_strlist_qty()):
            ...
            if ida_strlist.get_strlist_item(si, index) and si.ea == ea:
                ...
                if content:
                    return si.ea, content.decode('utf-8', errors='replace'), si.length, si.type
        return None
    ...
    def get_type(self, ea: ea_t) -> Union[int, StringType]:
        """
        ...
        Returns:
            String type (StringType enum or raw int) or -1 if not a string.
        """
        result = self.get_at(ea)
        return result[3] if result else -1
    ...
    def find_strings_containing(
        ...
        Returns:
            Iterator over matching strings (address, content).
        """
            ...
            if ida_strlist.get_strlist_item(si, index):
                ...
                if content:
                    try:
                        ...
                        search_text = decoded if case_sensitive else decoded.lower()
