...
def get_idps(base_url, verify=True):
    """
    ...
    """
    idps = requests.get(glue_url(base_url, '/saml', '/idps'), headers={'Accept': 'application/json'}, verify=verify)  # nosec: B113 (currently not dealing with timeouts)
    ...
    if idps.status_code == codes.ok:
        ...
        raise HanskenAuthError(f'cannot query remote for SAML IDPs: {idps.status_code}: {idps.reason}')


def probe_openam_idp(idp_url, verify=True):
    """
    ...
    probe = requests.get(probe, params=omit_empty({'realm': idp_realm}), allow_redirects=False, verify=verify)  # nosec: B113 (currently not dealing with timeouts)

    if probe.headers.get('WWW-Authenticate') == 'Negotiate':
        # idp wants us to use Kerberos, construct the info we need
        params = omit_empty({'realm': idp_realm, 'module': 'WindowsDesktopSSO'})
        ...
        return idp_realm, f'{sso_url}?{params}'
    else:
...
def resolve(
    ...
    log.debug(
        'constructing authentication method from the following provided information: '
        'base_url={base_url}, '
    ...
    if idps and not idp_id:
        if len(idps) == 1:
            ...
            raise HanskenAuthError(
                '{} IDPs available at {}, none requested, choose from {}'.format(
                    len(idps), base_url, ', '.join(sorted(idps.keys()))
                )
            )
    ...
    if idp_id:
        ...
        if not idp_url or 'signOnServices' not in idp_url or SAML_SOAP_NS not in idp_url['signOnServices']:
            raise HanskenAuthError(f'selected IDP ID {idp_id} not available at {base_url}')
    ...
    if idp_url and not username:
        ...
        username = input(f'username [{env_user}]: ').strip()
        if not username:
...
class HanskenAuthBase(AuthBase):
    ...
    def handle_response(self, response, **kwargs):
        ...
        handler = getattr(self, f'handle_{response.status_code}', None)
        if callable(handler):
...
class SAMLProxyAuth(HanskenAuthBase):
    ...
    def handle_401(self, response, **kwargs):
        if not self.login_url:
            raise HanskenAuthError(
                f'cannot handle {response.status_code}: {response.reason}, no login url available', response=response
            )

        with self._lock, self:
            if self._active > 1:
                raise HanskenAuthError(
                    f'login failed, got a {response.status_code}: {response.reason} response '
                    'while handling authentication trigger',
                    response=response,
                )
            ...
            if auth_response.status_code != codes.ok:
                ...
                # response
                raise HanskenAuthError(
                    f'failed to handle {response.status_code}: {response.reason}, login failed', response=auth_response
                )
...
class SAMLAuthBase(HanskenAuthBase):
        ...
        with self._lock:
            if self._active or not response.headers.get('SOAPAction'):
                ...
                with self:
                    ...
                    response_consumer_url = re.search(
                        r'responseConsumerURL="(?P<url>.+?)"', response.text, flags=re.IGNORECASE
                    )
    ...
    def do_saml_response(self, saml_response, response_consumer_url):
        ...
        saml_response = self.session.post(
        ...
        if not saml_response.is_redirect:
            ...
            raise HanskenAuthError(
                f'got unexpected response {saml_response.status_code}: {saml_response.reason} '
                f'from SP, expected a redirect',
...
class OpenAMAuth(SAMLAuthBase):
    ...
    def get_auth_token(self):
        ...
        if self.idp_realm:
            ...
            login_url = f'{login_url}?realm={self.idp_realm}'

        # attempt auth through OpenAM REST
        user = self.username()

        log.info(
            'authenticating user {user} to realm {realm} using {idp_url}',
            user=user,
        ...
        raise HanskenAuthError(
            'failed to get valid authentication token from OpenAM '
            f'(response {auth_response.status_code}: {auth_response.reason}), '
            'cannot complete ECP login'
        )
    ...
    def do_saml_request(self, saml_request, auth_token):
        ...
        if saml_response.status_code != codes.ok:
            ...
            log.warn(
                'got {response.status_code}: {response.reason} from IDP endpoint {idp}, SAML auth failed',
                response=saml_response,
                idp=self.idp_url,
            )
            saml_response.raise_for_status()
...
class KeycloakAuth(SAMLAuthBase):
    ...
    def handle_paos_request(self, response):
        log.info(
            'posting SAML request with authorization for user {user} to IDP endpoint {idp}',
            user=self.username(),
            idp=self.idp_url,
        )
        saml_response = self.session.post(
