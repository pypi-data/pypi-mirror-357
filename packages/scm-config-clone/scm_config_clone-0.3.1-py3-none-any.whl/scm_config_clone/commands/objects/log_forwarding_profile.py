# scm_config_clone/commands/objects/log_forwarding_profile.py
...
def build_create_params(
    src_obj: LogForwardingProfileResponseModel,
    destination: str,
    context_type: str = "folder",
) -> Dict[str, Any]:
    """
    Construct the dictionary of parameters required to create a new log forwarding profile object.

    Given an existing LogForwardingProfileResponseModel (source object), a destination folder or snippet,
    and an optional context type, this function builds a dictionary with all necessary fields
    for creating a new log forwarding profile in the destination tenant. It uses `model_dump` on a Pydantic model
    to ensure only valid, explicitly set fields are included.

    Args:
        src_obj: The LogForwardingProfileResponseModel representing the source log forwarding profile.
    ...
    Returns:
        A dictionary containing the fields required for `LogForwardingProfile.create()`.
        This dictionary is validated and pruned by LogForwardingProfileCreateModel.
    """
    data = {
        "name": src_obj.name,
        context_type: destination,
        "description": src_obj.description if src_obj.description is not None else None,
        "match_list": src_obj.match_list if src_obj.match_list else [],
        "tag": src_obj.tag if hasattr(src_obj, "tag") and src_obj.tag else [],
    }

    create_model = LogForwardingProfileCreateModel(**data)
...
def log_forwarding_profiles(
    ...
    """
    Clone log forwarding profile objects from a source SCM tenant to a destination SCM tenant.

    This Typer CLI command automates the process of retrieving log forwarding profile objects
    ...
    3. Authenticate to the source tenant and retrieve log forwarding profile objects from the given folder or snippet.
    ...
    Args:
        ...
        source_folder: [DEPRECATED] The source folder from which to retrieve log forwarding profile objects.
        destination_folder: [DEPRECATED] The destination folder from which to push log forwarding profile objects.
    ...
    """
    typer.echo("🚀 Starting log forwarding profile objects cloning...")
    ...
    # Retrieve log forwarding profile objects from source
    try:
        source_log_forwarding_profile_api = LogForwardingProfile(
        ...
        source_objects = source_log_forwarding_profile_api.list(**list_params)

        logger.info(
            f"Retrieved {len(source_objects)} log forwarding profile objects from source {context_type} '{resolved_source}'."
        )
    except Exception as e:
        logger.error(
            f"Error retrieving log forwarding profile objects from source: {e}"
        )
        raise typer.Exit(code=1)

    # Retrieve log forwarding profile objects from destination
    try:
        destination_client_api = LogForwardingProfile(
    ...
    # Create log forwarding profile objects in destination
    destination_profiles = LogForwardingProfile(destination_client, max_limit=5000)
    created_objs: List[LogForwardingProfileResponseModel] = []
    error_objects: List[List[str]] = []

    for src_obj in objects_to_create:
        if dry_run:
            logger.info(
                f"Skipping creation of log forwarding profile object in destination (dry run): {src_obj.name}"
            )
            continue
        ...
        if create_report:
            with open("result.csv", "a") as f:
                ...
                f.write(
                    f"Log Forwarding Profile,{src_obj.name},{getattr(src_obj, context_property)}\n"
                )
        ...
        try:
            ...
            logger.info(
                f"Created log forwarding profile object in destination: {new_obj.name}"
            )
        except (
            InvalidObjectError,
            MissingQueryParameterError,
    ...
    if created_objs and not quiet_mode:
        typer.echo(
            "\nSuccessfully created the following log forwarding profile objects:"
        )
        created_table = []
    ...
    if error_objects and not quiet_mode:
        typer.echo("\nSome log forwarding profile objects failed to be created:")
        typer.echo(
            tabulate(
                error_objects,
    ...
    if commit_and_push and created_objs and context_type == "folder":
        try:
            commit_params = {
                ...
                "description": f"Cloned log forwarding profile objects from {context_type} {resolved_source}",
                "sync": True,
            }
            ...
            logger.error(
                f"Error committing log forwarding profile objects in destination: {e}"
            )
            raise typer.Exit(code=1)
        ...
        if created_objs and not commit_and_push:
            ...
            logger.info(
                "No new log forwarding profile objects were created, skipping commit."
            )

    typer.echo("🎉 Log forwarding profile objects cloning completed successfully! 🎉")
