# scm_config_clone/commands/objects/schedule.py
...
def build_create_params(
    src_obj: ScheduleResponseModel,
    destination: str,
    context_type: str = "folder",
) -> Dict[str, Any]:
    """
    Construct the dictionary of parameters required to create a new schedule object.

    Given an existing ScheduleResponseModel (source object), a destination folder or snippet,
    and an optional context type, this function builds a dictionary with all necessary fields for creating
    a new schedule in the destination tenant. It uses `model_dump` on a Pydantic model
    to ensure only valid, explicitly set fields are included.

    Args:
        src_obj: The ScheduleResponseModel representing the source schedule object.
    ...
    Returns:
        A dictionary containing the fields required for `Schedule.create()`.
        This dictionary is validated and pruned by ScheduleCreateModel.
    """
    data = {
        "name": src_obj.name,
        ...
        "schedule_type": src_obj.schedule_type,
    }

    create_model = ScheduleCreateModel(**data)
...
def schedules(
    ...
    """
    Clone schedule objects from a source SCM tenant to a destination SCM tenant.

    This Typer CLI command automates the process of retrieving schedule objects
    ...
    3. Authenticate to the source tenant and retrieve schedule objects from the given folder or snippet.
    ...
    Args:
        ...
        source_folder: [DEPRECATED] The source folder from which to retrieve schedule objects.
        destination_folder: [DEPRECATED] The destination folder from which to push schedule objects.
    ...
    """
    typer.echo("🚀 Starting schedule objects cloning...")
    ...
    # Retrieve schedule objects from source
    try:
        source_schedule_api = Schedule(source_client, max_limit=5000)
        ...
        source_objects = source_schedule_api.list(**list_params)

        logger.info(
            f"Retrieved {len(source_objects)} schedule objects from source {context_type} '{resolved_source}'."
        )
    except Exception as e:
        logger.error(f"Error retrieving schedule objects from source: {e}")
        raise typer.Exit(code=1)

    # Retrieve schedule objects from destination
    try:
        destination_client_api = Schedule(destination_client, max_limit=5000)
    ...
    if source_objects and not quiet_mode:
        ...
        for result in comparison_results:
            ...
            schedule_table.append([result["name"], status])
    ...
    # Create schedule objects in destination
    destination_schedules = Schedule(destination_client, max_limit=5000)
    created_objs: List[ScheduleResponseModel] = []
    error_objects: List[List[str]] = []

    for src_obj in objects_to_create:
        if dry_run:
            logger.info(
                f"Skipping creation of schedule object in destination (dry run): {src_obj.name}"
            )
            continue
        ...
        if create_report:
            with open("result.csv", "a") as f:
                ...
                f.write(
                    f"Schedule,{src_obj.name},{getattr(src_obj, context_property)}\n"
                )
        ...
        try:
            new_obj = destination_schedules.create(create_params)
            created_objs.append(new_obj)
            logger.info(f"Created schedule object in destination: {new_obj.name}")
        except (
            InvalidObjectError,
            MissingQueryParameterError,
            NameNotUniqueError,
    ...
    if created_objs and not quiet_mode:
        typer.echo("\nSuccessfully created the following schedule objects:")
        created_table = []
    ...
    if error_objects and not quiet_mode:
        typer.echo("\nSome schedule objects failed to be created:")
        typer.echo(
            tabulate(
                error_objects,
    ...
    if commit_and_push and created_objs and context_type == "folder":
        try:
            commit_params = {
                ...
                "description": f"Cloned schedule objects from {context_type} {resolved_source}",
                "sync": True,
            }
            result = destination_schedules.commit(**commit_params)
            job_status = destination_schedules.get_job_status(result.job_id)
            logger.info(
                f"Commit job ID {result.job_id} status: {job_status.data[0].status_str}"
            )
        except Exception as e:
            logger.error(f"Error committing schedule objects in destination: {e}")
            raise typer.Exit(code=1)
        ...
        if created_objs and not commit_and_push:
            ...
            logger.info("No new schedule objects were created, skipping commit.")

    typer.echo("🎉 Schedule objects cloning completed successfully! 🎉")
