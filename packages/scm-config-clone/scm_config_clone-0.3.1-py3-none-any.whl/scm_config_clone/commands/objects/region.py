# scm_config_clone/commands/objects/region.py
...
def build_create_params(
    src_obj: RegionResponseModel,
    destination: str,
    context_type: str = "folder",
) -> Dict[str, Any]:
    """
    Construct the dictionary of parameters required to create a new region object.

    Given an existing RegionResponseModel (source object), a destination folder or snippet,
    and an optional context type, this function builds a dictionary with all necessary fields for creating
    a new region in the destination tenant. It uses `model_dump` on a Pydantic model
    to ensure only valid, explicitly set fields are included.

    Args:
        src_obj: The RegionResponseModel representing the source region object.
    ...
    Returns:
        A dictionary containing the fields required for `Region.create()`.
        This dictionary is validated and pruned by RegionCreateModel.
    """
    data = {
        "name": src_obj.name,
        context_type: destination,
        "description": src_obj.description if src_obj.description is not None else None,
        "address": src_obj.address if src_obj.address else [],
        "geo_location": src_obj.geo_location if src_obj.geo_location else None,
        "tag": src_obj.tag if hasattr(src_obj, "tag") and src_obj.tag else [],
    }

    create_model = RegionCreateModel(**data)
...
def regions(
    ...
    """
    Clone region objects from a source SCM tenant to a destination SCM tenant.

    This Typer CLI command automates the process of retrieving region objects
    ...
    3. Authenticate to the source tenant and retrieve region objects from the given folder or snippet.
    ...
    Args:
        ...
        source_folder: [DEPRECATED] The source folder from which to retrieve region objects.
        destination_folder: [DEPRECATED] The destination folder from which to push region objects.
    ...
    """
    typer.echo("🚀 Starting region objects cloning...")
    ...
    # Retrieve region objects from source
    try:
        source_region_api = Region(source_client, max_limit=5000)
        ...
        source_objects = source_region_api.list(**list_params)

        logger.info(
            f"Retrieved {len(source_objects)} region objects from source {context_type} '{resolved_source}'."
        )
    except Exception as e:
        logger.error(f"Error retrieving region objects from source: {e}")
        raise typer.Exit(code=1)

    # Retrieve region objects from destination
    try:
        destination_client_api = Region(destination_client, max_limit=5000)
    ...
    if source_objects and not quiet_mode:
        ...
        for result in comparison_results:
            ...
            region_table.append([result["name"], status])
    ...
    # Create region objects in destination
    destination_regions = Region(destination_client, max_limit=5000)
    created_objs: List[RegionResponseModel] = []
    error_objects: List[List[str]] = []

    for src_obj in objects_to_create:
        if dry_run:
            logger.info(
                f"Skipping creation of region object in destination (dry run): {src_obj.name}"
            )
            continue
        ...
        if create_report:
            with open("result.csv", "a") as f:
                ...
                f.write(f"Region,{src_obj.name},{getattr(src_obj, context_property)}\n")
        ...
        try:
            new_obj = destination_regions.create(create_params)
            created_objs.append(new_obj)
            logger.info(f"Created region object in destination: {new_obj.name}")
        except (
            InvalidObjectError,
            MissingQueryParameterError,
            NameNotUniqueError,
    ...
    if created_objs and not quiet_mode:
        typer.echo("\nSuccessfully created the following region objects:")
        created_table = []
    ...
    if error_objects and not quiet_mode:
        typer.echo("\nSome region objects failed to be created:")
        typer.echo(
            tabulate(
                error_objects,
    ...
    if commit_and_push and created_objs and context_type == "folder":
        try:
            commit_params = {
                ...
                "description": f"Cloned region objects from {context_type} {resolved_source}",
                "sync": True,
            }
            result = destination_regions.commit(**commit_params)
            job_status = destination_regions.get_job_status(result.job_id)
            logger.info(
                f"Commit job ID {result.job_id} status: {job_status.data[0].status_str}"
            )
        except Exception as e:
            logger.error(f"Error committing region objects in destination: {e}")
            raise typer.Exit(code=1)
        ...
        if created_objs and not commit_and_push:
            ...
            logger.info("No new region objects were created, skipping commit.")

    typer.echo("🎉 Region objects cloning completed successfully! 🎉")
