# scm_config_clone/commands/objects/http_server_profile.py
...
def build_create_params(
    src_obj: HTTPServerProfileResponseModel,
    destination: str,
    context_type: str = "folder",
) -> Dict[str, Any]:
    """
    Construct the dictionary of parameters required to create a new HTTP server profile object.

    Given an existing HTTPServerProfileResponseModel (source object), a destination folder or snippet,
    and an optional context type, this function builds a dictionary with all necessary fields
    for creating a new HTTP server profile in the destination tenant. It uses `model_dump` on a Pydantic model
    to ensure only valid, explicitly set fields are included.

    Args:
        src_obj: The HTTPServerProfileResponseModel representing the source HTTP server profile.
    ...
    Returns:
        A dictionary containing the fields required for `HTTPServerProfile.create()`.
        This dictionary is validated and pruned by HTTPServerProfileCreateModel.
    """
    data = {
        "name": src_obj.name,
        context_type: destination,
        "description": src_obj.description if src_obj.description is not None else None,
        "server": src_obj.server if src_obj.server else [],
        "tag_registration": src_obj.tag_registration,
        "tag": src_obj.tag if hasattr(src_obj, "tag") and src_obj.tag else [],
    }

    create_model = HTTPServerProfileCreateModel(**data)
...
def http_server_profiles(
    ...
    """
    Clone HTTP server profile objects from a source SCM tenant to a destination SCM tenant.

    This Typer CLI command automates the process of retrieving HTTP server profile objects
    ...
    3. Authenticate to the source tenant and retrieve HTTP server profile objects from the given folder or snippet.
    ...
    Args:
        ...
        source_folder: [DEPRECATED] The source folder from which to retrieve HTTP server profile objects.
        destination_folder: [DEPRECATED] The destination folder from which to push HTTP server profile objects.
    ...
    """
    typer.echo("🚀 Starting HTTP server profile objects cloning...")
    ...
    # Retrieve HTTP server profile objects from source
    try:
        source_http_server_profile_api = HTTPServerProfile(
            source_client, max_limit=5000
        )
        ...
        source_objects = source_http_server_profile_api.list(**list_params)

        logger.info(
            f"Retrieved {len(source_objects)} HTTP server profile objects from source {context_type} '{resolved_source}'."
        )
    except Exception as e:
        logger.error(f"Error retrieving HTTP server profile objects from source: {e}")
        raise typer.Exit(code=1)

    # Retrieve HTTP server profile objects from destination
    try:
        destination_client_api = HTTPServerProfile(destination_client, max_limit=5000)
    ...
    if source_objects and not quiet_mode:
        ...
        for result in comparison_results:
            ...
            http_table.append([result["name"], status])
    ...
    # Create HTTP server profile objects in destination
    destination_profiles = HTTPServerProfile(destination_client, max_limit=5000)
    created_objs: List[HTTPServerProfileResponseModel] = []
    error_objects: List[List[str]] = []

    for src_obj in objects_to_create:
        if dry_run:
            logger.info(
                f"Skipping creation of HTTP server profile object in destination (dry run): {src_obj.name}"
            )
            continue
        ...
        if create_report:
            with open("result.csv", "a") as f:
                ...
                f.write(
                    f"HTTP Server Profile,{src_obj.name},{getattr(src_obj, context_property)}\n"
                )
        ...
        try:
            ...
            logger.info(
                f"Created HTTP server profile object in destination: {new_obj.name}"
            )
        except (
            InvalidObjectError,
            MissingQueryParameterError,
    ...
    if created_objs and not quiet_mode:
        typer.echo("\nSuccessfully created the following HTTP server profile objects:")
        created_table = []
    ...
    if error_objects and not quiet_mode:
        typer.echo("\nSome HTTP server profile objects failed to be created:")
        typer.echo(
            tabulate(
                error_objects,
    ...
    if commit_and_push and created_objs and context_type == "folder":
        try:
            commit_params = {
                ...
                "description": f"Cloned HTTP server profile objects from {context_type} {resolved_source}",
                "sync": True,
            }
            ...
            logger.error(
                f"Error committing HTTP server profile objects in destination: {e}"
            )
            raise typer.Exit(code=1)
        ...
        if created_objs and not commit_and_push:
            ...
            logger.info(
                "No new HTTP server profile objects were created, skipping commit."
            )

    typer.echo("🎉 HTTP server profile objects cloning completed successfully! 🎉")
