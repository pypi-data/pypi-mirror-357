...
def build_create_params(
    src_obj: AddressGroupResponseModel,
    destination: str,
    ...
    Given an existing AddressGroupResponseModel (source object), a destination folder or snippet,
...
def address_groups(
    ...
    Args:
        ...
        source_folder: [DEPRECATED] The source folder from which to retrieve address group objects.
        destination_folder: [DEPRECATED] The destination folder from which to push address group objects.
    ...
    try:
        source_address_group_api = AddressGroup(source_client, max_limit=5000)
        ...
        source_objects = source_address_group_api.list(**list_params)

        logger.info(
            f"Retrieved {len(source_objects)} address group objects from source {context_type} '{resolved_source}'."
        )
    except Exception as e:
    ...
    try:
        destination_client_api = AddressGroup(destination_client, max_limit=5000)
    ...
    for src_obj in objects_to_create:
        ...
        if create_report:
            with open("result.csv", "a") as f:
                ...
                f.write(
                    f"Address Group,{src_obj.name},{getattr(src_obj, context_property)}\n"
                )
    ...
    if commit_and_push and created_objs and context_type == "folder":
        try:
            commit_params = {
                ...
                "description": f"Cloned address group objects from {context_type} {resolved_source}",
                "sync": True,
            }
