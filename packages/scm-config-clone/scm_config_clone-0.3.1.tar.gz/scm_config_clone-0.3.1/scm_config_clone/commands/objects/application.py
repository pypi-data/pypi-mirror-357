...
def build_create_params(
    ...
    Given an existing ApplicationResponseModel (source object), a destination folder or snippet,
...
def applications(
    ...
    Args:
        ...
        source_folder: [DEPRECATED] The source folder from which to retrieve application objects.
        destination_folder: [DEPRECATED] The destination folder from which to push application objects.
    ...
    try:
        source_application_api = Application(source_client, max_limit=5000)
        ...
        source_objects = source_application_api.list(**list_params)

        logger.info(
            f"Retrieved {len(source_objects)} application objects from source {context_type} '{resolved_source}'."
        )
    except Exception as e:
    ...
    try:
        destination_client_api = Application(destination_client, max_limit=5000)
    ...
    for src_obj in objects_to_create:
        ...
        if create_report:
            with open("result.csv", "a") as f:
                ...
                f.write(
                    f"Application,{src_obj.name},{getattr(src_obj, context_property)}\n"
                )
    ...
    if commit_and_push and created_objs and context_type == "folder":
        try:
            commit_params = {
                ...
                "description": f"Cloned application objects from {context_type} {resolved_source}",
                "sync": True,
            }
