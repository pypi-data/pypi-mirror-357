...
async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> list[Content]:
    ...
    try:
        ...
        if name == "auth":
            ...
            if authenticate_with_token(token, force_staging):
                ...
                result_data = (
                    f"✅ Authentication successful ({environment_name})!\n"
                    f"User: {user_data.get('name', 'Unknown')}\n"
                    f"Role: {user_data.get('role', 'Unknown')}\n"
                    f"Organization: {org_data.get('name', 'None')}\n"
                    f"Environment: {auth_cache['environment']}\n"
                    f"Base URL: {auth_cache['base_url']}"
                )
            else:
                result_data = "❌ Authentication failed. Check logs for details."
            ...
            result_data = f"Switched to {environment_name} environment. Please re-authenticate using the 'auth' tool."
        ...
        # --- Kubernetes (k8s) Management ---
            ...
            result_data = make_authenticated_request("GET", f"/core/kubernetes/{project_id}/{namespace}/{resource}")

        # --- VM Management ---
        elif name == "vm_list":
            project_id = arguments.get("project_id")
            # FIX: The endpoint was incorrect. Added '/project/'
            result_data = make_authenticated_request("GET", f"/core/virtual-machine/project/{project_id}")
        elif name == "vm_detail":
            vm_id = arguments.get("vm_id")
            result_data = make_authenticated_request("GET", f"/core/virtual-machine/{vm_id}")

        # --- Container Registry ---
        elif name == "registry_list":
            project_id = arguments.get("project_id")
            result_data = make_authenticated_request("GET", f"/core/dekaregistry/project/{project_id}")
        elif name == "registry_repositories":
            registry_id = arguments.get("registry_id")
            result_data = make_authenticated_request("GET", f"/core/dekaregistry/{registry_id}/repository")

        # --- Deka Notebooks ---
        elif name == "notebook_list":
            project_id = arguments.get("project_id")
            result_data = make_authenticated_request("GET", f"/core/dekanotebook/project/{project_id}")
            ...
            result_data = make_authenticated_request("POST", f"/core/dekanotebook/project/{project_id}", data=payload)
        ...
        # --- API Token Management ---
            ...
            result_data = make_authenticated_request("GET", "/core/user/token")
        elif name == "token_create":
            payload = {
                "name": arguments.get("name"),
                "expired_at": arguments.get("expired_at")
            }
            result_data = make_authenticated_request("POST", "/core/user/token", data=payload)
        elif name == "token_delete":
            token_id = arguments.get("token_id")
            result_data = make_authenticated_request("DELETE", f"/core/user/token/{token_id}")
    ...
    return [CallToolResult(output=[TextContent(text=format_response(result_data))])]
