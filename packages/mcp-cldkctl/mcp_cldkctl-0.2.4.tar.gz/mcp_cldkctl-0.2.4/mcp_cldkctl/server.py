...
async def handle_call_tool(name: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        # --- User Profile ---
            ...
            result_data = make_authenticated_request("GET", "/core/user/profile")
        ...
        # --- Balance & Billing ---
            ...
            # This endpoint in Go API expects a POST with a body
            result_data = make_authenticated_request("POST", "/core/billing/v2/history", data=arguments)
        ...
        # --- Kubernetes (k8s) Management ---
            ...
            # These endpoints in Go API use query parameters, not path parameters
            endpoint_map = {
                "pods": "/core/pods",
        ...
        # --- VM Management ---
            ...
            result_data = make_authenticated_request("POST", "/core/virtual-machine", data=arguments)
        elif name == "vm_delete":
            payload = {"vm_id": arguments.get("vm_id")}
        ...
        # --- Deka Notebooks ---
            ...
            result_data = make_authenticated_request("POST", "/core/deka-notebook", data=arguments)
        ...
        # --- API Token Management ---
            ...
            result_data = make_authenticated_request("POST", "/core/cldkctl/token", data=arguments)
        ...
        # --- Utility Tools ---
            ...
            # This endpoint in Go API uses GET
        ...
        result_data = {"error": True, "message": error_message}
        is_error = True
        # Attempt to extract status code for better error reporting
        if hasattr(e, 'response') and e.response is not None:
             result_data["message"] = f"❌ API Error: {e.response.status_code} {e.response.reason} for url: {e.response.url}"


    # FIX: Manually create the final object and serialize it to a dict
    # to avoid serialization bugs in the MCP library.
    text_content = TextContent(type="text", text=format_response(result_data))
    final_result = CallToolResult(content=[text_content], isError=is_error)

    # By returning a dictionary, we bypass the faulty serialization in stdio_server.
