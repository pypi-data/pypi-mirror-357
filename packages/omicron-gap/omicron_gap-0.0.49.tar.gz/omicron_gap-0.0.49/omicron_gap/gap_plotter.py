...
matplotlib.use('agg')
datafind_servers = {'L1': 'GWDATAFIND_SERVER=datafind2.ligo-la.caltech.edu:443',
                    'H1': 'GWDATAFIND_SERVER=ldrslave.ligo-wa.caltech.edu:443'}
        ...
        for i in ifos:
            ...
            for seg_name in sorted(dq_segs[i]):
                try:
                    ...
                    logger.warning(f'No segments found for {seg_name} due to http error')
                    new_seg_name = seg_name + "-error"
                    seg_data = DataQualityFlag(name=new_seg_name, known=SegmentList(),
                                               active=SegmentList(), label=new_seg_name)
                    htdecode = shutil.which('htdecodetoken')
                    if htdecode:
                        ret = subprocess.run([htdecode, '-H'], check=True, capture_output=True)
