...
def dbt(
    ...
    upload_cmd = upload_to_gcs(environment,target_path)
...
def dbt_freshness_and_upload_cmd(
    ...
    @return: the complete command string
    """
    return dbt(profile_arg,target_arg, environment,target_path,f"source freshness {dbt_command}",vars_arg,other_args)
...
def dbt_test_and_upload_cmd(
    ...
    @return: the complete command string
    """
    return dbt(profile_arg,target_arg, environment,target_path,f"test {dbt_command}",vars_arg,other_args)
...
def dbt_run_and_upload_cmd(
    ...
    @return: the complete command string
    """
    return dbt(profile_arg,target_arg, environment,target_path,f"run {dbt_command}",vars_arg,other_args)
...
def dbt_seed_and_upload_cmd(
    ...
    @return: the complete command string
    """
    return dbt(profile_arg,target_arg, environment,target_path,f"seed {dbt_command}",vars_arg,other_args)
...
def dbt_run_only_failed(
    ...
    download_failed_models_cmd = download_failed_models(environment,target_path)
    upload_cmd = upload_to_gcs(environment,target_path)
    ...
    if [ {{{{ ti.try_number }}}} -eq 2 ]; then
        ...
        if [ -z "${target_path}" ]; then
            ...
            dbt {dbt_command} --select result:fail+ result:error+ --state /app/{target_path}/target --profiles-dir={profile_arg} {target_arg} {vars_arg} {other_args}; dbt_ret=$?;  
        fi
