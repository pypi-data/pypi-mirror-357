...
## Recommendations
''' + '\\n'.join('- ' + r for r in analysis.get('recommendations', [])) + '''
'''
...
def create_ai_code_review_workflow(pr_number: int):
    wf = workflow("ai-code-review")
...
def create_etl_pipeline(source: str, target: str):
...
<CardGroup cols={2}>
  ...
  <Card title="Advanced Workflows" icon="wrench" href="/workflows/advanced">
