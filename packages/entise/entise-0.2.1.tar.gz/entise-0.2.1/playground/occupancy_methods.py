...
class Hts:
    ...
    def save_weather(self, path: str = "./simx/weather.txt") -> None:
        """
        Saves the weather filename in the required SimulationX format

        :param path: path where to save the weather filename

        :return: None
        """

        # Get time values for weather filename (start and end)
        data_time = {
            "0": 0,
        ...
        with open(path, "w") as filename:
            filename.write("#1\n")
            # Time
            filename.write(f"double\tT_StartEnd ({df_time.shape[0]}, {df_time.shape[1]})\n")
            df_time.to_csv(filename, header=False, index=False, sep='\t', lineterminator='\n')
            # Weather
            filename.write("\n")
            filename.write(f"double\tWeather({self.weather.shape[0]}, {self.weather.shape[1]})\n")
            self.weather.to_csv(filename, header=False, index=False, sep='\t', lineterminator='\n')
    ...
    def format_power(self) -> pd.DataFrame:
        ...
        # Limit range to length of weather filename
    ...
    def calc_occupancy(self, method: str = "geoma") -> pd.DataFrame:
        ...
        if self.occup['timestamp'].iat[1] - self.occup['timestamp'].iat[1] != self.resolution:
            ...
            # Check if length of occupancy is equal to length of power filename
            if len(self.occup) < len(self.power):
                # Calculate the difference in length
                diff = len(self.power) - len(self.occup)

                # Append the last occupancy value to the occupancy filename with the correct timestamp
                for i in range(diff):
    ...
    def load_params(self) -> dict:
        ...
        params = {  # keys correspond to FMU variable names
            ...
            # Operation
            "building.UseIndividualPresence": True,                                 # use occupancy filename
            "building.PresenceFile": "./simx/occupancy.txt",                        # occupancy filename path
            "building.UseIndividualElecConsumption": True,                          # use electricity filename
            "building.ElConsumptionFile": "./simx/power.txt",                       # electricity filename path
            ...
            # Environment
            "environment.Filename": "./simx/weather.txt",                                  # weather filename path
    ...
    def get_fmu_path(self) -> str:
        """Sets the path of the FMU filename that is to be used."""
    ...
    def save_results(self, path: str = "./output/results.csv", keys: list = None, si_units: bool = False,
        ...
        """
        Obtains the desired output results and saves them to a csv filename.
    ...
    def save_simx_file(df: pd.DataFrame, header: str, path: str):
        ...
        with open(path, "w") as filename:
            filename.write("#1\n")
            df.to_csv(filename, index=False, sep='\t', lineterminator='\n',
