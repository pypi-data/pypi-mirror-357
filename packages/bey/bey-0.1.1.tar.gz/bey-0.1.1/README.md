...
to add `BEY_API_KEY="My API Key"` to your `.env` file
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `bey.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `bey.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `bey.APIError`.
...
except bey.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except bey.APIStatusError as e:
