...
from agentsphere_base.api.client.client import AuthenticatedClient
from agentsphere_base.connection_config import ConnectionConfig
from agentsphere_base.api.metadata import default_headers
from agentsphere_base.exceptions import (
    AuthenticationException,
    SandboxException,
    RateLimitException,
)
from agentsphere_base.api.client.types import Response
...
class ApiClient(AuthenticatedClient):
    """
    The client for interacting with the AGENTSPHERE API.
    """
        ...
        if require_api_key:
            if config.api_key is None:
                raise AuthenticationException(
                    ...
                    "You can either set the environment variable `AGENTSPHERE_API_KEY` "
                    'or you can pass it directly to the sandbox like Sandbox(api_key="agentsphere_...")',
                )
        ...
        if require_access_token:
            if config.access_token is None:
                raise AuthenticationException(
                    ...
                    "You can set the environment variable `AGENTSPHERE_ACCESS_TOKEN` or pass the `access_token` in options.",
                )
