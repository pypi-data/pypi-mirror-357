        ...
        for project_name, project_tasks in tasks.items():
            ...
            if filtered_tasks:
                ...
                for task in filtered_tasks:
                    task_text = self._convert_to_whatsapp_format(task)
                    sections.append(task_text)
                sections.append("------------------------")

        return "\n".join(sections)

    def _convert_to_whatsapp_format(self, task: str) -> str:
        """Convert markdown task format to WhatsApp format."""
        # Convert checkboxes to WhatsApp-friendly format
