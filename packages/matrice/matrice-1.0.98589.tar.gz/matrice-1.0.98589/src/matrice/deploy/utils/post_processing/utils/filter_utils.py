...
def apply_category_mapping(results: Any, index_to_category: Dict[int, str]) -> Any:
    ...
    """
    def map_detection(detection: Dict[str, Any]) -> Dict[str, Any]:
        """Map a single detection."""
        detection = detection.copy()
        category_id = detection.get("category_id", detection.get("category"))
        
        if isinstance(category_id, int) and category_id in index_to_category:
