...
class InferenceInterface:
    ...
    def __init__(
        ...
        default_post_processing_config: Optional[
        ...
        Args:
            ...
            default_post_processing_config: Default post-processing configuration
        ...
        self.default_post_processing_config = None
        if default_post_processing_config:
            self.default_post_processing_config = self._parse_post_processing_config(
    ...
    def _parse_post_processing_config(
        ...
        try:
            if isinstance(config, BaseConfig):
                ...
                # Use the appropriate utility function based on use case
                if usecase == "people_counting":
                    return create_people_counting_config(
                        category=category, **config_params
                    )
                elif usecase == "customer_service":
                    return create_customer_service_config(
                        category=category, **config_params
                    )
                elif usecase == "advanced_customer_service":
                    return create_advanced_customer_service_config(
                        category=category, **config_params
                    )
                elif usecase == "basic_counting_tracking":
                    return create_basic_counting_tracking_config(**config_params)
                else:
                    # Fallback to generic config creation
    ...
    async def inference(
        ...
        return await self._apply_post_processing(
            raw_results, input1, post_processing_config
        )
    ...
    async def _apply_post_processing(
        ...
        try:
            ...
            config_to_use = (
                post_processing_config or self.default_post_processing_config
            )

            if config_to_use is None and self.custom_post_processing_fn is None:
                self.logger.warning(
                    "No post-processing configuration or custom function provided"
                )
                return raw_results, None
            ...
            if config_to_use is None:
                self.logger.error("Failed to parse post-processing configuration")
                return raw_results, {
            ...
            result = self.post_processor.process(raw_results, config_to_use)
            ...
            self.logger.error(f"Post-processing failed: {str(e)}", exc_info=True)
            return raw_results, {
                "error": str(e),
                "status": "post_processing_failed",
                "processed_data": raw_results,
        ...
        try:
            ...
            for i, result in enumerate(batch_results):
                ...
                if apply_post_processing:
                    ...
                    processed_result, post_processing_result = (
                        await self._apply_post_processing(result, input1, config)
                    )
