...
class PPEComplianceUseCase(BaseProcessor):
    ...
    def process(self, data: Any, config: ConfigProtocol,
        ...
        try:
            ...
            processed_data = data
            # Apply category mapping to the original data for general counting summary
            mapped_data = data
            if config.index_to_category:
                mapped_data = apply_category_mapping(data, config.index_to_category)
                self.logger.debug("Applied category mapping for general summary")
            
            # General counting summary (all detections, not just violations)
            general_counting_summary = calculate_counting_summary(mapped_data)
            
            # Now continue with processed_data for PPE violations as before
