    ...
    def _generate_insights(self, counting_summary: Dict, zone_analysis: Dict, 
        ...
        if intensity_threshold is not None:
            ...
            if percentage < 20:
                ...
                insights.append(f"INTENSITY: ⚠️ High occupancy in the scene ({percentage:.1f}% of capacity)")
            else:
                insights.append(f"INTENSITY: ⚠️ Very high density in the scene ({percentage:.1f}% of capacity)")
        else:
            # Fallback to hardcoded thresholds if no alert config is set
            if total_people > 10:
                insights.append(f"INTENSITY: ⚠️ High density in the scene with {total_people} people")
            elif total_people == 1:
                insights.append(f"INTENSITY: Low occupancy in the scene")
        ...
        if zone_analysis:
            for zone_name, zone_counts in zone_analysis.items():
                ...
                if zone_total > 0:
                    ...
                    # Density insights
                    if zone_total > 10:
                        insights.append(f"⚠️ High density in zone '{zone_name}' with {zone_total} people")
                    elif zone_total == 1:
