...
class VideoColorConfig(BaseConfig):
    ...
    index_to_category: Optional[Dict[int, str]] = None  # Add this missing field
...
class VideoColorClassificationUseCase(BaseProcessor):
    ...
    def process(self, data: Dict[str, Any], config: ConfigProtocol,
        ...
        if not video_bytes or not predictions:
            return self.create_error_result("Missing required input (video_bytes or predictions)", context=context)
