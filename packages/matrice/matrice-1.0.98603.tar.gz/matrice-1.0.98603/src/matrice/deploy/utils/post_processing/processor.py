...
=======
    ...
    PPEComplianceUseCase
>>>>>>> 30f75bc3f2bd177b75cfc3a08056483509efbbca
)
...
class PostProcessor:
    ...
    def _register_use_cases(self) -> None:
        ...
        registry.register_use_case("general", "video_color_classification", VideoColorClassificationUseCase)
        
        # Register PPE compliance use case
        registry.register_use_case("ppe", "ppe_compliance_detection", PPEComplianceUseCase)

        registry.register_use_case("vehicle", "vehicle_monitoring", VehicleMonitoringUseCase)
    ...
    def process(self, data: Any, config: Union[BaseConfig, Dict[str, Any], str, Path], input_bytes: Optional[bytes] = None,
        ...
        try:
            ...
            # Get appropriate use case
            use_case_class = registry.get_use_case(parsed_config.category, parsed_config.usecase)
            if not use_case_class:
                return self._create_error_result(
                    f"Use case '{parsed_config.category}/{parsed_config.usecase}' not found",
                    "UseCaseNotFound",
                    parsed_config.usecase,
                    parsed_config.category,
                    context
                )
            ...
            if use_case_class == VideoColorClassificationUseCase:
                result = use_case.process(data, parsed_config, input_bytes, context)
            else:
                result = use_case.process(data, parsed_config, context)
    ...
    def process_simple(self, data: Any, usecase: str, 
        ...
        try:
            ...
            return self.process(data, config, context)
