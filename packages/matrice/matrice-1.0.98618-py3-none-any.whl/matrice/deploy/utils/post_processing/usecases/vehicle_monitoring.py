...
from ..core.config import  ZoneConfig, AlertConfig
from ..utils import (
...
class VehicleMonitoringConfig(BaseConfig):
    ...
    # Target vehicle categories for monitoring
    ...
    def validate(self) -> List[str]:
        """Validate vehicle monitoring configuration."""
        ...
        if not self.vehicle_categories:
            errors.append("vehicle_categories cannot be empty")
        
        if not self.target_vehicle_categories:
            errors.append("target_vehicle_categories cannot be empty")
        
        # Validate that target_vehicle_categories are a subset of vehicle_categories
        if not all(cat in self.vehicle_categories for cat in self.target_vehicle_categories):
            errors.append("All target_vehicle_categories must be in vehicle_categories")
        
        # Validate index_to_category matches vehicle_categories
        if self.index_to_category and set(self.index_to_category.values()) != set(self.vehicle_categories):
            errors.append("index_to_category values must match vehicle_categories")
