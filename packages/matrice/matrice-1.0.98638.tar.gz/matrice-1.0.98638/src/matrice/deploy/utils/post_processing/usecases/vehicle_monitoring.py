...
class VehicleMonitoringConfig(BaseConfig):
    ...
    vehicle_categories: List[str] = field(
        default_factory=lambda: ["bus", "car", "microbus", "motorbike", "pickup van", "truck"]
    )
...
class VehicleMonitoringUseCase(BaseProcessor):
    ...
    def create_default_config(self, **overrides) -> VehicleMonitoringConfig:
        ...
        defaults = {
            ...
            "vehicle_categories": ["bus", "car", "microbus", "motorbike", "pickup van", "truck"],
        }
    ...
    def _generate_insights(self, counting_summary: Dict, zone_analysis: Dict, 
        ...
        if intensity_threshold is not None:
            ...
            if percentage < 20:
                ...
                insights.append(f"INTENSITY: ⚠️ Heavy traffic in the scene ({percentage:.1f}% of capacity)")
            else:
                insights.append(f"INTENSITY: ⚠️ Severe congestion in the scene ({percentage:.1f}% of capacity)")
        else:
            # Fallback to hardcoded thresholds if no alert config is set
            if total_vehicles > 15:
                insights.append(f"INTENSITY: ⚠️ Heavy traffic in the scene with {total_vehicles} vehicles")
        ...
        if zone_analysis:
            for zone_name, zone_counts in zone_analysis.items():
                ...
                if zone_total > 0:
                    ...
                    # Density insights
                    if zone_total > 10:
                        insights.append(f"⚠️ High traffic density in zone '{zone_name}' with {zone_total} vehicles")
                    elif zone_total == 1:
