...
from calicropyield.credentials import ensure_shared_credentials
...
class DataDownloader:
    ...
    def authenticate_drive(self):
        creds_path = ensure_shared_credentials()
    ...
    def download_USDA(self, county_names: list, years: list = None, crop_types: list = None):
        if years is None:
            years = [str(y) for y in range(2008, 2023) if y != 2012]
        else:
            years = [str(y) for y in years]
        ...
        for county in county_names:
            for year in years:
                filename = f"{county}_yield_{year}.csv"
                path = self._download_single_file(county, year, filename, dataset_type="usda")

                if crop_types and os.path.exists(path):
                    try:
                        # Read the CSV file
                        df = pd.read_csv(path)

                        # Filter rows based on crop_types
                        filtered_df = df[df['crop_name'].isin(crop_types)]

                        # Define new filename with "_filtered" suffix
                        filtered_filename = filename.replace('.csv', '_filtered.csv')
                        filtered_path = os.path.join(os.path.dirname(path), filtered_filename)
                        ...
                        print(f"Saved filtered file to: {filtered_path}")
                    except Exception as e:
