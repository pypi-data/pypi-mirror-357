...
class dropmailplus:
    ...
    def _generate_subdomain_extension(self, min_sub: int = 3, max_sub: int = 10) -> str:
        """Генерирует случайное расширение для email адреса"""
        length = random.randint(min_sub, max_sub)
        chars = string.ascii_lowercase + string.digits
        return ''.join(random.choice(chars) for _ in range(length))
    
    def _apply_subdomain(self, email: str, min_sub: int = 3, max_sub: int = 10) -> str:
        """Применяет расширение к email адресу согласно правилам"""
        username, domain = email.split('@')
        
        # Генерируем расширения
        username_extension = self._generate_subdomain_extension(min_sub, max_sub)
        domain_extension = self._generate_subdomain_extension(min_sub, max_sub)
        
        # Добавляем расширение к имени пользователя (без плюса!)
        separator = random.choice(['-', '.'])
        username = f"{username}{separator}{username_extension}"
        
        # Добавляем поддомен
        domain = f"{domain_extension}.{domain}"
    ...
    def create(self, domain: Optional[str] = None, subdomain: bool = False, 
        ...
        # Применяем расширение к основному email если нужно
        main_email = session["addresses"][0]["address"]
        if subdomain:
            main_email = self._apply_subdomain(main_email, min_sub, max_sub)
