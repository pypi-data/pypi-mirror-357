		...
		uni.request({
			...
			success: (res) => {
				if (res.statusCode >= 200 && res.statusCode < 300) {
					...
					var error_detail = (res.data)["detail"];
					uni.showToast({
						title: `请求失败: ${error_detail.error_code},  ${error_detail.detail}`,
						icon: 'none',
						duration: 3000
					})
					reject(new Error(`请求失败: ${error_detail.error_code},${error_detail.detail}`))
				}
			},
			fail: (err) => {
