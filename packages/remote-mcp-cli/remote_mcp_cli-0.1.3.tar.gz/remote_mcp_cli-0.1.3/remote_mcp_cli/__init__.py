...
def _reorder_global_options(argv: list[str]) -> list[str]:
    """Allow global options after the SERVER_URL argument."""
    if len(argv) > 2 and not argv[1].startswith("-"):
        ...
        while idx < len(rest):
            arg = rest[idx]
            if arg in {"--token", "--header", "-h", "--help"}:
                global_opts.append(arg)
                if arg in {"--token", "--header"} and idx + 1 < len(rest):
                    global_opts.append(rest[idx + 1])
                    idx += 2
                else:
                    idx += 1
            else:
                break
        reordered = [argv[0]] + global_opts + [server] + rest[idx:]
        return reordered
    return argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    if argv is None:
        argv = sys.argv
    argv = _reorder_global_options(list(argv))
    sys.argv = argv
    cli()
