...
def _save_tool_info(tools: list[dict], server: str) -> str:
    ...
    lines = [_format_tool_info(t) for t in tools]
    path.write_text("\n\n".join(lines) + "\n", encoding="utf-8")
    return str(path)
...
@click.option(
    "--save",
    is_flag=True,
    help="Save tool details to a text file in ./mcp_servers",
)
@click.pass_obj
def list_tools(obj, save: bool):
    ...
    if save and lines:
        file_path = _save_tool_info(tools_list or [], server)
        click.echo(f"Saved tool info to {file_path}")
