...
from src.usage_analyzer.models.data_structures import SessionBlock
...
class JSONFormatter:
    ...
    def _block_to_dict(self, block: SessionBlock) -> Dict[str, Any]:
        ...
        result = {
            ...
            "perModelStats": self._format_per_model_stats(
                block.per_model_stats, per_model_costs
            ),
            "limits": self._format_limit_messages(block.limit_messages),
            "burnRate": None,
            "projection": None,
        }
    ...
    def _format_limit_messages(self, limit_messages) -> List[Dict[str, Any]]:
        """Format limit messages for JSON output."""
        formatted_limits = []
        for limit_msg in limit_messages:
            formatted_limits.append(
                {
                    "type": limit_msg.type,
                    "timestamp": self._format_timestamp(limit_msg.timestamp),
                    "content": limit_msg.content,
                    "resetTime": self._format_timestamp(limit_msg.reset_time)
                    if limit_msg.reset_time
                    else None,
                }
            )
    ...
    def _format_timestamp(self, timestamp) -> str:
        """Format datetime to UTC with milliseconds precision."""
