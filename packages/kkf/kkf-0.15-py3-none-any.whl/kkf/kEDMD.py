...
class KoopmanOperator:
    ...
    def __init__(
        ...
        dynamical_system: DynamicalSystem, concatenate_indentity: bool = False,
        exact_lift_back: bool = True
    ):
        self.kernel_function = kernel_function
        self.dynamical_system = dynamical_system
        self.X: Optional[NDArray[np.float64]] = None
        self.nx: int = dynamical_system.nx
        self.ny: int = dynamical_system.ny
        ...
        self.concatenate_indentity = concatenate_indentity
        self.exact_lift_back = exact_lift_back
    ...
    def compute_edmd(self, n_features: int, optimize: bool = True, n_restarts_optimizer: int = 10) -> None:
        ...
        # Define feature map
        if self.concatenate_indentity:
            # If concatenate_indentity is True, concatenate the identity mapping
            # with the kernel function to compute the feature map
            if isinstance(self.kernel_function, Kernel):
                self.phi = lambda x: np.concatenate((x.T, self.kernel_function(self.X, x)), axis=0)
            else:
                raise ValueError("Kernel function must be an instance Kernel of sklearn")

        else:
            # If concatenate_indentity is False, use only the kernel function
            # to compute the feature map
            if isinstance(self.kernel_function, Kernel):
                self.phi = lambda x: self.kernel_function(self.X, x)
            else:
                raise ValueError("Kernel function must be an instance Kernel of sklearn")

        
        # Compute Gram matrix
        self.G = self.phi(self.X) 
        G_inv = np.linalg.pinv(self.G)
        
        # Compute Koopman operator approximation
        next_states = f(self.X.T).T
        self.A = self.phi(next_states) 
        self.U = self.A @ G_inv
        
        # Compute output and state transformation matrices
        self.C = g(self.X.T) @ G_inv

        if self.concatenate_indentity and self.exact_lift_back:
            self.B = np.concatenate((np.eye(self.nx), np.zeros((self.nx, n_features))), axis=1)
        else:
