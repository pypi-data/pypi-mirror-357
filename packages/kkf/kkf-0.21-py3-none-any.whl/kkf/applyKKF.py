...
def apply_koopman_kalman_filter(
    ...
    ### Initialization ###
    x0 = initial_distribution.mean.reshape(1, -1)  # Initial state estimate
    z0 = phi(x0)  # Initial state in feature space

    # Get problem dimensions
    n_timesteps, n_states = observations.shape[0], x0.shape[1]
    n_features = z0.shape[0]
...
def _initialize_state_arrays(
    ...
    return (np.zeros((n_timesteps, n_dim, 1)), 
            np.zeros((n_timesteps, n_dim, 1)))
...
def _initialize_covariance_arrays(
    ...
    return (np.zeros((n_timesteps, 1, n_dim)), 
            np.zeros((n_timesteps, 1, n_dim)))
