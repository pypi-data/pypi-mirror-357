...
def convert_example(example):
    ...
    if (
        ...
        pb = problem.replace("Can you point out", "Locate every").replace(
        ...
        pb = problem.replace("Locate the exact position of", "Locate every").replace(
...
def collate_fn(examples):
    ...
    for example in examples:
        ...
        strings = [
            f'{{"bbox_2d": [{bbox[0]}, {bbox[1]}, {bbox[2]}, {bbox[3]}], "label": "{cate}"}}'
            for cate, bbox in zip(categories, turned_bboxes)
        ]
        string = ",\n\t".join(strings)
        messages.append(
            {
