...
from namo.r1.trainer.qwen25_trainer import Qwen25VLGRPOTrainer
from trl import (
    GRPOConfig,
    GRPOTrainer,
    ModelConfig,
...
from namo.models.symbols import DEFAULT_IMAGE_TOKEN
from namo.utils.coor_utils import generalized_box_iou, accum_probs, box_iou
import pathlib
...
def extract_bbox_label(text):
    ...
    try:
        pattern = re.compile(
            r'\{"bbox_2d": \[(\d{1,3}|1000), (\d{1,3}|1000), (\d{1,3}|1000), (\d{1,3}|1000)\], "label": "([^"]+)"\}'
        )
            ...
            for item in extracted:
                try:
                    ...
                    if isinstance(bbox, list) and len(bbox) == 4:
                        ...
                        ):  # filter negative bbox
                            output.append(
                                {"bbox": bbox, "category_name": category_name}
                            )
                except Exception:
...
class GRPOScriptArguments(ScriptArguments):
    ...
    hard: Optional[bool] = field(
        default=False, metadata={"help": "whether filter the instance to over 10"}
    )
    eval_match: Optional[bool] = field(default=False)
    adaptive: Optional[bool] = field(default=False)
    use_liger: Optional[bool] = field(default=True)
...
def recall_reward(
    ...
    **kwargs,
    ...
    middle_brackets_pat = re.compile(
    ...
    for (
        ...
        if len(dt):
            ...
            dt_class = torch.zeros(
                [dt_bboxes.size()[0], max(target_id_map.values()) + 1],
                dtype=torch.float32,
            )  # +1 due to set the class 0 to the background
            ...
            gt_dict = {"labels": torch.tensor(gt_class_id), "boxes": gt_bboxes}
...
def main(script_args, training_args, model_args):
    ...
    try:
        dataset = load_dataset(
            ...
            cache_dir=f"./data/{script_args.dataset_name}_cache/",
        )
    except:
        dataset = load_from_disk(script_args.dataset_name)

    if script_args.use_liger:
        from liger_kernel.transformers import apply_liger_kernel_to_qwen2_5_vl

        apply_liger_kernel_to_qwen2_5_vl(fused_linear_cross_entropy=False)
        logger.info(f"will use liger for Qwen2.5 VL.")

    def make_conversation_image(example, qwen_enable):
        # change the prompt to qwen mode
        if qwen_enable:
            if (
                "Examine the image for any objects from the category set. Report the coordinates of each detected object."
                in example["problem"]
            ):
                pb = example["problem"].replace(
                    "Examine the image for any objects from the category set. Report the coordinates of each detected object.",
                    "Locate every item from the category list in the image and output the coordinates in JSON format.",
                )
                # pb = "Please output bbox coordinates and names of every item in this image."
            elif "Can you point out" in example["problem"]:
                pb = (
                    example["problem"]
                    .replace("Can you point out", "Locate every")
                    .replace(
                        "in the image and provide the coordinates of its location?",
                        "in the image and output the coordinates in JSON format.",
                    )
                )
            elif "Locate the exact position of" in example["problem"]:
                pb = (
                    example["problem"]
                    .replace("Locate the exact position of", "Locate every")
                    .replace(
    ...
    def filter_has_instance(example, minimal=1, maximum=30):
        return (
            int(example["num_instances"]) >= minimal
            and int(example["num_instances"]) <= maximum
        )
