...
from namo.utils.utils import rank0_print as auto_rank0_print
...
class Qwen25VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            model_id = model
            torch_dtype = (
                torch_dtype if torch_dtype else model_init_kwargs.get("torch_dtype")
            )
            if (
    ...
    def compute_loss(
        ...
        mini_batch_size = (
            self.args.logit_computation_mini_batch_size
            if hasattr(self.args, "logit_computation_mini_batch_size")
            else 1
        )  # need newest trl
        ...
        input_heights = (
            copy.deepcopy(image_grid_thw[:, 1])
            .detach()
        ...
        input_widths = (
            copy.deepcopy(image_grid_thw[:, 2])
            .detach()
            .repeat_interleave(self.num_generations, dim=0)
            .cpu()
            * 14
