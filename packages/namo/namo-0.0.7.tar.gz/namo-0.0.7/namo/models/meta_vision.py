...
class NamoMetaVisionForCausalLM(ABC):
    ...
    def encode_images(self, images, pixel_attention_mask=None, spatial_shapes=None):
        image_features = self.get_namo().get_vision_tower()(
            images, pixel_attention_mask, spatial_shapes
        )
        image_features = self.get_namo().conn_ve_llm(
            image_features, image_sizes=spatial_shapes
        )
        # normalization visual features
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features = self.encode_images(
                images, pixel_attention_mask, image_sizes
            )
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    # hack: support QwenVL VE features
                    cur_image_features = image_features[cur_image_idx]
                    cur_image_idx += 1
