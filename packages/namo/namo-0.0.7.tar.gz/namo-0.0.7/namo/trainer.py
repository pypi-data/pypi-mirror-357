...
from namo.utils.hf_utils import GPUUsageMonitorStepCallback
...
class NamoTrainer(Trainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.add_callback(GPUUsageMonitorStepCallback())
            ...
            if self.args.conn_ve_llm_lr is not None:
                ...
                if self.args.ve_lr is not None:
                    vision_tower_parameters = [
                        name
                        for name, _ in opt_model.named_parameters()
                        if ".ve." in name and "patch_merger" not in name
                    ]
                    # excluded patch_merger as it need larger lr
                    # if has
                    patch_merger_params = [
                        name
                        for name, _ in opt_model.named_parameters()
                        if "patch_merger" in name
                    ]
                    optimizer_grouped_parameters = [
                        {
                            "params": [
                                ...
                                if (
                                    ...
                                    and n not in patch_merger_params
                                    and p.requires_grad
                                )
                            ],
                    ...
                    if len(patch_merger_params) > 0:
                        optimizer_grouped_parameters.extend(
                            [
                                # add for patch merge lr
                                {
                                    "params": [
                                        p
                                        for n, p in opt_model.named_parameters()
                                    ...
                                    "weight_decay": self.args.weight_decay,
                                    "lr": (
                                        self.args.conn_ve_llm_lr
                                        if self.args.ve_merger_lr is None
                                        else self.args.ve_merger_lr
                                    ),
                                },
                                {
                                    "params": [
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, "tune_conn_ve_llm", False):
            ...
            if self.args.local_rank == 0 or self.args.local_rank == -1:
                ...
                if not os.path.exists(os.path.join(output_dir, TRAINER_STATE_NAME)):
            ...
            if self.args.local_rank == 0 or self.args.local_rank == -1:
                ...
                logger.info(f"saving image processor to: {output_dir}")
