...
class NamoHydraVL(VLBase):
    ...
    def get_msg(self, text, image=None, max_size=640):
        if image is None:
            return {
                "role": "user",
                "content": [
    ...
    def generate(
        ...
        # print(input_ids)
        if verbose:
            print(f'input_ids: {input_ids} {self.tokenizer.decode([i for i in input_ids[0] if i != -200])}')
        
        inputs_new = dict()
        inputs_new["input_ids"] = input_ids.to(self.device)
        if "image_grid_thw" in inputs:
            inputs_new["image_sizes"] = inputs["image_grid_thw"].to(self.device)
            inputs_new["pixel_values"] = inputs["pixel_values"].to(self.device)
        ...
        generated_ids = self.model.generate(
            **inputs_new, do_sample=False, max_new_tokens=500, streamer=streamer
        )
