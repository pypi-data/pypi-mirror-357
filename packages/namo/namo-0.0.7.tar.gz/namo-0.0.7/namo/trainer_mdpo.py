...
class MultimodalDPOTrainer(DPOTrainer):
    def __init__(self, compute_dtype=torch.bfloat16, *args, **kwargs):
    ...
    def concatenated_inputs(
        ...
        if "pixel_values" in batch:
            # todo: check is B,S,H or B,H,W,C
            if len(batch["pixel_values"].shape) == 4:
                concatenated_batch["pixel_values"] = (
                    batch["pixel_values"].repeat(2, 1, 1, 1).to(dtype)
                )
                print(
                    f'==> [warn] this is unexpected! batch pixel_values: {batch["pixel_values"].shape}!!'
                )
                for k, v in batch.items():
                    print(f"{k}: {v.shape}")
                print("----- something wrong batch -----")
            else:
                # force to training
                concatenated_batch["pixel_values"] = (
                    batch["pixel_values"].repeat(2, 1, 1).to(dtype)
                )

            if "spatial_shapes" in batch:
                concatenated_batch["spatial_shapes"] = batch["spatial_shapes"].repeat(
                    2, 1
                )
                concatenated_batch["pixel_attention_mask"] = batch[
    ...
    def concatenated_forward(
        ...
        if self.is_vision_model:
            ...
            model_kwargs["image_sizes"] = concatenated_batch["spatial_shapes"]
        ...
        try:
            ...
            for k, v in concatenated_batch.items():
                print(f"{k} {v.shape}")
            print("------ concatenated_batch done ------")

        all_logits = outputs.logits
        # print(f"all_logits: {all_logits.shape} {all_logits.dtype}")
        ...
        (
            ...
            concatenated_batch["pixel_values"],
            concatenated_batch["spatial_shapes"],
            concatenated_batch["pixel_attention_mask"],
        )
        # print(f"labels_new: {labels_new.shape}")
    ...
    def get_batch_logps(
        ...
        if logits.shape[:-1] != labels.shape:
            raise ValueError(
                f"Logits (batch and sequence length dim) and labels must have the same shape. {logits.shape} vs {labels.shape}"
            )
