...
from pandora.integrations.pytest_html_reporter.template import html_template
from pandora.integrations.pytest_html_reporter.time_converter import time_converter
...
class HTMLReporter(object):
    ...
    def pytest_runtest_makereport(self, item, call):
        ...
        trace = call.excinfo.tb if call.excinfo else ""
            ...
            if getattr(rep, "when", None) == "call":
                if hasattr(rep, "wasxfail"):
                    ...
                    if rep.longrepr:
                        ...
                        self.update_test_traceback(trace)
            else:
    ...
    def append_test_metrics_row(self):
        global _test_metrics_content, _pvalue, _duration, _current_error, _current_traceback
        ...
        _current_error = html.escape(_current_error.lstrip(), quote=False)
        _current_traceback = html.escape(_current_traceback.lstrip(), quote=False)
        if (self.rerun is not None) and (max_rerun() is not None):
            ...
            if (_pvalue == max_rerun() + 1) or (_test_status == 'PASS'):
                ...
                test_row_text = test_row_text.replace("__trace__", str(_current_traceback[-1000:]))
                current_time = time.time()
                floating_trace_text = floating_trace_text.replace("__runt__", str(current_time).replace('.', ''))

                if len(_current_traceback) < 999:
                    test_row_text = test_row_text.replace("__floating_trace_text__", str(''))
                else:
                    test_row_text = test_row_text.replace("__floating_trace_text__", str(floating_trace_text))
                    test_row_text = test_row_text.replace("__full_trace__", str(_current_traceback))
        ...
        self.json_data['content']['suites'].setdefault(len(_test_suite_name), {}).setdefault('tests', {}).setdefault(
            len(_scenario) - 1, {})['trace_back'] = str(_current_traceback)
        self.json_data['content']['suites'].setdefault(len(_test_suite_name), {}).setdefault('tests', {}).setdefault(
    ...
    def update_test_traceback(self, trace):
        global _current_traceback
        if not trace:
            _current_traceback = ""
            return
        extracted_list = traceback.extract_tb(trace)
        trace_list = traceback.StackSummary.from_list(extracted_list).format()
        trace_str = "".join(trace_list)
        _current_traceback = trace_str
