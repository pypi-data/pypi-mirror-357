...
1. {"Check network connectivity and DNS resolution" if "DNS resolution failed" in server_instance.connection_error else "Verify network connectivity"}
2. Confirm the API endpoint is accessible (current: {server_instance.vme_api_base_url})
3. {"Update API token in configuration" if "Authentication failed" in server_instance.connection_error else "Verify API token is valid"}
