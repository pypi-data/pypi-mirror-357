...
def check_audio_dependencies() -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        audio.terminate()
        logger.info("✅ PyAudio with PortAudio: Available")
    except ImportError:
        missing.append("pyaudio")
        logger.warning("❌ PyAudio not installed")
    except OSError as e:
        if "PortAudio" in str(e) or "libportaudio" in str(e):
            missing.append("portaudio-devel")
            logger.warning("❌ PortAudio library not found")
        else:
            missing.append("audio-system")
            logger.warning(f"❌ Audio system error: {e}")
    except Exception as e:
        missing.append("audio-unknown")
        logger.warning(f"❌ Unknown audio error: {e}")
...
def get_installation_instructions(missing_deps: List[str]) -> List[str]:
    ...
    if "pyaudio" in missing_deps:
        instructions.append("uv add pyaudio")
