...
asymm_auth_ta010_router = APIRouter(
    prefix="/asymm_auth_ta010", tags=["TA010_ASYMMETRIC_AUTH_APIs"]
)
uc_object = None


@asymm_auth_ta010_router.post("/setup")
def setup():
    """
    Sets up TA010 Asymmetric Authentication usecase.
...
@asymm_auth_ta010_router.post("/generate_resources")
def generate_resources(user_inputs=Body()):
    """
    Generate TA010 Asymmetric Authentication resources based on user inputs.

    Args:
        user_inputs (dict): The input data provided by the user, expected to be in the form of a dictionary.

    Returns:
        UsecaseResponseModel: The response model containing the result of TA010 Asymmetric Authentication resources generation.
    """
    try:
        response = UsecaseResponseModel(status=True, message="", log="")
        assert (
            uc_object is not None
        ), "TA010 Asymmetric Authentication usecase object is not initialized. Please restart usecase!"
...
@asymm_auth_ta010_router.post("/proto_provision")
def proto_provision(user_inputs=Body()):
    """
    Provisions TA010 Asymmetric Authentication protocol for the given user inputs.

    Args:
        user_inputs (Body): The user inputs required for provisioning the TA010 Asymmetric Authentication.
...
@asymm_auth_ta010_router.post("/teardown")
def teardown():
    """
    Teardown function for TA010 Asymmetric Authentication.

    This function performs the necessary cleanup for TA010 Asymmetric Authentication
    ...
    try:
        ...
        assert (
            uc_object is not None
        ), "TA010 Asymmetric Authentication usecase object is not initialized. Teardown failed!"
