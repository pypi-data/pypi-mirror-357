...
symm_auth_ta010_router = APIRouter(
    prefix="/symm_auth_ta010", tags=["TA010_SYMMETRIC_AUTH_APIs"]
)
uc_object = None


@symm_auth_ta010_router.post("/setup")
def setup():
    """
    Sets up TA010 Symmetric Authentication usecase.
...
@symm_auth_ta010_router.post("/generate_resources")
def generate_resources(user_inputs=Body()):
    """
    Generate TA010 Symmetric Authentication resources based on user inputs.

    Args:
        user_inputs (dict): The input data provided by the user, expected to be in the form of a dictionary.

    Returns:
        UsecaseResponseModel: The response model containing the result of TA010 Symmetric Authentication resources generation.
    """
    try:
        response = UsecaseResponseModel(status=True, message="", log="")
        assert (
            uc_object is not None
        ), "TA010 Symmetric Authentication usecase object is not initialized. Please restart usecase!"
...
@symm_auth_ta010_router.post("/proto_provision")
def proto_provision(user_inputs=Body()):
    """
    Provisions TA010 Symmetric Authentication protocol for the given user inputs.

    Args:
        user_inputs (Body): The user inputs required for provisioning the TA010 Symmetric Authentication.
...
@symm_auth_ta010_router.post("/teardown")
def teardown():
    """
    Teardown function for TA010 Symmetric Authentication.

    This function performs the necessary cleanup for TA010 Symmetric Authentication
    ...
    try:
        ...
        assert (
            uc_object is not None
        ), "TA010 Symmetric Authentication usecase object is not initialized. Teardown failed!"
