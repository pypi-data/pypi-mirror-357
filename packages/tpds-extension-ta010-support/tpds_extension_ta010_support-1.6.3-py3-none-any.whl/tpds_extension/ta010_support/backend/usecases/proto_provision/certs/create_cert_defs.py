...
from tpds.cert_tools.cert_utils import get_ca_status, get_device_sn_cert
...
class CertDef:
    ...
    def get_cert_params(self):
        """
        Retrieve certificate parameters for a given X.509 certificate.

        Returns:
            dict: A dictionary containing certificate parameters, including type, template ID, chain ID,
                private key slot, serial number source, certificate serial number device location,
                issue and expire date formats, and standard certificate elements.
        """
        template_data = self.cert.public_bytes(encoding=serialization.Encoding.DER)
    ...
    def get_cert_elements(self):
        """
        Retrieve certificate elements for a given X.509 certificate.

        Returns:
            list: A list of dictionaries, each representing a certificate element with its ID, device location,
                certificate location, and any necessary transformations.
        """
        template_data = self.cert.public_bytes(encoding=serialization.Encoding.DER)
        asn1_cert = asn1crypto.x509.Certificate().load(template_data, strict=True)
    ...
    def __get_signer_template(self, cert):
        """
        Generate a signer template for a given certificate.

        Args:
            cert (Certificate): The certificate for which the signer template is to be generated.

        Returns:
            dict: A Python definition of the signer template.
        """
    ...
    def __params_atcacert_definitions(template_data, params, elements):
        """
        Create and configure an ATCA certificate definition object.

        This static method initializes an `atcacert_def_t` object using provided parameters, template data,
        and certificate elements. It sets up the certificate template and elements within the definition.

        Args:
            template_data (bytes): The binary data representing the certificate template.
            params (dict): A dictionary of parameters to initialize the `atcacert_def_t` object.
            elements (list): A list of dictionaries representing certificate elements, each suitable for
                            initializing an `atcacert_cert_element_t` object.

        Returns:
            atcacert_def_t: An initialized and configured certificate definition object.
        """
    ...
    def __device_loc_to_c(device_loc, indent=""):
        """
        Convert device location information into a formatted C structure string.

        This method takes a device location object and formats its properties into a string representation
        suitable for inclusion in C code. The properties include zone, slot, is_genkey, offset, and count.

        Args:
            device_loc (object): An object representing the device location, expected to have `zone`, `slot`,
                                `is_genkey`, `offset`, and `count` attributes.
            indent (str, optional): The string used for indentation at the beginning of each line.
                                    Defaults to an empty string.

        Returns:
            str: A string representing the device location as a C structure.
        """
        lines = [
    ...
    def __cert_loc_to_c(cert_loc, indent=""):
        """
        Convert certificate location information into a formatted C structure string.


        Args:
            cert_loc (object): An object representing the certificate location, expected to have
                                `offset` and `count` attributes.
            indent (str, optional): The string used for indentation at the beginning of each line.
                                    Defaults to an empty string.

        Returns:
            str: A string representing the certificate location as a C structure, with the specified indentation.
        """
        lines = [
    ...
    def __c_hex_array(data, indent="    ", bytes_per_line=16):
        """
        Convert binary data into a formatted C hexadecimal array string.

        Args:
            data (bytes): The binary data to be converted into a hexadecimal array.
            indent (str, optional): The string used for indentation at the beginning of each line.
                                    Defaults to four spaces.
            bytes_per_line (int, optional): The number of bytes to include per line in the output. Defaults to 16.

        Returns:
            str: A string representing the binary data as a C hexadecimal array, formatted with the specified
                indentation and bytes per line.
        """
        lines = []
