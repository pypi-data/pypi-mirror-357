...
from tpds.cert_tools.cert_utils import get_ca_status
...
class WPCCertDef:
    ...
    def get_qi_extension_location(self, cert):
        """
        Determines the location and size of the Qi extension within a given certificate.

        Args:
            cert (asn1crypto.x509.Certificate): The certificate object in which to search for the Qi extension.

        Returns:
            tuple: A tuple containing the offset and size of the modified Qi extension within the certificate.
                If the Qi extension is not found, returns (0, 0).
        """
        cert_mod = cert.copy()
        if self.chain_level:
