...
class BackupManager:
    ...
    async def create_file_backup(
        ...
        # Store the backup
        backup_path = await self.storage.store_file_backup(file_path, metadata)
    ...
    async def create_folder_backup(
        ...
        # Store the backup
        backup_path = await self.storage.store_folder_backup(
            folder_path, 
            metadata,
            include_patterns,
            exclude_patterns
    ...
    async def restore_file_backup(
        ...
        try:
            ...
            global_meta = await self.storage.load_global_metadata()
            backup_info = global_meta["backups"][backup_id]
            backup_storage_path = Path(backup_info["storage_path"])
            
            # Restore the file
            await safe_copy_file(backup_storage_path, target_path)
            
            # Verify checksum
            restored_checksum = await calculate_checksum(target_path)
            ...
            if safety_backup_path and safety_backup_path.exists():
                await safe_copy_file(safety_backup_path, target_path)
            raise e
    ...
    async def list_backups_for_path(
        ...
        """
        file_path = validate_path(file_path)
        backups = await self.storage.list_backups_for_path(file_path)
    ...
    async def list_all_backups(self) -> List[Dict[str, Any]]:
        ...
        """
        backups = await self.storage.list_all_backups()
    ...
    async def _create_safety_backup(self, path: Path) -> Path:
        ...
        if path.is_file():
            await safe_copy_file(path, safety_path)
        else:
