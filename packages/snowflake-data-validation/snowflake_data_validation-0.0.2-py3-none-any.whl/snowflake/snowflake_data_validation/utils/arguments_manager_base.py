...
from snowflake.snowflake_data_validation.utils.helpers import (
    ...
    create_temp_dir,
    load_datatypes_mapping_templates_from_yaml,
)
...
class ArgumentsManagerBase(ABC):
    ...
    def setup_validation_environment(
        ...
        source_template_loader_manager = TemplatesLoaderManager(
            templates_directory_path=source_templates_dir_path,
        ...
        datatypes_file_path = source_templates_dir_path.joinpath(
            COLUMN_DATATYPES_MAPPING_NAME_FORMAT.format(
                platform=self.source_platform.value
            )
        )

        datatypes_mappings = load_datatypes_mapping_templates_from_yaml(
            yaml_path=datatypes_file_path, target_platform=self.target_platform.value
        )
