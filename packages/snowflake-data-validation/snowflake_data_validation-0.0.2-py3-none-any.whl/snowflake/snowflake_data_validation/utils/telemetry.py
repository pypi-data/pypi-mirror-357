...
class DataValidationTelemetryManager(TelemetryClient):
    ...
    def __init__(
        ...
        if SNOWFLAKE_DIRS:
            self.dv_folder_path = Path(SNOWFLAKE_DIRS.user_config_path) / "snowflake-data-validation-telemetry"
        else:
            self.dv_folder_path = Path.home() / ".snowflake" / "snowflake-data-validation-telemetry"
        ...
        try:
            ...
            for event in batch:
                ...
                if message is not None:
                    ...
                    file_path = self.dv_folder_path / f"{timestamp}_telemetry_{event_type}.json"
    ...
    def _dv_is_telemetry_enabled(self) -> bool:
        ...
        return os.getenv("SNOWFLAKE_DATA_VALIDATION_TELEMETRY_ENABLED", "true").lower() == "true"

    def _dv_is_telemetry_testing(self) -> bool:
        """Check if in testing mode."""
        is_testing = os.getenv("SNOWFLAKE_DATA_VALIDATION_TELEMETRY_TESTING", "false").lower() == "true"
        if is_testing:
...
def _get_metadata() -> dict:
    ...
    if SNOWPARK_VERSION:
        metadata["snowpark_version"] = ".".join(str(x) for x in SNOWPARK_VERSION if x is not None)
...
def validation_started_event(telemetry_data: dict, param_data: dict) -> tuple[str, dict]:
    ...
    if config_model:
        ...
        if hasattr(config_model, 'database_mappings'):
            telemetry_data[IS_DATABASE_MAPPING_USED_KEY] = bool(config_model.database_mappings)
        
        # Extract schema mappings usage
        if hasattr(config_model, 'schema_mappings'):
            telemetry_data[IS_SCHEMA_MAPPING_USED_KEY] = bool(config_model.schema_mappings)
        ...
        if hasattr(config_model, 'validation_configuration') and config_model.validation_configuration:
            validation_config = config_model.validation_configuration
            if hasattr(validation_config, 'schema_validation'):
                telemetry_data[SCHEMA_VALIDATION_KEY] = validation_config.schema_validation
            if hasattr(validation_config, 'metrics_validation'):
                telemetry_data[METRICS_VALIDATION_KEY] = validation_config.metrics_validation
            if hasattr(validation_config, 'columnar_validation'):
                telemetry_data[COLUMNAR_VALIDATION_KEY] = validation_config.columnar_validation
...
def _extract_table_config_data(telemetry_data: dict, param_data: dict) -> dict:
    ...
    if TABLE_CONTEXT_KEY in param_data:
        ...
        if hasattr(table_config, 'column_selection_list'):
            telemetry_data[COLUMN_SELECTION_USED_AS_EXCLUDED_KEY] = table_config.use_column_selection_as_exclude_list
...
def metrics_validation_event(telemetry_data: dict, param_data: dict) -> tuple[str, dict]:
...
def orchestration_event(telemetry_data: dict, param_data: dict, func_name: str) -> tuple[str, dict]:
...
def handle_result(
    ...
    elif func_name in ["run_sync_comparison", "run_async_generation", "run_async_comparison"]:
...
def report_telemetry(
    ...
    def report_telemetry_decorator(func: fn) -> fn:
        ...
        def wrapper(*args, **kwargs):
            ...
            if os.getenv("SNOWFLAKE_DATA_VALIDATION_TELEMETRY_ENABLED", "true").lower() == "false":
