...
class xy_outlook:
	...
	def check_folder_object(self, input_value=""):
		"""
	...
	def count_mails_in_folder(self, input_folder_name):
		...
		:return:
		"""
		folder_obj = self.check_folder_object(input_folder_name)
	...
	def count_unread_mails_in_folder(self, input_folder):
		...
		folder_obj = self.check_folder_object(input_folder)
	...
	def filter_mails_in_mails_by_body_with_xsql(self, input_mails, xsql):
		...
		for index, one_mail in enumerate(mail_l1d):
			if self.rex.search_with_xsql(xsql, one_mail.Body):
	...
	def get_all_property_for_mail(self, input_mail):
		...
		result = self.utilx.get_all_properties_for_object(input_mail)
		return result
	...
	def get_empty_mail(self):
		...
		:return:
		"""
		list_1d = self.get_all_property_name_for_mail_obj()
		mail_dic = {}
		for one in list_1d:
			mail_dic[one] = None
		return mail_dic
	...
	def get_folder_obj_by_any_input(self, input_value):
		...
		:return:
		"""
		result = self.check_folder_object(input_value)
		return result
	...
	def get_information_for_mail_v1(self, input_mail):
		...
		abc = self.utilx.get_all_properties_for_object(input_mail)
		print(abc)
	...
	def get_mails_between_today_to_nth_day_before_in_folder(self, input_folder, input_no, sort_by=""):
		...
		dt_to = self.timex.get_dt_obj_for_ymd_list_as_23h_59m_59s()
		dt_from = self.timex.get_dt_obj_for_ymd_list_as_0h_0m_0s()
		dt_from = self.timex.shift_day_for_dt_obj(dt_from, -1 * int(input_no))
	...
	def get_mails_in_folder_between_date1_and_date2(self, input_folder, start_date, end_date, sort_by=""):
		...
		:return:
		"""
		dt_obj_from = self.timex.change_anytime_to_dt_obj(start_date)
		dt_obj_to = self.timex.change_anytime_to_dt_obj(end_date)
	...
	def get_mails_in_folder_by_receive_date_as_yyyymmdd(self, input_folder="", input_yyyy_mm_dd=""):
		...
		folder_obj = self.check_folder_object(input_folder)
		dt_obj_to = self.timex.change_anytime_to_dt_obj(input_yyyy_mm_dd)
	...
	def get_mails_in_folder_for_today(self):
		...
		"""
		folder_obj = self.get_inbox_folder()
		ymd_list = self.timex.get_ymd_list_for_today()
		dt_to = self.timex.get_dt_obj_for_ymd_list_as_23h_59m_59s(ymd_list)
		dt_from = self.timex.get_dt_obj_for_ymd_list_as_0h_0m_0s(ymd_list)
	...
	def get_mails_in_folder_between_ymdlist1_and_ymdlist2(self, input_folder, ymdlist1, ymdlist2):
		...
		"""
		folder_obj = self.check_folder_object(input_folder)
		dt_from = self.timex.get_dt_obj_for_ymd_list_as_0h_0m_0s(ymdlist1)
		dt_to = self.timex.get_dt_obj_for_ymd_list_as_23h_59m_59s(ymdlist2)
		result = folder_obj.Items.Restrict("[ReceivedTime] >= '" + dt_from.strftime('%Y-%m-%d %H:%M') + "' AND [ReceivedTime] < '" + dt_to.strftime('%Y-%m-%d %H:%M') + "'")
		return result
	...
	def get_mails_in_folder_from_today_to_nth_day_later(self, input_folder, input_day_no):
		...
		folder_obj = self.check_folder_object(input_folder)
		dt_obj_to = self.timex.get_dt_obj_for_today()
	...
	def get_mails_in_folder_obj_by_sender_or_reciver(self, input_mail_folder_obj, sender_name, receiver):
		...
		:return:
		"""
		folder_obj = self.check_folder_object(input_mail_folder_obj)
	...
	def get_nea_mails_in_folder_obj(self, folder_obj, input_no=5):
		...
		"""
		result = []
		folder_obj = self.check_folder_object(folder_obj)
	...
	def get_nea_mails_in_inbox_folder(self, input_no=5):
		...
		:return:
		"""
		folder_obj = self.check_folder_object(input_no)
	...
	def get_nth_mails_in_folder(self, input_folder_no, index_no=6):
		...
		:return:
		"""
		folder_obj = self.check_folder_object(input_folder_no)
	...
	def get_sub_folder_index_in_top_folder_name_n_sub_folder_name(self, top_folder_name="", sub_folder_name=""):
		...
		if type(sub_folder_name) == type(123):
			...
			sub_folder_data = self.get_all_sub_folder_name_for_top_folder_name(top_folder_index)
	...
	def get_unread_mails_in_folder_by_folder_index(self, input_index):
		...
		:return:
		"""
		folder_obj = self.check_folder_object(input_index)
	...
	def move_mail_to_target_folder(self, input_mail, target_folder):
		...
		:return:
		"""
		folder_obj = self.check_folder_object(target_folder)
	...
	def print_basic_datas_for_mails(self, input_mails):
		...
		try:
			for one_mail in input_mails:
				...
				temp.append(self.timex.change_dt_obj_to_text_time_with_format(one_mail.ReceivedTime))
	...
	def sort_mails_by_mail_property(self, mail_list, input_property):
		...
		sorted_l2d = self.utilx.sort_list_2d_by_index(new_mail_list, 0)
