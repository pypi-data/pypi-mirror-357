...
class StructureValidator:
    ...
    def _validate_ten_step_numbering(self, result: ValidationResult, 
                                    numbered_dirs: List[tuple]):
        ...
        for dir_name, number in numbered_dirs:
            if number % 10 == 0:  # Base number
                ...
                if lower_base not in actual_numbers:
                    result.add_warning(f"Intermediate number without base: {dir_name} (missing {lower_base:02d}_*)")
    
    def _validate_sequential_numbering(self, result: ValidationResult,
                                      numbered_dirs: List[tuple]):
        ...
        for i in range(len(numbers) - 1):
            if numbers[i + 1] - numbers[i] > 1:
                ...
                result.add_warning(f"Gap in numbering: {gap_start:02d} to {gap_end:02d}")
        
        # Check if starts from 00
        if numbers[0] != 0:
            result.add_warning(f"Numbering doesn't start from 00, starts from {numbers[0]:02d}")
    
    def _validate_tier_compliance(self, result: ValidationResult):
        """Validate directory tier compliance."""
        tier_counts = {tier: 0 for tier in DirectoryTier}
            ...
            if item.is_dir() and not item.name.startswith('.'):
                ...
                if classification.tier == DirectoryTier.SYSTEM:
                    if not item.name.startswith('_'):
                        result.add_error(f"System directory doesn't start with '_': {item.name}")
                
                elif classification.tier == DirectoryTier.CORE:
                    if classification.number is None:
                        result.add_error(f"Core directory missing number prefix: {item.name}")
                
                elif classification.tier == DirectoryTier.AUXILIARY:
                    if item.name.startswith('_') or classification.number is not None:
                        result.add_warning(f"Auxiliary directory has unexpected prefix: {item.name}")
        
        # Report tier distribution
        result.add_info(f"Directory distribution - System: {tier_counts[DirectoryTier.SYSTEM]}, "
    ...
    def _validate_readme_coverage(self, result: ValidationResult):
        ...
        if missing_readmes:
            result.add_warning(f"Directories missing README.md: {', '.join(missing_readmes)}")
        else:
    ...
    def _validate_metadata_compliance(self, result: ValidationResult):
        ...
        total_content_files = len(markdown_files) - len(list(self.vault_path.rglob("README.md")))
        
        if total_content_files > 0:
            metadata_coverage = ((total_content_files - files_without_frontmatter) / 
                               total_content_files * 100)
            result.add_info(f"Metadata coverage: {metadata_coverage:.1f}% "
                           f"({total_content_files - files_without_frontmatter}/{total_content_files} files)")
            
            if files_without_frontmatter > 0:
                result.add_warning(f"{files_without_frontmatter} files missing frontmatter metadata")
    
    def _check_unexpected_directories(self, result: ValidationResult, 
                                     expected_structure: Dict[str, Dict[str, str]]):
...
class StructureHealthMonitor:
    ...
    def get_health_trend(self, days: int = 7) -> Dict[str, Any]:
        ...
        trend_analysis = {
            ...
            "latest_passed": recent_entries[-1].get("passed", False) if recent_entries else False
        }
        
        return trend_analysis


def validate_structure(vault_path: Path, hybrid_config: HybridStructureConfig) -> ValidationResult:
...
def monitor_structure_health(vault_path: Path, hybrid_config: HybridStructureConfig) -> ValidationResult:
