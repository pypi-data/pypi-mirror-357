...
class QueryCondition:
    ...
    comparison: Optional[QueryComparison] = None
    negate: bool = False
...
class ObsidianQueryBuilder:
    ...
    def success_rate(self, rate: int, comparison: QueryComparison = QueryComparison.GREATER_EQUAL) -> 'ObsidianQueryBuilder':
        """Filter by success rate."""
        self.conditions.append(QueryCondition("success_rate", rate, comparison=comparison))
        return self
    ...
    def created_after(self, date: str) -> 'ObsidianQueryBuilder':
        ...
        self.conditions.append(QueryCondition("created", date, comparison=QueryComparison.GREATER_THAN))
        return self
    
    def updated_after(self, date: str) -> 'ObsidianQueryBuilder':
        """Filter by update date (after)."""
        self.conditions.append(QueryCondition("updated", date, comparison=QueryComparison.GREATER_THAN))
        return self
    ...
    def _build_dataview_condition(self, condition: QueryCondition) -> str:
        ...
        # Handle array fields
        if field in ["tech", "domain", "team", "projects", "claude_model", "claude_feature", "tags"]:
            if condition.negate:
...
class PredefinedQueries:
    ...
    def cleanup_candidates() -> ObsidianQueryBuilder:
        ...
        return (ObsidianQueryBuilder()
                ...
                .sort_by("updated", True))  # Oldest first
...
def generate_obsidian_queries_file() -> str:
    ...
    queries = {
        ...
        "Technology Focus": {
            ...
            "React Development": ObsidianQueryBuilder().tech("react").domain("web-dev").build(),
            "Docker & DevOps": ObsidianQueryBuilder().tech(["docker", "kubernetes"]).build(),
        },
        
        "Team Views": {
            "Frontend Team": PredefinedQueries.frontend_development().build(),
            "Backend Resources": ObsidianQueryBuilder().team("backend").exclude_status("deprecated").build(),
            "ML Research": ObsidianQueryBuilder().team(["ml", "research"]).domain("machine-learning").build(),
            "DevOps Tools": ObsidianQueryBuilder().team("devops").type(["code", "resource"]).build(),
        },
        ...
        "Claude-Specific": {
            ...
            "Analysis & Review": ObsidianQueryBuilder().claude_feature(["analysis", "code-review"]).build(),
            "Debugging Help": ObsidianQueryBuilder().claude_feature("debugging").confidence("high").build(),
