...
class StructureCompatibilityManager:
    ...
    def detect_current_structure(self) -> str:
        ...
        ten_step_dirs = [d for d in existing_dirs if d.startswith(('00_', '10_', '20_', '30_'))]
        system_dirs = [d for d in existing_dirs if d.startswith('_')]
        
        if ten_step_dirs and system_dirs:
            return "hybrid"
        elif any(d.startswith(('00_', '01_', '02_', '03_', '04_', '05_')) for d in existing_dirs):
            return "legacy"
        elif existing_dirs:
            return "unknown"
        else:
    ...
    def ensure_legacy_access(self) -> bool:
        ...
        if not self.config.hybrid_structure.enabled or not self.config.hybrid_structure.legacy_support:
        ...
        for legacy_name, hybrid_name in self.legacy_mappings.items():
            ...
            if hybrid_path.exists() and not legacy_path.exists():
                try:
                    ...
                    bridge_success = self._create_compatibility_bridge(legacy_path, hybrid_path)
                    if not bridge_success:
                        success = False
        
        return success
    
    def _create_compatibility_bridge(self, legacy_path: Path, hybrid_path: Path) -> bool:
...
"""
            ...
            print(f"❌ Failed to create compatibility bridge for {legacy_path.name}: {e}")
            return False
            ...
            if legacy_path.exists():
                if legacy_path.is_symlink():
                    ...
                    if (readme_path.exists() and 
                        "Compatibility bridge created by CKC" in readme_path.read_text()):
...
class BackwardCompatibilityManager:
    ...
    def get_appropriate_manager(self, metadata_manager) -> ObsidianVaultManager:
        ...
        if self.config.hybrid_structure.enabled:
            # Use hybrid manager
            return HybridObsidianVaultManager(self.vault_path, metadata_manager, self.config)
        else:
            # Use legacy manager
    ...
    def ensure_compatibility(self) -> bool:
        ...
        if current_structure == "hybrid" and self.config.hybrid_structure.legacy_support:
    ...
    def validate_compatibility(self) -> List[str]:
        ...
        if self.config.hybrid_structure.enabled and current_structure == "legacy":
            if not self.config.hybrid_structure.legacy_support:
                issues.append("Hybrid structure enabled but legacy support disabled with legacy vault")
        
        # Check for conflicting directories
        if current_structure == "hybrid":
            legacy_dirs = [self.vault_path / name for name in self.compatibility.legacy_mappings.keys()]
            hybrid_dirs = [self.vault_path / name for name in self.compatibility.legacy_mappings.values()]
            
            for legacy_path, hybrid_path in zip(legacy_dirs, hybrid_dirs):
                if (legacy_path.exists() and hybrid_path.exists() and 
                    not legacy_path.is_symlink() and 
                    legacy_path.resolve() != hybrid_path.resolve()):
                    issues.append(f"Conflicting directories: {legacy_path.name} and {hybrid_path.name}")
...
class LegacyFileHandler:
    ...
    def handle_legacy_sync(self, source_path: Path, target_manager: ObsidianVaultManager,
                          metadata: KnowledgeMetadata, project_name: str | None = None) -> bool:
...
class MigrationSafetyValidator:
    ...
    def validate_migration_prerequisites(self) -> Dict[str, Any]:
        ...
        if not self._check_write_permissions():
            result["errors"].append("Insufficient write permissions for vault directory")
            result["safe_to_migrate"] = False
        ...
        if free_space < vault_size * 2:  # Need 2x space for backup
            result["warnings"].append(f"Low disk space. Have: {free_space}MB, Need: {vault_size * 2}MB")
        
        # Check for active file handles
        if self._check_active_file_handles():
            result["warnings"].append("Vault may be open in other applications. Close before migration.")
        
        # Check for custom modifications
        custom_mods = self._detect_custom_modifications()
        if custom_mods:
            result["warnings"].extend(custom_mods)
            result["recommendations"].append("Review custom modifications before migration")
        ...
        if obsidian_dir.exists():
            ...
            for config_file in config_files:
                if (obsidian_dir / config_file).exists():
                    modifications.append(f"Custom Obsidian configuration: {config_file}")
        
        # Check for non-standard directory structures
        expected_dirs = {"00_", "01_", "02_", "03_", "04_", "05_"}
        actual_dirs = {d.name[:3] for d in self.vault_path.iterdir() if d.is_dir() and len(d.name) >= 3}
        
        unexpected = actual_dirs - expected_dirs - {".ob", "_te"}  # Allow .obsidian, _templates etc
        if unexpected:
            modifications.append(f"Custom directory structure detected: {', '.join(unexpected)}")
