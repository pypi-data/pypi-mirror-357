...
class EnapSearchElastic(Page):
	...
	def normalize_noticia(self, item):
		def get_val(campo):
			prefixos = [
				"enap_designsystem_enapnoticiaimportada__",
				"enap_designsystem_enapnoticia__"
			]
			for prefixo in prefixos:
				valor = item.get(f"{prefixo}{campo}")
				if valor:
					return valor
			return ""

		return {
			"titulo": get_val("titulo_filter"),
			"descricao": get_val("descricao_filter"),
			"url": get_val("url_filter"),
			"data_atualizacao": get_val("data_atualizacao_filter"),
			"imagem": get_val("imagem_filter"),
