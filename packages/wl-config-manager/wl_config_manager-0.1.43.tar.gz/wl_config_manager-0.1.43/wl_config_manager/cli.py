...
Examples:
  ...
  wl_config_manager env MYAPP_ --format=json
    ...
    parser.add_argument('--no-dotenv', action='store_true',
                      help='Disable loading .env files')
...
def format_output(data: Any, format_type: Optional[str] = None) -> str:
    ...
    if isinstance(data, SimpleNamespace):
        from .dot_notation import namespace_to_dict
...
def _format_dict(data: Dict, indent: int = 0) -> str:
    ...
    for key, value in data.items():
        if isinstance(value, dict) or isinstance(value, SimpleNamespace):
            ...
            lines.append(' ' * indent + f"{key}: {display_value}")
    return '\n'.join(lines)
...
def _mask_sensitive_value(key: str, value: Any) -> Any:
    ...
    if any(word in key_lower for word in sensitive_keys):
...
def cmd_set(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if not os.path.exists(args.config_file) and args.create:
            config = ConfigManager(load_dotenv=not args.no_dotenv)
        else:
            config = ConfigManager(config_path=args.config_file,
                                 load_dotenv=not args.no_dotenv)
...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.template:
            config = ConfigManager(config_path=args.template,
                                 load_dotenv=not args.no_dotenv)
        else:
            config = ConfigManager(load_dotenv=not args.no_dotenv)
...
def cmd_convert(args: argparse.Namespace) -> int:
    ...
    try:
        # Load the input file
        config = ConfigManager(config_path=args.input_file,
                             load_dotenv=not args.no_dotenv)
...
def cmd_env(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        config = ConfigManager.from_env(args.prefix)
