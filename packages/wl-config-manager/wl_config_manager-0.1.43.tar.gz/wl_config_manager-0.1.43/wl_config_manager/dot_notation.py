...
# YAML Representers and Constructors
def iterable_namespace_representer(dumper, data):
    """
    Represent IterableNamespace as a regular dict in YAML
    """
    return dumper.represent_dict(namespace_to_dict(data))


def iterable_namespace_constructor(loader, node):
    """
    Construct IterableNamespace from YAML node
    """
    values = loader.construct_mapping(node)
    return dict_to_namespace(values)


# Register the representer for both safe and unsafe dumpers
yaml.add_representer(IterableNamespace, iterable_namespace_representer)
yaml.SafeDumper.add_representer(IterableNamespace, iterable_namespace_representer)

# Optional: Register a custom tag if you want to preserve type on load
yaml.add_constructor('!IterableNamespace', iterable_namespace_constructor)
yaml.SafeLoader.add_constructor('!IterableNamespace', iterable_namespace_constructor)


# Helper functions for easy usage
def dump_config(config, stream=None, **kwargs):
    """
    Dump configuration to YAML with clean output
    
    Args:
        config: IterableNamespace or dict to dump
        stream: File-like object to write to (None returns string)
        **kwargs: Additional arguments for yaml.dump
        
    Returns:
        YAML string if stream is None
    """
    # Set default flow style to False for better readability
    kwargs.setdefault('default_flow_style', False)
    kwargs.setdefault('sort_keys', False)
    
    return yaml.dump(config, stream, **kwargs)


def safe_dump_config(config, stream=None, **kwargs):
    """
    Safely dump configuration to YAML with clean output
    ...
    return yaml.safe_dump(config, stream, **kwargs)


def load_config(stream, preserve_namespace=False):
    """
    Load configuration from YAML
    
    Args:
        stream: File-like object or string to load from
        preserve_namespace: If True, converts dicts to IterableNamespace
    ...
    if preserve_namespace and isinstance(data, dict):
