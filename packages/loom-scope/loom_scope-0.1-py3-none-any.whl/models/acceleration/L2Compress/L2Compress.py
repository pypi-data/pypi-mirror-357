...
from .L2Compress_utils.monkey_patch import replace_llama, replace_mistral

logger.remove()
logger.add(sys.stdout,colorize=True, format="<level>{message}</level>")
class L2Compress(AccelerationModel):
    ...
    def deploy(self):
        ...
        with open(
            "./models/acceleration/L2Compress/config/L2Compress_config.json", "r"
        ) as f:
            config_dict = json.load(f)
            model_config.max_capacity_prompt = config_dict["max_capacity_prompt"]
            model_config.skip_layers = config_dict["skip_layers"]
        ...
        if "long" in self.model_path.lower() or "mamba" in self.model_path.lower():
            ...
            if "llama" in self.model_path.lower():
                from .L2Compress_utils.causal_model_forward import (
                ...
                from .L2Compress_utils.llama_mlp_forward import llama_mlp_forward
