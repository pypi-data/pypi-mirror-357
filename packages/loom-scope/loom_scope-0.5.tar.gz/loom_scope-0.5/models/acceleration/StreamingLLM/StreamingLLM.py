...
from .streaming_llm_utils.monkey_patch import replace_llama, replace_mistral

logger.remove()
logger.add(sys.stdout,colorize=True, format="<level>{message}</level>")
class StreamingLLM(AccelerationModel):
    ...
    def deploy(self):
        ...
        with open(
            "./models/acceleration/StreamingLLM/config/streamingllm_config.json", "r"
        ) as f:
        ...
        if "long" in self.model_path.lower() or "mamba" in self.model_path.lower():
            ...
            if "llama" in self.model_path.lower():
                from .streaming_llm_utils.causal_model_forward import (
                ...
                from .streaming_llm_utils.llama_mlp_forward import llama_mlp_forward
