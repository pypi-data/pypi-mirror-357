...
from .snapkv_utils.monkey_patch import replace_llama, replace_mistral

logger.remove()
logger.add(sys.stdout,colorize=True, format="<level>{message}</level>")
class SnapKV(AccelerationModel):
    ...
    def deploy(self):
        ...
        # load config from ./config/H2O_config.json
        with open("./models/acceleration/SnapKV/config/SnapKV_config.json","r") as f:
        ...
        if "long" in self.model_path.lower() or "mamba" in self.model_path.lower():
            ...
            if "llama" in self.model_path.lower():
                from .snapkv_utils.causal_model_forward import (
                ...
                from .snapkv_utils.llama_mlp_forward import llama_mlp_forward
