...
try:
    ...
    if HAS_LIBP2P:
        ...
        def inject_libp2p_into_high_level_api(api_class):
            ...
            """
            try:
                # Create AnyIO-compatible extension function
                return extend_high_level_api_class_anyio(api_class)
...
def extend_high_level_api_class_anyio(high_level_api_cls):
    """Extend the IPFSSimpleAPI class with libp2p peer discovery functionality using AnyIO.
    ...
    the full IPFS daemon. All async operations use AnyIO for backend flexibility.

    Args:
        high_level_api_cls: The IPFSSimpleAPI class to extend
    """
    # Check if libp2p and high-level API are available
    if not HAS_LIBP2P:
        logger.warning("libp2p not available")
            ...
            for source in discovery_sources:
                ...
                try:
                    if source == "mdns":
                        ...
                        from ..libp2p.enhanced_dht_discovery import EnhancedDHTDiscovery
                        ...
                        dht_peers = anyio.run(run_with_timeout)
    ...
    def request_content_from_peer(self, peer_id, cid, timeout=30):
        ...
        try:
            ...
            # Define async function for content request with anyio timeout
            async def request_with_timeout():
                with anyio.fail_after(timeout):
                    # Use the async version of request_content if available
                    if hasattr(self.libp2p_peer, "request_content_async"):
                        return await self.libp2p_peer.request_content_async(cid)
                    else:
                        # Fall back to sync version if needed
                        return self.libp2p_peer.request_content(cid, timeout=timeout)

            # Run the request using anyio
            content = anyio.run(request_with_timeout)
            ...
            logger.error(f"Timeout requesting content {cid} from peer {peer_id}")
            return result
        except Exception as e:
            result["error"] = str(e)
...
def apply_high_level_api_integration(api_class=None):
    """Apply the High-Level API integration using dependency injection with AnyIO support.
