...
from .chat import chat, prompt_template
...
class AIManager:
    ...
    def prompt_template(self, name, data):
        """
        ...
        try:
            if name not in self.prompts:
                self.logger.error(f"Prompt '{name}' not found in available prompts.")
                return None
                
            prompt = self.prompts[name]
            if prompt is None:
                self.logger.error(f"Prompt '{name}' exists but is None.")
                return None
            ...
            if isinstance(prompt, dict):
                ...
                if 'system' in prompt and prompt['system']:
                    try:
                        ...
                        self.logger.error(f"Missing data key for system prompt formatting: {e}")
                        return None
                ...
                if 'user' in prompt and prompt['user']:
                    try:
                        ...
                        self.logger.error(f"Missing data key for user prompt formatting: {e}")
                        return None
            ...
            # Handle standard string prompts
                ...
                try:
                    ...
                    self.logger.error(f"Missing data key for prompt formatting: {e}")
                    return None
                    
        except Exception as e:
            self.logger.error(f"Error formatting prompt '{name}': {str(e)}")
            return None
