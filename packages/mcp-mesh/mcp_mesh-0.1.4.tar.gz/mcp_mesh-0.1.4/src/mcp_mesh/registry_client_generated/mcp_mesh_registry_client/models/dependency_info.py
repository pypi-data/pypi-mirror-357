...
class DependencyInfo(BaseModel):
    ...
    capabilities: list[StrictStr] | None = Field(
        default=None, description="Capabilities provided by this dependency"
    )
    version: StrictStr | None = Field(
        default=None, description="Version of the provider agent"
    )
    metadata: dict[str, Any] | None = Field(
        default=None, description="Additional metadata about the provider"
    )
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(["available", "unavailable", "degraded"]):
            raise ValueError(
                "must be one of enum values ('available', 'unavailable', 'degraded')"
            )
        return value
