...
async def analyze_etf_technical(etf_code: str = '510300', with_market_style: bool = True, base_date: str = None, return_days: int = 5) ->dict:
    ...
    if df is not None:
        ...
        return {"results": df.tail(return_days).to_dict(orient="records")}
    else:
        return {"error": "无法获取数据"}

@app.get("/api/stock/{stock_code}", summary="股票技术指标分析")
async def analyze_stock_hist_technical(stock_code: str = '000001', with_market_style: bool = True, base_date: str = None) ->dict:
    """
    获取股票技术指标数据
