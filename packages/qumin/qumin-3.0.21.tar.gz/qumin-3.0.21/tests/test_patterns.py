...
class PatternsTestCase(unittest.TestCase):
    ...
    def test_gen_alt_doesnt_introduce_spaces(self):
        lex1 = (segments.Form("d i"), segments.Form("d a b"))
        lex2 = (segments.Form("s i"), segments.Form("s a b"))
        p1 = patterns.Pattern(("cellA", "cellB"), lex1)
        p2 = patterns.Pattern(("cellA", "cellB"), lex2)
        p1._generalize_alt(p2)
        # Ensure this does not insert empty elements (will end up adding extra spaces)
        for cell in p1.alternation:
            for alt_part in p1.alternation[cell]:
                if len(alt_part) > 1:
                    self.assertNotIn("", alt_part, "Generalizing should not introduce empty strings in alternations")

    def test_forms_normalize_spacing(self):
        for bl in [False, True]:
            forms = [segments.Form("d a ", resegment=bl),
                     segments.Form(" d a", resegment=bl),
                     segments.Form("d  a", resegment=bl)]
            for f in forms:
                self.assertNotIn("", f.tokens, "Forms should not allow empty tokens to be generated")
