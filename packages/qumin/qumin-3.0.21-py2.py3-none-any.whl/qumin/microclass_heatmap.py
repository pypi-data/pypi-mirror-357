...
def distance_matrix(patterns, microclasses, **kwargs):
    ...
    for pair in patterns:
        # We group together the patterns of overabundant lexemes.
        feature_space.loc[exemplars, [pair]] = \
            patterns[pair].groupby('lexeme', observed=False).pattern.apply(
                lambda x: ";".join(sorted(x.map(repr))))
