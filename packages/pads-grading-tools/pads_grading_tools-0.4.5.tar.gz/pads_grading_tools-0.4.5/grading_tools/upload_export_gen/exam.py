...
def gen_rwth(output_file: str, **config):
    ...
    write_rwthonline_csv(rwth_export, complete_file_path(output_file,
                                                         default_file_name=config.pop('DEFAULT_OUTPUT_FILE_NAME',
                                                                                      None)))
...
def gen_moodle(output_file: str, **config):
    ...
    moodle_export, _ = create_moodle_export(considered_grades, moodle_file=config.pop('moodle_file', None), **config)
    write_moodle_csv(moodle_export,
                     complete_file_path(output_file, default_file_name=config.pop('DEFAULT_OUTPUT_FILE_NAME', None)))
...
def create_moodle_export(scores_df: pd.DataFrame,
    ...
    use_feedback = (moodle_feedback_column in moodle_df.columns) and (used_feedback_column in considered_grades.columns)
    exported_cols = [moodle_grading_column, moodle_feedback_column] if use_feedback else [moodle_grading_column]

    # only students listed on moodle can receive a grade there
    moodle_export = pd.merge(considered_grades, moodle_df[
        list(naming_dictionary.MOODLE_EXCEL_IDENTITY_COLS) + [naming_dictionary.MOODLE_EXCEL_MATR_COL,
                                                              naming_dictionary.MOODLE_EXCEL_ID_COL] + exported_cols],
                             how='inner',
                             left_on=naming_dictionary.MATR_COL, right_on=naming_dictionary.MOODLE_EXCEL_MATR_COL)
    ...
    if use_feedback:
        moodle_changed_rows_idx |= moodle_export[moodle_feedback_column] != moodle_export[used_feedback_column]
    proj_cols = [used_grading_column, moodle_grading_column]
    if use_feedback:
        proj_cols += [used_feedback_column, moodle_feedback_column]
    moodle_changed_rows = project(moodle_export.loc[moodle_changed_rows_idx], proj_cols,
    ...
    moodle_overwritten_rows = project(moodle_overwritten_rows, proj_cols, naming_dictionary=naming_dictionary)
    ...
    moodle_export = moodle_export[
        [naming_dictionary.MOODLE_EXCEL_ID_COL,
         naming_dictionary.MATR_COL] + list(naming_dictionary.MOODLE_EXCEL_IDENTITY_COLS) + exported_cols]
