...
from grading_tools.grading_table_gen.grading_table_commons import mk_node_name, skip_in_formula
...
def gen_overview_figs(grades_file: str,
    ...
    melted_scores = df.melt(id_vars=[index_col],
    ...
    melted_grades = df.melt(id_vars=[index_col],
...
def gen_pie_fig(*, grades_file: str, index_column: str, naming_dictionary: NamingDictionary, **config):
    df = read_generic_table(grades_file, table_name=config.pop('grades_table', None),
                            sheet_name=config.pop('grades_sheet', None))
    melted_grades = df.melt(id_vars=[index_column],
                            value_vars=[naming_dictionary.EXAM_GRADE_COL, naming_dictionary.COURSE_GRADE_COL],
                            var_name='Kind',
                            value_name='Grade')
    melted_grades['Category'] = melted_grades['Grade'].map(
        {g: 'passed' for g in get_possible_grades(absent=False)} | {'X': 'no show', '5.0': 'failed'})
    return px.pie(melted_grades, facet_col='Kind', names='Category', title='Exam/Course Outcomes')
...
def gen_per_question_fig(gradable_spec: str, grading_file: str, naming_dictionary: NamingDictionary, **config):
    ...
    figs = {}
    for n in spec.get_level(groups_level):
        faceted_cols = [c for c in n.iter_level(facet_level_index - groups_level_index) if not skip_in_formula(c)]
        facet_rows = len(faceted_cols)
        titles = [mk_node_name(c, start_depth=facet_level_index) for c in faceted_cols]
        if facet_rows > 0:
            fig = make_subplots(rows=facet_rows, subplot_titles=titles, x_title='Points')
            for i, (facet_name, fn) in enumerate(zip(titles, faceted_cols), 1):
                leaves = [mk_node_name(leaf, start_depth=groups_level_index) for leaf in fn.leaves_iter() if
                          not skip_in_formula(leaf)]

                summarized_cols = [col for col in points_df.columns if
                                   any(leaf in col for leaf in leaves)]  # or startswith
                facet_totals = points_df.set_index(index_col)[summarized_cols].sum(axis='columns')
                facet_totals = facet_totals.dropna()
                g = go.Histogram(x=facet_totals,
                                 name=facet_name,
            ...
            fig.update_layout(height=facet_rows * 300,
...
def configure_parser_overview(parser: argparse.ArgumentParser, naming_dictionary: NamingDictionary, **defaults) -> None:
    parser.add_argument('-gr', '--grades-file', required=True, help='Path to the grading/scores file.',
                        dest='grades_file')
    group = parser.add_mutually_exclusive_group(required=False)
...
def configure_base_parser(parser: argparse.ArgumentParser, naming_dictionary: NamingDictionary, **defaults) -> None:
    ...
    parser.add_argument('-ic', '--index-column', required=False, type=str, default=naming_dictionary.MATR_COL,
                        help='Column to use as index, e.g., Matr No or Group ID.')
    parser.add_argument('-hn', '--histnorm',
...
def configure_all(parser: argparse.ArgumentParser, naming_dictionary: NamingDictionary, **defaults) -> None:
    ...
    configure_parser_per_question(parser, naming_dictionary=naming_dictionary, **defaults)
