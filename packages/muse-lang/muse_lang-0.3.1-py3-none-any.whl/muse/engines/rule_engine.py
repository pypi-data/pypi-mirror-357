...
def _extract_fields(expression):
...
def use_rule(rules: dict, df: pl.DataFrame) -> pl.DataFrame:
    ...
    for _, rule in rules.items():
        fields = _extract_fields(rule)
        all_filter_fields = all_filter_fields + fields

    all_filter_fields = list(set(all_filter_fields))
    # 将空值和inf过滤出来
    for col in all_filter_fields:
        inf_df = df.filter(pl.col(col).is_infinite())
        if not inf_df.is_empty():
            inf_df = inf_df.with_columns(pl.lit('除0异常').alias('监控结果'), pl.lit(monitor_rule).alias('监控规则'))
            result_dfs.append(inf_df)
        none_df = df.filter(pl.col(col).is_null() | pl.col(col).is_nan())
        if not none_df.is_empty():
            none_df = none_df.with_columns(pl.lit('其他异常').alias('监控结果'), pl.lit(monitor_rule).alias('监控规则'))
            result_dfs.append(none_df)
    try:
        expressions = {
...
def build_expression(rule_str: str, df_columns: list) -> pl.Expr:
    ...
    for col in sorted(df_columns, key=len, reverse=True):
        rule_logic = rule_str.replace(col, f'pl.col("{col}")')
    ...
    for k, v in results.items():
        if k == '监控规则':
            ...
            if isinstance(v, pl.DataFrame):
                result_col = get_result_col(list(v.columns))
                if result_col:
                    v = v.rename({result_col: k})
            r[k] = v
