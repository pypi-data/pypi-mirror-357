...
def dev(
    ...
    prompt_file: str | None = typer.Option(None, "-pf", "--prompt-file", help="Markdown file containing the prompt"),
    ...
    Examples:
        ...
        evo dev feature-branch -pf refactor-plan.md
    """
    ...
    if prompt and prompt_file:
        typer.echo("Error: Cannot use both -p/--prompt and -pf/--prompt-file options", err=True)
        raise typer.Exit(1)
    ...
    if prompt_file:
        ...
        if not prompt_path.suffix == '.md':
            typer.echo(f"Error: Prompt file must be a markdown file (.md), got '{prompt_path.suffix}'", err=True)
            raise typer.Exit(1)
        try:
            final_prompt = prompt_path.read_text()
        except Exception as e:
            typer.echo(f"Error reading prompt file: {e}", err=True)
            raise typer.Exit(1) from None
    ...
    if runtime == Runtime.local:
        _handle_local_runtime(branch_name, cli_tool, final_prompt, repo_root)
    elif runtime == Runtime.isolated:
        _handle_isolated_runtime(branch_name, cli_tool, final_prompt, repo_root, disable_firewall)
