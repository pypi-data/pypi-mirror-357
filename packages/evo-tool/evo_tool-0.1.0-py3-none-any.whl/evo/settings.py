...
class Settings:
    ...
    def is_completion_installed(self) -> bool:
        """Check if shell completion is already installed."""
        config = self._load_config()
        return config.get("completion_installed", False)

    def mark_completion_installed(self) -> None:
        """Mark that completion has been installed."""
        config = self._load_config()
        config["completion_installed"] = True
        self._save_config(config)

    def auto_install_completion(self) -> None:
        """Automatically install shell completion if not already done."""
        if self.is_completion_installed():
            return
        ...
        try:
            # Try to install completion silently
            shell_name = Path(shell).name
            if shell_name in ["bash", "zsh", "fish"]:
                # Run the completion installation command
                result = subprocess.run(
                    ["evo", "--install-completion"],
                    capture_output=True,
                    text=True,
                    input="y\n",  # Automatically confirm
                )
                if result.returncode == 0:
                    self.mark_completion_installed()
        except Exception:
            # Silently fail if auto-installation doesn't work
            pass
