...
class TestLinus(unittest.TestCase):
    ...
    def test_data(self):
        ...
        """
        filename = get_dataset_path('linus_ref.mat')
        ...
        self.assertEqual(s.shape, outputs[0].shape,
                         'Wrong signal shape in linus')
        assert_array_equal(s, outputs[0], 'Wrong sample values in linus')
        self.assertEqual(fs, outputs[1], 'Wrong sampling frequency in linus')


if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(TestLinus)
