...
root_path = Path(r"D:\2- Postdoc\2- Travaux\3_CWatM_EBR\results\processed_results\PaysBasque\plots\3rd visu")
...
if coords_data == 'auto':
    ...
    coords_df = pd.DataFrame(coords_data, columns = ['coords', 'add_title', 'site'])
...
var_dict = {
    ...
    'discharge': [dict(en = "Discharge", fr = "Débits"), 1, ['m<sup>3</sup>/s', 'm<sup>3</sup>/s']],
    'discharge_local': [dict(en = "Discharge (local)", fr = "Débits (local)"), 100, ['%', '%']],
    'discharge_distant': [dict(en = "Discharge (upstream)", fr = "Débits (amont)"), 1, ['m<sup>3</sup>/s', 'm<sup>3</sup>/s']],
...
if coords_data == 'auto':
    ...
    if len(param_df.crs.unique()) > 1:
        ...
        for gauge in gauge_ds[gauge_column].unique():
            coords_data.append(
                [(gauge_ds[gauge_ds[gauge_column] == gauge].iloc[0].geometry.x,
                  gauge_ds[gauge_ds[gauge_column] == gauge].iloc[0].geometry.y),
                 gauge, 
                 # gauge,
...
if len(run_dict) <= 6:
    color_map = cmg.discrete(sequence_name = 'ibm', alpha = 0.8, black = False,
...
elif (len(run_dict) > 6) & (len(run_dict) <= 9):
...
for var in var_list.index:
    ...
    for i_coords in coords_df.index: # i_coords = count of coords
        ...
        for run in run_dict:
            ...
            # Load results
            results = geo.time_series(
                input_file = res_path / Path(run) / Path(var + '_' + timestep + '.nc'),
                coords = coords_df.loc[i_coords].coords, 
                epsg_coords = epsg_coords,
                epsg_data = epsg_coords
                )  
...
for var in var_list.index:
    ...
    for i_coords in coords_df.index: # i_coords = count of coords
        ...
        if gauge_ds is not None:
            # Reference
            ref = gauge_ds.loc[gauge_ds.code_station == coords_df.loc[i_coords].site, ['time', 'resultat_obs_elab']]
