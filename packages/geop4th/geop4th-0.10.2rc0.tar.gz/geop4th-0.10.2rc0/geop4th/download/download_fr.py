...
def valid_request(response, file_format, prev_frame = None):
    isvalid = False
    frame = prev_frame
...
def valid_single_request(response, file_format):
...
def bnpe(*, dst_folder, start_year = None, end_year = None, masks = None, 
         departments = None, communes = None, file_formats = 'geojson'):
...
def hydrometry(*, dst_folder, masks, start_year, end_year = None, 
                quantities = 'QmnJ', file_formats = 'geojson'):
    """
    ...
    * : TYPE
    ...
    dst_folder : TYPE
        DESCRIPTION.
    masks : (list of) str or pathlib.Paths
        DESCRIPTION.
    start_year : TYPE
        DESCRIPTION.
    end_year : TYPE, optional
        DESCRIPTION. The default is None.
    quantities : (list of) str, optional, default 'QmnJ'
...
def sim2(*,
         var_list, dst_folder: str,
         first_year: int=None, last_year: int=None,
         extension = '.nc', mask: str=None):
    """
    ...
    first_year : int, optional, default None
        Data will be extracted from 1st January of first_year to 31st December of last_year.
        If None, the earliest date will be considered.
    last_year : int, optional, default None
        End date of data to be extracted. 
        If None, the current date will be used instead.
    extension : {'.nc', '.csv'}, optional, default '.nc'
        Output file type. When ``extension = '.nc'``, the ``mask`` can be applied
        to clip data. The ``mask`` will also be taken into account to determine
        which data are necessary to download.
    mask : str or pathlib.Path, optional, default None
        Shapefile path to indicate how to clip
        the netcdf files that are stored on the dst_folder folder.
        Note: The only purpose of using ``mask`` at this step is to save space on disk.

    Returns
    -------
    Create or update the netcdf files.
    """
    ...
    if len(filelist) > 0: # folder is not empty
        for file in filelist:
            ...
            if extension == '.csv':
                ...
                # Dates
                if pd.date_range(start = df_temp.DATE[0], 
                                 end = df_temp.DATE[-1], 
                                 freq = 'D').size == df_temp.DATE.size: # all time values are contiguous
                    local_data.loc[var, 'start_date'] = pd.to_datetime(df_temp.DATE[0])
                    local_data.loc[var, 'end_date'] = pd.to_datetime(df_temp.DATE[-1])
                ...
                with geo.load_any(os.path.join(dst_folder, file), decode_coords = 'all', decode_times = True) as ds_temp:
                    ...
                    if mask is not None:
                        ...
                        mask_gdf = geo.reproject(mask_gdf, dst_crs = ds_temp.rio.crs.to_epsg())
...
# =============================================================================
                    ...
                    if extension == '.csv':
                        ...
                        ds = geo.convert_to_cwatm(df, 'SIM2')
    ...
    if extension == '.csv':
        ...
        print("Merging...")
        merged_df = geo.merge_data(data_to_merge)
        ...
        for var in data_to_merge[0].data_vars: 
            ...
            if mask is not None:
                ...
                for d in data_to_merge:
                    ...
                    clipped_ds.append(geo.reproject(d[[var]], mask = mask))
            ...
            merged_ds = geo.merge_data(clipped_ds)
