        ...
        try:
            ...
            if pricing_info.get('has_pricing'):
                ...
                if pricing_info.get('credits_per_call'):
                    # Convert to float if it's a string (from NUMERIC database field)
                    credits_per_call = pricing_info['credits_per_call']
                    if isinstance(credits_per_call, str):
                        credits_per_call = float(credits_per_call)
                    cost_info['call_credits'] = credits_per_call
                
                # Token-based cost
                if pricing_info.get('credits_per_token') and usage_info.get('total_tokens'):
                    # Convert to float if it's a string (from NUMERIC database field)
                    credits_per_token = pricing_info['credits_per_token']
                    if isinstance(credits_per_token, str):
                        credits_per_token = float(credits_per_token)
                    cost_info['token_credits'] = credits_per_token * usage_info['total_tokens']
