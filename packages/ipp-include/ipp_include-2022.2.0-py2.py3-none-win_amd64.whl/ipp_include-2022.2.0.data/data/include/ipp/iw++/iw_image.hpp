...
public:
    ...
    template <typename T>
    IwiSize(T size // Size of square rectangle
    )
    {
    ...
    IwiSize(IwSize _width, // Width of rectangle
    ...
    IwiSize(IppiSize size // IppiSize structure
    )
    {
    ...
    IwiSize(::IwiSize size // C IwiSize structure
    )
    {
        Set(size.width, size.height);
    }
    ...
    IwiSize(IppiRect rect // IppiRect structure
    )
    {
    ...
    IwiSize(IwiRoi rect // IwiRoi structure
    )
    {
        Set(rect.width, rect.height);
    }
    ...
    void Set(IwSize _width, // Width of rectangle
             IwSize _height // Height of rectangle
    )
    {
        width = _width;
    ...
    inline IwSize Area() const { return IPP_ABS(this->width * this->height); }
...
public:
    ...
    template <typename T>
    IwiPoint(T point // Position of point
    )
    {
    ...
    IwiPoint(IwSize _x, // X coordinate of point
    ...
    IwiPoint(IppiPoint point // IppiPoint structure
    )
    {
        Set(point.x, point.y);
    }
    ...
    IwiPoint(IppiPointL point // IppiPointL structure
    )
    {
        Set(point.x, point.y);
    }
    ...
    // Constructor from IppiRect structure
    IwiPoint(IppiRect rect // IppiRect structure
    )
    {
        Set(rect.x, rect.y);
    }
    ...
    IwiPoint(::IwiRoi rect // C IwiRoi structure
    )
    {
        Set(rect.x, rect.y);
    }
    ...
    void Set(IwSize _x, // X coordinate of point
             IwSize _y  // Y coordinate of point
    )
    {
        x = _x;
...
public:
    ...
    template <typename T>
    IwiRoi(T size // Size of rectangle
    )
    {
    ...
    IwiRoi(IwSize _x,     // X coordinate of rectangle
    ...
    IwiRoi(IppiSize size) { Set(0, 0, size.width, size.height); }

    // Constructor from C IwiSize structure. Sets position to 0 and size to IwiSize value
    IwiRoi(::IwiSize size) { Set(0, 0, size.width, size.height); }

    // Constructor from IwiSize class. Sets position to 0 and size to IwiSize value
    IwiRoi(IwiSize size) { Set(0, 0, size.width, size.height); }

    // Constructor from IppiRect class
    IwiRoi(IppiRect rect) { Set(rect.x, rect.y, rect.width, rect.height); }

    // Constructor from C IwiRoi structure
    IwiRoi(::IwiRoi rect) { Set(rect.x, rect.y, rect.width, rect.height); }

    // Sets rectangle to specified values
    void Set(IwSize _x,     // X coordinate of rectangle
             IwSize _y,     // Y coordinate of rectangle
             IwSize _width, // Width of rectangle
             IwSize _height // Height of rectangle
    )
...
public:
    // Default constructor
    IwiBorderType() { m_type = ippBorderRepl; }

    // Default constructor with border type
    IwiBorderType(::IwiBorderType borderType) { m_type = borderType; }

    // Constructor for borders combination
    IwiBorderType(int borderType) { m_type = (::IwiBorderType)(borderType); }
    ...
    inline void SetType(::IwiBorderType type) { m_type = (::IwiBorderType)(StripType() | (0xF & type)); }

    // Set new border flags without affecting the type
    inline void SetFlags(int flags) { m_type = (::IwiBorderType)(StripFlags() | (flags & (~0xF))); }

    // Return border flags without type
    inline int StripType() const { return (m_type & (~0xF)); }

    // Return border type without flags
    inline IwiBorderType StripFlags() const { return (m_type & 0xF); }
    ...
    bool operator!=(const IwiBorderType &rhs) const { return !(*this == rhs); }
    bool operator==(const ::IwiBorderType &rhs) const
    {
        if (this->m_type == rhs)
            return true;
    ...
    bool operator!=(const ::IwiBorderType &rhs) const { return !(*this == rhs); }
    bool operator!() const { return !(m_type); }
...
public:
    ...
    IwiBorderSize(IwSize _left,  // Size of border to the left
    ...
    IwiBorderSize(::IwiBorderSize border // IwiBorderSize structure
    )
    {
    ...
    IwiBorderSize(IppiBorderSize border // IwiBorderSize structure
    )
    {
    ...
    IwiBorderSize(IwiSize imageSize, // Size of the image
    ...
    IwiBorderSize(IppiMaskSize mask // Processing mask size enumerator
    )
    {
    ...
    IwiBorderSize(IwiSize kernel // Processing kernel size
    )
    {
    ...
    void Set(IwSize _left,  // Size of border to the left
             IwSize _top,   // Size of border to the top
             IwSize _right, // Size of border to the right
             IwSize _bottom // Size of border to the bottom
    )
    ...
    void Set(IwiSize imageSize, // Size of the image
             IwiRoi imageRoi    // Image ROI
    )
    {
        left = imageRoi.x;
    ...
    bool Empty() const { return !(this->left || this->top || this->right || this->bottom); }
...
IW_DECL_CPP(IwiSize)
iwiMaskToSize(IppiMaskSize mask // Kernel or mask size enumerator
)
{
...
IW_DECL_CPP(IwiBorderSize)
iwiSizeToBorderSize(IwiSize kernelSize // Size of kernel as from iwiMaskToSize() or arbitrary
)
{
...
public:
    ...
    IwiImage(const IwiImage &image // Source image
    )
    {
        iwiImage_Init(this);
    ...
    IwiImage(IwiSize size,                                // Image size, in pixels
    ...
    ~IwiImage()
    {
        try {
            ...
            // Release() can throw `ipp::IwException`,
            // but it is prohibited to throw exceptions from destructors by default
        }
    }
    ...
    IppStatus Init(IwiSize size,                                // Image size, in pixels
                   IppDataType dataType,                        // Image pixel type
                   int channels,                                // Number of image channels
                   IwiBorderSize inMemBorder = IwiBorderSize(), // Size of border around image or NULL if there is no border
                   void *pBuffer = NULL,                        // Pointer to the external buffer image buffer
    ...
    // Returns:
    //      ippStsNoErr                         no errors
    IppStatus Alloc(IwiSize size,                               // Image size, in pixels
                    IppDataType dataType,                       // Image pixel type
                    int channels,                               // Number of image channels
                    IwiBorderSize inMemBorder = IwiBorderSize() // Size of border around image or NULL if there is no border
    )
    ...
    inline void *ptr(IwSize y = 0, // y shift, as rows
    ...
    // Returns:
    //      Pointer to the image data
    inline const void *ptrConst(IwSize y = 0, // y shift, as rows
                                IwSize x = 0, // x shift, as columns
                                int ch = 0    // channels shift
    ) const
    {
    ...
    // Returns:
    //      ippStsNoErr                         no errors
    IppStatus RoiSet(ipp::IwiRoi roi // Roi rectangle of the required sub-image
    )
    {
    ...
    IwiImage GetRoiImage(ipp::IwiRoi roi // Roi rectangle of the required sub-image
    ) const
    {
    ...
    inline bool Exists() const { return (this->m_ptrConst) ? true : false; }

    // Returns true if image doesn't have an assigned buffer or its dimensions have zero size
    inline bool Empty() const { return (Exists() && this->m_size.width && this->m_size.height) ? false : true; }
    ...
    {
        if (this->m_dataType == rhs.m_dataType && this->m_channels == rhs.m_channels)
            return true;
        else
            return false;
    }
...
public:
    // Default constructor
    IwiImageArray(const IwiImage &image1 = IwiImage(), const IwiImage &image2 = IwiImage(), const IwiImage &image3 = IwiImage(),
...
public:
    // Default constructor.
    IwiTile() { this->m_initialized = 0; }

    // Constructor with initialization.
    IwiTile(const ::IwiRoi &tileRoi // [in] Tile offset and size
    )
    {
    ...
    // for tile parameters.
    void SetRoi(const ::IwiRoi &tileRoi // [in] Tile offset and size
    )
    {
    ...
    IwiTile &operator=(const ::IwiRoi &tileRoi // [in] Tile offset and size
    )
    {
        SetRoi(tileRoi);
        return *this;
    ...
    static IwiBorderType GetTileBorder(IwiRoi roi,               // Tile position and size
    ...
    // Returns:
    //      Minimal tile size
    static IwiSize GetMinTileSize(::IwiBorderType border,  // Border type
                                  IwiBorderSize borderSize // Border size
    )
    {
    ...
    // Returns:
    //      Corrected ROI
    static IwiRoi CorrectBordersOverlap(IwiRoi tile,              // [in]     Tile position and size to be checked and corrected
...
protected:
    ...
    template <class T> T *CloneT() const
    {
...
public:
    ...
    virtual void *Clone() const { return NULL; }

private:
    // Source ROI getter
    virtual int GetSrcRoi(ipp::IwiRoi, ipp::IwiRoi &) const { return -1; }
};
...
public:
    ...
    //      ippStsNullPtrErr                    unexpected NULL pointer
    IwiTilePipeline(IwiSize tileSizeMax,                               // [in] Maximum tile size for intermediate buffers size calculation
                    ...
                    const IwiTileTransform &transformStruct =
                        IwiTileNoTransform() // [in] Initialized transform structure if operation performs geometric transformation
    )
    {
        this->pTransform = NULL;
        this->m_initialized = 0;
    ...
    //      ippStsBadArgErr                     incorrect arg/param of the function
    //      ippStsNullPtrErr                    unexpected NULL pointer
    IwiTilePipeline(IwiTilePipeline &parent,                           // [in] IwiTile structure of previous operation
                    const IwiBorderType &borderType = IwiBorderType(), // [in] Border type for the current operation
                    const IwiBorderSize &borderSize = IwiBorderSize(), // [in] Border size for the current operation
                    const IwiTileTransform &transformStruct =
                        IwiTileNoTransform() // [in] Initialized transform structure if operation performs geometric transformation
    ...
    // Returns:
    //      ippStsNoErr                         no errors
    IppStatus Init(IwiSize tileSizeMax,                               // [in] Maximum tile size for intermediate buffers size calculation
                   IwiSize dstImageSize,                              // [in] Destination image size for current operation
                   const IwiBorderType &borderType = IwiBorderType(), // [in] Border type for the current operation
                   const IwiBorderSize &borderSize = IwiBorderSize(), // [in] Border size for the current operation
                   const IwiTileTransform &transform =
                       IwiTileNoTransform() // [in] Initialized transform structure if operation performs geometric transformation
    )
    {
        ...
        IppStatus status = ::iwiTilePipeline_Init(this, tileSizeMax, dstImageSize, &border, ((borderSize.Empty()) ? NULL : &borderSize),
                                                  ((pTransform) ? ((const ::IwiTileTransform *)(*pTransform)) : (NULL)));
        OWN_ERROR_CHECK(status);
        return status;
    }
    ...
    // Returns:
    //      ippStsNoErr                         no errors
    IppStatus InitChild(IwiTilePipeline &parent,                           // [in] IwiTile structure of previous operation
                        const IwiBorderType &borderType = IwiBorderType(), // [in] Border type for the current operation
                        const IwiBorderSize &borderSize = IwiBorderSize(), // [in] Border size for the current operation
                        const IwiTileTransform &transform =
                            IwiTileNoTransform() // [in] Initialized transform structure if operation performs geometric transformation
    )
    {
        ...
        if (this->pTransform)
            delete this->pTransform;
        pTransform = (IwiTileTransform *)transform.Clone();
        IppStatus status = ::iwiTilePipeline_InitChild(this, &parent, &border, ((borderSize.Empty()) ? NULL : &borderSize),
                                                       (pTransform) ? ((const ::IwiTileTransform *)(*pTransform)) : NULL);
        OWN_ERROR_CHECK(status);
        return status;
    }
    ...
    // Returns:
    //      ippStsNoErr                         no errors
    IppStatus SetTile(IwiRoi tileRoi // [in] Tile offset and size
    )
    {
    ...
    IwiBorderType GetTileBorder(::IwiBorderType border // [in] Extrapolation algorithm for out of image pixels
    )
    {
        if (!this->m_initialized)
            OWN_ERROR_THROW_ONLY(ippStsContextMatchErr);
    ...
    // Returns:
    //      ippStsNoErr                         no errors
    IppStatus BuildBorder(IwiImage &srcImage,   // [in,out] Pointer to the source image for which to build border
    ...
    IwiSize GetChildSrcImageSize(IwiSize srcOrigSize // [in]     Original source image size
    )
    {
        if (!this->m_initialized)
            OWN_ERROR_THROW_ONLY(ippStsContextMatchErr);
    ...
    // Returns:
    //      Full image size with InMem part
    IwiSize GetChildDstImageSize(IwiSize dstOrigSize // [in]     Original destination image size
    )
    {
        if (!this->m_initialized)
            OWN_ERROR_THROW_ONLY(ippStsContextMatchErr);
