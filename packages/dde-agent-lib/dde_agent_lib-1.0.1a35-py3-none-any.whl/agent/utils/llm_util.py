...
async def handle_stream_output(rid: str, stream, request_increase_mode: bool):
    ...
    for chunk in stream:
        if "original" in log_mode:
            logger.info(f"llm_client, request_increase_mode={request_increase_mode} , rid = {rid} , chunk = {chunk}")
        ...
        if request_increase_mode:
            log_llm_output(content, reasoning_content, rid)
            yield {
                "content": content,
            ...
            log_llm_output(all_content, all_reasoning, rid)
            yield {
                "content": all_content,
...
def log_llm_output(content, reasoning_content, rid: str):
    system_config = get_system_config_from_nacos()
    log_mode = system_config["llm_config"]["log_mode"]
    if "all" in log_mode:
        logger.info(f"llm_client,  rid = {rid} , content = {content} , reasoning_content = {reasoning_content} ")
        ...
        logger.info(f"llm_client,  rid = {rid} , log_content = {log_content} ,  log_reasoning_content = {log_reasoning_content} ")
