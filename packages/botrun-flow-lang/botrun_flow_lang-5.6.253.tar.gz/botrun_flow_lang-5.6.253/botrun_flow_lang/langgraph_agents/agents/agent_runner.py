...
async def langgraph_runner(
    ...
    profile_first_stream_logged = False
    async for event in graph.astream_events(
        ...
        if event["event"] == "on_chat_model_stream":
            # Profiling: First stream response
            if not profile_first_stream_logged:
                profile_first_stream = time.time()
                profile_log(
                    f"[PROFILE] FIRST agent on_chat_model_stream event(LLM 第一個回應): {profile_first_stream - profile_before_stream:.3f}s"
                )
                logging.info(
                    f"[PROFILE] FIRST agent on_chat_model_stream event: {profile_first_stream - profile_before_stream:.3f}s"
                )
                profile_first_stream_logged = True
...
async def agent_runner(
    ...
    async for event in graph.astream_events(
        ...
        if not profile_first_event_logged:
            profile_first_event = time.time()
            profile_log(
                f"[PROFILE] FIRST agent graph.astream_events event(LangGraph 第一個回應): {profile_first_event - profile_before_stream:.3f}s"
            )
            logging.info(
