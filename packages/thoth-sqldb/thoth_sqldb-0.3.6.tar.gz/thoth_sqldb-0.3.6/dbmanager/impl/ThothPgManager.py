...
class ThothPgManager(ThothDbManager):
    ...
    def get_columns(self, table_name: str) -> List[Dict[str, Any]]:
        ...
        pk_columns = inspector.get_pk_constraint(table_name, schema=self.schema).get('constrained_columns', [])
        ...
        with self.engine.connect() as connection:
            for col_meta in columns_metadata:
                ...
                comment = ''
                try:
                    # PostgreSQL specific way to get column comment
                    # Need to get table OID first
                    table_oid_result = connection.execute(text(f"""
                        SELECT c.oid FROM pg_catalog.pg_class c JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname = '{self.schema}' AND c.relname = '{table_name}';
                    """)).scalar()
                    if table_oid_result:
                        comment_result = connection.execute(text(f"""
                            SELECT col_description('{table_oid_result}'::oid, {col_meta['ordinal_position']});
                        """)).scalar()
                        if comment_result:
                            comment = comment_result
                except SQLAlchemyError as e:
                    logging.warning(f"Could not retrieve comment for column {column_name} in table {table_name}: {e}")
                ...
                columns_info.append({
                    ...
                    'ordinal_position': col_meta.get('ordinal_position')
                })
        return columns_info
