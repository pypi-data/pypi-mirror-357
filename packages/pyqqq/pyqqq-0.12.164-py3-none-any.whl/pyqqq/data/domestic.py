...
def _get_tickers(date: dtm.date, market: str = None, adjusted: bool = True):
    ...
    # None, 0 값 처리: upper_limit, lower_limit, previous_close
    mask_upper_limit = df["upper_limit"].isna() | (df["upper_limit"] == 0)
    mask_lower_limit = df["lower_limit"].isna() | (df["lower_limit"] == 0)
    mask_previous_close = df["previous_close"].isna() | (df["previous_close"] == 0)
