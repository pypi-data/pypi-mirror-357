...
# Copyright (C) 2000-2025 Hiroshi Watabe hwatabe@tohoku.ac.jp
#$Id: bldanaimg.py,v 1ae9c20046b5 2025/01/09 07:10:24 watabe $
import numpy
...
def frame_size(hdr):
    ...
    if hdr.dim[4]>1: #4 dimension
        fr_size = hdr.dim[1].astype(numpy.int64)*hdr.dim[2].astype(numpy.int64)*hdr.dim[3].astype(numpy.int64)*hdr.dim[4].astype(numpy.int64)*hdr.bitpix/8
        return(fr_size)
    else:
        fr_size = hdr.dim[1].astype(numpy.int64)*hdr.dim[2].astype(numpy.int64)*hdr.dim[3].astype(numpy.int64)*hdr.bitpix/8
        return(fr_size)
