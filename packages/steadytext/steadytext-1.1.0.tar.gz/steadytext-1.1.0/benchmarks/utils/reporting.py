...
def format_speed_key_findings(results: Dict[str, Any]) -> List[str]:
    ...
    if 'cache_hit' in results and 'cache_miss' in results:
        hit_time = sum(results['cache_hit']['times']) / len(results['cache_hit']['times'])
        miss_time = sum(results['cache_miss']['times']) / len(results['cache_miss']['times'])
        ...
        for op_name, op_data in data['results'].items():
            if 'times' in op_data and op_data['times']:
                row = {
                    ...
                    'mean_time_ms': sum(op_data['times']) / len(op_data['times']) * 1000,
