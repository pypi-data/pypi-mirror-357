...
@click.option(
    "--stdin", is_flag=True, help="Read candidates from stdin"
)
@click.option(
    "--top", type=int, default=1, help="Number of top results to return"
)
@click.option(
    "--metric",
...
def search(
    ...
    if stdin or (not candidates and not sys.stdin.isatty()):
        ...
        click.echo("Error: No candidates provided. Use --stdin or --candidates.", err=True)
        sys.exit(1)
...
def average(texts: Tuple[str, ...], output_json: bool):
    ...
    if output_json:
        ...
        click.echo(f"Average embedding (first 50 values):")
