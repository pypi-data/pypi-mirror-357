...
def demo_index_creation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Get index info
        print("\n=== Index Information ===")
        info_output = run_command(f'st index info python_docs.faiss')
        print(info_output)
...
def demo_index_search():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for query in queries:
            print(f"\nQuery: '{query}'")
            result = run_command(f'st index search docs.faiss "{query}" --top-k 2 --json')
...
def demo_context_generation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = run_command('st generate "What libraries are used for machine learning?" --no-index')
        print(f"Response: {result[:200]}...")
        
        # Generate with context
        print("\nGeneration WITH index:")
        result = run_command('st generate "What libraries are used for machine learning?" --index-file knowledge.faiss')
        print(f"Response: {result[:200]}...")
        
        # Show what context was used
        print("\nContext chunks used:")
        search_result = run_command('st index search knowledge.faiss "machine learning libraries" --top-k 3 --json')
...
def demo_incremental_index():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        output = run_command(f'st index create {tmpdir}/*.txt --output my_index.faiss --force')
