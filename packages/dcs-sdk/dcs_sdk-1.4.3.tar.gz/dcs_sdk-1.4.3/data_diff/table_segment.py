...
class TableSegment:
    ...
    def get_sample_data(self, limit: int = 100) -> list:
        ...
        if self.where:
            select = (
                self.source_table.select(*self._relevant_columns_repr).where(self.where).order_by(*exprs).limit(limit)
            )
        else:
            select = self.source_table.select(*self._relevant_columns_repr).order_by(*exprs).limit(limit)
