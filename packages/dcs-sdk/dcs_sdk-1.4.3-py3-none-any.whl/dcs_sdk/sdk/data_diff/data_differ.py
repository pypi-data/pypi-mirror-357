...
class DBTableDiffer:
    ...
    def diff_tables(
        ...
        self.config.advanced_configuration.bisection_threshold = max(
            self.config.advanced_configuration.bisection_threshold, 1000
        )
        ...
        columns_with_unmatched_data_type, columns_not_compared, exc_to_src, exc_to_tgt = calculate_column_differences(
        ...
        if not is_value_mismatch:
            table_1_sample_data = convert_to_masked_if_required(
                table_sample_data=table_1_sample_data,
                masking_character=masking_character,
                masking_columns=src_masking_cols,
                columns_order_wise=columns_order_wise_src,
            )

            table_2_sample_data = convert_to_masked_if_required(
                table_sample_data=table_2_sample_data,
                masking_character=masking_character,
                masking_columns=tgt_masking_cols,
                columns_order_wise=columns_order_wise_target,
            )
            ...
            def get_composite_key(row, key_columns):
                return tuple(row[k] for k in key_columns)

            def map_rows_by_pk(dicts, key_columns):
                return {get_composite_key(row, key_columns): row for row in dicts}

            def build_comparison_record_list(source_map, target_map):
                sample_values_record_list = []
                for pk in source_map:
                    if pk in target_map:
                        sample_values_record_list.append(source_map[pk])
                        sample_values_record_list.append(target_map[pk])
                return sample_values_record_list

            sample_value_source_map = map_rows_by_pk(sample_value_source_dicts, self.table1.key_columns)
            sample_value_target_map = map_rows_by_pk(sample_value_target_dicts, self.table2.key_columns)

            sample_values_record_list = build_comparison_record_list(sample_value_source_map, sample_value_target_map)
