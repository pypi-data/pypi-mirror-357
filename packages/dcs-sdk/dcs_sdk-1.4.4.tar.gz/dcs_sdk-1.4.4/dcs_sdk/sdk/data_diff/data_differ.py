...
class DBTableDiffer:
    ...
    def diff_tables(
        ...
        if not is_value_mismatch:
            ...
            for pk in grouped_source.keys() | grouped_target.keys():
                ...
                remaining_sources_sorted = sorted(
                    remaining_sources, key=lambda row: sort_key(row, self.table1.key_columns, self.table1.extra_columns)
                )

                remaining_targets_sorted = sorted(
                    remaining_targets, key=lambda row: sort_key(row, self.table2.key_columns, self.table2.extra_columns)
                )
