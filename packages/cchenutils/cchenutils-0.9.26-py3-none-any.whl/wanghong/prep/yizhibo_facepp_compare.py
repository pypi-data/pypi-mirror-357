...
if __name__ == '__main__':
    ...
    fp_dst = r"D:\yizhibo_facepp_compare.csv"
    ...
    if os.path.exists(fp_sample):
        ...
        df = df[df['probs'] > 0.99]
        df_sampled = df.groupby(['Yizhibo_UID', 'Yizhibo_VID']) \
            .sample(3, replace=True, random_state=99) \
            .drop_duplicates() \
    ...
    if os.path.exists(fp_center):
        ...
        df_center = df_sampled.groupby(['Yizhibo_UID', 'Yizhibo_VID']) \
            .sample(3, replace=True, random_state=99) \
            .drop_duplicates() \
            .reset_index(drop=True)
    ...
    if not (writer_header := not os.path.exists(fp_dst)):
        ...
        read = set(df_done.apply(
            lambda x: (f'D:/img/{x["Yizhibo_UID"][2:]}/{x["Yizhibo_VID_c"][2:]}/{x["snippet_c"]:05d}.zip',
                       f'D:/img/{x["Yizhibo_UID"][2:]}/{x["Yizhibo_VID_p"][2:]}/{x["snippet_p"]:05d}.zip'),
            axis=1))
    ...
    with open(fp_dst, 'a', encoding='utf-8') as o, tqdm(total=len(p1p2)) as bar:
        ...
        if writer_header:
            csvwriter.writerow(['Yizhibo_UID',
                                'Yizhibo_VID_c', 'snippet_c', 'frame_c',
                                'Yizhibo_VID_p', 'snippet_p', 'frame_p',
                                'confidence', 'threshold-3', 'threshold-4', 'threshold-5',
                                'face_tokens_c', 'face_tokens_p', 'image_id_c', 'image_id_p', 'request_id'])
        for uid in set(df_center['Yizhibo_UID']):
            this_center = df_center[df_center['Yizhibo_UID'] == uid]
            this_point = df_sampled[df_sampled['Yizhibo_UID'] == uid]
            for _, row1 in this_center.iterrows():
                for _, row2 in this_point.iterrows():
                    ...
                    if path1 == path2 or (path1, path2) in read:
                        continue
                    with ZipFile(path1) as myzip1, myzip1.open(f'{row1["frame"]:05d}.jpg') as myfile1, \
                            ZipFile(path2) as myzip2, myzip2.open(f'{row2["frame"]:05d}.jpg') as myfile2:
                        ...
                        csvwriter.writerow([
                            uid,
                            row1["Yizhibo_VID"], row1["snippet"], row1["frame"],
                            row2["Yizhibo_VID"], row2["snippet"], row2["frame"],
                            data['confidence'],
                            data['thresholds']['1e-3'], data['thresholds']['1e-4'], data['thresholds']['1e-5'],
                            '|'.join(face['face_token'] for face in data['faces1']),
                            '|'.join(face['face_token'] for face in data['faces2']),
                            data['image_id1'], data['image_id2'], data['request_id']
                        ])
