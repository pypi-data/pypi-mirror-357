...
def make_method(tests, test_index, case_index):
    def method(self):
...
for test in tests:
    for test_index, test in enumerate(tests):
        ...
        for case_index, test_case in enumerate(test.cases):
            setattr(TestVisualTests, f"test_visual__{test_slug}__{test_case.name}", make_method(tests, test_index, case_index))
