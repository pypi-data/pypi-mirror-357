...
def test_timeout_mechanism():
    print("测试 find_half_energy_time 函数的超时机制")
    ...
    result1 = rust_pyfunc.find_half_energy_time(times, prices, time_window=5.0)
    end_time = time.time()
    ...
    result2 = rust_pyfunc.find_half_energy_time(times, prices, time_window=5.0, timeout_seconds=60)
    end_time = time.time()
    print(f"   结束时间: {datetime.now().strftime('%H:%M:%S')}, 耗时: {end_time - start_time:.2f}秒")
    ...
    result3 = rust_pyfunc.find_half_energy_time(times, prices, time_window=5.0, timeout_seconds=0.1)
    end_time = time.time()
    print(f"   结束时间: {datetime.now().strftime('%H:%M:%S')}, 耗时: {end_time - start_time:.2f}秒")
    ...
    result4 = rust_pyfunc.find_half_energy_time(times[:1000], prices[:1000], time_window=5.0, direction="pos")
    end_time = time.time()
    print(f"   结束时间: {datetime.now().strftime('%H:%M:%S')}, 耗时: {end_time - start_time:.2f}秒")
    ...
    result5 = rust_pyfunc.find_half_energy_time(times[:1000], prices[:1000], time_window=5.0, direction="neg")
    end_time = time.time()
    print(f"   结束时间: {datetime.now().strftime('%H:%M:%S')}, 耗时: {end_time - start_time:.2f}秒")
