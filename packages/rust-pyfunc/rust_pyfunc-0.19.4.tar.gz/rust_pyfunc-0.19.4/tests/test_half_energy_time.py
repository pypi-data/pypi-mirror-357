...
def test_with_real_data():
    ...
    # 测试 Rust 实现
    start = time.time()
    rust_result = rust_find_half_energy_time(times, prices, time_window=3600*24)  # 24小时窗口
    rust_time = time.time() - start
    print(f"Rust 实现耗时: {rust_time:.4f} 秒")
    
    # 测试 Python 实现
    start = time.time()
    python_result = python_find_half_energy_time(times, prices, time_window=3600*24)
...
def test_with_synthetic_data():
    ...
    # 测试 Rust 实现
    start = time.time()
    rust_result = rust_find_half_energy_time(times, prices, time_window=60)  # 60秒窗口
    rust_time = time.time() - start
    print(f"Rust 实现耗时: {rust_time:.4f} 秒")
    
    # 测试 Python 实现
    start = time.time()
    python_result = python_find_half_energy_time(times, prices, time_window=60)
    python_time = time.time() - start
    print(f"Python 实现耗时: {python_time:.4f} 秒")
...
def test_edge_cases():
    ...
    rust_result = rust_find_half_energy_time(times, prices, time_window=3.0)
    python_result = python_find_half_energy_time(times, prices, time_window=3.0)
    ...
    print("3. 空数组处理测试")
    try:
        rust_result = rust_find_half_energy_time(np.array([]), np.array([]), time_window=1.0)
        print(f"Rust结果: {rust_result}, 长度: {len(rust_result)}")
    except Exception as e:
        print(f"Rust异常: {e}")
        
    try:
        python_result = python_find_half_energy_time(np.array([]), np.array([]), time_window=1.0)
    ...
    rust_result = rust_find_half_energy_time(times, prices, time_window=1.0)
    python_result = python_find_half_energy_time(times, prices, time_window=1.0)
...
def test_performance_scaling():
    ...
    for size in sizes:
        ...
        # 测试 Rust 实现
        start = time.time()
        rust_find_half_energy_time(times, prices, time_window=60)  # 60秒窗口
        rust_time = time.time() - start
...
if __name__ == "__main__":
    print("=== 开始测试 find_half_energy_time 函数 ===")
