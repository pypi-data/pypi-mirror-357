...
class TestAesGcmCipher(unittest.TestCase):
    ...
    def test_encrypt_decrypt(self):
        ciphertext = self.cipher.encrypt(self.plaintext, tag)
        ...
        decrypted = self.cipher.decrypt(ciphertext, tag)
        self.assertEqual(decrypted, self.plaintext)

    def test_different_ciphertexts_for_same_plaintext(self):
        ciphertext1 = self.cipher.encrypt(self.plaintext, tag)
        ciphertext2 = self.cipher.encrypt(self.plaintext, tag)
