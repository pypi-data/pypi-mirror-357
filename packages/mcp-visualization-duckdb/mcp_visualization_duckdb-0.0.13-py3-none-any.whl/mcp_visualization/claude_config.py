...
class ClaudeDesktopConfigManager:
    ...
    def create_server_config(self, 
        ...
        if database_path is not None:
            ...
            if create_sample_db:
                print(f"📊 Skipping sample database creation due to known DuckDB connection issues on Windows")
                print(f"⚠️  You can create databases manually or connect to existing databases via Claude Desktop")
                print(f"💡 Tip: Try using option 2 (No default database) instead")
                # Remove the database environment variable since we're not creating it
                if "env" in config and "DUCKDB_DATABASE_PATH" in config["env"]:
                    del config["env"]
        else:
