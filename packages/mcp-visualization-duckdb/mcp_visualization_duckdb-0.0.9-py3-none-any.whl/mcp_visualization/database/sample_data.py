...
def create_sample_database(db_path: Optional[str] = None) -> str:
    ...
    try:
        ...
        if db_file.exists():
            print(f"📋 Database file already exists, checking if accessible...")
            try:
                # Quick test connection
                test_conn = duckdb.connect(db_path)
                test_conn.close()
                print(f"✅ Database file is accessible")
            except Exception as e:
                print(f"⚠️ Database file exists but may be locked: {e}")
        ...
        try:
            print(f"🔄 Attempting simple database creation...")
            with duckdb.connect(db_path) as conn:
                ...
                print(f"✅ Created minimal database successfully")
            return db_path
        except Exception as e2:
            logger.error(f"Even simple database creation failed: {e2}")
            print(f"❌ Simple database creation also failed: {e2}")
