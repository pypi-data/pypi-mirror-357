...
@pytest.fixture(scope="module")
def test_query0():
    return TQuery(attr1=["value1"])


@pytest.fixture(scope="module")
def test_query1():
    return TQuery(id=[2], attr1=["value1"], attr2=["value3"])


@pytest.fixture(scope="module")
def test_query2():
    return TQuery(id=[3], attr1=["value3"], attr2=["value2"])


@pytest.mark.parametrize(
    "query, expected",
    [
        (TQuery(attr1="value1"), (["value1"], [])),
        (TQuery(attr2="value2"), ([], ["value2"])),
    ],
)
...
def test_query_convert_attr():
    converted = TQuery.convert_attr({"id": 0, "attr1": "test"}, "attr4")
...
def test_query_from_kwargs():
    ...
    query = TQuery.from_kwargs(**kwargs)
