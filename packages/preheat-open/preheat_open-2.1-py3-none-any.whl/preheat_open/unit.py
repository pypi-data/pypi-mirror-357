...
class Component:
    ...
    max: float = None
    unit: str = ""
    measurements: dict[TimeResolution, LoadableData] = field(
        default_factory=_standard_measurement_dict
    )
    std_unit: str = ""
    std_unit_divisor: float = 1.0
    parent: Unit | Device = None
    ...
    def get_measurement(
        ...
        measurements = self.measurements[date_range.resolution].get(
    ...
    def __eq__(self, other: Component) -> bool:
        ...
        return (
            ...
            and self.parent.location.id == other.parent.location.id
        )
...
class Device:
    ...
    def __post_init__(self):
        """
        Post-initialization processing to set up components.
        """
...
class ControlSetting:
    ...
    :vartype unit: str
    :ivar name: The name of the control setting.
    :vartype name: str
    """
...
class ControlSettings:
    ...
    :ivar settings: A list of control settings.
    :vartype settings: list[ControlSetting]
    """

    settings: dict[str, ControlSetting]
    parent: Unit = None
    ...
    @classmethod
    def from_building_model_dict(cls, data: dict, adapter: Adapter) -> ControlSettings:
        """
...
class Unit(Device):
    ...
    descriptors: UnitDescriptors = field(default_factory=UnitDescriptors)
    zones: list[Zone] = field(default_factory=list)
    shared: bool = False
    shared_from: int | None = None
    shared_locations: list[int] = field(default_factory=list)
    parent: Unit | None = None
    children: list[Unit] = field(default_factory=list)
    related_units: list[Unit] = field(default_factory=list)
    covers_building: bool = None
    buildings_covered: int = None
    ...
    def __post_init__(self):
        """
        Post-initialization processing to set up child units.
        """
        super().__post_init__()
    ...
    def get_sub_units(
        ...
        **kwargs,
        ...
        yield from query_stuff(
            obj=self,
            queried_type=Unit,
            sub_obj_attrs=["children"]
            if include_self
            else ["children", "related_units"],
            sub_obj_attrs_for_removal=["related_units"],
            query=query,
            query_type=self.location.adapter.building_model.unit_query_type,
            include_obj=include_self,
