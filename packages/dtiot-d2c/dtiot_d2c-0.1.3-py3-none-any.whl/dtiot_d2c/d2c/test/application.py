...
def verify_application(api:ApiClient, name:str, props:dict):
    ...
    elem_path = "lbl"
    elem_value = f'type:{props.get("type", APPL_TYPE_DEFAULT)}'
    def operation():
        if not tutils.call_has_element_value(onem2m_dict, elem_path, elem_value):
            raise Exception(f"Value of element {elem_path} does not meet required value {elem_value}.")
    tutils.exec_test(f"Verifing {elem_path} for {elem_value}", operation)

    for (key, val) in props.get("connection_properties", {}).items():
        elem_path = "lbl"
    ...
    elem_path = "poa"
    elem_value = props.get("urls", None)
    def operation():
        if not tutils.call_has_element_value(onem2m_dict, elem_path, elem_value):
            raise Exception(f"Value of element {elem_path} does not meet required value {elem_value}.")
...
class TestCase_CreateApplication(TestCase):
    ...
    def run(self):
        if not (props := self.cfg.get("create_props", None)):
...
# MARK: Tests 00
###    
class Application00Tests(TestSet):
    def __init__(self, api: ApiClient):
        super().__init__(api, "Application Tests 01")
    
    def get_sample_configuration_varname(self)->str:
        return "Application01Cfg"

    def get_test_cases(self, cfg:dict)->List[TestCase]:
        return [
            TestCase_CreateApplication(api=self.api, cfg=cfg),
            TestCase_ModifyApplication(api=self.api, cfg=cfg),
        ]
    
    def test_pre_cleanup(self, cfg:dict)->str:
        delete_application(self.api, cfg.get("name"))

    def test_post_cleanup(self, cfg:dict)->str:
        delete_application(self.api, cfg.get("name"))
    
        
class FunctionalTest_Application00CLI(TestCLI_Base):
    def get1LineHelp(self):
        return "Runs the application test case: create, modify."

    def get_test_set(self, api: ApiClient):
        return Application00Tests(api)


###
# MARK: Tests 01
###    
class Application01Tests(TestSet):
    def __init__(self, api: ApiClient):
        super().__init__(api, "Application Tests 01")
    ...
    def get_test_cases(self, cfg:dict)->List[TestCase]:
        return [
            TestCase_CreateApplication(api=self.api, cfg=cfg),
            TestCase_ModifyApplication(api=self.api, cfg=cfg),
            TestCase_DeleteApplication(api=self.api, cfg=cfg)
        ]
...
class FunctionalTest_Application01CLI(TestCLI_Base):
    def get1LineHelp(self):
        return "Runs the application test case: create, modify, delete application."

    def get_test_set(self, api: ApiClient):
        return Application01Tests(api)
