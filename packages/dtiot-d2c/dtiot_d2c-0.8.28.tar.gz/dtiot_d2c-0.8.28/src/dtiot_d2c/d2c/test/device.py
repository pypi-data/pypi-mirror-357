...
def verify_device(api: ApiClient, name:str, device_props:dict):
    ...
    for (key, val) in [["profile", DEVICE_PROFILES.SCS_LWM2M],
                        ...
                        *[(k, v) for (k, v) in device_props.get("device_properties", {}).items()],
                        *[(f"UplinkProperty_{k}", v) for (k, v) in device_props.get("uplink_properties", {}).items()]
    ]:
        if val == None:
            continue
        elem_path = "lbl"
...
class TestCase_CreateDevice(TestCase):
    ...
    def run(self):
        ...
        if not (create_props := self.cfg.get("create_props", None)):
