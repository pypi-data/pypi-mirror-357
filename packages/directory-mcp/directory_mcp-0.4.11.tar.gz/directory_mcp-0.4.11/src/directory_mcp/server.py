"""MCP server implementation for Directory v0.4.11."""
import os
import sys
...
mcp = FastMCP("directory-mcp", version="0.4.11")
...
def run_server():
    """Run the MCP server."""
    try:
        # Initialize the server components first
        import asyncio
        asyncio.run(initialize())
        
        # Run the MCP server (sync function)
        mcp.run()
