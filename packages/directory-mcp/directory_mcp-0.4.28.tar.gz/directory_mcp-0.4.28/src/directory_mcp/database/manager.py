...
class DatabaseManager:
    ...
    async def get_person(self, person_id: int) -> Optional[Dict[str, Any]]:
        """Get a person by ID."""
        query = """
        SELECT person_id, names, email, secondary_emails, company_id, title, 
               department, location, timezone, handles, aliases, bio, skills, 
    ...
    async def get_company(self, company_id: int) -> Optional[Dict[str, Any]]:
        """Get a company by ID."""
        query = """
        SELECT company_id, name, domain, domains, industry, size, location_hq, 
    ...
    async def get_channel(self, channel_id: int) -> Optional[Dict[str, Any]]:
        """Get a channel by ID."""
        query = """
        SELECT channel_id, platform, ext_id, name, type, purpose, owner_id, 
               company_id, project_id, is_active, member_count, created_at, updated_at 
    ...
    async def get_project(self, project_id: int) -> Optional[Dict[str, Any]]:
        """Get a project by ID."""
        query = """
        SELECT project_id, name, code, description, status, company_id, lead_id, 
               start_date, end_date, tags, urls, created_at, updated_at 
    ...
    async def add_person_to_channel(
        ...
        """Add a person to a channel with a specific role."""
    ...
    async def add_person_to_project(
        ...
        """Add a person to a project with role and allocation."""
        return await self.create_relationship(
            "person_project",
            person_id=person_id,
    ...
    async def add_person_to_company(
        ...
        """Add a person to a company with employment details."""
        relationship_data = {
            "person_id": person_id,
        ...
        if start_date:
            relationship_data["start_date"] = start_date
        if end_date:
            relationship_data["end_date"] = end_date
            
        return await self.create_relationship("person_company", **relationship_data)
    ...
    async def update_company(self, company_id: int, **kwargs) -> bool:
        ...
        if 'domains' in kwargs and isinstance(kwargs['domains'], list):
            kwargs['domains'] = json.dumps(kwargs['domains'])
        if 'tags' in kwargs and isinstance(kwargs['tags'], list):
            kwargs['tags'] = json.dumps(kwargs['tags'])
        
        set_clause = ', '.join([f"{key} = ?" for key in kwargs.keys()])
        query = f"UPDATE company SET {set_clause} WHERE company_id = ?"
        
        rows_affected = await self.execute_update(
            query, 
            tuple(list(kwargs.values()) + [company_id])
        )
    ...
    async def update_channel(self, channel_id: int, **kwargs) -> bool:
        ...
        set_clause = ', '.join([f"{key} = ?" for key in kwargs.keys()])
        query = f"UPDATE channel SET {set_clause} WHERE channel_id = ?"
        
        rows_affected = await self.execute_update(
            query, 
            tuple(list(kwargs.values()) + [channel_id])
        )
        return rows_affected > 0
    ...
    async def update_project(self, project_id: int, **kwargs) -> bool:
        ...
        if 'tags' in kwargs and isinstance(kwargs['tags'], list):
            kwargs['tags'] = json.dumps(kwargs['tags'])
        if 'urls' in kwargs and isinstance(kwargs['urls'], dict):
            kwargs['urls'] = json.dumps(kwargs['urls'])
        
        set_clause = ', '.join([f"{key} = ?" for key in kwargs.keys()])
        query = f"UPDATE project SET {set_clause} WHERE project_id = ?"
        
        rows_affected = await self.execute_update(
            query, 
            tuple(list(kwargs.values()) + [project_id])
        )
        return rows_affected > 0
    ...
    async def search_people(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Search for people using text search."""
        return await self.search_entities('person', query, limit)
    
    async def search_companies(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Search for companies using text search."""
        return await self.search_entities('company', query, limit)
    
    async def search_channels(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Search for channels using text search."""
        return await self.search_entities('channel', query, limit)
    
    async def search_projects(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Search for projects using text search."""
        return await self.search_entities('project', query, limit)
    
    async def find_people_by_criteria(self, criteria: Dict[str, str], limit: int = 20) -> List[Dict[str, Any]]:
        ...
        if 'company' in criteria:
            ...
            where_clauses.append("""
                person_id IN (
                    ...
                    WHERE (c.name LIKE ? OR c.domain LIKE ?) AND pc.is_current = TRUE
                )
            """)
            company_pattern = f"%{criteria['company']}%"
        ...
        if 'skills' in criteria:
            ...
            params.append(f"%{criteria['skills']}%")
    ...
    async def get_person_channels(self, person_id: int) -> List[Dict[str, Any]]:
        ...
        query = """
        SELECT c.channel_id, c.platform, c.ext_id, c.name, c.type, c.purpose,
               c.owner_id, c.company_id, c.project_id, c.is_active, c.member_count,
               c.created_at, c.updated_at, pc.role, pc.joined_at, pc.is_active as membership_active
        FROM channel c
    ...
    async def get_person_projects(self, person_id: int) -> List[Dict[str, Any]]:
        ...
        query = """
        SELECT p.project_id, p.name, p.description, p.status, p.company_id,
               p.start_date, p.end_date, p.tags, p.urls,
               p.created_at, p.updated_at, pp.role, pp.allocation, pp.start_date as member_start_date, 
               pp.end_date as member_end_date, pp.is_active
        FROM project p
    ...
    async def get_person_connections(self, person_id: int) -> Dict[str, Any]:
        """Get all connections for a person (companies, projects, channels)."""
        ...
        companies = []
        company_query = """
        SELECT c.company_id, c.name, c.domain, c.domains, c.industry, c.size, 
               c.location_hq, c.description, c.tags, c.created_at, c.updated_at,
               pc.role, pc.start_date, pc.end_date, pc.is_current
        FROM company c
        JOIN person_company pc ON c.company_id = pc.company_id
        WHERE pc.person_id = ?
        """
        company_results = await self.execute_query(company_query, (person_id,))
        ...
        # Get projects
        projects = await self.get_person_projects(person_id)
        
        # Get channels
        channels = await self.get_person_channels(person_id)
    ...
    async def get_company_people(self, company_id: int, exclude_id: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        query = """
        SELECT p.person_id, p.names, p.email, p.secondary_emails, p.company_id, 
               p.title, p.department, p.location, p.timezone, p.handles, p.aliases, 
               p.bio, p.skills, p.created_at, p.updated_at, pc.role, pc.is_current
        FROM person p
        JOIN person_company pc ON p.person_id = pc.person_id
    ...
    async def get_company_projects(self, company_id: int) -> List[Dict[str, Any]]:
        """Get all projects for a company."""
        query = """
        SELECT p.project_id, p.name, p.description, p.status, p.company_id,
               p.start_date, p.end_date, p.tags, p.urls,
               p.created_at, p.updated_at, COUNT(pp.person_id) as team_size
        FROM project p
        LEFT JOIN person_project pp ON p.project_id = pp.project_id AND pp.is_active = TRUE
        WHERE p.company_id = ?
    ...
    async def get_company_channels(self, company_id: int) -> List[Dict[str, Any]]:
        """Get all channels for a company."""
        query = "SELECT * FROM channel WHERE company_id = ?"
        return await self.execute_query(query, (company_id,))
    ...
    async def get_project_people(self, project_id: int) -> List[Dict[str, Any]]:
        ...
        SELECT p.person_id, p.names, p.email, p.secondary_emails, p.company_id, 
               p.title, p.department, p.location, p.timezone, p.handles, p.aliases, 
               p.bio, p.skills, p.created_at, p.updated_at, 
               pp.role, pp.allocation, pp.start_date, pp.end_date, pp.is_active
        FROM person p
    ...
    async def get_channel_people(self, channel_id: int) -> List[Dict[str, Any]]:
        """Get all people in a channel."""
        query = """
        SELECT p.person_id, p.names, p.email, p.secondary_emails, p.company_id, 
               p.title, p.department, p.location, p.timezone, p.handles, p.aliases, 
               p.bio, p.skills, p.created_at, p.updated_at,
               pc.role, pc.joined_at, pc.is_active
        FROM person p
        JOIN person_channel pc ON p.person_id = pc.person_id
        WHERE pc.channel_id = ? AND pc.is_active = TRUE
        """
        results = await self.execute_query(query, (channel_id,))
        for result in results:
            result['names'] = json.loads(result['names'])
    ...
    async def get_project_channels(self, project_id: int) -> List[Dict[str, Any]]:
        ...
        query = """
        SELECT channel_id, platform, ext_id, name, type, purpose,
               owner_id, company_id, project_id, is_active, member_count,
               created_at, updated_at
        FROM channel WHERE project_id = ?
        """
