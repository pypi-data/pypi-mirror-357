...
def register_project_tools(mcp: FastMCP, db, config, embedder):
    ...
    async def create_project(
        ...
        try:
            ...
            # Map priority to tags since priority isn't a database field
            tags = []
            if priority:
                tags.append(f"priority:{priority.lower()}")
            if budget:
                # Store budget in URLs field as metadata
                urls = {"budget": str(budget)}
            else:
                urls = None
            ...
            # Remove binary vector data for JSON serialization
            if project and 'vector' in project:
                project.pop('vector', None)
