...
def register_relationship_tools(mcp: FastMCP, db, config, embedder):
    ...
    async def assign_person_to_project(
        ...
        try:
            ...
            # Assign person to project (using existing method)
            # Convert allocation from float to integer percentage
            allocation_int = int(allocation * 100) if allocation else 50
            await db.add_person_to_project(person_id, project_id, role.lower(), allocation_int)
    ...
    async def link_person_to_company(
        ...
        try:
            ...
            # Link person to company (using existing method)
