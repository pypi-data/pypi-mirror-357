...
def register_company_tools(mcp: FastMCP, db, config, embedder):
    ...
    async def create_company(
        ...
        try:
            ...
            if company and 'vector' in company:
                company.pop('vector', None)
    ...
    async def get_company_overview(company: str) -> dict:
        ...
        try:
            ...
            # Remove binary vector data for JSON serialization
            if 'vector' in company_data:
                company_data.pop('vector', None)
            
            # Get all people at this company
            people = await db.get_company_people(company_id)
            
            # Remove vectors from people
            ...
            # Remove vectors from projects and channels
            for project in projects:
                if 'vector' in project:
