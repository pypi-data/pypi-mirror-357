...
def register_channel_tools(mcp: FastMCP, db, config, embedder):
    ...
    async def create_channel(
        ...
        try:
            ...
            # Use channel name as ext_id for now
            channel_id = await db.create_channel(
                platform=platform.strip().lower(),
                ext_id=name.strip(),  # Using name as external ID
                name=name.strip(),
                type=channel_type.strip().lower() if channel_type else "public",
                purpose=description.strip() if description else None,
            ...
            if channel and 'vector' in channel:
                channel.pop('vector', None)
