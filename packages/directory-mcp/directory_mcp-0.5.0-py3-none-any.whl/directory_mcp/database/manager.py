"""Database manager for Directory MCP."""
import aiosqlite
import json
from datetime import datetime
from pathlib import Path
from typing import List, Dict, Any, Optional, Union
from contextlib import asynccontextmanager
import logging

from .schema import SCHEMA
from .migration import migrate_to_enhanced_fts5

logger = logging.getLogger(__name__)

class DatabaseManager:
    """Manages all database operations."""
    
    def __init__(self, db_path: Path):
        self.db_path = db_path
        self._connection = None
    
    async def initialize(self):
        """Initialize the database with schema and run migrations."""
        # Create parent directory if it doesn't exist
        self.db_path.parent.mkdir(parents=True, exist_ok=True)
        
        # Check if database exists to determine if this is a fresh install
        is_fresh_install = not self.db_path.exists()
        
        # Execute schema
        async with self._get_connection() as conn:
            await conn.executescript(SCHEMA)
            await conn.commit()
        
        # Run migrations for existing databases
        if not is_fresh_install:
            logger.info("Running database migrations...")
            try:
                migration_success = await migrate_to_enhanced_fts5(self.db_path)
                if migration_success:
                    logger.info("Database migrations completed successfully")
                else:
                    logger.warning("Database migrations failed - continuing with existing schema")
            except Exception as e:
                logger.warning(f"Migration error (continuing): {e}")
        else:
            logger.info("Fresh database install - skipping migrations")
        
        logger.info(f"Database initialized at {self.db_path}")
    
    @asynccontextmanager
    async def _get_connection(self):
        """Get a database connection with proper context management."""
        conn = await aiosqlite.connect(str(self.db_path))
        conn.row_factory = aiosqlite.Row
        try:
            yield conn
        finally:
            await conn.close()
    
    async def execute_query(self, query: str, params: tuple = ()) -> List[Dict[str, Any]]:
        """Execute a query and return results as list of dicts."""
        async with self._get_connection() as conn:
            async with conn.execute(query, params) as cursor:
                rows = await cursor.fetchall()
                return [dict(row) for row in rows]
    
    async def execute_update(self, query: str, params: tuple = ()) -> int:
        """Execute an update query and return affected rows."""
        async with self._get_connection() as conn:
            cursor = await conn.execute(query, params)
            await conn.commit()
            return cursor.rowcount
    
    async def execute_insert(self, query: str, params: tuple = ()) -> int:
        """Execute an insert query and return the last row id."""
        async with self._get_connection() as conn:
            cursor = await conn.execute(query, params)
            await conn.commit()
            return cursor.lastrowid
    
    # Person CRUD operations
    async def _check_person_duplicates(self, email: Optional[str], names: List[str]) -> Optional[int]:
        """Check for duplicate persons and return existing person_id if found."""
        if email:
            # Check for exact email match (highest priority)
            query = "SELECT person_id FROM person WHERE email = ?"
            results = await self.execute_query(query, (email,))
            if results:
                logger.info(f"Found existing person with email: {email}")
                return results[0]['person_id']
        
        # Only check for name duplicates if no email was provided
        # (Different people can have same name if they have different emails)
        if not email and names:
            primary_name = names[0]
            # Use JSON_EXTRACT for SQLite to search in JSON array
            query = """
            SELECT person_id FROM person 
            WHERE JSON_EXTRACT(names, '$[0]') = ? AND email IS NULL
            """
            results = await self.execute_query(query, (primary_name,))
            if results:
                logger.info(f"Found existing person with name (no email): {primary_name}")
                return results[0]['person_id']
        
        return None

    async def create_person(
        self,
        names: List[str],
        email: Optional[str] = None,
        secondary_emails: Optional[List[str]] = None,
        company_id: Optional[int] = None,
        title: Optional[str] = None,
        department: Optional[str] = None,
        location: Optional[str] = None,
        timezone: Optional[str] = None,
        handles: Optional[Dict[str, str]] = None,
        aliases: Optional[List[str]] = None,
        bio: Optional[str] = None,
        skills: Optional[List[str]] = None,
        vector: Optional[bytes] = None,
        allow_duplicates: bool = False
    ) -> int:
        """Create a new person with duplicate detection."""
        
        # Check for duplicates unless explicitly allowed
        if not allow_duplicates:
            existing_id = await self._check_person_duplicates(email, names)
            if existing_id:
                if email:
                    raise ValueError(f"Person with email '{email}' already exists (ID: {existing_id})")
                else:
                    raise ValueError(f"Person with name '{names[0]}' already exists (ID: {existing_id})")
        
        query = """
        INSERT INTO person (
            names, email, secondary_emails, company_id, title, department,
            location, timezone, handles, aliases, bio, skills, vector
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        
        params = (
            json.dumps(names),
            email,
            json.dumps(secondary_emails or []),
            company_id,
            title,
            department,
            location,
            timezone,
            json.dumps(handles or {}),
            json.dumps(aliases or []),
            bio,
            json.dumps(skills or []),
            vector
        )
        
        person_id = await self.execute_insert(query, params)
        
        # Also insert primary email into email table if provided
        if email:
            await self.create_email(email, person_id, is_primary=True)
        
        # Insert secondary emails
        for sec_email in (secondary_emails or []):
            await self.create_email(sec_email, person_id, is_primary=False)
        
        return person_id
    
    async def get_person(self, person_id: int) -> Optional[Dict[str, Any]]:
        """Get a person by ID."""
        query = """
        SELECT person_id, names, email, secondary_emails, company_id, title, 
               department, location, timezone, handles, aliases, bio, skills, 
               created_at, updated_at 
        FROM person WHERE person_id = ?
        """
        results = await self.execute_query(query, (person_id,))
        if results:
            person = results[0]
            # Parse JSON fields
            person['names'] = json.loads(person['names'])
            person['secondary_emails'] = json.loads(person['secondary_emails'])
            person['handles'] = json.loads(person['handles'])
            person['aliases'] = json.loads(person['aliases'])
            person['skills'] = json.loads(person['skills'])
            return person
        return None
    
    async def update_person(self, person_id: int, **kwargs) -> bool:
        """Update a person's data."""
        # Convert lists/dicts to JSON
        json_fields = ['names', 'secondary_emails', 'handles', 'aliases', 'skills']
        for field in json_fields:
            if field in kwargs and kwargs[field] is not None:
                kwargs[field] = json.dumps(kwargs[field])
        
        # Build update query
        fields = []
        values = []
        for key, value in kwargs.items():
            fields.append(f"{key} = ?")
            values.append(value)
        
        if not fields:
            return False
        
        values.append(person_id)
        query = f"UPDATE person SET {', '.join(fields)} WHERE person_id = ?"
        
        affected = await self.execute_update(query, tuple(values))
        return affected > 0
    
    async def delete_person(self, person_id: int) -> bool:
        """Delete a person and related data."""
        # Delete from relationship tables first
        await self.execute_update("DELETE FROM person_company WHERE person_id = ?", (person_id,))
        await self.execute_update("DELETE FROM person_channel WHERE person_id = ?", (person_id,))
        await self.execute_update("DELETE FROM person_project WHERE person_id = ?", (person_id,))
        await self.execute_update("DELETE FROM email WHERE person_id = ?", (person_id,))
        
        # Delete the person
        query = "DELETE FROM person WHERE person_id = ?"
        affected = await self.execute_update(query, (person_id,))
        return affected > 0
    
    # Company CRUD operations
    async def create_company(
        self,
        name: str,
        domain: Optional[str] = None,
        domains: Optional[List[str]] = None,
        industry: Optional[str] = None,
        size: Optional[str] = None,
        location_hq: Optional[str] = None,
        description: Optional[str] = None,
        tags: Optional[List[str]] = None,
        vector: Optional[bytes] = None
    ) -> int:
        """Create a new company."""
        query = """
        INSERT INTO company (
            name, domain, domains, industry, size, location_hq,
            description, tags, vector
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        
        params = (
            name,
            domain,
            json.dumps(domains or []),
            industry,
            size,
            location_hq,
            description,
            json.dumps(tags or []),
            vector
        )
        
        return await self.execute_insert(query, params)
    
    async def get_company(self, company_id: int) -> Optional[Dict[str, Any]]:
        """Get a company by ID."""
        query = """
        SELECT company_id, name, domain, domains, industry, size, location_hq, 
               description, tags, created_at, updated_at 
        FROM company WHERE company_id = ?
        """
        results = await self.execute_query(query, (company_id,))
        if results:
            company = results[0]
            company['domains'] = json.loads(company['domains'])
            company['tags'] = json.loads(company['tags'])
            return company
        return None
    
    # Channel CRUD operations
    async def create_channel(
        self,
        platform: str,
        ext_id: str,
        name: Optional[str] = None,
        type: Optional[str] = None,
        purpose: Optional[str] = None,
        owner_id: Optional[int] = None,
        company_id: Optional[int] = None,
        project_id: Optional[int] = None,
        is_active: bool = True,
        member_count: Optional[int] = None,
        vector: Optional[bytes] = None
    ) -> int:
        """Create a new channel."""
        query = """
        INSERT INTO channel (
            platform, ext_id, name, type, purpose, owner_id,
            company_id, project_id, is_active, member_count, vector
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        
        params = (
            platform, ext_id, name, type, purpose, owner_id,
            company_id, project_id, is_active, member_count, vector
        )
        
        return await self.execute_insert(query, params)
    
    async def get_channel(self, channel_id: int) -> Optional[Dict[str, Any]]:
        """Get a channel by ID."""
        query = """
        SELECT channel_id, platform, ext_id, name, type, purpose, owner_id, 
               company_id, project_id, is_active, member_count, created_at, updated_at 
        FROM channel WHERE channel_id = ?
        """
        results = await self.execute_query(query, (channel_id,))
        return results[0] if results else None
    
    # Project CRUD operations
    async def create_project(
        self,
        name: str,
        code: Optional[str] = None,
        description: Optional[str] = None,
        status: str = 'active',
        company_id: Optional[int] = None,
        lead_id: Optional[int] = None,
        start_date: Optional[str] = None,
        end_date: Optional[str] = None,
        tags: Optional[List[str]] = None,
        urls: Optional[Dict[str, str]] = None,
        vector: Optional[bytes] = None
    ) -> int:
        """Create a new project."""
        query = """
        INSERT INTO project (
            name, code, description, status, company_id, lead_id,
            start_date, end_date, tags, urls, vector
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        
        params = (
            name, code, description, status, company_id, lead_id,
            start_date, end_date, json.dumps(tags or []),
            json.dumps(urls or {}), vector
        )
        
        return await self.execute_insert(query, params)
    
    async def get_project(self, project_id: int) -> Optional[Dict[str, Any]]:
        """Get a project by ID."""
        query = """
        SELECT project_id, name, code, description, status, company_id, lead_id, 
               start_date, end_date, tags, urls, created_at, updated_at 
        FROM project WHERE project_id = ?
        """
        results = await self.execute_query(query, (project_id,))
        if results:
            project = results[0]
            project['tags'] = json.loads(project['tags'])
            project['urls'] = json.loads(project['urls'])
            return project
        return None
    
    # Email operations
    async def create_email(
        self,
        address: str,
        person_id: int,
        is_primary: bool = False,
        is_verified: bool = False
    ) -> int:
        """Create a new email record."""
        domain = address.split('@')[1] if '@' in address else None
        
        query = """
        INSERT INTO email (address, person_id, is_primary, is_verified, domain)
        VALUES (?, ?, ?, ?, ?)
        """
        
        params = (address, person_id, is_primary, is_verified, domain)
        
        try:
            return await self.execute_insert(query, params)
        except aiosqlite.IntegrityError:
            # Email already exists, update it
            update_query = """
            UPDATE email SET person_id = ?, is_primary = ?, is_verified = ?
            WHERE address = ?
            """
            await self.execute_update(update_query, (person_id, is_primary, is_verified, address))
            return 0
    
    # Relationship operations
    async def create_relationship(self, table: str, **kwargs) -> bool:
        """Create a relationship between entities."""
        fields = list(kwargs.keys())
        placeholders = ['?' for _ in fields]
        
        query = f"""
        INSERT INTO {table} ({', '.join(fields)})
        VALUES ({', '.join(placeholders)})
        """
        
        try:
            await self.execute_insert(query, tuple(kwargs.values()))
            return True
        except aiosqlite.IntegrityError:
            # Relationship already exists
            return False
    
    async def add_person_to_channel(
        self,
        person_id: int,
        channel_id: int,
        role: str = "member"
    ) -> bool:
        """Add a person to a channel with a specific role."""
        return await self.create_relationship(
            "person_channel",
            person_id=person_id,
            channel_id=channel_id,
            role=role,
            joined_at=datetime.now().isoformat(),
            is_active=True
        )
    
    async def add_person_to_project(
        self,
        person_id: int,
        project_id: int,
        role: str = "contributor",
        allocation: int = 50
    ) -> bool:
        """Add a person to a project with role and allocation."""
        return await self.create_relationship(
            "person_project",
            person_id=person_id,
            project_id=project_id,
            role=role,
            allocation=allocation,
            start_date=datetime.now().isoformat(),
            is_active=True
        )
    
    async def add_person_to_company(
        self,
        person_id: int,
        company_id: int,
        role: str = "employee",
        start_date: str = None,
        end_date: str = None,
        is_current: bool = True
    ) -> bool:
        """Add a person to a company with employment details."""
        relationship_data = {
            "person_id": person_id,
            "company_id": company_id,
            "role": role,
            "is_current": is_current
        }
        
        # Only add dates if provided
        if start_date:
            relationship_data["start_date"] = start_date
        if end_date:
            relationship_data["end_date"] = end_date
            
        return await self.create_relationship("person_company", **relationship_data)
    
    async def update_company(self, company_id: int, **kwargs) -> bool:
        """Update company information."""
        if not kwargs:
            return False
        
        # Handle special fields
        if 'domains' in kwargs and isinstance(kwargs['domains'], list):
            kwargs['domains'] = json.dumps(kwargs['domains'])
        if 'tags' in kwargs and isinstance(kwargs['tags'], list):
            kwargs['tags'] = json.dumps(kwargs['tags'])
        
        set_clause = ', '.join([f"{key} = ?" for key in kwargs.keys()])
        query = f"UPDATE company SET {set_clause} WHERE company_id = ?"
        
        rows_affected = await self.execute_update(
            query, 
            tuple(list(kwargs.values()) + [company_id])
        )
        return rows_affected > 0
    
    async def update_channel(self, channel_id: int, **kwargs) -> bool:
        """Update channel information."""
        if not kwargs:
            return False
        
        set_clause = ', '.join([f"{key} = ?" for key in kwargs.keys()])
        query = f"UPDATE channel SET {set_clause} WHERE channel_id = ?"
        
        rows_affected = await self.execute_update(
            query, 
            tuple(list(kwargs.values()) + [channel_id])
        )
        return rows_affected > 0
    
    async def update_project(self, project_id: int, **kwargs) -> bool:
        """Update project information."""
        if not kwargs:
            return False
        
        # Handle special fields
        if 'tags' in kwargs and isinstance(kwargs['tags'], list):
            kwargs['tags'] = json.dumps(kwargs['tags'])
        if 'urls' in kwargs and isinstance(kwargs['urls'], dict):
            kwargs['urls'] = json.dumps(kwargs['urls'])
        
        set_clause = ', '.join([f"{key} = ?" for key in kwargs.keys()])
        query = f"UPDATE project SET {set_clause} WHERE project_id = ?"
        
        rows_affected = await self.execute_update(
            query, 
            tuple(list(kwargs.values()) + [project_id])
        )
        return rows_affected > 0
    
    # Search operations
    def _escape_fts5_query(self, query: str) -> str:
        """Escape FTS5 query to handle special characters safely."""
        # Characters that cause FTS5 syntax errors
        special_chars = ['@', "'", '#', '-', '&', ':', '?', '=', '(', ')', '[', ']']
        
        # If query contains special characters, wrap in double quotes for exact match
        if any(char in query for char in special_chars):
            # Escape any existing double quotes in the query
            escaped_query = query.replace('"', '""')
            return f'"{escaped_query}"'
        
        return query
    
    async def _fallback_exact_search(
        self,
        entity_type: str,
        query: str,
        limit: int = 10
    ) -> List[Dict[str, Any]]:
        """Fallback exact match search when FTS5 fails."""
        if entity_type == 'person':
            # Search in names (JSON array), email, title, bio, skills
            search_query = """
            SELECT * FROM person 
            WHERE names LIKE ? 
               OR email LIKE ?
               OR title LIKE ?
               OR bio LIKE ?
               OR skills LIKE ?
            LIMIT ?
            """
            like_query = f"%{query}%"
            params = (like_query, like_query, like_query, like_query, like_query, limit)
            
        elif entity_type == 'company':
            search_query = """
            SELECT * FROM company
            WHERE name LIKE ?
               OR domain LIKE ?
               OR description LIKE ?
            LIMIT ?
            """
            like_query = f"%{query}%"
            params = (like_query, like_query, like_query, limit)
            
        elif entity_type == 'channel':
            search_query = """
            SELECT * FROM channel
            WHERE name LIKE ?
               OR ext_id LIKE ?
               OR purpose LIKE ?
            LIMIT ?
            """
            like_query = f"%{query}%"
            params = (like_query, like_query, like_query, limit)
            
        elif entity_type == 'project':
            search_query = """
            SELECT * FROM project
            WHERE name LIKE ?
               OR description LIKE ?
            LIMIT ?
            """
            like_query = f"%{query}%"
            params = (like_query, like_query, limit)
            
        else:
            logger.error(f"Unknown entity type for fallback search: {entity_type}")
            return []
        
        results = await self.execute_query(search_query, params)
        
        # Parse JSON fields based on entity type
        for result in results:
            if entity_type == 'person':
                result['names'] = json.loads(result['names'])
                result['secondary_emails'] = json.loads(result['secondary_emails'])
                result['handles'] = json.loads(result['handles'])
                result['aliases'] = json.loads(result['aliases'])
                result['skills'] = json.loads(result['skills'])
            elif entity_type == 'company':
                result['domains'] = json.loads(result['domains'])
                result['tags'] = json.loads(result['tags'])
            # Add other entity types as needed
        
        return results
    
    async def search_entities(
        self,
        entity_type: str,
        query: str,
        limit: int = 10
    ) -> List[Dict[str, Any]]:
        """Search entities using full-text search with special character handling."""
        fts_table = f"{entity_type}_fts"
        
        # Escape the query to handle special characters
        escaped_query = self._escape_fts5_query(query)
        
        search_query = f"""
        SELECT * FROM {entity_type}
        WHERE {entity_type}_id IN (
            SELECT {entity_type}_id FROM {fts_table}
            WHERE {fts_table} MATCH ?
            ORDER BY rank
            LIMIT ?
        )
        """
        
        try:
            results = await self.execute_query(search_query, (escaped_query, limit))
        except Exception as e:
            # If FTS5 still fails, fall back to exact match search
            if "fts5: syntax error" in str(e).lower():
                logger.warning(f"FTS5 search failed for query '{query}', falling back to exact match: {e}")
                return await self._fallback_exact_search(entity_type, query, limit)
            raise
        
        # Parse JSON fields based on entity type
        for result in results:
            if entity_type == 'person':
                result['names'] = json.loads(result['names'])
                result['handles'] = json.loads(result['handles'])
                result['aliases'] = json.loads(result['aliases'])
                result['skills'] = json.loads(result['skills'])
                result['secondary_emails'] = json.loads(result['secondary_emails'])
            elif entity_type == 'company':
                result['domains'] = json.loads(result['domains'])
                result['tags'] = json.loads(result['tags'])
            elif entity_type == 'project':
                result['tags'] = json.loads(result['tags'])
                result['urls'] = json.loads(result['urls'])
        
        return results
    
    # Search methods for each entity type
    async def search_people(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Search for people using text search."""
        return await self.search_entities('person', query, limit)
    
    async def search_companies(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Search for companies using text search."""
        return await self.search_entities('company', query, limit)
    
    async def search_channels(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Search for channels using text search."""
        return await self.search_entities('channel', query, limit)
    
    async def search_projects(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Search for projects using text search."""
        return await self.search_entities('project', query, limit)
    
    async def find_people_by_criteria(self, criteria: Dict[str, str], limit: int = 20) -> List[Dict[str, Any]]:
        """Find people based on specific criteria."""
        where_clauses = []
        params = []
        
        if 'company' in criteria:
            # Search by company name or domain
            where_clauses.append("""
                person_id IN (
                    SELECT p.person_id FROM person p
                    JOIN person_company pc ON p.person_id = pc.person_id
                    JOIN company c ON pc.company_id = c.company_id
                    WHERE (c.name LIKE ? OR c.domain LIKE ?) AND pc.is_current = TRUE
                )
            """)
            company_pattern = f"%{criteria['company']}%"
            params.extend([company_pattern, company_pattern])
        
        if 'title' in criteria:
            where_clauses.append("title LIKE ?")
            params.append(f"%{criteria['title']}%")
        
        if 'skills' in criteria:
            where_clauses.append("skills LIKE ?")
            params.append(f"%{criteria['skills']}%")
        
        if 'location' in criteria:
            where_clauses.append("location LIKE ?")
            params.append(f"%{criteria['location']}%")
        
        if not where_clauses:
            return []
        
        query = f"""
        SELECT DISTINCT * FROM person
        WHERE {' AND '.join(where_clauses)}
        LIMIT ?
        """
        params.append(limit)
        
        results = await self.execute_query(query, tuple(params))
        
        # Parse JSON fields
        for result in results:
            result['names'] = json.loads(result['names'])
            result['handles'] = json.loads(result['handles'])
            result['aliases'] = json.loads(result['aliases'])
            result['skills'] = json.loads(result['skills'])
            result['secondary_emails'] = json.loads(result['secondary_emails'])
        
        return results
    
    # Utility methods
    async def get_company_name(self, company_id: int) -> Optional[str]:
        """Get company name by ID."""
        query = "SELECT name FROM company WHERE company_id = ?"
        results = await self.execute_query(query, (company_id,))
        return results[0]['name'] if results else None
    
    async def get_project_name(self, project_id: int) -> Optional[str]:
        """Get project name by ID."""
        query = "SELECT name FROM project WHERE project_id = ?"
        results = await self.execute_query(query, (project_id,))
        return results[0]['name'] if results else None
    
    async def get_person_channels(self, person_id: int) -> List[Dict[str, Any]]:
        """Get all channels a person is in."""
        query = """
        SELECT c.channel_id, c.platform, c.ext_id, c.name, c.type, c.purpose,
               c.owner_id, c.company_id, c.project_id, c.is_active, c.member_count,
               c.created_at, c.updated_at, pc.role, pc.joined_at, pc.is_active as membership_active
        FROM channel c
        JOIN person_channel pc ON c.channel_id = pc.channel_id
        WHERE pc.person_id = ?
        """
        return await self.execute_query(query, (person_id,))
    
    async def get_person_projects(self, person_id: int) -> List[Dict[str, Any]]:
        """Get all projects a person is involved in."""
        query = """
        SELECT p.project_id, p.name, p.description, p.status, p.company_id,
               p.start_date, p.end_date, p.tags, p.urls,
               p.created_at, p.updated_at, pp.role, pp.allocation, pp.start_date as member_start_date, 
               pp.end_date as member_end_date, pp.is_active
        FROM project p
        JOIN person_project pp ON p.project_id = pp.project_id
        WHERE pp.person_id = ?
        """
        results = await self.execute_query(query, (person_id,))
        for result in results:
            result['tags'] = json.loads(result['tags'])
            result['urls'] = json.loads(result['urls'])
        return results
    
    async def get_person_connections(self, person_id: int) -> Dict[str, Any]:
        """Get all connections for a person (companies, projects, channels)."""
        person = await self.get_person(person_id)
        if not person:
            return {}
        
        # Get companies
        companies = []
        company_query = """
        SELECT c.company_id, c.name, c.domain, c.domains, c.industry, c.size, 
               c.location_hq, c.description, c.tags, c.created_at, c.updated_at,
               pc.role, pc.start_date, pc.end_date, pc.is_current
        FROM company c
        JOIN person_company pc ON c.company_id = pc.company_id
        WHERE pc.person_id = ?
        """
        company_results = await self.execute_query(company_query, (person_id,))
        for company in company_results:
            company['domains'] = json.loads(company['domains'])
            company['tags'] = json.loads(company['tags'])
            companies.append(company)
        
        # Get projects
        projects = await self.get_person_projects(person_id)
        
        # Get channels
        channels = await self.get_person_channels(person_id)
        
        return {
            "person": person,
            "companies": companies,
            "projects": projects,
            "channels": channels
        }
    
    async def get_company_people(self, company_id: int, exclude_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """Get all people in a company."""
        query = """
        SELECT p.person_id, p.names, p.email, p.secondary_emails, p.company_id, 
               p.title, p.department, p.location, p.timezone, p.handles, p.aliases, 
               p.bio, p.skills, p.created_at, p.updated_at, pc.role, pc.is_current
        FROM person p
        JOIN person_company pc ON p.person_id = pc.person_id
        WHERE pc.company_id = ? AND pc.is_current = TRUE
        """
        params = [company_id]
        
        if exclude_id:
            query += " AND p.person_id != ?"
            params.append(exclude_id)
        
        results = await self.execute_query(query, tuple(params))
        for result in results:
            result['names'] = json.loads(result['names'])
            result['handles'] = json.loads(result['handles'])
            result['aliases'] = json.loads(result['aliases'])
            result['skills'] = json.loads(result['skills'])
            result['secondary_emails'] = json.loads(result['secondary_emails'])
        return results
    
    async def get_company_projects(self, company_id: int) -> List[Dict[str, Any]]:
        """Get all projects for a company."""
        query = """
        SELECT p.project_id, p.name, p.description, p.status, p.company_id,
               p.start_date, p.end_date, p.tags, p.urls,
               p.created_at, p.updated_at, COUNT(pp.person_id) as team_size
        FROM project p
        LEFT JOIN person_project pp ON p.project_id = pp.project_id AND pp.is_active = TRUE
        WHERE p.company_id = ?
        GROUP BY p.project_id
        ORDER BY p.created_at DESC
        """
        results = await self.execute_query(query, (company_id,))
        for result in results:
            result['tags'] = json.loads(result['tags'])
            result['urls'] = json.loads(result['urls'])
        return results
    
    async def get_company_channels(self, company_id: int) -> List[Dict[str, Any]]:
        """Get all channels for a company."""
        query = "SELECT * FROM channel WHERE company_id = ?"
        return await self.execute_query(query, (company_id,))
    
    async def find_shared_channels(self, person1_id: int, person2_id: int) -> List[Dict[str, Any]]:
        """Find channels that two people share."""
        query = """
        SELECT c.channel_id, c.platform, c.ext_id, c.name, c.type, c.purpose,
               c.owner_id, c.company_id, c.project_id, c.is_active, c.member_count,
               c.created_at, c.updated_at
        FROM channel c
        WHERE c.channel_id IN (
            SELECT channel_id FROM person_channel WHERE person_id = ? AND is_active = TRUE
            INTERSECT
            SELECT channel_id FROM person_channel WHERE person_id = ? AND is_active = TRUE
        )
        """
        return await self.execute_query(query, (person1_id, person2_id))
    
    async def find_shared_projects(self, person1_id: int, person2_id: int) -> List[Dict[str, Any]]:
        """Find projects that two people share."""
        query = """
        SELECT p.project_id, p.name, p.description, p.status, p.company_id,
               p.start_date, p.end_date, p.tags, p.urls,
               p.created_at, p.updated_at
        FROM project p
        WHERE p.project_id IN (
            SELECT project_id FROM person_project WHERE person_id = ? AND is_active = TRUE
            INTERSECT
            SELECT project_id FROM person_project WHERE person_id = ? AND is_active = TRUE
        )
        """
        results = await self.execute_query(query, (person1_id, person2_id))
        for result in results:
            result['tags'] = json.loads(result['tags'])
            result['urls'] = json.loads(result['urls'])
        return results
    
    async def get_project_people(self, project_id: int) -> List[Dict[str, Any]]:
        """Get all people working on a project."""
        query = """
        SELECT p.person_id, p.names, p.email, p.secondary_emails, p.company_id, 
               p.title, p.department, p.location, p.timezone, p.handles, p.aliases, 
               p.bio, p.skills, p.created_at, p.updated_at, 
               pp.role, pp.allocation, pp.start_date, pp.end_date, pp.is_active
        FROM person p
        JOIN person_project pp ON p.person_id = pp.person_id
        WHERE pp.project_id = ?
        """
        results = await self.execute_query(query, (project_id,))
        for result in results:
            result['names'] = json.loads(result['names'])
            result['handles'] = json.loads(result['handles'])
            result['aliases'] = json.loads(result['aliases'])
            result['skills'] = json.loads(result['skills'])
            result['secondary_emails'] = json.loads(result['secondary_emails'])
        return results
    
    async def get_channel_people(self, channel_id: int) -> List[Dict[str, Any]]:
        """Get all people in a channel."""
        query = """
        SELECT p.person_id, p.names, p.email, p.secondary_emails, p.company_id, 
               p.title, p.department, p.location, p.timezone, p.handles, p.aliases, 
               p.bio, p.skills, p.created_at, p.updated_at,
               pc.role, pc.joined_at, pc.is_active
        FROM person p
        JOIN person_channel pc ON p.person_id = pc.person_id
        WHERE pc.channel_id = ? AND pc.is_active = TRUE
        """
        results = await self.execute_query(query, (channel_id,))
        for result in results:
            result['names'] = json.loads(result['names'])
            result['handles'] = json.loads(result['handles'])
            result['aliases'] = json.loads(result['aliases'])
            result['skills'] = json.loads(result['skills'])
            result['secondary_emails'] = json.loads(result['secondary_emails'])
        return results
    
    async def get_project_channels(self, project_id: int) -> List[Dict[str, Any]]:
        """Get all channels for a project."""
        query = """
        SELECT channel_id, platform, ext_id, name, type, purpose,
               owner_id, company_id, project_id, is_active, member_count,
               created_at, updated_at
        FROM channel WHERE project_id = ?
        """
        return await self.execute_query(query, (project_id,))
    
    async def close(self):
        """Close database connection."""
        if self._connection:
            await self._connection.close()
            self._connection = None