...
if (!win.__stencil_cssshim && needsShim()) {
    ...
    var regex = /\/limepkg-scrive-lwc-components(\.esm)?\.js($|\?|#)/;
    var scriptElm = currentScript || Array.from(doc.querySelectorAll('script')).find(function(s) {
      return regex.test(s.src) || s.getAttribute('data-stencil-namespace') === "limepkg-scrive-lwc-components";
    });

    var resourcesUrl = scriptElm ? scriptElm.getAttribute('data-resources-url') || scriptElm.src : '';
    var start = function() {
      var url = new URL('./p-a4960c0c.system.js', resourcesUrl);
