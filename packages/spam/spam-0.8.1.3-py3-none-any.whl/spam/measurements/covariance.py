...
def covarianceAlongAxis(im, d, mask=None, axis=[0, 1, 2]):
    ...
    This function computes the covariance of a field (grey values or binary) with or without a mask.
    The covariance is computed at integer distances (in voxels). No interpolation is made but specific direction can be asked.
    ...
    if type(d[0]) == float:
        print(
            "spam.measurements.covariance.covarianceAlongAxis: d={}. Should be a list of integers.".format(
                type(d[0])
            )
        )
        print("exit function.")
    ...
    if any([max(d) >= im.shape[i] for i in axis]):
        print(
            "spam.measurements.covariance.covarianceAlongAxis: max(d)={}. Should be smaller than the image.".format(
                max(d)
            )
        )
        print("exit function.")
    ...
    if mask is not None:
        E = (
            numpy.mean(im, dtype=numpy.float32)
            * numpy.size(mask)
            / float(numpy.sum(mask))
        )
    ...
    for j, a in enumerate(axis):
        for (i, x) in enumerate(d):
            if mask is not None:
                ...
                mask_eff = numpy.logical_and(
                    mask, spam.helpers.singleShift(mask, x, a, sub=False)
                )
                ...
                n = (im.shape[a] - x) * numpy.prod(
                    [s for i, s in enumerate(im.shape) if i != a]
                )  #
                # Step 2.2: Multiply the image
                im_multi = numpy.multiply(
                    im, spam.helpers.singleShift(im, x, a, sub=0), dtype=numpy.float32
                )
