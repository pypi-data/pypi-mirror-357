...
from spambind.measurements.measurementsToolkit import (
...
def porosityField(
    ...
    if poreValue is None or solidValue is None:
        print(
            "spam.measurements.porosityField(): Need to give me the greyvalue of the pores and solids"
        )
        return

    if poreValue == solidValue:
        print(
            "spam.measurements.porosityField(): Need to give me the correct greyvalue of the pores and solids, where greyvalue of pores always smaller than that of solids"
        )
        return
    ...
    if poreValue < solidValue:
        imPorosity[image > poreValue] = (
            numpy.true_divide(100, solidValue - poreValue)
        ) * (solidValue - image[image > poreValue])
        imPorosity[image <= poreValue] = 100
        imPorosity[image >= solidValue] = 0

    if poreValue > solidValue:
        imPorosity[image > solidValue] = (
            numpy.true_divide(100, poreValue - solidValue)
        ) * (poreValue - image[image > solidValue])
    ...
    porosityFieldBinaryCPP(imPorosity, pointsArray, hwsArray, porosities)
