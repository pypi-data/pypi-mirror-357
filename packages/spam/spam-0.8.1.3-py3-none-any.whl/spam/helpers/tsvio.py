...
def writeRegistrationTSV(fileName, regCentre, regReturns):
    ...
    numpy.savetxt(fileName, outMatrix.T, fmt="%.7f", delimiter="\t", newline="\n", comments="", header=header)
...
def writeStrainTSV(fileName, points, decomposedFfield, firstColumn="StrainPointNumber", startRow=0):
    ...
    outMatrix = numpy.array([numpy.arange(points.shape[0]), points[:, 0], points[:, 1], points[:, 2]]).T
    ...
    numpy.savetxt(fileName, outMatrix, delimiter="\t", fmt="%.7f", newline="\n", comments="", header=header)


def readCorrelationTSV(fileName, fieldBinRatio=1.0, readOnlyDisplacements=False, readConvergence=True, readPixelSearchCC=False, readError=False, readLabelDilate=False, verbose=True):
    """
    ...
    output = {"fieldDims": fieldDims, "numberOfLabels": numberOfLabels, "fieldCoords": fieldCoords}
    if readConvergence:
        output.update({"returnStatus": RS, "deltaPhiNorm": deltaPhiNorm, "iterations": iterations})
    if readError:
...
def TSVtoTIFF(fileName, fieldBinRatio=1.0, lab=None, returnRS=False, outDir=None, prefix=None):
    ...
    PhiComponents = [["Zdisp", 0], ["Ydisp", 1], ["Xdisp", 2]]
    ...
    if fi["numberOfLabels"] != 0:
        if lab:
            ...
            for component in PhiComponents:
                tifffile.imwrite("{}/{}-{}.tif".format(outDir, prefix, component[0]), spam.label.convertLabelToFloat(labelled, displacements[:, component[1]]).astype("<f4"))
            if returnRS:
                tifffile.imwrite("{}/{}-RS.tif".format(outDir, prefix), spam.label.convertLabelToFloat(labelled, fi["returnStatus"]).astype("<f4"))
        else:
    ...
    # if not, is a ldic result
        ...
        for component in PhiComponents:
            tifffile.imwrite("{}/{}-{}.tif".format(outDir, prefix, component[0]), displacements[:, component[1]].reshape(dims).astype("<f4"))

        if returnRS:
            tifffile.imwrite("{}/{}-RS.tif".format(outDir, prefix), fi["returnStatus"].reshape(dims).astype("<f4"))
...
def TSVtoVTK(fileName, fieldBinRatio=1.0, pixelSize=1.0, returnRS=False, outDir=None, prefix=None):
    ...
    if fi["numberOfLabels"] != 0:
        ...
        if not returnRS:
            ...
            pointData = {"displacements": PhiField[1:, :-1, 3] * pixelSize, "returnStatus": fi["returnStatus"][1:]}
    ...
    # if not, is a ldic result
        ...
        if not returnRS:
            ...
            cellData = {"displacements": (PhiField[:, :-1, 3] * pixelSize).reshape((dims[0], dims[1], dims[2], 3)), "returnStatus": fi["returnStatus"].reshape(dims[0], dims[1], dims[2])}
