...
def plotOrientations(
    ...
    if plot == "points" or plot == "both":
        ...
        matplotlib.pyplot.axis((0, numpy.pi * 2, 0, radiusMax))
        ...
        for angle in radiusGridAngles:
            ...
            radiusGridValues.append(spam.orientations.projectOrientation([0, angle * numpy.pi / 180.0, 1], "spherical", projection)[1][1])
    ...
    if plot == "bins" or plot == "both":
        ...
        for vectorN in range(numberOfPoints):
            ...
            angularBin = int(numpy.floor((angle) / (2 * numpy.pi / float(numberOfAngularBinsPerRing[ringNumber])))) + 1
        ...
        #              theta   radius    width
        bars.append([0, radiusMax, 2 * numpy.pi])
        bars.append([0, radiusMax, 2 * numpy.pi])
        # bars.append(ax.bar(0,   radiusMax,    2*numpy.pi, bottom=0.0))
        # bars.append(ax.bar(0,   radiusMax,    2*numpy.pi, bottom=0.0))
        ...
        for ringNumber in range(numberOfRings)[::-1]:
            ...
            deltaThetaRad = 2 * numpy.pi / float(numberOfAngularBinsPerRing[ringNumber])
