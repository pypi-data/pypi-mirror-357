...
def save_last_checkpoint(trainer: "pl.Trainer") -> None:

    try:
        logdir = trainer.logger.experiment.config["runtime"]["logdir"]
        filepath = logdir + '/checkpoint-ema/last.ckpt'
    except Exception as e: # noqa
        filepath = '/tmp/checkpoint-ema/last.ckpt'
...
class EMA(Callback):
    ...
    def __init__(
        self, decay: float, validate_original_weights: bool = True, every_n_steps: int = 1, cpu_offload: bool = False,
    ):
        if not (0 <= decay <= 1):
    ...
    def on_train_epoch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        ...
        checkpoint_callback: ModelCheckpoint = trainer.checkpoint_callback # noqa

        self.swap_model_weights(trainer)
        if checkpoint_callback is not None:
            save_last_checkpoint(trainer)
        self.swap_model_weights(trainer)
