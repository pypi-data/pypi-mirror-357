...
def init(
    ...
    if shell:
        if append_autocomplete(shell):
            ...
            console.print(
                "[yellow]Autocompletar já estava configurado ou não foi possível configurar automaticamente.[/yellow]"
            )
    else:
        console.print(
            "[yellow]Shell não reconhecido. Configure o autocompletar manualmente se desejar.[/yellow]"
        )
