...
class SyncManager:
    ...
    def init_git_repo(self, remote_url: Optional[str] = None) -> bool:
        ...
        try:
            ...
            console.print(f"[green]✓ Git repository initialized in {self.config_dir}[/green]")
            return True
    ...
    def get_status(self) -> Dict[str, str]:
        ...
        if self.is_git_repo and self.repo:
            try:
                ...
                status["remote"] = "origin" if "origin" in [r.name for r in self.repo.remotes] else "none"
                status["uncommitted_changes"] = str(bool(self.repo.is_dirty()))
                status["last_commit"] = self.repo.head.commit.hexsha[:8] if self.repo.head.is_valid() else "none"
            except Exception as e:
                status["error"] = str(e)
    ...
    def commit_changes(self, message: Optional[str] = None) -> bool:
        ...
        try:
            ...
            # Commit changes
            commit_message = message or f"Update snippets - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    ...
    def sync(self, auto_commit: bool = True, commit_message: Optional[str] = None) -> bool:
        """Perform full sync: commit, pull, push."""
        if not self.is_git_repo:
            console.print("[red]Not in a Git repository. Use 'pypet sync init' to initialize.[/red]")
            return False
