...
from wind_farm_loads.tool_agnostic import SmoothPotFunctions
...
def sequential_solver(
    ...
    flow_field.turbulence_intensity_field_sorted_avg = np.mean(
        turbine_turbulence_intensity, axis=(2, 3)
    )[:, :, None, None]
...
def full_flow_sequential_solver(
    ...
    for i in range(flow_field_grid.n_turbines):
        ...
        r = np.sqrt(
            (deficit_model_args["x"] - x_i) ** 2
            + (deficit_model_args["y"] - y_i) ** 2
            + (deficit_model_args["z"] - z_i) ** 2
        )
        weight = SmoothPotFunctions.pot_sharp(r, 0.75 * rotor_diameter_i)
