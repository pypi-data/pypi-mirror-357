...
def predict_loads_pod(
    ...
    *additional_inputs,
    ...
    # Ensure that the additional inputs have shape (wt, wd, ws), then reshape to column array.
    # We take the sizes from the flow map, rather than fmodel, because the flow map might contain a subset of it.
    theta = [
        _findex_to_ilk(
            th,
            flow_map["wt"].size,
...
def predict_loads_sector_average(
    ...
    *additional_inputs,
    ...
    # Ensure that the additional inputs have shape (wt, wd, ws), then reshape to column array.
    # We take the sizes from the sector average, rather than fmodel, because the sector average might contain a subset of it.
    theta = [
        _findex_to_ilk(
            th,
