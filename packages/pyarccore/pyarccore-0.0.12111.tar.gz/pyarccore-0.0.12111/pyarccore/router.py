    ...
    def _get_route_path(filepath: Path, base_path: Path) -> str:
        ...
        for part in relative_path.parts:
            ...
            if part.startswith('_'):
                continue
            if part.startswith('{') and part.endswith('}'):  # Modification ici pour les paramètres
                part = f"{{{part[1:-1]}}}"
            parts.append(part)
    ...
    def register_routes(cls, router: APIRouter, base_path: Union[Path, str]):
        ...
        try:
            for filepath in base_path.glob('**/*.py'):
                ...
                try:
                    # Gestion spéciale pour les fichiers paramétrés (ex: {id}.py)
                    ...
                    module_path = '.'.join([
                        *filepath.relative_to(base_path.parent).with_suffix('').parts
                    ])
