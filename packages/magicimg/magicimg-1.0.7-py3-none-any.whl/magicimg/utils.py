...
def create_debug_dir(base_dir: Union[str, Path], name: str = "debug") -> str:
    ...
    logger.debug(f"Đã tạo thư mục debug: {debug_dir}")
...
def clean_debug_dir(debug_dir: Union[str, Path], keep_latest: int = 5) -> None:
    ...
    try:
        ...
        if deleted_count > 0:
            logger.debug(f"Đã xóa {deleted_count} file cũ trong thư mục debug")
...
def check_quality_array(image: np.ndarray, **kwargs) -> Tuple[bool, dict, Optional[np.ndarray]]:
    """
    ...
    Returns:
        Tuple[bool, dict, Optional[np.ndarray]]: 
            - is_good: Ảnh có đạt chất lượng không
            - quality_info_dict: Thông tin chất lượng (dạng dict)
            - enhanced_image: Ảnh đã tăng cường (nếu cần)
    """
    processor = ImageProcessor(**kwargs)
    is_good, quality_metrics, enhanced_image = processor.check_quality(image)
    
    # Chuyển ImageQualityMetrics thành dict để consistent
    quality_info_dict = quality_metrics.to_dict() if quality_metrics else {}
    
    return is_good, quality_info_dict, enhanced_image
...
def enhance_image_array(image: np.ndarray, **kwargs) -> np.ndarray:
    ...
    # Lấy quality metrics trước
    quality_metrics = processor._check_image_quality(image)
    
    # Tăng cường ảnh dựa trên quality metrics
    enhanced = processor.enhance_image(image, quality_metrics)
