...
class ImageProcessor:
    ...
    def check_quality(self, image, prefix=None):
        ...
        Returns:
            tuple: (is_good, quality_info, enhanced_image)
                is_good: Boolean cho biết ảnh có đạt chất lượng không
                quality_info: Dict chứa thông tin chất lượng
                enhanced_image: Ảnh đã được nâng cao chất lượng nếu cần
        """
    ...
    def enhance_image(self, image: np.ndarray, quality_info: ImageQualityMetrics) -> np.ndarray:
        ...
        if self.has_gpu:
            with gpu_memory_manager():
                ...
                if quality_info.brightness < self.config["min_brightness"]:
                    alpha = self.config["min_brightness"] / quality_info.brightness
                    img_tensor = img_tensor * alpha
                    
                if quality_info.contrast < self.config["min_contrast"]:
                    beta = self.config["min_contrast"] / quality_info.contrast
                    img_tensor = (img_tensor - img_tensor.mean()) * beta + img_tensor.mean()
                
                # Giới hạn giá trị pixel
                img_tensor = torch.clamp(img_tensor, 0, 255)
                
                # Chuyển về CPU
                return self._to_cpu(img_tensor).astype(np.uint8)
        else:
            # Xử lý trên CPU
            result = image.copy()
            ...
            if quality_info.contrast < self.config["min_contrast"]:
                beta = self.config["min_contrast"] / quality_info.contrast
                result = cv2.convertScaleAbs(result, alpha=beta, beta=0)
    ...
    def process_image(self, input_path: str, output_path: str = None) -> ProcessingResult:
        ...
        try:
            ...
            with gpu_memory_manager():
                ...
                if not self.config["skip_rotation"]:
            ...
            processing_result = ProcessingResult(
                ...
                processing_steps=[
                    "check_quality",
                    "enhance_image",
                    "rotate_image" if angle != 0 else None,
                    "enhance_for_ocr"
                ],
