...
def has_final(jamo_or_syllable: str) -> bool:
    """
    한 글자가 종성을 가지는지 여부를 반환합니다.
    
    Parameters:
        jamo_or_syllable (str): 한글 완성형 문자 하나
    
    Returns:
        bool: 종성이 있으면 True, 없으면 False
    """
    if not jamo_or_syllable or len(jamo_or_syllable) != 1:
        return False

    ch = jamo_or_syllable[0]
    code = ord(ch)
...
__all__ = ['split', 'add', 'heavy_split', 'heavy_add', 'getChoseong', 'josa', 'compjosa', 'toCompat', 'fromCompat', 'native_number', 'native_day', 'has_final']
