...
def heavy_add(jamo_str: str) -> str:
    """
    쌍자음을 합쳐서 자모 + 쌍자음/겹모음을 합쳐 완성형 한글로 조합.
    수정: 낱자(pair)를 맥락에 따라 합칠지 말지 결정.
    """
    i = 0
    result_jamo = []
    length = len(jamo_str)
    ...
    while i < length:
        # 우선 가능한 pair인지 확인
        if i + 1 < length:
            first, second = jamo_str[i], jamo_str[i+1]
            pair = (first, second)
            if pair in SIMPLE_TO_HEAVY:
                heavy = SIMPLE_TO_HEAVY[pair]  # 합쳐질 jamo (예: ('ㄴ','ㅈ') -> 'ㄵ', ('ㅗ','ㅏ')->'ㅘ')
                # heavy가 겹자음(종성)인지, 겹모음(중성)인지 확인
                is_final_candidate = heavy in FINAL_SET
                is_medial_candidate = heavy in MEDIAL_SET  # e.g. 'ㅘ','ㅙ' 등

                # 뒤따르는 낱자가 중성인 경우, 두 번째 낱자는 다음 음절 초성일 가능성이 크므로
                # 겹자음(fianl) 결합을 건너뜀.
                next_is_medial = (i + 2 < length and jamo_str[i+2] in MEDIAL_SET)

                if is_medial_candidate:
                    # 겹모음은 문맥 상 합쳐주어도 문제 없음
                    result_jamo.append(heavy)
                    i += 2
                    continue
                elif is_final_candidate and not next_is_medial:
                    # 겹자음 결합이 타당한 상황: 합쳐서 final로 사용
                    result_jamo.append(heavy)
                    i += 2
                    continue
                # 그 외 (예: is_final_candidate이지만 next_is_medial=True 상황),
                # 또는 heavy가 neither final nor medial(보통 없음)인 경우: 합치지 않음.
        # 합치지 않은 경우:
        result_jamo.append(jamo_str[i])
        i += 1

    # 최종적으로 생성된 jamo sequence를 add로 합쳐 한글 완성형 반환
