...
class TestConfigureIBMCOSIAM:
    """Test IBM COS IAM configuration."""
    
    def test_configure_ibm_cos_iam_basic(self, clean_env):
        """Test basic IBM COS IAM configuration."""
        result = configure_ibm_cos_iam(
            api_key="iam_api_key",
            instance_crn="crn:v1:bluemix:public:cloud-object-storage:global:a/account::instance:instance-id",
            bucket="cos-bucket"
        )
    ...
    def test_configure_ibm_cos_iam_custom_endpoint(self, clean_env):
        """Test IBM COS IAM configuration with custom endpoint."""
        custom_endpoint = "https://s3.eu-de.cloud-object-storage.appdomain.cloud"
        result = configure_ibm_cos_iam(
            api_key="iam_api_key",
            instance_crn="crn:v1:bluemix:public:cloud-object-storage:global:a/account::instance:instance-id",
            bucket="cos-bucket",
...
class TestConvenienceFunctions:
    ...
    @patch('chuk_artifacts.config.configure_filesystem')
    @patch('chuk_artifacts.config.create_store')
    def test_testing_setup_custom_dir(self, mock_create_store, mock_configure_filesystem):
...
class TestProductionSetup:
    ...
    @patch('chuk_artifacts.config.configure_ibm_cos_iam')
    @patch('chuk_artifacts.config.create_store')
    def test_production_setup_ibm_cos_iam(self, mock_create_store, mock_configure_ibm_cos_iam):
        """Test production_setup with IBM COS IAM storage type."""
        mock_store = Mock()
        mock_create_store.return_value = mock_store
        ...
        result = production_setup(storage_type="ibm_cos_iam", **kwargs)
        
        mock_configure_ibm_cos_iam.assert_called_once_with(**kwargs)
        mock_create_store.assert_called_once()
        assert result == mock_store
...
class TestParameterValidation:
    ...
    def test_configure_ibm_cos_iam_required_parameters(self):
        """Test that configure_ibm_cos_iam requires necessary parameters."""
        with pytest.raises(TypeError):
            configure_ibm_cos_iam()  # Missing required parameters
        
        with pytest.raises(TypeError):
            configure_ibm_cos_iam(api_key="key")  # Missing instance_crn and bucket
...
class TestReturnValues:
    ...
    def test_all_configure_functions_return_dict(self, clean_env):
        ...
        functions_and_args = [
            ...
            (configure_filesystem, {}),
            (configure_redis_session, {}),
            (configure_s3, {"access_key": "key", "secret_key": "secret", "bucket": "bucket"}),
            (configure_ibm_cos, {"access_key": "key", "secret_key": "secret", "bucket": "bucket"}),
            (configure_ibm_cos_iam, {"api_key": "key", "instance_crn": "crn", "bucket": "bucket"}),
        ]
