...
from chuk_artifacts.grid import canonical_prefix, artifact_key, GridError
...
class TestSecurityInputSanitization:
    ...
    async def test_malicious_filenames(self):
        ...
        async with ArtifactStore(storage_provider="memory", session_provider="memory") as store:
...
class TestEnhancedErrorHandling:
        ...
        async with ArtifactStore(storage_provider="memory", session_provider="memory") as store:
            ...
            for error_type, error_message in error_scenarios:
                with patch.object(store._core, '_store_with_retry') as mock_store:
                    ...
                    with pytest.raises((ProviderError, SessionError, ArtifactStoreError)):
                        await store.store(
                            data=b"test data",
                            mime="text/plain",
                            summary="Error test",
    ...
    async def test_partial_failure_recovery(self):
        ...
        async with ArtifactStore(storage_provider="memory", session_provider="memory") as store:
            ...
            with patch.object(store._batch, '_store_with_retry', side_effect=mock_store_with_retry):
...
class TestPerformanceStress:
    ...
    async def test_batch_operation_performance(self):
        ...
        async with ArtifactStore(storage_provider="memory", session_provider="memory") as store:
            ...
            print(f"Batch operation: {batch_size} items in {duration:.2f}s "
    ...
    async def test_rapid_session_operations(self):
        ...
        async with ArtifactStore(storage_provider="memory", session_provider="memory") as store:
            ...
            print(f"Rapid session operations: {rapid_operations} cycles in {duration:.2f}s")
...
class TestResourceManagement:
    ...
    async def test_cleanup_on_exceptions(self):
        ...
        try:
            ...
            original_method = store._core.store
    ...
    def test_filesystem_resource_cleanup(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Run the test
            artifact_ids = asyncio.run(test_filesystem_operations())
    ...
    async def test_complete_artifact_lifecycle(self):
        ...
        async with ArtifactStore(storage_provider="memory", session_provider="memory") as store:
            ...
            try:
                ...
                assert download_url.startswith(("http://", "https://", "file://", "memory://"))
            ...
            for user, session_id in sessions.items():
                for artifact_id in artifacts[user]:
                    try:
                        ...
                        print(f"Failed to verify artifact {artifact_id} for {user}: {e}")
