...
- **`is_sdk=True`**: SDK mode - expects a complete response object from the LLM SDK
...
- Function returns a complete response object (.to_json())
- Extracts content and metadata from the full response
- Handles tool calls and usage information from the response structure
- More suitable for SDK-based implementations
