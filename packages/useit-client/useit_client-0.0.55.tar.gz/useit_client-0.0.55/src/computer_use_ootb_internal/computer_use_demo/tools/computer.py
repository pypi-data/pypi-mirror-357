...
class ComputerTool(BaseAnthropicTool):
        ...
        **kwargs,
        ...
        if action in (
            ...
            if action == "left_click":
                ...
                # Ensure we move to the exact position before clicking to eliminate any coordinate discrepancy
                pyautogui.moveTo(x, y)
                time.sleep(0.1)  # Brief pause to ensure cursor is positioned
                # Verify mouse position before clicking
                actual_x, actual_y = pyautogui.position()
                print(f"[DEBUG] Left click - Target: ({x}, {y}), Mouse position: ({actual_x}, {actual_y})")
                show_click(x, y)
                
                # Try alternative clicking method if coordinates don't match
                if abs(actual_x - x) > 5 or abs(actual_y - y) > 5:
                    print(f"[DEBUG] Coordinate mismatch detected, trying alternative windll method")
                    self.marbot_auto_gui.click(x=x, y=y)
                else:
                    # For WebRDP, try both methods to ensure click is registered
                    pyautogui.click(x=x, y=y)
                    # Small delay and verification
                    time.sleep(0.05)
                    # Verify click position one more time
                    final_x, final_y = pyautogui.position()
                    print(f"[DEBUG] Post-click mouse position: ({final_x}, {final_y})")
    ...
    def test_webrdp_clicking(self, test_x=100, test_y=100):
        ...
        print(f"[DEBUG] Testing windll method...")
