...
class MarbotAutoGUI:
    ...
    def middleClick(self, x=None, y=None, duration=0.3):
        if x is not None and y is not None:
            self.moveTo(x, y, duration)
        self._mouse_middle_click()

    def mouseDown(self, x=None, y=None, duration=0.3):
        if x is not None and y is not None:
            self.moveTo(x, y, duration)
        self._mouse_down()

    def mouseUp(self, x=None, y=None, duration=0.3):
        if x is not None and y is not None:
            self.moveTo(x, y, duration)
        self._mouse_up()
    ...
    def _mouse_right_click(self):
        inputs = (INPUT * 2)()
        inputs[0].type = INPUT_MOUSE
        inputs[0].mi = MOUSEINPUT(0, 0, 0, MOUSEEVENTF_RIGHTDOWN, 0, ctypes.pointer(self.extra))
        inputs[1].type = INPUT_MOUSE
        inputs[1].mi = MOUSEINPUT(0, 0, 0, MOUSEEVENTF_RIGHTUP, 0, ctypes.pointer(self.extra))
        ctypes.windll.user32.SendInput(2, ctypes.byref(inputs), ctypes.sizeof(INPUT))

    def _mouse_middle_click(self):
        inputs = (INPUT * 2)()
        inputs[0].type = INPUT_MOUSE
        inputs[0].mi = MOUSEINPUT(0, 0, 0, MOUSEEVENTF_MIDDLEDOWN, 0, ctypes.pointer(self.extra))
        inputs[1].type = INPUT_MOUSE
        inputs[1].mi = MOUSEINPUT(0, 0, 0, MOUSEEVENTF_MIDDLEUP, 0, ctypes.pointer(self.extra))
        ctypes.windll.user32.SendInput(2, ctypes.byref(inputs), ctypes.sizeof(INPUT))
    ...
    def _mouse_up(self):
        inputs = (INPUT * 1)()
        inputs[0].type = INPUT_MOUSE
        inputs[0].mi = MOUSEINPUT(0, 0, 0, MOUSEEVENTF_LEFTUP, 0, ctypes.pointer(self.extra))
        ctypes.windll.user32.SendInput(1, ctypes.byref(inputs), ctypes.sizeof(INPUT))
