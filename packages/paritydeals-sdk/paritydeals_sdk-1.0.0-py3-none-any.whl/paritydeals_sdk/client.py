...
# Import the SDK's logger (assuming logger is defined in paritydeals_sdk/__init__.py)
from . import logger

from .exceptions import APIError, AuthenticationError, InvalidRequestError, ServerError, NotFoundError
from .constants import DEFAULT_API_URL, API_BASE_PATH_PREFIX, DEFAULT_API_VERSION, EDGE_API_URL
...
# Type variable for the ParityDeals class itself, used in factory method return types
TParityDeals = TypeVar("TParityDeals", bound="ParityDeals")


class ParityDeals:
    """
    Unified Python SDK Client for interacting with the ParityDeals API.
    ...
    Synchronous usage:
        client = ParityDeals.create_sync_client(access_token="YOUR_TOKEN")
    ...
    Asynchronous usage:
        async_client = ParityDeals.create_async_client(access_token="YOUR_TOKEN")
    ...
    def __init__(self,
        ...
        """
        Initializes the ParityDeals API client.
        It's recommended to use `ParityDeals.create_sync_client` or
        `ParityDeals.create_async_client` factory methods for instantiation.
        ...
        actual_base_url = base_url if base_url is not None else DEFAULT_API_URL
        self.edge_api_url_to_use = edge_api_url if edge_api_url is not None else EDGE_API_URL  # Store the edge URL
        ...
        self.logger = logger
        logger.info(
            f"ParityDeals client initialized. Mode: {self._mode}, Base URL: {self.base_url}, API Version: {self.api_version}"
        )
    ...
    def create_sync_client(
        ...
        """
        Factory method to create a ParityDeals client configured for synchronous operations.
        """
        logger.debug("Creating synchronous ParityDeals client via factory.")
        return cls(
    ...
    def create_async_client(
        ...
        """
        Factory method to create a ParityDeals client configured for asynchronous operations.
        """
        logger.debug("Creating asynchronous ParityDeals client via factory.")
        return cls(
            access_token=access_token,
            mode='async',
            base_url=base_url,
    ...
    def close(self):
        ...
        if self._mode == 'sync' and isinstance(self._http_client, httpx.Client):
            if not self._http_client.is_closed:
                logger.info("Closing synchronous ParityDeals client.")
    ...
    async def aclose(self):
        ...
        if self._mode == 'async' and isinstance(self._http_client, httpx.AsyncClient):
            if not self._http_client.is_closed:
                logger.info("Closing asynchronous ParityDeals client.")
    ...
    # --- Context Management ---
    def __enter__(self: TParityDeals) -> TParityDeals:
        if self._mode != 'sync':
    ...
    async def __aenter__(self: TParityDeals) -> TParityDeals:
        if self._mode != 'async':
