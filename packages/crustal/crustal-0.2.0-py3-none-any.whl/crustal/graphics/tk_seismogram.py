...
class TkSeismogram():
    ...
    def __init__(self,
        ...
        self.root=tk.Tk()
        self.root.title("Seismogram")
        self.root.geometry("800x600")

        canvas = FigureCanvasTkAgg(fig, master=self.root)
        canvas.draw()
        canvas.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, padx=10, pady=10, expand=True)


        close_button = tk.Button(self.root, text="Close", font=("Helvetica", 14, "bold"), command=self.root.destroy)
