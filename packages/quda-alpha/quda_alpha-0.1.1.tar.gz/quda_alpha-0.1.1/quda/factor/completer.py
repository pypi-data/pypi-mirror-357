...
class MissingChecker:
    ...
    def check_date(fac: 'Factor', beg_date, end_date) -> list[str]:
        ...
        if not quda.has(fac.tb_name):
            return dateList
        # 查询本地有数据的日期
        fac_path = quda.tb_path(fac.tb_name)
        ...
        sql = f"""SELECT date
                ...
                GROUP BY date 
                HAVING count() > 0;"""
        exist_dateList = quda.sql(sql, lazy=False)["date"].cast(pl.Utf8).to_list()
    ...
    def check_times(fac: 'Factor', date: str, beg_time: str, end_time: str, freq: str) -> set:
        ...
        """
        tb_name = fac.tb_name
        tb_path = quda.tb_path(tb_name) / f"date={date}"
            ...
            with ygo.pool(n_jobs=n_jobs) as go:
                ...
                if dataset:
                    for tb_name, data_list in dataset.items():
                        ...
                        quda.put(data, tb_name=tb_name, partitions=[FIELD_DATE, FIELD_TIME])
