...
from .services.api import TraeApi
...
log_file_path = os.path.join(os.path.dirname(__file__), 'logs', "backend_code_gen.log")
...
# Create MCP szetop_backend_code_mcp
mcp = FastMCP(
    "szetop-trae-dev",
...
async def generate_api_code_prompt(project_folder: str, image_path: list[str], entity_paths: list[str],
    ...
    # 设置Trae的整体规则
    TraeEnv.set_rules(project_folder, entity_paths)
    ...
    try:
        # return TraeApi.get_backend_code_gen_prompt(image_path, entity_paths, business_logic)
...
def main():
    """Entry point for the MCP szetop_backend_code_mcp."""
    # 检查环境
