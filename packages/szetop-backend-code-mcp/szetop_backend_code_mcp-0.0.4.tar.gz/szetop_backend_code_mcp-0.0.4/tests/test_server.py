...
from szetop_backend_code_mcp import describe_image, describe_image_from_file, describe_image_from_url
...
async def test_describe_image_function(mock_vision_client):
    ...
    with patch('image_recognition_server.szetop_backend_code_mcp.get_vision_client', return_value=mock_vision_client):
        with patch('image_recognition_server.szetop_backend_code_mcp.validate_base64_image', return_value=True):
...
async def test_describe_image_invalid_data(mock_vision_client):
    ...
    with patch('image_recognition_server.szetop_backend_code_mcp.get_vision_client', return_value=mock_vision_client):
        with patch('image_recognition_server.szetop_backend_code_mcp.validate_base64_image', return_value=False):
...
async def test_describe_image_from_file_function(mock_vision_client, tmp_path):
    ...
    with patch('image_recognition_server.szetop_backend_code_mcp.get_vision_client', return_value=mock_vision_client):
        with patch('image_recognition_server.szetop_backend_code_mcp.image_to_base64', return_value=(TEST_IMAGE_DATA, "image/png")):
...
async def test_describe_image_from_url_function(mock_vision_client):
    ...
    with patch('image_recognition_server.szetop_backend_code_mcp.get_vision_client', return_value=mock_vision_client):
        with patch('image_recognition_server.szetop_backend_code_mcp.url_to_base64', return_value=(TEST_IMAGE_DATA, "image/jpeg")):
...
async def test_describe_image_from_url_invalid(mock_vision_client):
    ...
    with patch('image_recognition_server.szetop_backend_code_mcp.get_vision_client', return_value=mock_vision_client):
        with patch('image_recognition_server.szetop_backend_code_mcp.url_to_base64', side_effect=ValueError("Failed to fetch image")):
