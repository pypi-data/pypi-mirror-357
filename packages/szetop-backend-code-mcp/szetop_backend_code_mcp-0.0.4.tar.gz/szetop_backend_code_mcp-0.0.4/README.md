...
```bash
uvx pip install szetop-backend-code-mcp
```
...
```bash
pip install szetop-backend-code-mcp
```
...
```bash
python -m etop_mcp_dev.szetop_backend_code_mcp
```
...
```json
{
  "mcpServers": {
    "image-recognition": {
      ...
      "args": [
        "-m",
        "backend_gen_server.szetop_backend_code_mcp"
      ],
...
```bash
run.bat test szetop_backend_code_mcp
