...
def get_final_minimized_structure(em_tpr, em_trr, final_minimized):
    ...
    print("="*100)
    run_command(f"echo 14 | gmx trjconv -s {em_tpr} -f {em_trr} -o {final_minimized} -pbc nojump")
...
def Run_NPT_Equilibration(topology_file, npt_tpr, nvt_gro, final_last_npt_pdb):
    print("\n" + "="*100)
...
def main():
    ...
    equilibrator_steps = []
    # === Protein Topology ===
    equilibrator_steps.append(("Generate topology for protein", lambda: generate_topology_protein(protein_file, topology_file, protein_gro)))
