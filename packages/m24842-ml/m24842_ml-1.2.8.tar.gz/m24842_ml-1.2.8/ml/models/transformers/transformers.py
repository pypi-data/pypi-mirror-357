...
class DiffusionTransformer(nn.Module):
    def __init__(self, emb_dim, input_dim, output_dim,
        ...
        encoder_depth = math.log(input_dim/emb_dim, 2)
        self.encoder = nn.Sequential()
        for i in range(int(encoder_depth)):
            self.encoder.add_module(f"encoder_conv_{i}", nn.Conv1d(1, 1, kernel_size=3, padding=1, bias=False, device=device))
            self.encoder.add_module(f"encoder_activation_{i}", nn.ReLU())
            self.encoder.add_module(f"encoder_maxpool_{i}", nn.MaxPool1d(kernel_size=2, stride=2))
        self.encoder.add_module(f"encoder_linear{i}", nn.Linear(int(input_dim/(2**int(encoder_depth))), emb_dim, bias=False, device=device))
        
        decoder_depth = math.log(output_dim/emb_dim, 2)
        self.decoder = nn.Sequential()
        self.decoder.add_module(f"decoder_linear_{i}", nn.Linear(emb_dim, math.ceil(output_dim/(2**int(decoder_depth))), bias=False, device=device))
        for i in range(int(decoder_depth)):
            self.decoder.add_module(f"decoder_upsample_{i}", nn.Upsample(scale_factor=2, mode='nearest'))
            self.decoder.add_module(f"decoder_conv_{i}", nn.Conv1d(1, 1, kernel_size=3, padding=1, bias=False, device=device))
            if i < decoder_depth-1: self.decoder.add_module(f"decoder_activation_{i}", nn.ReLU())
    ...
    def forward(self, x):
        ...
        x = self.encoder(x.reshape(bsz*seq_len, 1, -1)).reshape(bsz, seq_len, self.emb_dim)
        for layer in self.layers:
            x = layer.norm1(x)
            if layer.abs_pos_encoding is not None:
                pos = torch.arange(seq_len, device=x.device, dtype=torch.long).unsqueeze(0).expand(x.size(0), -1)
            ...
            x = x + layer.dropout2(ff_out)
        x = self.norm_f(x)
        x = self.decoder(x.reshape(bsz*seq_len, 1, -1)).reshape(bsz, seq_len, self.emb_dim)
