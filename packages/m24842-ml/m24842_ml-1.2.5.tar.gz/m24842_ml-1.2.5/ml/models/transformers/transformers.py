...
class DiffusionTransformer(nn.Module):
    def __init__(self, emb_dim, input_dim, output_dim,
                 n_layers=1, n_heads=1, mlp_dim=None, attn_sink=False,
                 dropout=0.0, encoder_decoder_depth=4,
                 mlp_bias=True, attn_bias=True,
                 pos_encoding=None, pos_encoding_max_len=None,
                 device="cpu"):
        super().__init__()
        ...
        encoder_dims = torch.linspace(input_dim, emb_dim, encoder_decoder_depth, device=device).to(torch.long)
        self.encoder = nn.Sequential(
            nn.Linear(encoder_dims[0], encoder_dims[1], bias=False, device=device),
            nn.ReLU(),
            nn.Linear(encoder_dims[1], encoder_dims[2], bias=False, device=device),
            nn.ReLU(),
            nn.Linear(encoder_dims[2], encoder_dims[3], bias=False, device=device),
        )
        
        decoder_dims = torch.linspace(emb_dim, output_dim, encoder_decoder_depth, device=device).to(torch.long)
        self.decoder = nn.Sequential(
            nn.Linear(decoder_dims[0], decoder_dims[1], bias=False, device=device),
            nn.ReLU(),
            nn.Linear(decoder_dims[1], decoder_dims[2], bias=False, device=device),
            nn.ReLU(),
            nn.Linear(decoder_dims[2], decoder_dims[3], bias=False, device=device),
        )
