...
{
Compiler_Error(__eCNameSpace__eC__i18n__GetTranslatedString("ec", "No set defined for property %s\n", (((void *)0))), dataMember->name);
continue;
}
recursionCount--;
return 0;
...
sprintf(instTemp, "__eCInstance%d", tempCount);
exp->type = 23;
...
exp->__anon1.compound->__anon1.compound.declarations = MkListOne(QMkDeclaration(inst->_class->__anon1.__anon1.name, MkInitDeclarator(MkDeclaratorIdentifier(MkIdentifier(instTemp)), MkInitializerAssignment(newCall))));
exp->__anon1.compound->__anon1.compound.statements = MkListOne(MkExpressionStmt((expList = MkList())));
instExp = QMkExpId(instTemp);
...
if(exp->usage)
{
struct Expression * tmpExp = QMkExpId(instTemp);
