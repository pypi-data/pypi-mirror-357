...
QtWidgets.QApplication.setAttribute(QtCore.Qt.AA_ShareOpenGLContexts) # 两行
...
class DisplayEmbryo:
    ...
    def annotation_to(self):
        ...
        if self.column_name.value not in adata.obs.columns:
            ...
            if self.cluster_anno.value not in cat:
                ...
                adata.obs[self.column_name.value] = adata.obs[self.embryo.tissue_id].cat.set_categories(new_cat)
    ...
    def _on_dock_closed(self):
        ...
        # # 注册重建回调
        # self.viewer.window._dock_widgets['console'].dockWidget.visibilityChanged.connect(
        #     lambda v: self._rebuild_if_needed(v)
        # )
