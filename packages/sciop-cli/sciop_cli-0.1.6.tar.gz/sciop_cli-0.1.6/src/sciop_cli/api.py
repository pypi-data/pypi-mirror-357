...
def api_call(
    ...
    if res.status_code == 403 and config.token and not autorefresh_token:
...
def login(username: str, password: str, **kwargs: Any) -> str:
    """
    Get a token from the API with a username and password
...
def upload_torrent(path: Path, timeout: float | None = 60.0) -> TorrentFile:
...
def create_upload(
    ...
    Args:
        ...
        timeout (float | None): Timeout in seconds to wait for the upload to complete.
            Defaults to 60 seconds since processing uploads can take some time,
            explicitly pass `None` to use `config.request_timeout`
    """
