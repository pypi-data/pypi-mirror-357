...
class QoreClient:
    ...
    def __init__(
        ...
        self.organization_ops = OrganizationAPI(self._request)
        self.drive_ops = DriveAPI(self._request)
        self.folder_ops = FolderAPI(self._request)
        self.file_ops = FileAPI(self._request)
        self.module_ops = ModuleImportManager(self.get_file, self.upload_file)
        self.workflow_ops = WorkflowAPI(self._request)
        self.workspace_ops = WorkspaceAPI(self._request)
        self.webhook_ops = WebhookAPI(self._request)
