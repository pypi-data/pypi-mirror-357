...
class GradioWebUIBuilder:
    ...
    def _build_input_component(self, variable: WorkflowVariableConfig) -> gradio.Component:
        ...
        if variable.type == "string":
            return gradio.Textbox(label=label, value="", info=info)

        if variable.type == "number":
            return gradio.Number(label=label, value="", precision=None, info=info)
        
        if variable.type == "integer":
            return gradio.Number(label=label, value="", precision=0, info=info)
        ...
        return gradio.Textbox(label=label, value=default, info=f"Unsupported type: {variable.type}")
