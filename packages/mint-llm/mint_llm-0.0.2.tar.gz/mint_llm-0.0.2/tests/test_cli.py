...
def test_cli_infuse(monkeypatch, tmp_path):
    ...
    def fake_attach(model, sim, alpha):
        called["attach"] = (model, sim, alpha)
        return DummyModel()

    monkeypatch.setattr("mint.cli._attach_layer", fake_attach)
    ...
    result = runner.invoke(
        app,
        ["infuse", str(model_dir), "sim.pt", str(out_dir), "--alpha", "0.5"],
    )

    assert result.exit_code == 0
    assert f"Model infused with similarity data saved to {out_dir}" in result.stdout
    assert called["attach"] == (str(model_dir), "sim.pt", 0.5)
...
def test_cli_pick(monkeypatch, tmp_path):
    ...
    result = runner.invoke(app, ["pick", "mymodel", "--dest", str(tmp_path)])
