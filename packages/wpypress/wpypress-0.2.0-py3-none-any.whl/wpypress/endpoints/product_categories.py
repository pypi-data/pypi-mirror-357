...
class ProductCategoriesEndpoint:
    def __init__(self, client):
        self.client = client
        self.endpoint = f"{self.client.base_url}/wp-json/wc/v3/products/categories"

    def list(self, params=None):
        """List all product categories (with optional filters) and pagination metadata"""
    ...
    def delete(self, category_id):
        """Permanently delete a product category"""
        url = f"{self.endpoint}/{category_id}"
        params = {'force': 'true'}
        headers = self.client.auth.get_headers() if self.client.auth else {}
