...
# Try to import OmniParser utilities
try:
    # First try to import from the package's util module
    from .util.utils import get_som_labeled_img, get_caption_model_processor, get_yolo_model, check_ocr_box
except ImportError:
    try:
        # Try to import from OmniParser installation
        ...
        if OMNIPARSER_PATH.exists():
            ...
            raise ImportError("OmniParser not found")
    except ImportError as e:
        # Provide helpful error message
        raise ImportError(
            f"Failed to import OmniParser utilities. Please ensure OmniParser is properly installed.\n"
            f"You can:\n"
            f"1. Clone OmniParser to the parent directory: git clone https://github.com/microsoft/OmniParser.git\n"
            f"2. Or install OmniParser as a package\n"
            f"Original error: {e}"
        )
