...
mcp = FastMCP(
    ...
    instructions="""This server provides access to the MAG graph '{graph_name}' in parallel execution mode. 
    {description}
...
def execute_agents(input_text: str, conversation_id: str = None) -> str:
    ...
    """
    try:
        payload = {{"input_text": input_text, "graph_name": "{graph_name}", "parallel": True}}
        if conversation_id:
