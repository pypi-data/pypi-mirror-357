...
class MySQLConnectionManager:
    ...
    def get_connection(self):
        ...
        while attempt < self.retry_attempts:
            try:
                connection = pymysql.connect(**self.conn_config)
                logging.debug(f"成功连接到数据库 {self.conn_config['host']}")
                return connection
            except (pymysql.err.OperationalError, pymysql.err.InterfaceError) as e:
                attempt += 1
                wait_time = self.retry_delay * (2 ** (attempt - 1))  # 指数退避
                logging.warning(f"连接失败({e})，尝试第{attempt}次重试（{wait_time}秒后）")
                time.sleep(wait_time)
        
        # 所有重试失败后抛出异常
        error_msg = f"无法连接数据库 {self.conn_config['host']} 错误号：2003"
