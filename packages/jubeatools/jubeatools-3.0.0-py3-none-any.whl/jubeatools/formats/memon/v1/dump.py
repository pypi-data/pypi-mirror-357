...
def _dump_memon_1_0_0(song: jbt.Song, **kwargs: Any) -> SongFile:
    ...
    memon_file_timing = dump_file_timing(song)
    charts = {
        diff: dump_chart(chart, chart_timing, memon_file_timing)
        for diff, chart, chart_timing in song.iter_charts_with_applicable_timing()
    }
    file = memon.File(
        version="1.0.0",
        metadata=metadata,
        timing=memon_file_timing,
        data=charts,
    )
...
def dump_chart(
    chart: jbt.Chart,
    chart_timing: jbt.Timing,
    memon_file_timing: Optional[memon.Timing],
) -> memon.Chart:
    return memon.Chart(
        level=chart.level,
        resolution=None,
        timing=dump_chart_timing(chart_timing, memon_file_timing, chart.hakus),
