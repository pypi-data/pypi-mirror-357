...
def make_chart_from_events(events: Iterable[Event], beat_snap: int = 240) -> song.Chart:
    ...
    tempo_events_by_time = group_by(events_by_command[Command.TEMPO], lambda e: e.time)
    for time, tempo_events in tempo_events_by_time.items():
        if len(tempo_events) > 1:
            warnings.warn(
                f"Duplicate TEMPO event at time {time}, the last one in order will be kept"
            )

    bpms = [
        BPMAtSecond(
            seconds=ticks_to_seconds(time), BPM=value_to_truncated_bpm(events[-1].value)
        )
        for time, events in tempo_events_by_time.items()
    ]
