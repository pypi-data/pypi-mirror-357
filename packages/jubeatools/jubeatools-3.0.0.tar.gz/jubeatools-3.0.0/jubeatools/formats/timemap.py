...
from jubeatools.utils import fraction_to_decimal, group_by, mixed_number_format
...
class TimeMap:
    ...
    def from_beats(cls, events: List[BPMAtBeat], offset: SecondsAtBeat) -> TimeMap:
        ...
        for time, events_at_time in grouped_by_time.items():
            if len(events_at_time) > 1:
                raise ValueError(
                    f"Multiple BPMs defined at beat {mixed_number_format(time)} : {events}"
                )
    ...
    def from_seconds(cls, events: List[BPMAtSecond]) -> TimeMap:
        ...
        for time, events_at_time in grouped_by_time.items():
            if len(events_at_time) > 1:
                raise ValueError(
                    f"Multiple BPMs defined at {float(time):.2f} seconds : {events}"
                )
    ...
    def bpm_at_seconds(self, seconds: Union[song.SecondsTime, Fraction]) -> Fraction:
    ...
    def bpm_at_beat(self, beat: song.BeatsTime) -> Fraction:
