...
def make_events_from_chart(
    notes: Sequence[AnyNote], timing: song.Timing, hakus: Optional[Set[song.BeatsTime]]
) -> List[Event]:
...
def make_note_events(notes: Sequence[AnyNote], time_map: TimeMap) -> List[Event]:
...
def choose_end_beat(
    notes: Sequence[AnyNote], hakus: Optional[Set[song.BeatsTime]]
) -> song.BeatsTime:
...
def compute_last_note_beat(notes: Sequence[AnyNote]) -> song.BeatsTime:
