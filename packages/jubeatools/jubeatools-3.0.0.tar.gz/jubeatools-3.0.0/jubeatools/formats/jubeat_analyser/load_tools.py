...
class JubeatAnalyserParser:
    ...
    symbols: Dict[str, BeatsTime] = field(
        default_factory=partial(deepcopy, SYMBOL_TO_BEATS_TIME)
    )
    section_starting_beat: BeatsTime = BeatsTime(0)
    current_tempo: Decimal = Decimal(120)
    timing_events: List[BPMEvent] = field(default_factory=list)
    offset: int = 0
    beats_per_section: BeatsTime = BeatsTime(4)
    bytes_per_panel: int = 2
    level: Decimal = Decimal(1)
    difficulty: Optional[str] = None
    title: Optional[str] = None
    artist: Optional[str] = None
    jacket: Optional[str] = None
    preview_start: Optional[int] = None
    hold_by_arrow: bool = False
