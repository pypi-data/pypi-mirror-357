...
class MonoColumnLoadedSection:
    ...
    def blocs(self, bpp: int = 2) -> Iterator[List[List[str]]]:
        if bpp not in (1, 2):
            ...
            split_line = lambda line: list(line)
...
class MonoColumnParser(JubeatAnalyserParser):
    ...
    def append_chart_line(self, line: str) -> None:
        ...
        if expected_length != actual_length:
            raise SyntaxError(
                f"Invalid chart line. Since #bpp={self.bytes_per_panel}, a "
                f"chart line should be {expected_length} bytes long, but "
                f"{line!r} is {actual_length} bytes long"
            )
