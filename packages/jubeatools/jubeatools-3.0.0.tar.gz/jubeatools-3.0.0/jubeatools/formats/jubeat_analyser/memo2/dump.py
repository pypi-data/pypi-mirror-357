...
FULLWIDTH_EMPTY_BEAT_SYMBOL = "−"  # U+02212 : MINUS SIGN
FULLWIDTH_EMPTY_POSITION_SYMBOL = "□"  # U+025A1 : WHITE SQUARE
...
class Frame:
    ...
    empty_position_symbol: str = FULLWIDTH_EMPTY_POSITION_SYMBOL
    spacing: str = " "
    ...
    def dump(self) -> Iterator[str]:
        ...
        for pos, bar in zip_longest(self.dump_positions(), self.dump_bars()):
            if bar is None:
                ...
                yield f"{pos}{self.spacing}{bar}"

    def dump_positions(self) -> Iterator[str]:
        for y in range(4):
            yield "".join(
                self.positions.get(NotePosition(x, y), self.empty_position_symbol)
                for x in range(4)
            )
...
class Memo2Section:
    ...
    note_symbols: List[str] = field(default_factory=lambda: NOTE_SYMBOLS)
    empty_beat_symbol: str = FULLWIDTH_EMPTY_BEAT_SYMBOL
    frame_factory: Callable[[], Frame] = lambda: Frame()
    ...
    def _dump_notes(self, circle_free: bool = False) -> Iterator[str]:
        ...
        symbols_iterator = iter(self.note_symbols)
        for bar_index in range(4):
            ...
            for i in range(bar_length):
                ...
                bar.append(bar_event.note or self.empty_beat_symbol)
