...
def timing_info(
    ...
    if with_bpm_changes:
        ...
        sorted_bpm_changes = raw_bpm_changes.map(
            lambda l: sorted(l, key=get_bpm_change_time)  # noqa: E741
        )
...
def chart(
    draw: st.DrawFn,
    timing_strat: st.SearchStrategy[Optional[Timing]] = timing_info(),
    ...
    level_strat: st.SearchStrategy[Optional[Union[int, Decimal]]] = level(),
) -> Chart:
    level = draw(level_strat)
...
# some codepoint ranges that are valid in shift-jis
shift_jis_safe_text = partial(
    st.text,
    alphabet=st.one_of(
        *(
...
def metadata(
    draw: st.DrawFn,
    text_strat: st.SearchStrategy[str] = metadata_text(),
    path_strat: st.SearchStrategy[str] = metadata_path(),
) -> Metadata:
    return Metadata(
