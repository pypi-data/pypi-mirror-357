...
class FormulaParser:
    ...
    def parse_formula(self, formula, data=None):
        ...
        # Parse response variable (handle transformations)
        response_var, resp_func = self._parse_response(response_part)
        ...
        Y = self._apply_response_transformation(response_var, resp_func)
        
        return response_var, X.columns.tolist(), Y, X
    
    def _parse_response(self, response_part):
        """Parse response variable, handling transformations."""
    ...
    def _evaluate_term(self, term):
        ...
        term = re.sub(r'(\w+)\^\(?([\d\.]+)\)?', 
                     lambda m: f"pow__{m.group(2).replace('.', '_')}({m.group(1)})", 
                     term)
    ...
    def _handle_interaction(self, term):
        """Handle interaction terms like A:B or A:B:C."""
        parts = term.split(':')
    ...
    def _apply_function(self, func_name, inner_term):
        ...
        for col_name, col_values in inner_matrix.items():
            if func_name == 'log':
                ...
                result[f'{col_name}^{power}'] = transformed
            else:
    ...
    def _apply_response_transformation(self, response_var, resp_func):
