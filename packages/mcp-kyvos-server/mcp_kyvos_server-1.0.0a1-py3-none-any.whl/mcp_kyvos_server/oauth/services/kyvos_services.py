...
class ClientRegistrationService:
    ...
    def register_client(self, client_req):
        ...
        logger.info(f"Get clients request body: {client_req}")
        ...
        logger.info(f"Check registered clients: {self._clients}")
...
class AuthService:
    async def handle_callback(self, code: str, code_verifier: str) -> Dict:
        ...
        logger.info(f"Decoded the jwt access token. {decoded}")

        # Extract email
        email = decoded.get("email") or decoded.get("sub")
        logger.info(f"Fetched the email id from jwt access token. {email}")

        bound_code = await client_auth_service.generate_bound_code(access_token)
        logger.info(f"Bound authorization code successfully generated for client access token exchangee: {bound_code}")
...
class TokenService:
    async def get_client_refreshed_access_token(self, email: str) -> dict:       
        ...
        logger.info(f"Received client's refresh token: {client_refresh_token}")
