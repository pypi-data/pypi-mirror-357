...
from mcp_kyvos_server.kyvos.auth_config import config as cg
...
async def run_server(transport: str = "stdio", port: int = 8000, sslkeyfile: Optional[str] = None,
                     sslcertifile: Optional[str] = None, auth_type: str="oauth") -> None:
    """Run the MCP Kyvos server with the specified transport."""
    logger.info(InfoLogs.SERVER_STARTED)
    logger.info(f"server auth type: {auth_type}")
    ...
    try:
        if transport == "sse":
            ...
            async def handle_post_message(scope: Scope, receive: Receive, send: Send) -> None:
                ...
                logger.info(f"{InfoLogs.RECEIVED_JSON} {body}")

                try:
                    message = JSONRPCMessage.model_validate_json(body)
                    logger.info(f"Check JSONRPCMessage Body: {message}")

                    auth_token = request.headers.get("Authorization")
                    auth_from_client = request.headers.get("auth_type")
                    logger.info(f"Authorization type: {auth_from_client}")
                    ...
                    if auth_type == "basic":
                        if hasattr(message, "root") and message.root:
                            ...
                            if hasattr(root, "params") and isinstance(root.params, dict):
                                ...
                                if "arguments" in params and isinstance(params["arguments"], dict):
                                    ...
                                    params["arguments"] = {"auth_token": auth_token}
                                logger.info("Updated message instance: %s", params["arguments"])
                            else:
                        ...
                        logger.info(f"Received valid access token from MCP Client: {access_token}")
    
                        email = get_email_by_client_access_token(access_token)                       
                        access_token = get_tokens(email)["access_token"]
                        logger.info(f"Retrieved valid access token for user: {email}")
                ...
                logger.debug(DebugLogs.SENDING_MESSAGE.format(message=message))
