...
async def authorize(request: Request):
    ...
    logger.info(f"Query params received from client: {params}")
    ...
    username = os.getenv("KYVOS_USERNAME")
    if username:
        login_hint = username
    else:
        login_hint = "kyvos_user"
    ...
    logger.info(f"Redirecting client to authorization URL: {auth_url}")
...
async def callback(request: Request):
    ...
    logger.info(f"Auth callback received with params: {params}")
    ...
    logger.info(f"Check code verifier: {code_verifier} - {state} - {get_tokens_state(state)['state']}")
    ...
    logger.info(f"Check the retrieved access token: {get_tokens(authorized_user_email)["access_token"]}")
...
async def generate_client_access_token(request: Request):
    ...
    if refresh_token:
        ...
        logger.info(f"Client access token expired. Check email-id: {email_id}")
    ...
    logger.info(f"Check if the email is fetched dependent on bound code: {user_email_id}")
